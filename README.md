...
* 国储会拥有发起发行数字公民币、销毁数字公民币、增删改新省储会、增删改新省储行等提案投票和多签名pallet_address交易功能，并承担数据归档和引导节点；
* 国储会发起发行数字公民币、增删改新省储会、增删改新省储行等流程的，使用联合投票流程；发起销毁数字公民币、更换国储会多签管理员等流程的，使用内部投票流程。
...
* 省储会发起销毁数字公民币、更换省储会多签管理员等流程的，使用内部投票流程。
...
* 省储行发起销毁数字公民币、更换省储行多签管理员等流程的，使用内部投票流程。
...
* 各省储行质押的数字公民币，由区块链支付100年质押利息,质押利息存入各省储行多签名模块地址（pallet_address），利息归各省储行所有，用于省储行运营和资助民运人士；
...
```
   ...
   │
   ├── frontend/                # 前端应用
   │   ├── fcrc-frontend/       # 公民储备委员会前端（国储会+省储会+省储行管理员操作、治理界面等）
   │   ├── fullnode-frontend/   # 全节点前端（矿工 PoW状态、区块浏览器）
   │   └── wuminapp-frontend/   # 轻节点前端（钱包、公民投票、资产管理）
   │
...
* 除runtime升级以外的超级管理员权限，由国储会承担，使用国储会内部投票执行，国储会内部投票通过的则具备超级管理员权限，反之则否决；
* 链的运行时状态函数升级（runtimee升级）由联合投票决定，通过则升级，反之则否决。
...
### 5.3.1.链上交易/onchain-transaction-fee
...
### 5.3.2.链下交易/offchain-transaction-fee
...
* 交易：国储会、省储会、省储行交易地址（pallet_address）使用“内部投票”支出，内部投票即使用管理员私钥签名；
