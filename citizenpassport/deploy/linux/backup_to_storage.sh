...
BACKEND_ENV="/etc/citizenpassport/citizenpassport-backend.env"
BACKUP_ENV="/etc/citizenpassport/backup.env"
...
LOCAL_BACKUP_DIR="/var/backups/citizenpassport/${DATE_TAG}"
...
tar -C /var/lib/citizenpassport -czf "${RUNTIME_TAR_FILE}" runtime
...
if [[ "${LOCAL_RETENTION_DAYS}" =~ ^[0-9]+$ ]] && [[ "${LOCAL_RETENTION_DAYS}" -gt 0 ]]; then
  find /var/backups/citizenpassport -mindepth 1 -maxdepth 1 -type d -mtime +"${LOCAL_RETENTION_DAYS}" -exec rm -rf {} +
fi
