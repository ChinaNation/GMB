...
struct QrGenerateData {
    qr_payload: crate::archive::ArchiveQrPayload,
    qr_content: String,
}
...
struct StatusExportData {
    file_name: String,
    export_file: archive::CpmsStatusExportFile,
}

#[derive(Serialize)]
struct StatusExportStateData {
    state: archive::CpmsStatusExportState,
}
...
async fn create_archive(
    ...
    let citizen_status = req
        .citizen_status
        .unwrap_or_else(|| archive::CITIZEN_STATUS_NORMAL.to_string());
    archive::validate_citizen_status(&citizen_status)?;
    ...
    let election_scope_level = archive::validate_election_scope_level(
        req.election_scope_level
            .as_deref()
            .unwrap_or(archive::ELECTION_SCOPE_PROVINCE),
    )?;

    let now_ts = Utc::now().timestamp();
    let voting = archive::resolve_voting_eligible(
        &citizen_status,
        birth_date,
        req.voting_eligible,
        true,
    ...
    let valid_from = archive::archive_valid_from(now_ts);
    let valid_until =
        archive::archive_valid_until(now_ts, archive::archive_validity_years(now_ts, birth_date));
    let archive = Archive {
        archive_id,
...
async fn save_archive_wallet(
    ...
    if archive.citizen_status != archive::CITIZEN_STATUS_NORMAL || !archive.voting_eligible {
    ...
    archive::clear_archive_qr_payload(&state, &archive_id, now).await?;
...
async fn update_archive(
    ...
    if let Some(v) = req.citizen_status {
        archive::validate_citizen_status(&v)?;
    ...
    archive.voting_eligible = archive::resolve_voting_eligible(
...
async fn generate_archive_qr(
    ...
    let qr_payload = archive::build_archive_qr_payload(&state, &archive).await?;
...
async fn export_status_file(
    ...
    let export_file = archive::build_and_record_cpms_status_export(&state).await?;
...
async fn status_export_state(
    ...
    authz::require_user_group(&state, &headers, "admins").await?;
    let export_state = archive::status_export_state(&state).await?;
...
async fn ensure_archive_qr_ready(
    ...
    let election_scope_level =
        archive::validate_election_scope_level(&archive.election_scope_level)?;
    if election_scope_level == archive::ELECTION_SCOPE_CITY
        || election_scope_level == archive::ELECTION_SCOPE_TOWN
    {
        ensure_required_text(&archive.city_code, "archive qr requires residence city")?;
        ensure_required_text(&archive.birth_city_code, "archive qr requires birth city")?;
    }
    if election_scope_level == archive::ELECTION_SCOPE_TOWN {
        ensure_required_text(&archive.town_code, "archive qr requires residence town")?;
        ensure_required_text(&archive.birth_town_code, "archive qr requires birth town")?;
    }
    if archive.citizen_status != archive::CITIZEN_STATUS_NORMAL {
        return Err(err(
            StatusCode::BAD_REQUEST,
            1001,
    ...
    if !archive::is_voting_age_at(Utc::now().timestamp(), birth_date) {
        return Err(err(
            StatusCode::BAD_REQUEST,
            1001,
...
mod tests {
    ...
    fn sample_archive() -> Archive {
        Archive {
            ...
            election_scope_level: archive::ELECTION_SCOPE_PROVINCE.to_string(),
            status: "ACTIVE".to_string(),
