...
## 4. 两码流程
1. CID 生成 `CID_CPMS_V1 / INSTALL`。
2. CPMS 离线解析 INSTALL，校验协议类型、字段格式、省市名称与 `cid_number` 解码结果一致；CPMS 不引入外置 CID 公钥验签流程，ARCHIVE 是否可信由 CID 侧最终验真。
3. CPMS 根据 INSTALL 的 R5 段从内置 CID 行政区快照中重建当前市镇和地址段表。
4. CPMS 生成本机 `ARCHIVE` 签发密钥，公钥保存为 `cpms_pubkey`。
5. CPMS 创建档案时由 `number` 模块同步生成一对一绑定的档案号和护照号；档案号供 CID 使用，护照号印刷在护照上。
6. 用户在 citizenwallet 电子护照页出示投票账户地址二维码，CPMS 扫描后保存投票账户。
7. CPMS 生成携带钱包地址/公钥的 ARCHIVE 二维码。
8. CID 扫描 ARCHIVE，解 `geo_seal`、验 CPMS 签名，并在 CID 绑定阶段要求 citizenwallet 钱包签名。

## 5. 数据库表
| 表 | 说明 |
|------|------|
| `system_install` | INSTALL 安装授权状态，显式保存 `cid_number / province_code / city_code` |
...
- CPMS 尚未正式发行前，开发期 migration 只保留当前完整基线 `0001_init_citizenpassport_pg.sql`；修改基线后必须执行 `./citizenpassport.sh --reset` 重建开发库。
...
- 年度报告类型固定为 `CID_CPMS_V1 / CPMS_STATUS_EXPORT`，只导出给 CID 手工导入的绑定状态 JSON。
...
- `binding_release_records` 只导出当年度满 100 年硬删除后需要 CID 释放三者绑定关系的 `archive_no / released_at / release_reason`。
- 年度报告不得导出姓名、出生日期、地址、护照号；护照号是 CPMS 内部号码，与 CID 导入无关。
...
- 开发默认文件正文保存在 `data/archive-materials/<archive_id>/`；正式离线安装包固定通过
  `CPMS_MATERIALS_DIR=/var/lib/citizenpassport/materials` 写入本机资料目录。
...
- 左侧导航保留 CPMS 当前“返回列表”图标；“档案详情、资料库、操作记录”图标语义对齐 CID 机构详情共享导航的房子、文件夹和历史记录图标。
...
- 正式安装包按 CPU 架构分为 `citizenpassport-ubuntu24-amd64.run` 和 `citizenpassport-ubuntu24-arm64.run`，均包含后端、
  `frontend/dist`、PostgreSQL/nginx/openssl 等 Ubuntu 24.04 离线 deb 依赖、systemd、nginx
  配置、证书生成脚本和 `CitizenPassport安装配置手册.md`；安装过程不得联网。
- 后端正式部署只监听 `127.0.0.1:8080`，局域网入口统一由 nginx 提供
  `https://www.citizenpassport.com/`。客户端 DNS 由公安局内网自行配置到 CPMS 主机地址。
- 安装时生成 `/etc/citizenpassport/certs/citizenpassport-root-ca.crt` 和 `www.citizenpassport.com` 服务端证书；客户端需要信任该
  本机私有 CA 后访问 HTTPS。
- 安装手册安装到 `/opt/citizenpassport/docs/CitizenPassport安装配置手册.md`，手动 CI artifact 也单独包含同一份 Markdown 手册。
...
| `CPMS_MATERIALS_DIR` | 公民资料库文件正文保存目录；正式安装为 `/var/lib/citizenpassport/materials` | `data/archive-materials` |
...
| `CPMS_CHINA_DB` | 开发库派生的离线行政区 SQLite 快照路径；安装包默认随附 | `/opt/citizenpassport/data/china.sqlite` |
...
`403`。完整规则见 `memory/05-modules/citizenpassport/ERROR_CODES.md`。
...
`memory/05-modules/citizenpassport/ARCHIVE_WALLET_PROOF.md`。无钱包地址的档案不得签出
...
- CPMS 后端 `china` 模块运行时用 rusqlite 只读 `china.sqlite`，路径三层兜底：
  ① 环境变量 `CPMS_CHINA_DB`（生产由 install_host 写入 `/opt/citizenpassport/data/china.sqlite`）；
  ② 二进制旁 `<exe 目录>/../data/china.sqlite`（部署自定位，env 丢失也能找到）；
  ③ 编译期 `CARGO_MANIFEST_DIR` 相对的 CID 唯一源（本地 `cargo run` 零配置即通）。
  发行包随附该 SQLite 的只读拷贝（安装到 `/opt/citizenpassport/data/china.sqlite`）。
- `citizenpassport/scripts/build_linux_host_installer.sh` 把 CID 唯一源 `china.sqlite` 拷入安装包 payload；
  任何在 CPMS 源码树恢复 `province.rs`/`city_codes` 第二份行政区源码的改动都属于残留回退。
- 一个 CPMS 通用发行包可以安装到任意市公安局；运行时由 CID 签发的 INSTALL 安装码锁定
  唯一市公安局。
...
- 发行产物名称固定为 `citizenpassport-ubuntu24-amd64.run` 和 `citizenpassport-ubuntu24-arm64.run`，分别由 GitHub
...
- `citizenpassport/scripts/build_linux_host_installer.sh` 构建自解压 `.run`：payload 包含 `citizenpassport-backend`、
...
- `citizenpassport/deploy/linux/install_host.sh` 只从 payload 的 `debs/` 安装依赖，禁止 `apt-get update`
  或访问外部 apt 源；目标机无需联网。
...
- 安装后后端服务为 `citizenpassport-backend.service`，工作目录 `/var/lib/citizenpassport`，环境文件
  `/etc/citizenpassport/citizenpassport-backend.env`，资料目录 `/var/lib/citizenpassport/materials`。
- nginx 站点文件为 `/etc/nginx/sites-available/citizenpassport.conf`，启用后监听 80/443，80 自动跳转
  HTTPS，443 反代到 `127.0.0.1:8080`。
- 证书由 `/opt/citizenpassport/bin/generate_citizenpassport_certs.sh` 安装时生成；Root CA 路径为
  `/etc/citizenpassport/certs/citizenpassport-root-ca.crt`，服务端证书只绑定 `DNS:www.citizenpassport.com`。
- 备份脚本 `/opt/citizenpassport/bin/backup_to_storage.sh` 同时备份 PostgreSQL dump、`/var/lib/citizenpassport/runtime`
  和 `/var/lib/citizenpassport/materials`，并生成同批次 sha256 校验文件。
- 卸载脚本只移除 CPMS 服务、nginx 站点和 `/opt/citizenpassport/bin` 内程序；PostgreSQL、数据库、
  `/etc/citizenpassport`、`/var/lib/citizenpassport` 和 `/var/backups/citizenpassport` 默认保留，避免误删实名档案资料。
