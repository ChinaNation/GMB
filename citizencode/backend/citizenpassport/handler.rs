...
use crate::number::validate_cid_number_format;
use crate::*;
...
impl Db {
    pub(crate) fn get_cpms_site(&self, cid_number: &str) -> Result<Option<CpmsSiteKeys>, String> {
        let cid_number = cid_number.trim().to_string();
        self.with_client(move |conn| {
            let row = conn
                .query_opt(
                    "SELECT cid_number, province_code, city_code, status, install_token_status,
        ...
        self.with_client(move |conn| {
            ...
            conn.execute(
                "INSERT INTO cpms_sites (
                    cid_number, province_code, city_code, status, install_token_status,
    ...
    fn delete_cpms_site(&self, cid_number: &str) -> Result<bool, String> {
        let cid_number = cid_number.trim().to_string();
        self.with_client(move |conn| {
            let affected = conn
                .execute(
                    "DELETE FROM cpms_sites WHERE cid_number = $1",
                    &[&cid_number],
                )
            ...
            let rows = conn
                .query(
                    "SELECT cid_number, province_code, city_code, status, install_token_status,
                     ...
                     ORDER BY province_code ASC, city_code ASC, cid_number ASC",
                    &[&province_code],
                )
    ...
    fn find_cpms_target_institution_by_cid(
        ...
        self.with_client(move |conn| {
            let row = conn
                ...
                .map_err(|e| format!("query cpms target institution by cid failed: {e}"))?;
            Ok(row.map(|row| {
                (
                    row.get(0),
                    row.get(1),
...
fn append_cpms_audit_log_best_effort(
    ...
    crate::core::runtime_ops::append_audit_log(state, action, actor_account, target_cid, detail);
}
...
pub(crate) async fn generate_cpms_install_qr(
    ...
    let cid_number = match validate_cid_number_format(input.cid_number.trim()) {
        Ok(v) => v,
    ...
    let sign_source =
        build_install_sign_source(&cid_number, &province, &city, &install_secret_hash);
...
pub(crate) async fn archive_verify(
    ...
    append_cpms_audit_log_best_effort(
        ...
        "CPMS_ARCHIVE_VERIFY",
        &ctx.admin_account,
        Some(verified.cid_number.clone()),
    ...
    Json(ApiResponse {
        ...
        data: CpmsArchiveVerifyOutput {
            ...
            cid_number: verified.cid_number,
            status: "verified",
        },
    })
    .into_response()
...
pub(crate) async fn revoke_install_token(
    ...
    update_cpms_site_token_status(state, headers, cid_number, InstallTokenStatus::Revoked).await
}
...
pub(crate) async fn reissue_install_token(
    ...
    let cid_number = match validate_cid_number_format(cid_number.as_str()) {
        Ok(v) => v,
        Err(msg) => return api_error(StatusCode::BAD_REQUEST, 1001, msg),
    };
    let grant_payload = serde_json::json!({ "target": cid_number.clone() });
    if let Err(resp) = require_admin_security_grant(
        &state,
        &headers,
        &ctx,
    ...
    let mut site = match load_scoped_site(&state, &ctx, &cid_number) {
        Ok(v) => v,
        Err(resp) => return resp,
    };
...
pub(crate) async fn delete_cpms_keys(
    ...
    let site = match load_scoped_site(&state, &ctx, &cid_number) {
        Ok(v) => v,
        Err(resp) => return resp,
    };
    ...
    if let Err(err) = state.db.delete_cpms_site(&cid_number) {
...
pub(crate) async fn list_cpms_keys(
    ...
    rows.sort_by(|a, b| a.cid_number.cmp(&b.cid_number));
    let total = rows.len();
...
pub(crate) async fn get_cpms_site_by_institution(
    ...
    let site = match state.db.get_cpms_site(&cid_number) {
        Ok(v) => v,
        Err(err) => {
...
fn cpms_site_keys_to_list_row_simple(
    ...
    CpmsSiteKeysListRow {
        cid_number: site.cid_number.clone(),
...
pub(crate) async fn verify_cpms_archive_qr(
    ...
    if archive_code.proto != "CID_CPMS_V1" {
        return Err((
            StatusCode::BAD_REQUEST,
            1001,
            "proto must be CID_CPMS_V1".to_string(),
        ));
    }
    ...
    bind_cpms_pubkey_if_needed(state, site.cid_number.as_str(), cpms_pubkey_hash.as_str()).await?;
    Ok(VerifiedCpmsArchive {
        ...
        province_code: extract_province_code_from_cid(seal.cid_number.as_str()),
        city_code: extract_city_code_from_cid(seal.cid_number.as_str()),
...
fn validate_geo_seal_against_site(
    ...
    if seal.cid_number != site.cid_number {
        return Err((
            StatusCode::UNPROCESSABLE_ENTITY,
            2004,
...
fn build_install_sign_source(
    ...
    format!(
        "cid-cpms-v1|install|{}|{}|{}|{}",
        cid_number, province_name, city_name, install_secret_hash
    )
}
...
pub(super) fn extract_province_code_from_cid(cid_number: &str) -> String {
    let segments: Vec<&str> = cid_number.split('-').collect();
...
pub(super) fn extract_city_code_from_cid(cid_number: &str) -> String {
    let segments: Vec<&str> = cid_number.split('-').collect();
