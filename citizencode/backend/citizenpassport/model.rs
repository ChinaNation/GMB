...
pub(crate) struct CpmsSiteKeys {
    /// CPMS 机构的 CID 号,内部结构和外部协议统一使用 cid_number。
...
pub(crate) struct GenerateCpmsInstallInput {
    /// 机构自身 cid_number,作为 cpms_sites 写入键(= 详情页读取键)。
    pub(crate) cid_number: String,
    /// province_name/city_name/institution 仅用于安全授权(passkey grant)绑定,机构真源按 cid_number 反查。
