//! CPMS 年度报告导入。
//!
//! 中文注释：CPMS 是档案号、钱包、公民状态和投票资格的真源；CID 是身份 ID
//! 的真源。本文件只按档案号覆盖已有 CID 绑定，不自动生成新的身份 ID。
...
const EXPORT_PROTO: &str = "CID_CPMS_V1";
...
pub(crate) async fn admin_import_cpms_status_export(
    ...
    let grant_payload = serde_json::json!({
        "target": file.cid_number.clone(),
        "cid_number": file.cid_number.clone(),
    ...
    if let Some(existing_hash) = match state
        .db
        .get_cpms_status_import_hash(&file.cid_number, file.export_year)
    {
        Ok(v) => v,
        Err(err) => {
...
async fn validate_cpms_status_export(
    ...
    let site = state
        .db
        .get_cpms_site(file.cid_number.as_str())
...
fn build_status_export_sign_source(
    ...
    format!(
        "cid-cpms-v1|cpms-status-export|{}|{}|{}|{}|{}",
        cid_number, cpms_pubkey, export_batch_id, exported_at, records_hash
    )
}
            ...
            let row = conn
                .query_opt(
                    "SELECT records_hash
                     ...
                     WHERE cid_number = $1 AND export_year = $2",
                    &[&cid_number, &export_year],
                )
        ...
        self.with_client(move |conn| {
            conn.execute(
                "INSERT INTO citizen_status_imports (
                    cid_number, export_year, export_batch_id, records_hash,
...
mod tests {
    ...
    fn sample_file() -> CpmsStatusExportFile {
        let mut file = CpmsStatusExportFile {
            ...
            export_year: 2026,
            cid_number: "GD001-GZF06-123456789-2026".to_string(),
    ...
    #[test]
    fn normal_voting_citizen_can_remain_in_cid() {
    ...
    #[test]
    fn ineligible_citizen_must_be_deleted_from_cid() {
        let record = CpmsCitizenBindingRecord {
