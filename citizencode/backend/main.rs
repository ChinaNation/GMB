...
impl Db {
    ...
    pub(crate) fn delete_citizen_binding_record(
        ...
        let cid_number = record
            .cid_number
            .as_deref()
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .map(str::to_string)
            .ok_or_else(|| "citizen cid_number is required for delete".to_string())?;
        ...
        self.with_client(move |conn| {
            ...
            // 中文注释:CID 公民库只保留可投票公民;年度报告判定失格时同步删除索引行。
            tx.execute(
                "DELETE FROM citizens WHERE province_code = $1 AND cid_number = $2",
                &[&province_code, &cid_number],
            )
            .map_err(|e| format!("delete citizen row failed: {e}"))?;
            tx.execute(
                "DELETE FROM subjects
                 WHERE province_code = $1 AND cid_number = $2 AND kind = 'CITIZEN'",
                &[&province_code, &cid_number],
            )
            .map_err(|e| format!("delete citizen subject failed: {e}"))?;
            tx.execute(
                "DELETE FROM ids WHERE cid_number = $1 AND kind = 'CITIZEN'",
                &[&cid_number],
            )
    ...
    fn upsert_target_citizen_rows(
        ...
        conn.execute(
            "INSERT INTO citizens (
                cid_number, province_code, city_code, id, archive_no, wallet_pubkey, wallet_address,
            ...
            let rows = conn
                .query(
                    "SELECT COALESCE(c.id, 0), c.wallet_pubkey, c.wallet_address,
                                    c.archive_no, c.cid_number, c.citizen_status,
                             ...
                             WHERE c.bind_status = 'BOUND'
                               ...
                               AND (
                                    c.archive_no = $3 OR c.cid_number = $3
    ...
    pub(crate) fn purge_legacy_cid_rows(&self, dry_run: bool) -> Result<PurgeReport, String> {
        ...
        self.with_client(move |conn| {
            ...
            for row in conn
                .query("SELECT DISTINCT cid_number FROM cpms_sites", &[])
		                    ...
		                    "SELECT s.cid_number, s.name, s.category,
	                             ...
	                             JOIN gov g ON g.province_code = s.province_code AND g.cid_number = s.cid_number
	                                ...
	                                GROUP BY province_code, cid_number
		                             ...
		                             LEFT JOIN admins a ON lower(a.admin_account) = lower(s.created_by)
		                             LEFT JOIN cpms_sites cs ON cs.cid_number = s.cid_number
	                             WHERE s.kind = 'PUBLIC'
	                               ...
	                               AND s.province_code = $1
	                               AND ($2::text IS NULL OR s.city_code = $2)
	                             ORDER BY s.city_code ASC NULLS LAST, s.cid_number ASC
	                             LIMIT $3 OFFSET $4",
...
fn resolve_backend_bind_addr() -> Result<SocketAddr, String> {
    let raw = std::env::var("CID_BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:8899".to_string());
...
fn parse_backend_command() -> BackendCommand {
    ...
    match command {
        ...
        other => panic!("unknown citizencode-backend command: {other}"),
    }
}
...
fn ensure_gov_catalog_current_for_serve(state: &AppState) -> Result<(), String> {
    ...
    if !env_flag_enabled("CID_GOV_AUTO_RECONCILE") {
        return Err(
            "CID 公权机构目录已落后于当前 china.sqlite；请先执行 `citizencode-backend reconcile-gov --changed-only` 和 `citizencode-backend check-gov --strict`，或在本地开发显式设置 CID_GOV_AUTO_RECONCILE=1 后再启动。"
                .to_string(),
        );
    }
...
// 中文注释:链节点是 CID 的外部联调依赖,不能阻塞后端、管理员和 CPMS 基础服务启动。
...
fn main() {
    ...
    let redis_url = required_env("CID_REDIS_URL");
    let redis_client = RedisClient::open(redis_url.as_str())
        .unwrap_or_else(|e| panic!("invalid CID_REDIS_URL: {e}"));

    // 中文注释:CID 是联邦注册局运维的中心化独立系统,启动阶段只校验本地
    // 主签名密钥格式;链端签发机构/主账户/签名公钥只在链交互路径按需校验。
    {
        let seed_hex = required_env("CID_SIGNING_SEED_HEX");
    ...
    runtime.block_on(async move {
        ...
        let admin_routes = Router::new()
            ...
            .route(
                "/api/v1/admin/cpms-keys/by-institution/:cid_number",
                get(cpms::get_cpms_site_by_institution),
            )
            .route(
                "/api/v1/admin/cpms-keys/cid/generate",
            ...
            .route(
                "/api/v1/admin/cpms-keys/:cid_number",
                delete(cpms::delete_cpms_keys),
            )
            .route(
                "/api/v1/admin/cpms-keys/:cid_number/revoke-token",
                post(cpms::revoke_install_token),
            )
            .route(
                "/api/v1/admin/cpms-keys/:cid_number/reissue",
                post(cpms::reissue_install_token),
            )
            .route(
                "/api/v1/admin/cpms-keys/:cid_number/disable",
                put(cpms::disable_cpms_keys),
            )
            .route(
                "/api/v1/admin/cpms-keys/:cid_number/enable",
        ...
        // 生产环境在启动期强制校验为 cid.crcfrcn.com。
        admins::passkeys::validate_passkey_configuration()
            .unwrap_or_else(|e| panic!("invalid CID Passkey configuration: {e}"));
        ...
        info!("citizencode-backend listening on http://{}", addr);
...
fn cid_error_code(status: StatusCode, message: &str) -> &'static str {
    ...
    match message {
        ...
        "invalid signature hex" => "CID_BIND_SIGNATURE_FORMAT_INVALID",
        "archive_no already bound" => "CID_BIND_ARCHIVE_ALREADY_BOUND",
        "archive_no immutable after binding" => "CID_BIND_ARCHIVE_IMMUTABLE",
        "wallet_pubkey already bound" => "CID_BIND_WALLET_ALREADY_BOUND",
        "archive signature invalid" => "CID_CITIZEN_ARCHIVE_SIGNATURE_BAD",
        "geo_seal cannot be decrypted" => "CID_CITIZEN_ARCHIVE_GEO_SEAL_INVALID",
        "geo_seal install scope mismatch" => "CID_CITIZEN_ARCHIVE_SCOPE_MISMATCH",
        "cpms_pubkey does not match installed CPMS" => "CID_CITIZEN_ARCHIVE_PUBKEY_MISMATCH",
        "qr expired" => "CID_CITIZEN_QR_EXPIRED",
        "qr header invalid" => "CID_CITIZEN_QR_HEADER_INVALID",
        ...
        "federal admin province limit reached" => {
            "CID_ADMIN_FEDERAL_REGISTRY_PROVINCE_LIMIT_REACHED"
        }
        "city admin city limit reached" => "CID_ADMIN_CITY_REGISTRY_CITY_LIMIT_REACHED",
        "passkey required" => "CID_ADMIN_PASSKEY_REQUIRED",
