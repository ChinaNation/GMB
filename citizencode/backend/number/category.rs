...
//! 主体属性与机构分类枚举 — CID 编号协议和机构目录共用的基础分类。
//!
//! 中文注释:
//! 新版 CID 编号把主体属性放入第二段 `K3P1C1` 的首字符 `K1`。这里集中维护
//! `K1` 的合法值,并把所有注册型多签机构按业务域分为 3 类,便于前端按 tab
//! 过滤和按角色授权。分类规则由 `classify(subject_property, code, cid_full_name)` 决定:
//!
//! - PublicSecurity     公安局      公法人 + ZF + cid_full_name 以"公安局"结尾
...
/// 按 (subject_property, code, cid_full_name) 三元组决定机构分类。
/// 规则优先级:公安局 > 公权机构 > 私权机构。
///
    ...
    match subject_property {
        SubjectProperty::G => {
            ...
            if code == InstitutionCode::ZF
                && cid_full_name.ends_with(PUBLIC_SECURITY_INSTITUTION_SUFFIX)
            {
