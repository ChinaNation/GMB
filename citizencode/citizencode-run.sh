...
CID_BIND_ADDR="${CID_BIND_ADDR:-127.0.0.1:8899}"
CID_HEALTHCHECK_URL="${CID_HEALTHCHECK_URL:-http://${CID_BIND_ADDR}/api/v1/health}"
CID_PORT="${CID_BIND_ADDR##*:}"
CID_FRONTEND_PORT="${CID_FRONTEND_PORT:-5179}"
CID_LAUNCHD_LABEL="${CID_LAUNCHD_LABEL:-com.gmb.citizencode-backend}"
...
stop_launchd_backend() {
  ...
  local domain="gui/$(id -u)/${CID_LAUNCHD_LABEL}"
  if launchctl print "$domain" >/dev/null 2>&1; then
    echo "Stopping launchd service ${CID_LAUNCHD_LABEL}..."
    # 中文注释:本机曾通过 launchctl submit 启动同端口后端,只 kill PID 会被 launchd 立刻拉起。
    launchctl bootout "$domain" >/dev/null 2>&1 \
      || launchctl remove "$CID_LAUNCHD_LABEL" >/dev/null 2>&1 \
      || true
    sleep 1
  fi
}
...
check_database_ready() {
  if ! command -v pg_isready >/dev/null 2>&1; then
    echo "Missing pg_isready; install PostgreSQL client tools before starting CID."
    return 1
  fi
  ...
  local pgdata="${CID_POSTGRES_DATA_DIR:-/opt/homebrew/var/postgresql@17}"
...
launch_backend() {
  ...
  (
    ...
    cleanup_child() {
      ...
      stop_port_listeners "$CID_PORT" "backend" >/dev/null || true
    }
    trap '' HUP
    trap cleanup_child EXIT INT TERM
    "$ROOT_DIR/backend/target/debug/citizencode-backend" &
    child_pid="$!"
...
launch_frontend() {
  ...
  (
    ...
    cleanup_child() {
      ...
      stop_port_listeners "$CID_FRONTEND_PORT" "frontend" >/dev/null || true
    }
    trap '' HUP
    trap cleanup_child EXIT INT TERM
    npm run dev &
...
stop_launchd_backend
stop_port_listeners "$CID_FRONTEND_PORT" "frontend"
stop_port_listeners "$CID_PORT" "backend"
check_database_ready
...
"$ROOT_DIR/backend/target/debug/citizencode-backend" ensure-gov
launch_backend
...
wait_backend_ready() {
  ...
  for ((i=1; i<=retries; i++)); do
    ...
    if curl -fsS "$CID_HEALTHCHECK_URL" >/dev/null 2>&1; then
      return 0
    fi
    sleep 1
  done
  echo "Backend did not become ready on ${CID_HEALTHCHECK_URL} within ${retries}s"
  return 1
}
