...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "请使用 root 或 sudo 运行 update_citizencode_app.sh"
  exit 1
fi

RELEASE_ID="${1:-}"
BUNDLE_DIR="${2:-}"
APP_HOME="${APP_HOME:-/opt/citizencode}"
FRONTEND_ROOT="${FRONTEND_ROOT:-/var/www/cid}"
SERVICE_NAME="${SERVICE_NAME:-citizencode-backend}"
ENV_FILE="${ENV_FILE:-/etc/citizencode/citizencode.env}"
...
if [[ -z "${RELEASE_ID}" || -z "${BUNDLE_DIR}" ]]; then
  echo "用法: update_citizencode_app.sh <release_id> <bundle_dir>"
  exit 1
fi

if [[ ! -x "${BUNDLE_DIR}/backend/citizencode-backend" ]]; then
  echo "缺少后端二进制: ${BUNDLE_DIR}/backend/citizencode-backend"
  exit 1
fi
...
install -m 755 "${BUNDLE_DIR}/backend/citizencode-backend" \
  "${APP_HOME}/releases/${RELEASE_ID}/backend/citizencode-backend"
install -m 755 "${BUNDLE_DIR}/backend/citizencode-backend" \
  "${APP_HOME}/bin/citizencode-backend"
install -m 755 "${BUNDLE_DIR}/deploy/update_citizencode_app.sh" \
  "${APP_HOME}/scripts/update_citizencode_app.sh"
