...
BACKUP_DIR="/var/backups/citizencode"
DATE="$(date +%F_%H%M%S)"
DB_URL="${DATABASE_URL:-}"
STANDBY_USER="backup"
STANDBY_IP="10.0.0.22"
STANDBY_DIR="/data/cid-backups"
BACKUP_PASSPHRASE="${CID_BACKUP_PASSPHRASE:-}"
...
if [[ -z "${BACKUP_PASSPHRASE}" ]]; then
  echo "请先配置 CID_BACKUP_PASSPHRASE"
  exit 1
fi

mkdir -p "${BACKUP_DIR}"
RAW_FILE="${BACKUP_DIR}/cid_${DATE}.dump"
...
# 本地保留 14 天
find "${BACKUP_DIR}" -name 'cid_*.dump.gpg' -mtime +14 -delete
