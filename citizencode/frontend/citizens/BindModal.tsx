...
// 扫 CPMS ARCHIVE 档案码 -> 展示 citizenapp sign_request -> 扫 sign_response -> 提交 CID。
...
interface BindModalProps {
  ...
  onClose: () => void;
  onBound: (boundCidNumber?: string) => Promise<void> | void;
}

function bindErrorMessage(err: unknown): string {
  if (err instanceof ApiError) {
    switch (err.errorCode) {
      case 'CID_BIND_CHALLENGE_EXPIRED':
        return '签名请求已过期，请重新扫描档案码。';
      case 'CID_BIND_SIGNATURE_VERIFY_FAILED':
        return '签名校验失败，请确认使用档案码绑定的投票账户签名。';
      case 'CID_BIND_WALLET_MISMATCH':
        return '签名账户与本次投票账户不一致。';
      case 'CID_BIND_ARCHIVE_ALREADY_BOUND':
        return '该档案已完成电子护照绑定。';
      case 'CID_BIND_ARCHIVE_IMMUTABLE':
        return '档案号已和对应身份ID永久绑定，更换绑定必须扫描同一档案号的档案码。';
      case 'CID_BIND_WALLET_ALREADY_BOUND':
...
export function BindModal({ auth, open, record, onClose, onBound }: BindModalProps) {
  ...
  const onScanBindSignature = async (raw: string) => {
    ...
    try {
      ...
      onClose();
      await onBound(result.cid_number);
    } catch (err) {
      notice.error(bindErrorMessage(err));
    } finally {
      setArchiveCodeScanLoading(false);
        ...
        <>
          {record && (
            <Descriptions column={1} size="small" bordered style={{ marginBottom: 12 }}>
              ...
              <Descriptions.Item label="身份ID">{record.cid_number ?? '-'}</Descriptions.Item>
