...
import {
  ...
  type CpmsSiteRow,
} from '../citizenpassport/api';
import type { AdminAuth } from '../auth/types';
import { AccountList } from '../accounts/AccountList';
import { notice } from '../utils/notice';
import { CitizenPassportSitePanel } from '../citizenpassport/CitizenPassportSitePanel';
...
export const GovDetailPage: React.FC<Props> = ({ auth, cidNumber, canWrite, onBack, backLabel, loadDetail, adminListSection }) => {
  ...
  const loadCpms = useCallback(
    (instCidNumber: string) => {
      getCpmsSiteByInstitution(auth, instCidNumber)
  ...
  useEffect(() => {
    if (inst && inst.category === 'PUBLIC_SECURITY') {
      loadCpms(inst.cid_number);
    } else {
      setCpmsSite(null);
    }
  }, [inst?.cid_number, inst?.category, loadCpms]);
  ...
  const onGenerateCpms = async () => {
    ...
    try {
      ...
      // 生成请求额外带机构自身 cid_number，后端以它为写入键（= 详情页读取键），根治再次进入二维码丢失。
      const grantPayload = {
      ...
      const result = await generateCpmsInstallQr(
        auth,
        { ...grantPayload, cid_number: inst.cid_number },
        grant,
      );
      setCpmsSite({
        cid_number: result.cid_number,
      ...
      >
        <Row gutter={24}>
          ...
          {inst.category === 'PUBLIC_SECURITY' && cpmsSite && (
            <Col xs={24} md={12}>
              <CitizenPassportSitePanel
                auth={auth}
                site={cpmsSite}
                canWrite={canManageCpms}
                onChanged={() => loadCpms(inst.cid_number)}
              />
            </Col>
          )}
        </Row>
