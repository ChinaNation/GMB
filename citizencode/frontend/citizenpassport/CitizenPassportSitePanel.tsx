...
export const CitizenPassportSitePanel: React.FC<Props> = ({ auth, site, canWrite, onChanged }) => {
  ...
  const onReissue = async () => {
    setBusy(true);
    try {
      const grant = await runPasskeyChallengeAction('CPMS_REISSUE_INSTALL_TOKEN', { target: site.cid_number });
      await reissueInstallToken(auth, site.cid_number, grant);
  ...
  const onDisable = async () => {
    ...
    try {
      const grant = await runPasskeyChallengeAction('CPMS_DISABLE_KEYS', {
        target: site.cid_number,
        reason: normalizedReason,
      });
      await disableCpmsKeys(auth, site.cid_number, normalizedReason || undefined, grant);
      notice.success('已禁用');
      onChanged();
    } catch (err) {
      notice.error(err, '');
  ...
  const onEnable = async () => {
    ...
    try {
      ...
      await enableCpmsKeys(auth, site.cid_number, grant);
      notice.success('已启用');
      onChanged();
    } catch (err) {
      notice.error(err, '');
  ...
  const onRevoke = async () => {
    ...
    try {
      ...
      await revokeCpmsKeys(auth, site.cid_number, normalizedReason || undefined, grant);
      notice.success('已吊销');
      onChanged();
    } catch (err) {
      notice.error(err, '');
  ...
  const onDownload = () => {
    ...
    downloadQr({
      ...
      filename: `cpms-${label}-${site.cid_number}`,
