...
pub(crate) fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/v1/address/villages",
            get(list_villages).post(create_village),
        )
...
async fn create_town(
    ...
    if code.is_empty() || name.is_empty() {
        return Err(err(
            ...
            1001,
            "town_code and town_name required",
        ));
    }
...
async fn create_village(
    ...
    if town_code.is_empty() || name.is_empty() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            1001,
            "town_code and village_name required",
        ));
    }
    // 自动生成 village_id: town_code + 递增序号
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM address_villages WHERE town_code = $1")
            .bind(&town_code)
            .fetch_one(&state.db)
            .await
            .map_err(|_| {
    ...
    .map_err(|_| {
        err(
            StatusCode::INTERNAL_SERVER_ERROR,
            5001,
            "insert village failed",
        )
    })?;
    Ok(Json(ok(VillageRow {
        village_id: vid,
        town_code,
        village_name: name,
    })))
