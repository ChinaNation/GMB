...
/// 处理 SFID 签发的 INSTALL 安装码。
///
/// 中文注释：第 2 步不再生成中间注册码。CPMS 安装时一次性写入
/// `sfid_number / province_name / city_name / install_secret / sig`，同时生成本机 ARCHIVE 签名密钥。
...
async fn initialize_install(
    ...
    let existing_sfid: Option<String> =
        sqlx::query_scalar("SELECT sfid_number FROM system_install WHERE id = 1 FOR UPDATE")
    ...
    .bind(qr_payload.sfid_number.trim())
    ...
    write_audit(
        ...
        "INSTALL_INITIALIZE",
        "CPMS_INSTALL",
        Some(qr_payload.sfid_number.clone()),
        "SUCCESS",
        serde_json::json!({
            "sfid_number": qr_payload.sfid_number,
...
fn validate_sfid_install_qr(payload: &SfidInstallQrPayload) -> Result<(), String> {
    ...
    if payload.sfid_number.trim().is_empty()
...
mod tests {
    ...
    fn install_payload() -> SfidInstallQrPayload {
