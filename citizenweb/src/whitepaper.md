...
* 公民提供隐私优先的即时通信能力；私密聊天不上链、不依赖 CID，消息使用 OpenMLS 端到端加密，附件通过 WebRTC DataChannel 在设备间传输。消息、会话、联系人关系和附件只保存在通信双方设备；Cloudflare 不保存任何聊天内容，只提供当前请求内的瞬时信封/信令转发、无内容推送、一次性 KeyPackage 和短期 TURN 凭证。<br><span class="whitepaper-en">CitizenApp provides privacy-first instant messaging. Private chat is not placed on chain and does not depend on CID; messages use OpenMLS end-to-end encryption, while attachments travel device to device over WebRTC DataChannel. Messages, conversations, contact relationships, and attachments remain only on the communicating devices. Cloudflare stores no chat content and provides only transient per-request envelope and signaling relay, content-free wake notifications, one-time KeyPackages, and short-lived TURN credentials.</span>
