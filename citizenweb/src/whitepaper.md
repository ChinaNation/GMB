...
* 公民提供隐私优先的即时通信能力，采用 Cloudflare 密文 mailbox 和手机近场通信双模式实现；私聊和群聊不上链，不依赖 CID，消息内容必须使用 OpenMLS 端到端加密。Cloudflare 只保存密文 envelope、加密附件和必要投递元数据，不保存聊天明文，不托管钱包私钥或 Chat MLS 设备私钥。<br><span class="whitepaper-en">CitizenApp provides privacy-first instant messaging through a dual mode of Cloudflare encrypted mailbox and mobile near-field communication. Private and group chats shall not be placed on chain, shall not depend on CID, and message content must use OpenMLS end-to-end encryption. Cloudflare stores only encrypted envelopes, encrypted attachments, and necessary delivery metadata; it does not store plaintext chat content or custody wallet private keys or Chat MLS device private keys.</span>
* 公民的聊天账户使用钱包地址作为用户可见账号；Chat 设备密钥与钱包账户分层。创建钱包时由钱包主密钥一次性绑定 P-256 设备子密钥；此后聊天设备绑定和会话登录只使用受生物识别保护的 P-256 设备子密钥，钱包主密钥不再参与，也不用于 OpenMLS 消息加密或交给 Cloudflare mailbox。近场通信传输同一套密文消息，用于手机之间的近距离通信。<br><span class="whitepaper-en">CitizenApp uses the wallet address as the user-visible chat account. Chat device keys are separated from wallet accounts. When a wallet is created, its main key binds the P-256 device subkey once; subsequent Chat device binding and session login use only the biometrically protected P-256 device subkey. The wallet main key no longer participates, is not used for OpenMLS message encryption, and is never handed to the Cloudflare mailbox. Near-field communication transmits the same encrypted messages for short-range phone-to-phone communication.</span>
