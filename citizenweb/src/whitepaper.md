...
* 国家名称与公民主义：中华民族联邦共和国国家名称是基于中华各民族悠久历史与璀璨文化的沉淀，全称为：中华民族联邦共和国，简称为：中华联邦；中华民族联邦共和国是致力于推行“公民主义”的———「公民治理国家（民治）、实行民主共和（民主）、保障公民权利（民权）、建设民生社会（民生）、复兴民族文化（民族）」———联邦制共和国。<br><span class="whitepaper-en">State name and Citizenism: the state name Federal Republic of the China Nation is rooted in the long history and splendid cultures of the various ethnic groups of the China Nation. Its full name is the Federal Republic of the China Nation, and its abbreviated name is the China Federation. The Federal Republic of the China Nation is a federal republic committed to advancing “Citizenism”: the governance of the State by citizens (mínzhì), the practice of democratic republicanism (mínzhǔ), the protection of citizens’ rights (mínquán), the building of a society of citizens’ livelihood (mínshēng), and the revival of ethnic cultures (mínzú).</span>
...
* 公民提供隐私优先的即时通信能力；私密聊天不上链、不依赖 CID，消息使用 OpenMLS 端到端加密，附件通过 WebRTC DataChannel 在设备间直连传输。消息、会话、联系人关系和附件只保存在通信双方设备；Cloudflare 不保存任何聊天内容，只提供当前请求内的瞬时信封/信令转发、无内容推送和一次性 KeyPackage。公共 STUN 只用于发现候选地址，附件不经过云端中继。<br><span class="whitepaper-en">CitizenApp provides privacy-first instant messaging. Private chat is not placed on chain and does not depend on CID; messages use OpenMLS end-to-end encryption, while attachments travel directly between devices over WebRTC DataChannel. Messages, conversations, contact relationships, and attachments remain only on the communicating devices. Cloudflare stores no chat content and provides only transient per-request envelope and signaling forwarding, content-free wake notifications, and one-time KeyPackages. Public STUN is used only for candidate discovery; attachment bytes never pass through a cloud relay.</span>
* 公民的聊天账户使用钱包地址作为用户可见账号；Chat 设备密钥与钱包账户分层。创建钱包时由钱包主密钥一次性绑定硬件 P-256 设备子钥；此后聊天设备绑定和会话登录只使用该设备子钥静默签名，钱包主密钥不再参与，也不用于 OpenMLS 消息加密或交给任何边缘服务。接收设备不可达时，密文只保留在发送设备本地队列，由通用推送唤醒两端后自动重试。<br><span class="whitepaper-en">CitizenApp uses the wallet address as the user-visible chat account, while Chat device keys remain separate from wallet accounts. When a wallet is created, its main key binds a hardware-backed P-256 device subkey once. Subsequent Chat device binding and session login use silent signatures from that device subkey; the wallet main key no longer participates, is not used for OpenMLS message encryption, and is never handed to any edge service. When a receiving device is unavailable, ciphertext remains only in the sender's local queue and retries automatically after content-free wake notifications bring the devices online.</span>
