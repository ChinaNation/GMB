...
use super::types::{
    AccountWithBalance, ClearingBankNodeOnChainInfo, InstitutionDetail, InstitutionProposalPage,
};
...
pub fn fetch_institution_detail(sfid_id: &str) -> Result<Option<InstitutionDetail>, String> {
    ...
    let parent_sfid_id = match inst.parent_sfid_id {
        Some(v) => Some(
            String::from_utf8(v.into_inner())
                .map_err(|_| "parent_sfid_id 非 UTF-8".to_string())?,
        ),
        None => None,
    };
...
fn decode_account_name_from_key(full_key_hex: &str, sfid_prefix_hex: &str) -> Option<String> {
    ...
    let prefix = sfid_prefix_hex.strip_prefix("0x").unwrap_or(sfid_prefix_hex);
