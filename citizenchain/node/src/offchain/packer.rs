...
//!       `offchain_keystore::SigningKey` 后实现 `KeystoreBatchSigner`。
...
//!   的 `event_listener` 收到链上 `PaymentSettled` 事件时调用。
...
use super::ledger::{OffchainLedger, PendingPayment};
...
/// 批次签名消息的域分隔符,Step 2b-ii 新增。Step 2b-ii-β 接清算行 keystore 时
/// 使用同一前缀。runtime 端 `settlement.rs` 目前不校验 batch_signature,但
/// 我们仍按正确协议拼接消息,为 Step 2b-iv 链上校验做好准备。
...
///
/// Step 2b-ii-α:`NoopBatchSigner` 占位;Step 2b-ii-β 接入 `offchain_keystore`
/// 里存的清算行管理员私钥。
...
pub struct OffchainPacker {
    ...
    /// 清算行本地 batch_seq 递增计数(内存,Step 3 持久化)。
...
impl OffchainPacker {
    ...
    pub fn new(
        ...
        Self {
            ...
            batch_seq_counter: Arc::new(RwLock::new(0)),
        }
    }
    ...
    /// 的 `event_listener::on_payment_settled` 收到链上事件时完成。
...
///
/// Step 2b-iv 链上 `settlement::execute_clearing_bank_batch` 接入 batch_signature
/// 校验时,必须与本函数产生的消息**逐字节一致**。
...
/// 节点层 blake2b-256 包装(与 `ledger.rs::blake2_256` 用同一依赖 `blake2b_simd`)。
fn blake2_simd(data: &[u8]) -> [u8; 32] {
