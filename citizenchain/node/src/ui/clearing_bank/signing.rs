...
pub fn build_register_call_data(
    ...
    let mut call = Vec::with_capacity(2 + 1 + sfid_id.len() + 1 + peer_id.len() + 1 + rpc_domain.len() + 2);
...
pub fn build_register_sign_request(
    ...
    let summary = format!(
        "声明清算行节点 {sfid_id} @ {rpc_domain}:{rpc_port}"
    );
...
mod tests {
    ...
    fn register_call_rejects_empty_sfid() {
        let err = build_register_call_data("", "12D3KooWaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "x.com", 9944)
            .unwrap_err();
    ...
    fn parse_pubkey_normalizes_uppercase_and_strips_prefix() {
        let (clean, bytes) = parse_pubkey(
            "0xAABBCCDDEEFF00112233445566778899AABBCCDDEEFF00112233445566778899",
        )
        .unwrap();
        assert_eq!(clean.len(), 64);
        assert!(clean.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit()));
