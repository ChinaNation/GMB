...
// `offchain_keystore::OffchainKeystore` 加载到 `KeystoreBatchSigner` 的
...
pub fn verify_and_decrypt_admin(input: VerifyDecryptAdminInput) -> Result<DecryptedAdminInfo, String> {
    let response: QrSignResponse = serde_json::from_str(&input.response_json)
