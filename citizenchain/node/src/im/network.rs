...
pub(crate) fn handle_incoming_request(request: IncomingRequest) {
    let response = handle_payload(&request.payload, super::commands::mailbox());
...
fn handle_payload(payload: &[u8], mailbox: &Mutex<ImMailbox>) -> ImNetworkResponse {
...
///
/// 本函数不做 DHT、rendezvous 或 relay；它只把联系人包里的显式 multiaddr 写入
...
pub(crate) async fn send_direct_delivery(
    ...
    let (peer_id, multiaddr) =
        sc_network::config::parse_str_addr(&delivery.remote_endpoint.multiaddr)
            .map_err(|e| format!("IM 直连端点解析失败: {e}"))?;
    if peer_id.to_string() != delivery.remote_endpoint.peer_id {
        return Err("IM 直连端点 PeerId 与 multiaddr 不一致".to_string());
    }

    network.add_known_address(peer_id, multiaddr);

    let request = ImNetworkRequest::SubmitEnvelope(delivery.submit).encode_wire()?;
...
mod tests {
    ...
    fn response_wire_round_trip() {
        let response = ImNetworkResponse::Error("拒绝第三方 mailbox".to_string());
