...
import type { ClearingBankNodeOnChainInfo } from './clearing-bank-types';
...
export function ClearingBankNodeInfoPanel({ info, sfidId, admins, onChanged, onUnregistered }: Props) {
  ...
  const startUpdate = async () => {
    ...
    try {
      const r = await api.buildUpdateClearingBankEndpointRequest(
  ...
  const startUnregister = async () => {
    ...
    setError(null); setStep('qr'); setCountdown(90);
    try {
      const r = await api.buildUnregisterClearingBankRequest(selectedAdmin.pubkeyHex, sfidId);
      setSignRequest(r);
    } catch (e) { setError(sanitizeError(e)); setStep('error'); }
  };
  ...
  const handleScan = async (responseJson: string) => {
    ...
    try {
      let r;
      if (mode === 'update') {
        r = await api.submitUpdateClearingBankEndpoint(
        ...
        r = await api.submitUnregisterClearingBank(
          sr.requestId, selectedAdmin.pubkeyHex, sr.expectedPayloadHash,
...
// 让 lint 不抱怨未用 import
export const _formatHex = hexToSs58;
