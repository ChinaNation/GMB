...
import type { ConnectivityTestReport } from './clearing-bank-types';
...
export function ClearingBankDeclareNodePage({ sfidId, institutionName, onBack, onSuccess }: Props) {
  ...
  const runConnectivityTest = useCallback(async () => {
    ...
    setStep('testing');
    try {
      const r = await api.testClearingBankEndpointConnectivity(rpcDomain.trim(), portNum, peerId);
      setReport(r);
      setStep('tested');
    } catch (e) {
  ...
  const startSign = useCallback(async () => {
    ...
    setCountdown(90);
    try {
      const r = await api.buildRegisterClearingBankRequest(
  ...
  const handleScan = useCallback(async (responseJson: string) => {
    ...
    setStep('submit');
    try {
      const r = await api.submitRegisterClearingBank(
        sr.requestId,
        selectedAdmin.pubkeyHex,
