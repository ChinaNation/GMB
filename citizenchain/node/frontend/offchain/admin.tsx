...
export function ClearingBankAdminListPage({ shenfenId, onBack }: Props) {
  ...
  const refresh = useCallback(() => {
    ...
    Promise.all([
      ...
      offchainApi.listDecryptedAdmins(shenfenId).catch(() => [] as DecryptedAdminInfo[]),
    ])
      .then(([d, aa, dd]) => {
        setDetail(d);
        setActivated(aa);
  ...
  const startDecrypt = useCallback(async (pubkey: string) => {
    ...
    try {
      const r = await offchainApi.buildDecryptAdminRequest(pubkey, shenfenId);
  ...
  const handleDecryptScan = useCallback(async (responseJson: string) => {
    ...
    try {
      await offchainApi.verifyAndDecryptAdmin(
  ...
  const lockAdmin = useCallback(async (pubkeyHex: string) => {
    try {
      await offchainApi.lockDecryptedAdmin(pubkeyHex);
      refresh();
    } catch (e) {
