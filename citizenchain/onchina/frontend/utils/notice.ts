...
function translateErrorCode(code: string): string | null {
  const map: Record<string, string> = {
    ...
    ONCHINA_LOGIN_USER_CONTACT_INVALID: '管理员用户码无效，请使用当前 QR_V1 用户码重新扫描',
    ...
    ONCHINA_LOGIN_CHALLENGE_NOT_FOUND: '登录二维码不存在或已失效，请重新扫描管理员用户码',
    ONCHINA_LOGIN_CHALLENGE_CONSUMED: '登录二维码已使用，请重新扫描管理员用户码',
    ONCHINA_LOGIN_SESSION_MISMATCH: '登录会话不匹配，请关闭多余页面后重新扫描管理员用户码',
    ONCHINA_LOGIN_CHALLENGE_EXPIRED: '登录二维码已过期，请重新扫描管理员用户码',
...
const KNOWN_ENGLISH_MESSAGES: Record<string, string> = {
  ...
  'sign request not found': '登录二维码不存在或已失效，请重新扫描管理员用户码',
  'sign request already consumed': '登录二维码已使用，请重新扫描管理员用户码',
  'sign request session mismatch': '登录会话不匹配，请关闭多余页面后重新扫描管理员用户码',
  'sign request expired': '登录二维码已过期，请重新扫描管理员用户码',
  'signer_public_key must match targeted account_id': '签名账户和登录目标账户不一致',
