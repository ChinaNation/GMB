// 中文注释:本级法律列表(只读)。会话派生 scope,调 /api/legislation/laws/mine;
...
function versionLabel(row: LawView): string {
  ...
  if (row.effectiveVersion) {
    parts.push(`生效 v${row.effectiveVersion}`);
  }
  if (row.pendingVersion) {
    parts.push(`待生效 v${row.pendingVersion}`);
  }
  return parts.join(' / ');
}
...
export function LawListTable({ auth, onOpen }: Props) {
  ...
  const columns = useMemo<ColumnsType<LawView>>(
    () => [
      ...
      { title: '表决类型', key: 'voteType', render: (_, row) => voteTypeLabel(row.voteType) },
      { title: '状态', key: 'status', render: (_, row) => statusTag(row.status) },
