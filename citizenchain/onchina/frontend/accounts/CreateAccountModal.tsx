...
//   - 链上注册由区块链软件发起,成功后同步状态回 CID
//   - 链上派生公式由链端按 `account_name` 路由(默认账户/制度账户/自定义账户),
//     cid 前端不做地址预览(避免和链端公式漂移)
//
// 约束:
...
export const CreateAccountModal: React.FC<Props> = ({
  ...
  const { signWithScan, scanSignModal } = useScanSignGrant('账户创建签名确认');
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    setSubmitting(true);
    try {
      await createAccount(auth, cidNumber, name, signWithScan);
