...
pub enum QrKind {
    SignRequest = 1,
    SignResponse = 2,
    /// k=3 用户码(人/永久 CID):由 CitizenApp 用户主页出示,只能写入通讯录与转账。
    /// OnChina 不解析它——登录改由 k=5 钱包码提供目标账户,理由见 qr-protocol-spec.md 第 4 节。
    #[allow(dead_code)]
    UserContact = 3,
    /// k=4 收款码(一笔收款请求):只属于 CitizenApp,Rust 侧不构造也不解析。
