...
const SELECT_COLUMNS: &str = "cid_number, province_code, city_code, admin_account,
    ...
    admin_passkey_credential_id, admin_source_id, admin_profile_status,
    admin_profile_updated_at, created_by, chain_status, chain_tx_hash,
    chain_block_number, operation_log_id, created_at";
...
fn row_to_admin(row: &postgres::Row) -> InstitutionAdmin {
    ...
    let block_number: Option<i64> = row.get(19);
    InstitutionAdmin {
        ...
        admin_profile_status: row.get(14),
        admin_profile_updated_at: row.get(15),
        created_by: row.get(16),
        chain_status: row.get(17),
        chain_tx_hash: row.get(18),
        chain_block_number: block_number.and_then(|v| u64::try_from(v).ok()),
        operation_log_id: row.get(20),
...
pub(crate) fn upsert_institution_admin(db: &Db, admin: &InstitutionAdmin) -> Result<(), String> {
    ...
    db.with_client(move |conn| {
        ...
        let block_number = admin.chain_block_number.and_then(|v| i64::try_from(v).ok());
        conn.execute(
            "INSERT INTO institution_admins (
                ...
                admin_passkey_credential_id, admin_source_id, admin_profile_status,
                admin_profile_updated_at, created_by, chain_status, chain_tx_hash,
                chain_block_number, operation_log_id
             ) VALUES (
             ...
             ON CONFLICT (province_code, cid_number, admin_account) DO UPDATE SET
                ...
                admin_profile_status = EXCLUDED.admin_profile_status,
                admin_profile_updated_at = EXCLUDED.admin_profile_updated_at,
            ...
            &[
                ...
                &admin.admin_profile_status,
                &admin.admin_profile_updated_at,
                &admin.created_by,
                &admin.chain_status,
                &admin.chain_tx_hash,
                &block_number,
