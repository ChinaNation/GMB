//! CitizenApp 电子护照状态查询 handler。
//!
//! 公民由注册局直接录入并发护照；App 侧只查询 CID 已落库的结果。

use axum::{
    extract::{Query, State},
    http::StatusCode,
    response::IntoResponse,
    Json,
};

use crate::crypto::pubkey::ss58_to_pubkey_hex;
use crate::*;

/// CitizenApp 查询电子护照状态（公共接口）。
pub(crate) async fn app_myid_status(
    State(state): State<AppState>,
    Query(params): Query<MyIdStatusQuery>,
) -> impl IntoResponse {
    if params.wallet_address.trim().is_empty() {
        return api_error(StatusCode::BAD_REQUEST, 1001, "wallet_address is required");
    }
    let wallet_pubkey = match ss58_to_pubkey_hex(params.wallet_address.trim()) {
        Some(v) => v,
        None => return api_error(StatusCode::BAD_REQUEST, 1001, "invalid wallet_address"),
    };

    let result = match state.db.find_citizen_by_wallet(&wallet_pubkey) {
        Ok(Some(record)) => MyIdStatusOutput {
            found: true,
            wallet_address: record
                .wallet_address
                .clone()
                .or_else(|| Some(params.wallet_address.trim().to_string())),
            cid_number: Some(record.cid_number.clone()),
            passport_no: Some(record.passport_no.clone()),
            citizen_status: Some(record.citizen_status.clone()),
            voting_eligible: Some(record.voting_eligible),
            vote_status: Some(record.computed_vote_status()),
            identity_status: Some(record.computed_identity_status()),
            passport_valid_from: Some(record.passport_valid_from.clone()),
            passport_valid_until: Some(record.passport_valid_until.clone()),
            status_updated_at: record.status_updated_at,
            residence_province_code: Some(record.residence_province_code.clone()),
            residence_city_code: Some(record.residence_city_code.clone()),
            residence_town_code: Some(record.residence_town_code.clone()),
            birth_province_code: Some(record.birth_province_code.clone()),
            birth_city_code: Some(record.birth_city_code.clone()),
            birth_town_code: Some(record.birth_town_code.clone()),
        },
        Ok(None) => MyIdStatusOutput {
            found: false,
            wallet_address: None,
            cid_number: None,
            passport_no: None,
            citizen_status: None,
            voting_eligible: None,
            vote_status: None,
            identity_status: None,
            passport_valid_from: None,
            passport_valid_until: None,
            status_updated_at: None,
            residence_province_code: None,
            residence_city_code: None,
            residence_town_code: None,
            birth_province_code: None,
            birth_city_code: None,
            birth_town_code: None,
        },
        Err(err) => {
            tracing::error!(error = %err, "query myid status failed");
            return api_error(StatusCode::INTERNAL_SERVER_ERROR, 1004, "myid query failed");
        }
    };
    Json(ApiResponse {
        code: 0,
        message: "ok".to_string(),
        data: result,
    })
    .into_response()
}
