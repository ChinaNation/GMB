...
///
/// 中文注释:联邦注册局(FRG)的 admin_level 虽为 NATIONAL,但其管理员按省分区
/// (federal_registry_scope 映射),故先于 admin_level 特判为省级范围。其余机构按
/// admin_level 派生;私权法人/非法人无层级,暂沿用本市范围。任一档缺必要 scope 字段
/// 返回空范围,不制造伪行政区参与查询。
pub fn get_visible_scope(ctx: &AdminAuthContext) -> VisibleScope {
    if ctx.institution_code == "FRG" {
        return ctx
            .scope_province_name
            .clone()
