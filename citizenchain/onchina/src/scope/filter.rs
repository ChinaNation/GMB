...
pub trait HasProvinceCity {
    ...
    fn city(&self) -> &str;
    /// 镇为空字符串时视为"不限镇";无镇维度的记录类型用默认实现即可。
    fn town(&self) -> &str {
        ""
    }
}

fn in_scope<T: HasProvinceCity>(r: &T, scope: &VisibleScope) -> bool {
    scope.includes_province(r.province())
        && scope.includes_city(r.city())
        && scope.includes_town(r.town())
}
...
mod tests {
    ...
    #[test]
    fn province_scope_only_own_province() {
        let rows = vec![
            row("安徽省", "合肥市"),
            row("安徽省", "芜湖市"),
            row("广东省", "广州市"),
        ];
        let scope = VisibleScope::province("安徽省".to_string());
    ...
    #[test]
    fn city_scope_only_own_city() {
        let rows = vec![
            row("安徽省", "合肥市"),
            row("安徽省", "芜湖市"),
            row("广东省", "广州市"),
        ];
        let scope = VisibleScope::city("安徽省".to_string(), "合肥市".to_string());
        let filtered = filter_by_scope(&rows, &scope);
