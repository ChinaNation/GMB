...
//!
//! 管理员的唯一真源是链上 `admins-change::AdminAccounts`。本模块只在
...
use crate::domains::gov::service::federal_registry_admins;
use crate::AppState;
...
/// 中文注释:按创世常量反查联邦注册局管理员所属省,专用于修复旧库里错误的「全国」省域映射。
/// 这不是授权真源;授权仍以链上 Active 管理员集合为准。
pub(crate) fn federal_registry_bootstrap_province_for_account(
    admin_account: &str,
) -> Option<&'static str> {
    let normalized = normalize_admin_account(admin_account)?;
    let admins = federal_registry_admins()?;
    admins.iter().enumerate().find_map(|(idx, raw)| {
        let candidate = format!("0x{}", hex::encode(raw));
        if candidate.eq_ignore_ascii_case(normalized.as_str()) {
            Some(FEDERAL_ADMIN_PROVINCES[idx / FEDERAL_ADMINS_PER_PROVINCE])
        } else {
            None
        }
    })
    ...
    let seeded = state.db.with_client(move |conn| {
        for (idx, raw) in admins.iter().enumerate() {
            ...
            let admin = AdminUser {
                id,
                admin_account,
                // 中文注释:创世常量只给账户和省域,不伪造真实姓名;显示层统一回退为「联邦注册局管理员」。
                admin_name: String::new(),
                institution_code: "FRG".to_string(),
                built_in: true,
