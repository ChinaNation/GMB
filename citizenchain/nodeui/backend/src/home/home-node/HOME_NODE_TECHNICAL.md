...
- 首页需要展示本机节点是否运行、当前进程 PID 以及运行/停止状态文案。
- 首页需要展示链同步信息，包括当前区块高度、最终确认高度和同步状态。
- 首页需要展示节点身份信息，包括节点名称、P2P 地址（PeerId）和节点角色。
- 用户可以从首页启动本地节点；启动前必须输入并校验设备开机密码。
- 用户可以从首页停止本地节点；停止后界面需要及时反映最新状态。
- 用户可以修改节点名称；保存前必须输入并校验设备开机密码。
- 当节点未运行时，身份与区块信息仍需尽量返回可展示的本地配置，避免首页整块空白。
- 模块需要支撑前端定时刷新，允许单项数据失败时其余数据继续返回，并给出可读错误。
- 模块需要保证节点启停过程安全可控，避免使用未校验的二进制、残留明文临时文件或误报运行状态。
...
9. 启动后补齐奖励地址链上绑定，并校验 GRANDPA 生效。
   - 奖励地址绑定失败按 best-effort 记录日志，不阻断节点运行。
   - GRANDPA 启动后校验失败时，回滚刚拉起的节点进程和临时文件，避免出现“启动失败但节点仍在运行”。
10. 返回最新状态。
...
- 节点启停与退出清理在进程内串行执行，减少并发命令导致的状态错乱和临时文件互删风险。
