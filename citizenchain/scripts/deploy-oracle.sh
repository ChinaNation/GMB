...
NEED_SETUP=false
ssh "$SSH_TARGET" "test -f /etc/systemd/system/$SERVICE_NAME.service" 2>/dev/null || NEED_SETUP=true
...
if [ "$NEED_SETUP" = true ]; then
  ...
  # 上传 systemd 服务文件
  scp "$SCRIPT_DIR/citizenchain-node.service" "$SSH_TARGET:/tmp/$SERVICE_NAME.service"
  ssh "$SSH_TARGET" "
...
echo ""
echo ">>> 上传 node 二进制..."
scp "$NODE_BIN" "$SSH_TARGET:/tmp/node-new"
...
# 检查服务状态
if ssh "$SSH_TARGET" "sudo systemctl is-active --quiet $SERVICE_NAME"; then
  echo "✅ 节点运行中"
else
  echo "❌ 节点启动失败，查看日志："
  ssh "$SSH_TARGET" "sudo journalctl -u $SERVICE_NAME --no-pager -n 20"
  exit 1
fi
