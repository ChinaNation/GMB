...
#
# 启动后:节点挖矿 + 托管注册局 Web(http://127.0.0.1:8899)。
# 注册局登录与节点启动**解耦**:本市注册局管理员用冷钱包扫码、对链上 Active 管理员集合
# 鉴权(3b)即可登录;不是注册局管理员就不用管,也没有任何注册局权限。
set -euo pipefail

cleanup() {
    echo ""
    echo "==> 正在关闭节点 + 注册局 + 内嵌 PG..."
    pkill -f "citizenchain" 2>/dev/null || true
    pkill -f "target/debug/registry" 2>/dev/null || true
...
# ── 注册局(registry)dev 配置 ──
# 中文注释:启动节点**不需要**任何注册局鉴权/身份。这里只让本机能跑起注册局 Web 服务:
#   ① 构建 registry 二进制(节点同目录,registry_proc 自动拉起)+ 前端产物;
#   ② DB 用内嵌私有 PG(方案 A):借本机 PostgreSQL 二进制起一个 registry 专属实例(127.0.0.1)。
# 该市注册局的"系统签名钥 / 机构身份"是可选配置(签登录 QR / 签发凭证才需要),非启动前提。
echo "==> 构建注册局(registry)二进制 + 前端..."
( cd "$REPO_ROOT" && cargo build -p registry )
echo "==> 构建注册局前端产物..."
( cd "$REPO_ROOT/registry/frontend" && if [ ! -d node_modules ]; then npm ci; fi && npm run build )
PG_PREFIX=""
...
if [ -n "$PG_PREFIX" ]; then
    ...
    export CID_PG_DATA_DIR="$HOME/Library/Application Support/gmb.dev/registry-pgdata"
    echo "    内嵌私有 PG:$CID_PG_BIN_DIR(端口 $CID_PG_PORT)"
else
    echo "    [warn] 未找到本机 PostgreSQL(brew install postgresql@16);注册局 Web 仍可起但缺 DB,功能受限。"
fi
export CID_CHINA_DB="$REPO_ROOT/registry/src/cid/china/china.sqlite"
export REGISTRY_FRONTEND_DIST="$REPO_ROOT/registry/frontend/dist"
# 中文注释:本地开发让注册局启动时自动对账公权机构目录(全新内嵌 PG 是空库,
#   首启需把 40 万+ 公权机构从 china.sqlite 生成进库;首次较慢,之后增量对账很快),
#   否则启动期"目录落后"守卫会 panic、注册局起不来。
export CID_GOV_AUTO_RECONCILE=1

# ── dev 注册局身份(让"生成登录二维码"和"扫码登录"在本地可用)──
# 中文注释:以下是"该市注册局"的可选配置——签登录二维码挑战(系统签名钥)+ 登录闸读哪个机构的
#   链上 Active 管理员集合(机构身份)。**不是节点启动前提**(节点/Web 没有它们也能起,只是登录
...
echo "==> 注册局 Web(节点启动后浏览器访问):http://127.0.0.1:8899"
