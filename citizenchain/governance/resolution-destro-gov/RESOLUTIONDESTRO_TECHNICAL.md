...
- 每个机构同一时间只允许存在 1 个活跃销毁提案，避免并发提案造成执行目标冲突。
- 销毁金额必须大于 0，且执行时必须保证机构账户保留最小余额 `ED`。
- 提案投票通过后，系统应自动尝试执行销毁；若自动执行失败，提案保留为已通过状态，允许后续手动重试执行。
- 被否决、已执行、已失效或已被覆盖的提案必须能够清理，不能长期阻塞同机构的新提案。
- 已通过但长时间未执行的提案，需要有受控的 stale 解阻塞策略，避免机构治理永久卡死。
- 销毁执行本质上通过 `Currency::slash` 减少机构账户余额与总发行量，实现链上销毁。
...
5. `execute_destroy` / `cancel_stale_destroy` 目前是公开触发入口；如果后续制度要求“只能本机构管理员推动执行/清理”，需要再收紧 origin。  
