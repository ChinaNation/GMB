...
- 路径：`nodeuitauri/backend/src/mining/mining-dashboard/mod.rs`
- 对外命令：
...
- RPC 通过共享模块 `nodeuitauri/backend/src/shared/rpc.rs` 发起（`rpc::rpc_post`），与 Network Overview 复用同一连接池实现。
