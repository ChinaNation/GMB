...
/// 长度 8 字节（`b"org-mgmt"`）；admins 模块 / citizenwallet / citizenapp 三方解码必须保持一致。
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(crate) fn create_pending_admin_account_for_proposal(
            ...
            if is_public_admin_code(&institution_code) {
                return T::PublicAdminLifecycle::create_pending_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                    institution_code,
        ...
        pub(crate) fn activate_admin_account(
            ...
            if is_public_admin_code(&institution_code) {
                return T::PublicAdminLifecycle::activate_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                );
            ...
            if is_private_admin_code(&institution_code) {
                return T::PrivateAdminLifecycle::activate_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                );
        ...
        pub(crate) fn remove_pending_admin_account(
            ...
            let _ = if is_public_admin_code(&institution_code) {
                T::PublicAdminLifecycle::remove_pending_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                )
            } else if is_private_admin_code(&institution_code) {
                T::PrivateAdminLifecycle::remove_pending_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                )
        ...
        pub(crate) fn close_admin_account(
            ...
            if is_public_admin_code(&institution_code) {
                return T::PublicAdminLifecycle::close_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                );
            ...
            if is_private_admin_code(&institution_code) {
                return T::PrivateAdminLifecycle::close_admin_account_for_proposal(
                    proposal_id,
                    crate::MODULE_TAG,
                    institution_id,
                );
