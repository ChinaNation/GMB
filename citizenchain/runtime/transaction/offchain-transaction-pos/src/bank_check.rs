...
//! - 清算行在 SFID 系统注册时生成 sfid_id,并在链上 `duoqian-manage-pow` 注册
//!   主账户 + 费用账户两个多签账户。
//! - 本模块判定:某个地址能否作为"可被 L3 绑定的清算行主账户"。
//!
//! **解耦设计**:bank_check 不直接依赖 `duoqian-manage-pow`,而是通过
//! `SfidAccountQuery` trait 抽象机构登记表。runtime 层实现该 trait(内部委托
//! 给 `duoqian-manage-pow` 的 Storage),测试层可用 `()` 空实现或 mock,从
//! 而避免 pallet 之间形成强耦合、tests 需要完整 impl `duoqian_manage_pow::Config`。
...
///
/// 直接对字节做前缀匹配,不依赖 sfid-code-auth 或 SFID 后端。
...
///
/// 供 `institution-asset-guard` 的 `can_spend` / `is_protected` 实现时使用。
