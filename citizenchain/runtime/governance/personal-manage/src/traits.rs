//! 个人多签账户的查询 trait,供 duoqian-transfer / runtime config 等下游调用。
//!
//! `PersonalMultisigQuery` 与 organization-manage 的 `InstitutionMultisigQuery`
//! 是平行 trait,duoqian-transfer 在转账治理时按"先问 personal、再问 institution"
...
pub trait PersonalMultisigQuery<AccountId> {
    /// 输入个人多签地址,返回 admin 配置快照(管理员列表 + threshold + admin_count)。
