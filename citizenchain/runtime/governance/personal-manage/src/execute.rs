...
//!   + PersonalDuoqianInfo + 关闭 admin subject + 清 PendingCloseProposal
...
/// 执行关闭：转出余额 + 删除 PersonalDuoqians + PersonalDuoqianInfo + 关闭 admin subject。
...
/// 创建提案被否决/超时/终态失败时清理:
/// unreserve(amount + fee) + 删 PersonalDuoqians/PersonalDuoqianInfo/PendingPersonalCreate +
