...
pub mod pallet {
    ...
    pub struct Clause<T: Config> {
        ...
        pub text: TextOf<T>,
        /// 款下属各项
        pub items: BoundedVec<Item<T>, <T as Config>::MaxItemsPerClause>,
    }

    /// 法律条文(第 N 条)。`number` 为数字序号(对齐你给的 JSON `id`),用于不可修改条款比对。
    #[derive(
        Encode,
        Decode,
        DecodeWithMemTracking,
    ...
    pub struct LawVersion<T: Config> {
        ...
        pub articles: BoundedVec<Article<T>, <T as Config>::MaxArticlesPerLaw>,
        /// blake2_256(规范化 SCALE 条文),完整性 + 公投/签名绑定
    ...
    pub trait Config: frame_system::Config + votingengine::Config {
        ...
        #[pallet::constant]
        type MaxItemsPerClause: Get<u32>;
        #[pallet::constant]
        type MaxClausesPerArticle: Get<u32>;
        #[pallet::constant]
        type MaxArticlesPerLaw: Get<u32>;
        #[pallet::constant]
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn propose_enact_law(
            ...
            articles: BoundedVec<Article<T>, <T as Config>::MaxArticlesPerLaw>,
            effective_at: BlockNumberFor<T>,
        ) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ensure!(!title.is_empty(), Error::<T>::EmptyTitle);
            ensure!(!articles.is_empty(), Error::<T>::EmptyArticles);
            ...
            let summary = LawProposalSummary::<T> {
                ...
                title,
                title_en,
                content_hash: Self::hash_articles(&articles),
                effective_at,
            };
            let proposal_id =
                Self::dispatch_to_engine(&who, &houses, vote_type, &summary, &articles)?;
        ...
        pub fn propose_amend_law(
            ...
            if law.tier == Tier::Constitution {
                Self::ensure_immutable_preserved(law_id, law.current_version, &articles)?;
            }
            ...
            let proposal_id =
                Self::dispatch_to_engine(&who, &law.houses, vote_type, &summary, &articles)?;
            Self::deposit_event(Event::<T>::LawProposalCreated {
                proposal_id,
                action: LawAction::Amend,
                law_id: Some(law_id),
        ...
        pub fn propose_repeal_law(
            ...
            let empty: BoundedVec<Article<T>, <T as Config>::MaxArticlesPerLaw> =
                Default::default();
            let proposal_id =
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn ensure_immutable_preserved(
            law_id: u64,
            current_version: u32,
            new_articles: &BoundedVec<Article<T>, <T as Config>::MaxArticlesPerLaw>,
        ) -> DispatchResult {
            ...
            for &n in IMMUTABLE_CONSTITUTION_ARTICLES.iter() {
                let cur = current.articles.iter().find(|a| a.number == n);
                let new = new_articles.iter().find(|a| a.number == n);
                ensure!(cur == new, Error::<T>::ImmutableArticleViolation);
            }
            Ok(())
        }

        /// 规范化条文哈希(blake2_256(SCALE))。
        pub fn hash_articles(
            articles: &BoundedVec<Article<T>, <T as Config>::MaxArticlesPerLaw>,
        ) -> [u8; 32] {
            sp_io::hashing::blake2_256(&articles.encode())
        }
        ...
        pub fn apply_legislation_vote_result(
            ...
            let articles = Self::load_articles(proposal_id)?;
            let now = frame_system::Pallet::<T>::block_number();
            Self::write_law_version(proposal_id, summary, articles, now)?;
        ...
        /// 从 votingengine ProposalObject 读回并解码法律全文条文。
        fn load_articles(
            proposal_id: u64,
        ) -> Result<BoundedVec<Article<T>, <T as Config>::MaxArticlesPerLaw>, DispatchError>
        {
            let raw = votingengine::Pallet::<T>::get_proposal_object(proposal_id)
                .ok_or(Error::<T>::ProposalPayloadInvalid)?;
            BoundedVec::<Article<T>, <T as Config>::MaxArticlesPerLaw>::decode(&mut &raw[..])
                .map_err(|_| Error::<T>::ProposalPayloadInvalid.into())
        }
