...
use admin_primitives::{
    is_public_admin_code, AdminAccountKind, AdminAccountLifecycle, AdminAccountQuery, AdminProfile,
};
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + votingengine::Config {
        ...
        /// 兄弟机构生命周期查询入口，用于禁止同一 CID 在私权模块重复登记。
    ...
    /// 公权机构生命周期员资料集合(创建提案携带:每个管理员一条 `AdminProfile`)。
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_cid_public_institution(
            ...
            crate::institution::register::do_register_cid_public_institution::<T>(
                submitter,
                cid_number,
                cid_full_name,
                account_names,
    ...
    impl<T: Config> Pallet<T> {
        ...
        ///
        /// 这是 `register_cid_public_institution` / 机构创建 / 关闭等入口的唯一派生入口。
        ///
        ...
        // public-manage 的机构地址只走 derive_registered_account。
        ...
        /// 本机构【主账户】——即 admins 模块 里承载该机构唯一管理员集的键。这样公权机构生命周期员
...
//   (提案保留 PASSED,可用 cleanup_rejected_public_proposal 或手动重试处理);
