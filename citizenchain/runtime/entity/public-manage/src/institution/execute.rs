...
pub(crate) fn cleanup_pending_institution_create<T: Config>(
    ...
    // 中文注释：管理员 Pending 账户直接绑定机构主账户，并按机构码路由到公权/私权管理员模块。
    Pallet::<T>::remove_pending_admin_account(
        proposal_id,
        action.institution_code,
        action.main_account.clone(),
    );
    if emit_event {
...
pub(crate) fn execute_create_institution_with_finalizer<T: Config>(
    ...
    Pallet::<T>::activate_admin_account(
        proposal_id,
        action.institution_code,
        action.main_account.clone(),
    )?;
