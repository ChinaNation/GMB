...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        ProvincialBankDecodeFailed { year: u32, account_bytes: [u8; 32] },

        /// stake_amount 转换到运行时 Balance 发生饱和截断。
        ProvincialBankPrincipalOverflow { year: u32, account_bytes: [u8; 32] },

        /// 利率乘法发生溢出，跳过该省储行本年度铸币并让年度结算失败。
        ProvincialBankInterestOverflow { year: u32, account_bytes: [u8; 32] },

        /// 某一年度结算完成
        ProvincialBankYearSettled { year: u32 },

        /// 某一年度结算失败（未满足“43个省储行全部成功入账”）
        ProvincialBankYearSettlementFailed {
            year: u32,
            success_count: u32,
            total_count: u32,
        },
        ...
        ProvincialBankYearForceAdvanced { year: u32 },

        /// 省储行利息低于 Existential Deposit，跳过发币以防 dust 账户（链上可审计）。
        ProvincialBankInterestBelowED {
            year: u32,
            account_bytes: [u8; 32],
            amount: BalanceOf<T>,
        },
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn try_state(_n: BlockNumberFor<T>) -> Result<(), sp_runtime::TryRuntimeError> {
            ...
            frame_support::ensure!(
                last <= PROVINCIALBANK_INTEREST_DURATION_YEARS,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn force_advance_year(origin: OriginFor<T>, year: u32) -> DispatchResult {
            ...
            Self::deposit_event(Event::<T>::ProvincialBankYearForceAdvanced { year });
            Ok(())
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn resolve_bank_account(
            ...
            match T::AccountId::decode(&mut &bank.main_account[..]) {
                Ok(a) => Some(a),
                Err(_) => {
                    Self::deposit_event(Event::<T>::ProvincialBankDecodeFailed {
                        year,
                        account_bytes,
                    });
                    log::error!(
        ...
        fn settle_next_years(
                ...
                && iterations < max_years
            {
                ...
                Self::deposit_event(Event::<T>::ProvincialBankYearSettlementFailed {
                    year: settling_year,
                    success_count,
                    total_count,
                });
        ...
        fn interest_bp_for_year(year: u32) -> u32 {
            debug_assert!(
                year >= 1 && year <= PROVINCIALBANK_INTEREST_DURATION_YEARS,
        ...
        fn mint_interest_for_year(year: u32) -> (u64, u64, u32, u32) {
            ...
            for bank in CHINA_CH.iter() {
                ...
                if principal_back != bank.stake_amount {
                    Self::deposit_event(Event::<T>::ProvincialBankPrincipalOverflow {
                        year,
