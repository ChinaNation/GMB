...
//!
//! # 出块时间与难度
//! 运行时真实出块目标时间由 chain-phase-control 链上存储控制（开发期 30s / 运行期 6min）。
//! 本文件的 MILLISECS_PER_BLOCK 仅作为创世占位和 node 层 fallback 默认值，
//! 固定为开发期值 30,000 ms，不再通过 cfg(feature) 分裂。

/// PoW 初始难度（创世难度，链启动后由动态难度调整算法自动维护）。
/// 固定为低难度，确保开发期单机可出块。运行期通过难度调整算法自动攀升。
pub const POW_INITIAL_DIFFICULTY: u64 = 100;

/// 难度调整周期（每隔多少块调整一次难度）。
/// 600 块 × 30 秒 = 5 小时（开发期），600 块 × 6 分钟 = 60 小时（运行期）。
...
/// 生产代码已改为从 chain-phase-control 链上存储读取。
...
///
/// 编译期占位，固定为开发期值（30 秒）。
/// 运行时真实出块目标时间由 chain-phase-control::TargetBlockTimeMs 链上存储控制。
/// node 层矿工门控通过 ChainPhaseApi::target_block_time_ms() Runtime API 读取。
pub const MILLISECS_PER_BLOCK: u64 = 30_000; // 30,000 毫秒（30 秒）

/// 以下派生常量基于开发期 30 秒出块计算。
/// 运行期切换到 6 分钟后，涉及时间的链上逻辑需从 chain-phase-control 读取真实值。
pub const SECONDS_PER_BLOCK: u64 = MILLISECS_PER_BLOCK / 1_000;
pub const BLOCKS_PER_HOUR: u64 = 3_600_000 / MILLISECS_PER_BLOCK; // 每小时区块数（开发期 120）
pub const BLOCKS_PER_DAY: u64 = BLOCKS_PER_HOUR * 24; // 每天区块数（开发期 2,880）
pub const BLOCKS_PER_YEAR: u64 = BLOCKS_PER_DAY * 365; // 每年区块数（开发期 1,051,200）
