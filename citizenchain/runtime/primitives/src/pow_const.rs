...
/// 难度调整周期（每隔多少块调整一次难度）。600 块 × 30 秒 = 5 小时（创世期），600 块 × 6 分钟 = 60 小时（运行期）。
...
/// 目标调整窗口总时长（毫秒），仅用于 benchmark 和 test。生产代码已改为从 genesis-pallet 链上存储读取。
...
/// 省储行利息结算周期（每年区块数）。
/// 白皮书定义：每 87,600 个区块结算一次年度利息，与出块时间无关。因空块不允许上链，区块高度仅在有交易时推进，此值为纯区块计数口径。
