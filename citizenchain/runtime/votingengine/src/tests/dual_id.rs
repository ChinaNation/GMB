...
use super::*;
use crate::pallet::{
    ProposalDisplayId, ProposalsByInstitution, ProposalsByOrg, ProposalsByOwner, ProposalsByYear,
};
...
fn create_internal_proposal_with_data_via_engine(
    ...
    <VotingEngine as InternalVoteEngine<AccountId32>>::create_internal_proposal_with_data(
        who,
        org,
        institution,
        module_tag,
...
fn reverse_indexes_populated_after_register_proposal_data() {
    new_test_ext().execute_with(|| {
        ...
        let owner = pallet::ProposalOwner::<Test>::get(id).expect("owner present");
...
fn final_cleanup_removes_indexes_and_display_id() {
    new_test_ext().execute_with(|| {
        ...
        pallet::Proposals::<Test>::remove(id);
        pallet::ProposalOwner::<Test>::remove(id);
...
fn migration_v1_backfills_display_id_and_indexes_for_legacy_proposals() {
    use crate::migrations::v1::MigrateToV1;
    use crate::Proposal;
    ...
    new_test_ext().execute_with(|| {
        ...
        pallet::Proposals::<Test>::insert(legacy_id, proposal);

        // 模拟存量 owner(MODULE_TAG)
        let module_tag: BoundedVec<u8, <Test as pallet::Config>::MaxModuleTagLen> =
            b"legacy".to_vec().try_into().unwrap();
        pallet::ProposalOwner::<Test>::insert(legacy_id, module_tag.clone());
        ...
        assert!(ProposalsByOwner::<Test>::contains_key(module_tag, legacy_id));
