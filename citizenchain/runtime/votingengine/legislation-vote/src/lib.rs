...
//!
//! 立法机构专属投票模式(ADR-027,公民宪法第十八条)。投票引擎「头等模式」:
...
//!
//! 三种模式(由表决类型 + 院序列决定):
//! - 单院(市立法会 常规/重要/二审):一段 `STAGE_LEG_HOUSE`,委员一人一票。
//! - 两院(国家/省立法院 众→参;教委会→参议会):`STAGE_LEG_HOUSE` 内顺序两段。
//! - 特别案(含核心修宪):内部段全过后强制进入 `STAGE_LEG_REFERENDUM` 公投,两者皆须过。
//!
...
use votingengine::{
    ...
    Proposal, PROPOSAL_KIND_LEGISLATION, STAGE_LEG_HOUSE, STAGE_LEG_REFERENDUM, STATUS_PASSED,
...
impl<T: Config> Pallet<T> {
    ...
    pub fn do_create_legislation_proposal(
        ...
        with_transaction(|| {
            ...
            if !<votingengine::Pallet<T>>::is_admin_in_snapshot(id, first_account.clone(), &who) {
                frame_support::defensive!(
                    "do_create_legislation_proposal: proposer missing from admin snapshot"
                );
...
impl<T: Config> votingengine::LegislationVoteEngine<T::AccountId> for Pallet<T> {
    fn create_legislation_proposal(
        ...
        with_transaction(|| {
            let id = match Self::do_create_legislation_proposal(who.clone(), houses, vote_type) {
                Ok(id) => id,
                Err(err) => return TransactionOutcome::Rollback(Err(err)),
            };
            let now = <frame_system::Pallet<T>>::block_number();
            ...
            // 中文注释:发起人签名发起即自动对发起院记一票赞成,无需再发第二笔交易。
