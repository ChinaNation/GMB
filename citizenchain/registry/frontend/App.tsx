...
function AppInner() {
  ...
  // 中文注释:按登录角色决定默认落地 tab——联邦注册局管理员默认进「联邦注册局」
  // (其余 tab 对其不可见),市注册局管理员默认进首页(公民列表)。
  // 只依赖 registry_org_code,会话内不会覆盖用户的手动切换。
  useEffect(() => {
    if (!auth?.registry_org_code) return;
    const isFederal = auth.registry_org_code === 'FEDERAL_REGISTRY';
