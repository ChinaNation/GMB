...
fn parse_backend_command() -> BackendCommand {
    ...
    match command {
        ...
        other => panic!("unknown registry command: {other}"),
    }
}
...
fn ensure_gov_catalog_current_for_serve(state: &AppState) -> Result<(), String> {
    ...
    if !env_flag_enabled("CID_GOV_AUTO_RECONCILE") {
        return Err(
            "CID 公权机构目录已落后于当前 china.sqlite；请先执行 `registry reconcile-gov --changed-only` 和 `registry check-gov --strict`，或在本地开发显式设置 CID_GOV_AUTO_RECONCILE=1 后再启动。"
                .to_string(),
        );
    }
...
fn main() {
    ...
    // 中文注释(Card 05):桌面/小市内嵌私有 PostgreSQL——registry 自管 initdb/起停,
    ...
    runtime.block_on(async move {
        ...
        if core::chain_url::chain_ws_url().is_ok() {
            tokio::spawn(admins::login::revoke_stale_admin_sessions_loop(
                state.db.clone(),
            ));
        }
        ...
        let admin_routes = Router::new()
            ...
            .route(
                ...
                get(citizens::handler::admin_list_citizens)
                    .post(citizens::admin_entry::admin_create_citizen),
            )
            .route(
        ...
        // 中文注释:registry 同源托管管理员前端 dist;未匹配路径回退 index.html(SPA 路由)。
        // 静态服务置于限流层之后,静态资源不走全局限流;dist 路径可由 REGISTRY_FRONTEND_DIST 覆盖。
        let frontend_dist = std::env::var("REGISTRY_FRONTEND_DIST")
        ...
        serve_registry(addr, app).await;
    });
}

/// 中文注释(Card 05):内网 TLS——`CID_ENABLE_TLS` 开则 rcgen 自签 HTTPS,否则 HTTP(开发)。
async fn serve_registry(addr: SocketAddr, app: axum::Router) {
    let service = app.into_make_service_with_connect_info::<SocketAddr>();
    if core::tls::is_enabled() {
        let config = core::tls::load_or_generate_rustls_config()
            .await
            .unwrap_or_else(|e| panic!("registry tls config failed: {e}"));
        info!("registry listening on https://{}", addr);
        axum_server::bind_rustls(addr, config)
            .serve(service)
            .await
            .expect("run cid backend https server");
    } else {
        info!("registry listening on http://{}", addr);
