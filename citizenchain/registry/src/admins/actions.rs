//! 管理员安全动作:会话 + QR_V1 冷钱包扫码签名(ScanSign)。
//!
//! 中文注释(3c):passkey 已删。管理员治理动作、业务安全授权和短期挑战全部使用结构化表;
//! ScanSign 档 commit 校验冷签且 signer 须 ∈ 本机构链上 Active 集合。
...
pub(crate) struct CommitAdminActionInput {
    action_id: String,
    /// 冷钱包扫码签名(ScanSign 档必填;LoginState 动作不走 commit)。
    #[serde(default)]
...
/// 校验扫码签名 signer ∈ 本机构链上 Active 管理员集合(ScanSign step-up,与登录同源)。
async fn ensure_signer_on_chain_admin(signer_pubkey: &str) -> Result<(), axum::response::Response> {
    ...
    let normalized = chain_runtime::normalize_account_pubkey(signer_pubkey)
        .ok_or_else(|| api_error(StatusCode::FORBIDDEN, 2002, "not an on-chain admin"))?;
    if !onchain
        .iter()
...
pub(crate) async fn prepare_admin_action(
    ...
    let (payload_text, payload_hash, sign_request) =
        if preview.auth_type == AdminOperationAuth::ScanSign {
...
pub(crate) async fn commit_admin_action(
    ...
    // ── ScanSign step-up:冷钱包扫码签名 + signer ∈ 本机构链上 Active 集合(3c)。
    //    所有可 commit 动作(LoginState 已在上方拒绝)一律走此校验。
    ...
    if matches!(
        ...
        if let CommitAdminActionOutput::Applied(ref value) = output {
            ...
            let Some(target_32) = crate::admins::login::parse_sr25519_pubkey_bytes(&target_hex)
            else {
...
pub(crate) fn require_admin_security_grant(
    ...
    // 中文注释(3c):LoginState 档不再要求扫码签名 grant——会话已是链上已证管理员,
    // 仅按动作角色边界(联邦/省 scope)放行,不消费一次性 grant。ScanSign 档继续校验 grant。
...
fn validate_institution_deregister_conn(
    ...
    if !visible.includes_province(&inst.province_name) || !visible.includes_city(&inst.city_name) {
    ...
    let (account_name, scope) = match action_type {
        AdminActionType::InstitutionDeregister => (
            crate::subjects::service::ACCOUNT_NAME_MAIN.to_string(),
            SCOPE_INSTITUTION,
        ),
        AdminActionType::InstitutionAccountDeregister => {
            let name = input
                ...
                .to_string();
            if name == crate::subjects::service::ACCOUNT_NAME_MAIN {
                return Err(
    ...
    if account.chain_status != crate::subjects::model::MultisigChainStatus::ActiveOnChain {
    ...
    let target_hex = crate::accounts::derive::derive_account(&cid_number, &account_name)
