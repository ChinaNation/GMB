...
pub(crate) async fn list_city_registry_admins(
    ...
    if ctx.registry_org_code == RegistryOrgCode::CityRegistry
        && ctx
            .scope_city_name
            .as_deref()
            .unwrap_or("")
    ...
    let actor_city_name = if ctx.registry_org_code == RegistryOrgCode::CityRegistry {
...
pub(crate) fn can_manage_city_registry_conn(
    ...
    let city_registry_scope = repo::province_scope_for_registry_org_conn(
        ...
        &city_registry.registry_org_code,
    )?;
...
pub(crate) fn find_city_registry_by_id_conn(
    ...
    repo::get_admin_by_id_and_registry_org_conn(conn, id, &RegistryOrgCode::CityRegistry)
}
