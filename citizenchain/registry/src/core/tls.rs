//! registry 内网 API 自签 TLS(Card 05)。
//!
...
/// 证书持久化目录:`CID_TLS_DIR`(node 传 `base_path/registry-tls`);兜底 exe 同目录 `tls`。
fn tls_dir() -> PathBuf {
...
pub(crate) async fn load_or_generate_rustls_config() -> Result<RustlsConfig, String> {
    ...
    let dir = tls_dir();
    let cert_path = dir.join("registry-cert.pem");
    let key_path = dir.join("registry-key.pem");
    ...
    if !(cert_path.is_file() && key_path.is_file()) {
        ...
        tracing::info!(dir = %dir.display(), "registry self-signed TLS cert generated");
    }

    RustlsConfig::from_pem_file(cert_path, key_path)
        .await
        .map_err(|e| format!("load registry tls cert failed: {e}"))
}
