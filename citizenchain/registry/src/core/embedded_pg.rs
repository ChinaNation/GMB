//! 内嵌私有 PostgreSQL 生命周期(registry 自管)。
//!
//! 中文注释(Card 05):去中心化每市自治节点零依赖——registry 在启动期把随安装包打进来的
//! PostgreSQL 二进制拉成**本机私有实例**(127.0.0.1,不对外),首启 `initdb`,起 `postgres`,
//! 建 `registry` 库,返回 `DATABASE_URL`;退出期 `pg_ctl stop`。node 不碰 PG,只用 env 告诉
//! registry 二进制目录/数据目录/端口(见 `node/src/registry_proc`)。
//!
...
/// registry 业务库名 / 超级用户名(本机 trust 鉴权,口令为空)。
const PG_DB_NAME: &str = "registry";
...
/// 兜底用 registry 可执行文件同目录(开发期 `cargo build` 后手动放置或软链)。
...
/// 确保内嵌 PG 已就绪并返回 registry 业务库的 `DATABASE_URL`。
///
/// 幂等:已 initdb 不重做;postmaster 已在跑则复用;`registry` 库存在则跳过建库。
...
fn configure_postgresql_conf(data_dir: &Path) -> Result<(), String> {
    ...
    let mut extra =
        String::from("\n# ── registry 内嵌实例(Card 05)──\nlisten_addresses = '127.0.0.1'\n");
    if let Some(archive_dir) = std::env::var("CID_PG_WAL_ARCHIVE_DIR")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    {
        // 中文注释:每日 pg_basebackup + 持续 WAL 归档到 NAS = PITR(见 citizenchain/scripts/registry-{backup,restore}.sh)。
...
fn ensure_database() -> Result<(), String> {
    ...
    let exists: bool = client
        ...
        .map_err(|e| format!("query registry database failed: {e}"))?
        .get(0);
    if !exists {
        // 中文注释:库名为固定常量,无注入风险;CREATE DATABASE 不可在事务内。
        client
            .batch_execute(&format!("CREATE DATABASE \"{PG_DB_NAME}\""))
            .map_err(|e| format!("create registry database failed: {e}"))?;
