...
/// 由注册局管理员动作(ScanSign 冷签档)校验通过后调用;机构管理员持此凭证冷签 propose_close。
...
// 链上管理员集合读取(3b 去中心化鉴权)
//
// 真源:管理员集合落链端 `GenesisAdmins::AdminAccounts`(联邦注册局)/
// `PublicAdmins::AdminAccounts`(市注册局),与 3a 直设入口同一 storage。
// registry 登录验签后比对此集合放行,本地 admins 表仅作元数据/省映射缓存。
// ──────────────────────────────────────────────────────────────────

/// 联邦注册局机构码,镜像 `admin_primitives::FRG`(`*b"FRG\0"`)。
/// 中文注释:registry 不依赖 admin-primitives(避免引入 frame-support 重依赖),
...
/// 链上管理员集合所属 pallet(联邦 = 创世管理员;市 = 公权管理员)。
#[derive(Debug, Clone, Copy)]
...
/// 本注册局节点的链上机构身份(从 `CID_RUNTIME_ISSUER_*` 解析)。
pub(crate) struct NodeInstitutionIdentity {
    pub(crate) pallet: AdminPallet,
    pub(crate) main_account: [u8; 32],
    pub(crate) is_federal: bool,
}

/// 解析本节点所代表的链上注册局机构(联邦注册局 or 某市注册局)。
///
/// 中文注释:节点身份单源 = `CID_RUNTIME_ISSUER_CID_NUMBER`(判机构码)+
...
pub(crate) fn node_institution_identity() -> Result<NodeInstitutionIdentity, String> {
    ...
    if code == FRG_CODE {
        Ok(NodeInstitutionIdentity {
            pallet: AdminPallet::GenesisAdmins,
            main_account,
            is_federal: true,
        })
        ...
        && !primitives::code::is_fixed_governance_code(&code)
    {
        ...
        Err("node institution code is neither federal registry nor public registry".to_string())
    }
}

/// 读取某机构账户的链上 Active 管理员公钥集合(0x 小写 hex 列表)。
///
/// 返回:`Ok(Some(set))`=账户存在且 Active;`Ok(None)`=账户不存在或非 Active;
/// `Err`=链不可达或解码失败。读 latest 块(membership 变更治理级稀有,后台扫描持续复查)。
