...
async fn finalize_3_voters_no_observers() {
    ...
    for i in 0..3 {
        ...
        assert_eq!(
            net.peer(i).client().info().best_hash,
            hashof20,
            "Peer #{} failed to sync",
            i
        );
...
async fn transition_3_voters_twice_1_full_observer() {
    ...
    for (i, peer) in net.lock().peers().iter().enumerate() {
        ...
        assert_eq!(
            full_client.chain_info().best_number,
            1,
            "Peer #{} failed to sync",
            i
        );
...
async fn voter_persists_its_votes() {
    ...
    assert_eq!(
        net.peer(0).client().info().best_number,
        20,
        "Peer #{} failed to sync",
        0
    );
...
async fn grandpa_environment_respects_voting_rules() {
    ...
    let mut notification_service = peer
        .take_notification_service(&grandpa_protocol_name::NAME.into())
        .unwrap();
    ...
    peer.client()
        .finalize_block(hashes[18], None, false)
        .unwrap();
...
async fn grandpa_environment_checks_if_best_block_is_descendent_of_finality_target() {
    ...
    let hashof5_a = client
        .expect_block_hash_from_id(&BlockId::Number(5))
        .unwrap();
    let hashof10_a = client
        .expect_block_hash_from_id(&BlockId::Number(10))
        .unwrap();
    ...
    let fork = peer.generate_blocks_at(
        ...
        |mut builder| {
            builder
                .push_deposit_log_digest_item(DigestItem::Other(vec![1]))
                .unwrap();
            builder.build().unwrap().block
        },
        false,
...
async fn grandpa_environment_uses_round_base_block_for_voting_if_finality_target_errors() {
    ...
    let hashof7 = client
        .expect_block_hash_from_id(&BlockId::Number(7))
        .unwrap();
    let hashof8_a = client
    ...
    peer.client()
        .finalize_block(hashof8_a, None, false)
        .unwrap();
...
async fn justification_with_equivocation() {
    ...
    let pairs = (0..100)
        .map(|n| AuthorityPair::from_seed(&[n; 32]))
        .collect::<Vec<_>>();
    let voters = pairs
        .iter()
        .map(AuthorityPair::public)
        .map(|id| (id, 1))
        .collect::<Vec<_>>();
    ...
    let hashof2 = client
        ...
        .unwrap();
    let hashof3 = client
        .expect_block_hash_from_id(&BlockId::Number(3))
        .unwrap();
...
async fn revert_prunes_authority_changes() {
    ...
    let edit_block = |mut builder: TestBlockBuilder| {
        add_scheduled_change(
            &mut builder,
            ScheduledChange {
                next_authorities: make_ids(peers),
                delay: 0,
            },
        );
        builder.build().unwrap().block
    ...
    let hash = peer
        .generate_blocks_at(
            ...
            |mut builder| {
                builder
                    .push_deposit_log_digest_item(DigestItem::Other(vec![2]))
                    .unwrap();
                builder.build().unwrap().block
            },
            false,
