...
impl NetworkPeers for TestNetwork {
    ...
    fn report_peer(&self, peer_id: PeerId, cost_benefit: ReputationChange) {
        let _ = self
            .sender
            .unbounded_send(Event::Report(peer_id, cost_benefit));
    }
...
impl sc_network_gossip::ValidatorContext<Block> for TestNetwork {
    ...
    fn send_message(&mut self, who: &PeerId, data: Vec<u8>) {
        let _ = self
            .sender
            .unbounded_send(Event::WriteNotification(*who, data));
    }
...
impl NotificationService for TestNotificationService {
    ...
    fn send_sync_notification(&mut self, peer: &PeerId, notification: Vec<u8>) {
        let _ = self
            .sender
            .unbounded_send(Event::WriteNotification(*peer, notification));
    }
...
pub(crate) fn make_test_network() -> (impl Future<Output = Tester>, TestNetwork) {
    ...
    let notification_service = TestNotificationService {
        rx: notification_rx,
        sender: tx.clone(),
    };
...
fn good_commit_leads_to_relay() {
    ...
    let test = make_test_network()
        ...
        .then(move |tester| {
            ...
            tester
                .gossip_validator
                .new_peer(&mut NoopContext, &id, ObservedRole::Full);
...
fn peer_with_higher_view_leads_to_catch_up_request() {
    ...
    let test = tester
        .map(move |tester| {
            ...
            tester
                .gossip_validator
                .new_peer(&mut NoopContext, &id, ObservedRole::Authority);
            (tester, id)
        })
        .then(move |(tester, id)| {
