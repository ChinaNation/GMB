...
void main() {
  test('Cloudflare transport validates encrypted envelope bytes before config',
      () async {
    final transport = ImCloudflareTransport(
      ownerChatAccount: 'alice-wallet',
    ...
    expect(result.transportType, ImTransportType.cloudflare);
    expect(result.state, ImMessageDeliveryState.failed);
  ...
  test('Cloudflare transport posts encrypted envelope to mailbox API',
    ...
    final transport = ImCloudflareTransport(
      ...
      httpClient: MockClient((request) async {
        ...
        expect(body['sender_account'], envelope.senderChatAccount);
        expect(body['recipient_account'], envelope.recipientChatAccount);
    ...
    expect(result.state, ImMessageDeliveryState.sent);
  });
  ...
  test('Cloudflare transport fetches and consumes KeyPackages', () async {
    ...
    final packages = await transport.fetchKeyPackages(
      ...
      requesterChatAccount: 'alice-wallet',
    );
  ...
  test('Cloudflare transport fetches pending envelopes and acks them',
    ...
    expect(ImEnvelope.fromBuffer(pending.single.envelopeBytes).envelopeId,
        envelope.envelopeId);
    expect(acked, isTrue);
  });
  ...
  test('Cloudflare transport prepares, uploads, and completes attachment',
    ...
    final plan = await transport.prepareAttachmentUpload(
      ...
      manifestByteSize: 12,
      chunks: const [
        ImAttachmentChunkDraft(chunkId: 'chunk-001', byteSize: 24),
      ],
    );
