...
typedef ImAttachmentPrepareUploader = Future<ImAttachmentUploadPlan> Function({
...
typedef ImAttachmentObjectUploader = Future<void> Function({
  required Uri uploadUrl,
  required List<int> bytes,
  required String contentType,
});

typedef ImAttachmentCompleteUploader = Future<void> Function(
  ImAttachmentCompleteRequest input,
);

typedef ImAttachmentDownloadPreparer = Future<ImAttachmentDownloadPlan>
    Function(ImAttachmentDownloadRequest input);

typedef ImAttachmentObjectDownloader = Future<List<int>> Function(
  Uri downloadUrl,
);

typedef ImLocalAttachmentSaver = Future<void> Function({
  required String conversationId,
  required String attachmentId,
...
/// 待发送的本机明文附件。
class ImAttachmentDraft {
  const ImAttachmentDraft({
...
/// GMB_IM_V1 envelope、本地 Isar 和正式 transport 串起来。
class ImMessageFlow {
  const ImMessageFlow({
  ...
  Future<List<ImDeliveryResult>> sendAttachment({
    ...
    required ImAttachmentPrepareUploader prepareAttachmentUpload,
    required ImAttachmentObjectUploader uploadAttachmentObject,
    required ImAttachmentCompleteUploader completeAttachmentUpload,
    ImLocalAttachmentSaver? saveLocalAttachment,
  }) async {
    final now = DateTime.now().millisecondsSinceEpoch;
    ...
    final manifestPlaintext = utf8.encode(
      jsonEncode({
        'type': 'gmb_im_attachment_manifest_v1',
      ...
      final envelope = wireMessage.toEnvelope(
        ...
        attachmentManifestHash:
            wireMessage.messageKind == ImMlsMessageKind.application
                ? manifestHash
                : '',
        chunkRefs: wireMessage.messageKind == ImMlsMessageKind.application
  ...
  Future<int> fetchAndProcessPending({
    required Future<List<ImPendingEncryptedEnvelope>> Function() fetchPending,
    required Future<void> Function(String envelopeId) ackEnvelope,
    Future<ImDownloadedAttachment> Function(
    ...
    for (final row in rows) {
      ...
      if (result.accepted) {
        ...
        if (plaintext != null && _isAttachmentControlPlaintext(plaintext)) {
          ...
          final envelope = ImEnvelope.fromBuffer(row.envelopeBytes);
  ...
  static Future<ImDeliveryResult> deliverWithTransport({
    required ImTransport transport,
  ...
  static Future<ImDownloadedAttachment> downloadAttachment({
    required String conversationId,
    required String controlPlaintext,
    required Directory cacheDirectory,
    required ImAttachmentDownloadPreparer prepareAttachmentDownload,
    required ImAttachmentObjectDownloader downloadAttachmentObject,
  }) async {
  ...
  static Future<ImDownloadedAttachment> saveAttachmentBytesToCache({
    required String conversationId,
    required String attachmentId,
    required String fileName,
    required String contentType,
  ...
  static Future<ImDownloadedAttachment?> readCachedAttachment({
    required String conversationId,
    required String attachmentId,
    required String fileName,
    required String contentType,
...
ImMessageKind _messageKindFromPlaintext(String plaintext) {
  try {
    final decoded = jsonDecode(plaintext);
    if (decoded is Map && decoded['type'] == 'gmb_im_attachment_v1') {
      return ImMessageKind.attachment;
    }
  } catch (_) {
...
bool _isAttachmentControlPlaintext(String plaintext) {
  try {
    final decoded = jsonDecode(plaintext);
    return decoded is Map && decoded['type'] == 'gmb_im_attachment_v1';
  } catch (_) {
    return false;
  }
}
...
class _AttachmentControl {
  ...
  factory _AttachmentControl.fromPlaintext(String plaintext) {
    final decoded = jsonDecode(plaintext);
    if (decoded is! Map || decoded['type'] != 'gmb_im_attachment_v1') {
      throw const FormatException('不是有效的 IM 附件控制消息');
    }
...
class _AttachmentManifest {
  ...
  factory _AttachmentManifest.fromPlaintext(String plaintext) {
    final decoded = jsonDecode(plaintext);
    if (decoded is! Map || decoded['type'] != 'gmb_im_attachment_manifest_v1') {
      throw const FormatException('不是有效的 IM 附件 manifest');
    }
