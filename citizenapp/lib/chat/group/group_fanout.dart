...
// 同一 `mls_wire_message`,不同 `recipient_account`。服务端仍按 envelope 内
// `recipient_account` 路由,零存储不变。
...
class GroupFanout {
  ...
  static List<ChatEnvelope> fanOut({
    ...
    for (var index = 0; index < recipientAccounts.length; index++) {
      final recipient = recipientAccounts[index];
      envelopes.add(
        wire.toEnvelope(
