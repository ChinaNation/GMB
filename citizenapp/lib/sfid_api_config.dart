/// citizenapp 访问 SFID 系统的唯一地址策略。
///
/// 中文注释：这里刻意只允许生产域名和本地 USB 调试两条路径，禁止局域网 IP、
/// 任意 API URL 注入、旧端口默认值以及失败自动回退，避免电子护照账户被发送到
/// 不同信任环境。
class SfidApiConfig {
  const SfidApiConfig._();

  static const String environmentDefineName = 'CITIZENAPP_SFID_ENV';
  static const String productionEnvironment = 'prod';
  static const String devUsbEnvironment = 'dev_usb';

  static const String productionBaseUrl = 'https://sfid.crcfrcn.com';
    ...
    switch (environment.trim()) {
      ...
      default:
        throw UnsupportedError(
          '$environmentDefineName 只允许 prod 或 dev_usb，禁止配置其他 SFID 地址',
        );
    }
  }
  ...
  static String connectionErrorMessage(String baseUrl) {
    if (baseUrl == devUsbBaseUrl) {
      return '当前使用本地 USB 开发路径 $devUsbBaseUrl，请确认电脑 SFID 后端已启动，并已执行 adb reverse tcp:8899 tcp:8899';
    }
    return '当前使用生产 SFID 路径 $productionBaseUrl，请检查手机网络、HTTPS 证书或生产网关';
  }
}
