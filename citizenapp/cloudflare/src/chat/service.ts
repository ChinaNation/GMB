...
export async function registerChatDevice(request: Request, env: Env): Promise<Response> {
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO chat_device_binding_nonces (account_id, nonce_hash, expires_at, created_at)
        VALUES (?, ?, ?, ?)`,
    ).bind(accountId, nonceHash, expiresAt, createdAt).run();
  } catch {
  ...
  const deviceWrite = await env.DB.prepare(
    `INSERT INTO chat_devices
      (account_id, device_id, device_public_key_hex, push_provider, push_token, expires_at, created_at)
      SELECT ?, ?, ?, ?, ?, ?, ?
      WHERE EXISTS (SELECT 1 FROM chat_devices WHERE account_id = ? AND device_id = ?)
        OR (SELECT COUNT(*) FROM chat_devices WHERE account_id = ? AND expires_at > ?) < ?
    ...
    accountId, deviceId, devicePublicKeyHex, pushProvider, pushToken, expiresAt, createdAt,
    accountId, deviceId, accountId, createdAt, deviceLimit,
  ).run();
...
export async function publishChatKeyPackage(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const inserted = await env.DB.prepare(
      `INSERT INTO chat_keypackages
        (account_id, device_id, key_package_id, key_package, cipher_suite, created_at, expires_at)
        SELECT ?, ?, ?, ?, ?, ?, ?
        WHERE (SELECT COUNT(*) FROM chat_keypackages
          WHERE account_id = ? AND device_id = ? AND expires_at > ?) < ?`,
    ).bind(
      accountId, deviceId, keyPackageId, keyPackage, cipherSuite, createdAt, expiresAt,
      accountId, deviceId, nowMs(), keyPackageLimit.max_count!,
    ).run();
  ...
  return jsonResponse({ ok: true, account_id: accountId, device_id: deviceId, key_package_id: keyPackageId, expires_at: expiresAt });
}

export async function fetchChatKeyPackages(request: Request, env: Env): Promise<Response> {
  await requireSession(request, env);
  const url = new URL(request.url);
  const accountId = assertChatAccountId(url.pathname.split('/').pop());
  ...
  const rows = await env.DB.prepare(
    `SELECT kp.account_id, kp.device_id, d.device_public_key_hex, kp.key_package_id,
      ...
      JOIN chat_devices d ON d.account_id = kp.account_id AND d.device_id = kp.device_id
      WHERE kp.account_id = ? AND kp.expires_at > ? AND d.expires_at > ?
      ORDER BY kp.created_at ASC LIMIT ?`,
  ).bind(accountId, nowMs(), nowMs(), limit).all<ChatKeyPackageRow>();
  return jsonResponse({ ok: true, account_id: accountId, key_packages: rows.results ?? [] });
}
...
export async function consumeChatKeyPackage(request: Request, env: Env): Promise<Response> {
  ...
  const row = await env.DB.prepare(
    `SELECT kp.account_id, kp.device_id, d.device_public_key_hex, kp.key_package_id,
      ...
      FROM chat_keypackages kp
      JOIN chat_devices d ON d.account_id = kp.account_id AND d.device_id = kp.device_id
      WHERE kp.account_id = ? AND kp.key_package_id = ? AND kp.expires_at > ? AND d.expires_at > ?`,
  ).bind(accountId, keyPackageId, nowMs(), nowMs()).first<ChatKeyPackageRow>();
  if (!row) throw new HttpError(404, 'key_package_not_available', 'KeyPackage 不存在或已被消费');
  const deleted = await env.DB.prepare(`DELETE FROM chat_keypackages WHERE key_package_id = ? AND account_id = ?`)
    .bind(keyPackageId, accountId).run();
...
export async function submitChatEnvelope(request: Request, env: Env): Promise<Response> {
  ...
  const senderAccountId = assertChatAccountId(session.account_id);
  const senderDeviceId = assertDeviceId(body.sender_device_id);
  await requireActiveDevice(env, senderAccountId, senderDeviceId);
  const recipientAccountId = assertChatAccountId(body.recipient_account_id, 'invalid_recipient_account_id');
  ...
  const wakeSent = sent === 0 ? await sendChatWake(env, recipientAccountId, senderAccountId).catch(() => 0) : 0;
  return jsonResponse({
    ok: true,
    envelope_id: envelopeId,
...
export async function openChatWebSocket(request: Request, env: Env): Promise<Response> {
  ...
  await requireActiveDevice(env, session.account_id, deviceId);
  ...
  return requireChatRealtimeNamespace(env).getByName(session.account_id).fetch(internal);
}
...
async function requireActiveDevice(
  ...
  const row = await env.DB.prepare(
    `SELECT account_id, device_id, device_public_key_hex, expires_at
      FROM chat_devices WHERE account_id = ? AND device_id = ? AND expires_at > ?`,
  ).bind(accountId, deviceId, nowMs()).first<ChatDeviceRow>();
