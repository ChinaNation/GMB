...
export async function selfPostCopiesRoute(request: Request, env: Env): Promise<Response> {
  ...
  const result = await env.DB.prepare(
    `SELECT p.post_id, p.cid_number, p.account_id, p.post_category, p.content_format,
        ...
        u.post_category AS upload_post_category, u.manifest_hash,
        u.content_hash AS upload_content_hash,
        u.storage_receipt_id AS upload_storage_receipt_id,
        u.object_keys_json, u.status AS upload_status
      FROM square_posts p
...
function validateRow(sessionCidNumber: string, row: SelfPostCopyRow): string {
  ...
  if (
    (row.post_category !== 'normal' && row.post_category !== 'campaign') ||
    row.upload_post_category !== row.post_category
  ) {
    throw new HttpError(409, 'post_category_mismatch', '帖子与上传分类不一致');
  }
  if (row.content_format !== 'normal' && row.content_format !== 'article') {
    throw new HttpError(409, 'post_content_format_invalid', '帖子内容形态不合法');
  }
