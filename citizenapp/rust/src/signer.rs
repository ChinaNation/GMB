//! sr25519 原生签名 FFI（schnorrkel）。
//!
//! 取代 App 端纯 Dart `sr25519` 实现：纯 Dart 走 BigInt 软算标量乘，真机上一次
//! 「派生 + 签名」实测 **8.2 秒**，会把主线程/用户直接晾在那里；schnorrkel 是
//! Substrate 全家的官方实现，同样的活是毫秒级。
//!
...
pub const GMB_SIGNER_OK: i32 = 0;
pub const GMB_SIGNER_ERR_NULL_ARG: i32 = -1;
pub const GMB_SIGNER_ERR_BAD_KEY: i32 = -2;
pub const GMB_SIGNER_ERR_BAD_SIGNATURE: i32 = -3;
pub const GMB_SIGNER_ERR_VERIFY_FAILED: i32 = -4;
pub const GMB_SIGNER_ERR_PANIC: i32 = -5;
...
#[no_mangle]
pub unsafe extern "C" fn gmb_sr25519_derive_hard(
    seed: *const u8,
    chain_code: *const u8,
    out_child: *mut u8,
) -> i32 {
    if seed.is_null() || chain_code.is_null() || out_child.is_null() {
        return GMB_SIGNER_ERR_NULL_ARG;
    }
    ...
    .unwrap_or(GMB_SIGNER_ERR_PANIC)
}
...
#[no_mangle]
pub unsafe extern "C" fn gmb_sr25519_public_key(
    child: *const u8,
    out_public: *mut u8,
) -> i32 {
...
#[no_mangle]
pub unsafe extern "C" fn gmb_sr25519_sign(
    child: *const u8,
    message: *const u8,
    message_len: usize,
...
#[no_mangle]
pub unsafe extern "C" fn gmb_sr25519_verify(
    public: *const u8,
    signature: *const u8,
    message: *const u8,
    message_len: usize,
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let Ok(sig) = Signature::from_bytes(sig_bytes) else {
            return GMB_SIGNER_ERR_BAD_SIGNATURE;
        };
        if public_key
...
mod tests {
    ...
    fn hard_derive_is_deterministic() {
        ...
        unsafe {
            assert_eq!(
                gmb_sr25519_derive_hard(seed.as_ptr(), cc.as_ptr(), a.as_mut_ptr()),
                GMB_SIGNER_OK
            );
            assert_eq!(
                gmb_sr25519_derive_hard(seed.as_ptr(), cc.as_ptr(), b.as_mut_ptr()),
                GMB_SIGNER_OK
            );
        }
        assert_eq!(a, b);
    ...
    fn sign_then_verify_roundtrip() {
        ...
        unsafe {
            assert_eq!(
                gmb_sr25519_public_key(child.as_ptr(), public.as_mut_ptr()),
                GMB_SIGNER_OK
            );
            assert_eq!(
                gmb_sr25519_sign(child.as_ptr(), msg.as_ptr(), msg.len(), sig.as_mut_ptr()),
                GMB_SIGNER_OK
            );
            assert_eq!(
                gmb_sr25519_verify(public.as_ptr(), sig.as_ptr(), msg.as_ptr(), msg.len()),
                GMB_SIGNER_OK
            );
            let other = b"tampered";
            assert_eq!(
                gmb_sr25519_verify(public.as_ptr(), sig.as_ptr(), other.as_ptr(), other.len()),
                GMB_SIGNER_ERR_VERIFY_FAILED
            );
        }
    }
    ...
    fn null_arguments_are_rejected() {
        ...
        unsafe {
            assert_eq!(
                gmb_sr25519_derive_hard(
                    std::ptr::null(),
                    buf.as_ptr(),
                    buf.as_ptr() as *mut u8
                ),
                GMB_SIGNER_ERR_NULL_ARG
            );
            assert_eq!(
                gmb_sr25519_sign(std::ptr::null(), buf.as_ptr(), 1, buf.as_ptr() as *mut u8),
                GMB_SIGNER_ERR_NULL_ARG
            );
            assert_eq!(
                gmb_sr25519_verify(buf.as_ptr(), std::ptr::null(), buf.as_ptr(), 1),
                GMB_SIGNER_ERR_NULL_ARG
            );
        }
    }
