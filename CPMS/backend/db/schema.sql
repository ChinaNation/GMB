...
CREATE TABLE IF NOT EXISTS citizen_archives (
  ...
  CONSTRAINT ck_archive_index_no_format
    CHECK (archive_index_no ~ '^[A-Z]{2}[0-9]{3}[0-9][0-9]{9}[0-9]{8}$')
);
...
CREATE TABLE IF NOT EXISTS archive_sequence_counters (
  id BIGSERIAL PRIMARY KEY,
  province_code VARCHAR(2) NOT NULL CHECK (province_code ~ '^[A-Z]{2}$'),
  city_code VARCHAR(3) NOT NULL CHECK (city_code ~ '^[0-9]{3}$'),
  created_yyyymmdd VARCHAR(8) NOT NULL CHECK (created_yyyymmdd ~ '^[0-9]{8}$'),
  next_seq INT NOT NULL CHECK (next_seq >= 1),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_archive_seq_counter UNIQUE (province_code, city_code, created_yyyymmdd)
);
