...
/// 全链统一的省储行 pallet_id -> [u8; 8] 转码入口（其他模块统一调用这里，避免重复转码）。
#[inline]
...
/// 全链统一的省储行 fee_pallet_id -> [u8; 8] 转码入口。
#[inline]
pub fn fee_pallet_id_to_bytes(id: &str) -> Option<[u8; 8]> {
    pallet_id_to_bytes(id)
}
