...
pub const MILLISECS_PER_BLOCK: u64 = 360_000;  // 目标区块时间：360,000 毫秒（即 6 分钟）

pub const BLOCKS_PER_MINUTE: u64 = 60_000 / MILLISECS_PER_BLOCK;   // 每分钟区块数（6 分钟一个块，所以每分钟约为 1/6 个块）

pub const BLOCKS_PER_HOUR: u64 = 10;  // 每小时区块数 (60 / 6 = 10 个块)

pub const BLOCKS_PER_DAY: u64 = BLOCKS_PER_HOUR * 24;  // 每天区块数 (10 * 24 = 240 个块)

pub const BLOCKS_PER_YEAR: u64 = BLOCKS_PER_DAY * 365;  // 每年区块数（365 天：240 * 365 = 87,600 个块）
