...
- <details>
  <summary>1. 联邦公民储备委员会前端/fcrc-frontend</summary>
...
# 1.联邦公民储备委员会前端/fcrc-frontend
## 1.1.功能需求
### 1.1.1.高优级功能
* 管理员身份登录：国储会：使用管理员公钥登录；省储会：使用管理员公钥登录；省储行：使用管理员公钥登录。
* 交易：国储会：交易地址（pallet_address）支出（内部投票）；省储会：交易地址（pallet_address）支出（内部投票）；省储行：交易地址（pallet_address）支出（内部投票）；
* 查询：国储会：查看交易地址交易记录；省储会：查看交易地址交易记录；省储行：查看交易地址交易记录和本省链下手续费收入记录。
* 账户监控：国储会：交易地址余额显示；省储会：本省交易地址余额显示；省储行：质押地址和交易地址余额显示，设置本节点链下交易手续费。

### 1.1.2.中优级功能
* 治理：国储会：决议发行提案/投票、增删改新省储会/省储行提案/投票、决议销毁提案/投票；省储会：决议发行投票、决议销毁提案/投票；省储行：决议发行投票、决议销毁提案/投票。
* 安全与硬件支持：国储会：支持硬件钱包签名、2FA 二次验证。省储会：支持硬件钱包签名。省储行：支持硬件钱包签名。
* 日志与告警：国储会：查看全网操作日志。省储会：查看本省节点操作日志。省储行：查看本节点操作日志。
* 数据查询与分析：国储会：查询全网交易历史。省储会：查询本省交易历史。省储行：查询本省交易历史。

### 低优级功能
* 前端体验优化：多维度报表可视化。省储会：报表可视化。省储行：奖励/交易统计可视化。
* 通知与消息：国储会：全网提案提醒；省储会：本省提案签名提醒；省储行：本节点提案签名提醒、奖励到账提醒。
...
* 交易管理：节点本地钱包余额展示（链上公民币），支持链上链下交易发起、签名、广播，链上链下交易手续费计算显示；
* 交易池监控：待处理交易数量；
