...
PRIMARY_BIND_ADDR="10.0.0.21"
APP_DB="sfid_prod"
APP_ROLE="sfid_app"
APP_PASS="CHANGE_ME_APP_PASSWORD"
MIG_ROLE="sfid_migrator"
MIG_PASS="CHANGE_ME_MIGRATOR_PASSWORD"

for secret in "${REPL_PASS}" "${APP_PASS}" "${MIG_PASS}"; do
  if [[ "${secret}" == *"CHANGE_ME"* ]]; then
    echo "检测到 CHANGE_ME 占位凭据，请先替换后再执行。"
    exit 1
  fi
done
