...
const NOTICE_KEY = 'sfid-global-notice';
...
function translateErrorCode(code: string): string | null {
  const map: Record<string, string> = {
    ...
    SFID_AUTH_UNAUTHORIZED: '请先登录管理员账户',
    SFID_REQUEST_INVALID: '请求内容不正确，请检查后重试',
    SFID_RESOURCE_NOT_FOUND: '数据不存在',
    SFID_RESOURCE_CONFLICT: '数据状态冲突，请刷新后重试',
    SFID_RESOURCE_EXPIRED: '请求已过期，请重新操作',
    SFID_RATE_LIMITED: '请求过于频繁，请稍后重试',
    SFID_SERVICE_UNAVAILABLE: '服务暂不可用，请稍后重试',
  };
