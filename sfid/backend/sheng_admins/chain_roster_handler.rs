//! 中文注释:省管理员模块 `chain_` 链交互 HTTP handler。
//!
//! 提供两类 endpoint:
//!
...
//!      `list_roster_public`。
//!
//! ## 与 `institutions/chain_duoqian_info_handler.rs` 的对齐
//!
...
use crate::sheng_admins::chain_roster_query::fetch_roster;
use crate::AppState;
...
pub(crate) async fn list_roster_public(Query(q): Query<ListRosterQuery>) -> impl IntoResponse {
    let code = q.province.trim().to_ascii_uppercase();
    if code.is_empty() {
        return crate::api_error(StatusCode::BAD_REQUEST, 1001, "province query is required");
    }
    let Some(province_name) = province_name_by_code(code.as_str()) else {
        return crate::api_error(StatusCode::BAD_REQUEST, 1001, "unknown province code");
    };
