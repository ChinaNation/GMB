...
//! ```
//! A3(3) - R5(5) - T2P1C1(4) - N9(9) - D4(4)
//! 例:GFR  - AH001 - ZF0X     - 898100720 - 2026
//! ```
//!
...
//! - D4:4 字符,生成年份 YYYY
//!
//! 容量分析:同 (a3, 省, 市, 机构, year) 5 元组下,n9 是 hash mod 10^9 = 10 亿桶。
...
pub const SFID_NUMBER_SEGMENT_COUNT: usize = 5;
pub const SFID_NUMBER_SEGMENT_A3_LEN: usize = 3;
pub const SFID_NUMBER_SEGMENT_R5_LEN: usize = 5;
pub const SFID_NUMBER_SEGMENT_T2P1C1_LEN: usize = 4;
...
pub fn validate_sfid_number_format(raw: &str) -> Result<String, &'static str> {
    ...
    if segments[0].len() != SFID_NUMBER_SEGMENT_A3_LEN
        || !segments[0].chars().all(|c| c.is_ascii_uppercase())
    {
        return Err("site_sfid a3 segment invalid");
    }
    if segments[1].len() != SFID_NUMBER_SEGMENT_R5_LEN
        || !segments[1]
            .chars()
    ...
    if segments[2].len() != SFID_NUMBER_SEGMENT_T2P1C1_LEN
        || !segments[2]
            .chars()
            .all(|c| c.is_ascii_uppercase() || c.is_ascii_digit())
    {
    ...
    if segments[3].len() != SFID_NUMBER_SEGMENT_N9_LEN
        || !segments[3].chars().all(|c| c.is_ascii_digit())
    {
        return Err("site_sfid n9 segment invalid");
    }
    if segments[4].len() != SFID_NUMBER_SEGMENT_D4_LEN
...
mod tests {
    ...
    fn accepts_valid_format() {
        let r = validate_sfid_number_format("GFR-AH001-ZF0X-898100720-2026");
        assert_eq!(r, Ok("GFR-AH001-ZF0X-898100720-2026".to_string()));
    }

    #[test]
    fn rejects_bad_segment_count() {
        assert!(validate_sfid_number_format("GFR-AH001-ZF0X-898100720").is_err());
    }
    ...
    fn trims_whitespace() {
        assert_eq!(
            validate_sfid_number_format("  GFR-AH001-ZF0X-898100720-2026  "),
            Ok("GFR-AH001-ZF0X-898100720-2026".to_string())
        );
    }
}
