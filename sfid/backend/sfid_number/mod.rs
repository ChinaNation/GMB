// 中文注释:本模块集中导出 SFID 工具 API;部分导出由业务模块按需使用,
// 保留 allow 抑制"未使用重导出"告警。
#![allow(unused_imports)]

//! SFID 编码协议模块 — SFID 号码常量、枚举、生成、校验的唯一入口。
//!
//! 中文注释(铁律):
//! 任何新增的 SFID 编码逻辑(A3 类型、机构码、分类、校验、生成)都必须放在
//! 本模块下。行政区划真源属于 `crate::china`,本模块只在生成号码时读取省市代码。
//!
//! ## 子模块
//!
//! - [`a3`]               — A3 主体属性枚举(GMR/ZRR/ZNR/GFR/SFR/FFR)
//! - [`institution_code`] — 机构类型枚举(ZG/ZF/LF/SF/JC/JY/CB/CH/TG)
//! - [`category`]         — 机构分类(公安局/公权/私权),任务卡 2 使用
//! - [`validator`]        — SFID 号格式校验
//! - [`generator`]        — SFID 号生成
//! - [`model`]            — SFID admin 元信息 DTO
//! - [`admin`]            — SFID 管理端元信息接口

pub mod a3;
pub(crate) mod admin;
pub mod category;
pub mod generator;
pub mod institution_code;
pub(crate) mod model;
pub mod validator;

// 中文注释:对外聚合导出,方便业务模块只写 `use crate::sfid_number::*`。
pub use a3::{all_a3, A3};
pub use category::{classify, InstitutionCategory, PUBLIC_SECURITY_INSTITUTION_NAME};
pub use generator::{generate_sfid_code, GenerateSfidInput};
pub use institution_code::InstitutionCode;
#[allow(unused_imports)]
pub(crate) use model::*;
pub use validator::{
    validate_sfid_number_format, SFID_NUMBER_MAX_BYTES, SFID_NUMBER_SEGMENT_A3_LEN,
    SFID_NUMBER_SEGMENT_COUNT, SFID_NUMBER_SEGMENT_D4_LEN, SFID_NUMBER_SEGMENT_N9_LEN,
    SFID_NUMBER_SEGMENT_R5_LEN, SFID_NUMBER_SEGMENT_T2P1C1_LEN,
};
