...
use crate::models::{
    ...
    AdminPasskeyStatus, AdminSecurityGrant, AdminSecurityLevel,
};
use crate::*;

const ADMIN_ACTION_TTL_SECONDS: i64 = 300;
...
#[derive(Serialize)]
struct AdminSignedPayload<'a> {
    domain: &'static str,
    qr_proto: &'static str,
    action_id: &'a str,
    action_type: &'a str,
    actor_pubkey: &'a str,
    actor_province: &'a str,
    target: &'a str,
    request_hash: &'a str,
    before_hash: &'a str,
    after_hash: &'a str,
...
fn webauthn() -> Result<Webauthn, axum::response::Response> {
...
fn active_passkeys(store: &Store, admin_pubkey: &str) -> Vec<Passkey> {
...
fn cleanup_admin_security_challenges(store: &mut Store) {
    let now = Utc::now();
    store
        .admin_passkey_registration_challenges
...
fn signed_payload_text(payload: AdminSignedPayload<'_>) -> String {
...
fn field(key: &str, label: &str, value: &str) -> serde_json::Value {
...
fn verify_cold_wallet_signature(
