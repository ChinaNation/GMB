...
pub(crate) async fn handler(
    ...
    // SFID 不持有 admin slot 私钥,本期由 sheng_signer_cache 提供本省任一已登录 slot 的
    // 签名 Pair 作为 stand-in;链端验签若失败将返回 InvalidTx,由调用方观察。
    let Some(signer_pair) = state.sheng_signer_cache.any_for_province(province.as_str()) else {
        return crate::api_error(
...
mod tests {
    ...
    fn add_backup_call_bytes_layout_matches_pallet_index() {
        ...
        assert_eq!(call_bytes[1], 2, "call_idx must be 2 (add_sheng_admin_backup)");
    }
