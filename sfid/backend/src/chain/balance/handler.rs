//! 链余额查询 handler:`GET /api/v1/admin/chain/balance`。
//!
//! 调用 `frame_system::Account` 存储,截取 `data.free` 字段。
//! 链上 `Account` 结构是 `StorageMap<Blake2_128Concat, AccountId32, AccountInfo>`,
//! 因此 storage key = twox_128("System") + twox_128("Account") + blake2_128(account) + account。
//!
//! AccountInfo SCALE 编码布局:
//!   nonce: u32        (4 bytes)
...
use crate::chain::key_admins::call_chain_state_get_storage;
use crate::{api_error, require_admin_any, ApiResponse, AppState};

use super::dto::{ChainBalanceOutput, ChainBalanceQuery};

/// `frame_system::AccountInfo` 头部 4 个 u32(nonce/consumers/providers/sufficients)共 16 字节,
/// 紧接着是 `data: AccountData`,其中 `free: u128` 是 AccountData 的第一个字段。
...
///
/// 通过 SFID 后端配置的链节点(`SFID_CHAIN_WS_URL`)调用 `state_getStorage`
/// 拉取 `System.Account`,截取 AccountInfo 中的 `data.free`。
///
/// 账户在链上不存在(未上链)时返回 0。
pub(crate) async fn query_free_balance(account_pubkey_hex: &str) -> Result<u128, String> {
    ...
    let result = call_chain_state_get_storage(&storage_key).await?;
