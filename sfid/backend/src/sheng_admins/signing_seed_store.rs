...
//! 加密 seed 落盘:`storage/sheng_admins/signing_seeds/{province}_{pubkey_hex}.enc`
//! 加密算法:AES-256-GCM
...
fn master_kek() -> Result<[u8; 32], String> {
    let raw = std::env::var("SFID_MASTER_KEK_HEX")
        ...
        .map_err(|_| {
            "SFID_MASTER_KEK_HEX (or SFID_SIGNING_SEED_HEX fallback) not set".to_string()
        })?;
    let trimmed = raw
        .trim()
        .strip_prefix("0x")
...
mod tests {
    ...
    fn unique_tmp_dir(label: &str) -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        p.push(format!(
            "sfid_sheng_admins_signing_seed_test_{label}_{pid}_{nanos}"
        ));
        p
    }
