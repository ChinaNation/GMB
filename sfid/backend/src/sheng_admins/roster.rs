...
//!
//! Phase 4 起真实推链入口转移到 `crate::chain::sheng_admins::{add_backup, remove_backup}`,
...
/// 已被 `crate::chain::sheng_admins::add_backup` 取代,无活跃调用方。
...
/// 已被 `crate::chain::sheng_admins::remove_backup` 取代,无活跃调用方。
...
/// Phase 3 历史路径占位。真实推链已由 `crate::chain::sheng_admins::{add_backup, remove_backup}`
/// 在 Phase 7 切真,无活跃调用方;保留仅为暂不破坏 module 边界,后续清理。
async fn push_chain_placeholder(name: &str) -> Result<(), RosterError> {
    tracing::warn!(
        "legacy roster placeholder invoked for {name}; real path is chain::sheng_admins"
    );
