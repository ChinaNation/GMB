...
/// 开发期一次性清理：删除 sfid 未固化的机构关联的 cpms_site_keys。
///
/// 旧代码用随机 UUID 生成 site_sfid，与机构 sfid_id 不一致。
/// 新代码在首次生成 QR1 时固化 sfid_id（sfid_finalized=true）。
/// sfid_finalized=false 的机构如果有 cpms_site_keys，一定是旧代码产物——直接删除，
/// 让管理员用新逻辑重新生成 QR1。
pub(crate) async fn cleanup_stale_cpms_sites(state: &AppState) {
    ...
    // 遍历分片，收集需要删除的 site_sfid
    let mut stale_by_province: std::collections::HashMap<String, Vec<String>> =
        std::collections::HashMap::new();
    let scan_result = state
        .sharded_store
        .for_each_province(|province, shard| {
            for site in shard.cpms_site_keys.values() {
                if unfinalized.contains(&(
                    site.admin_province.clone(),
                    site.city_name.clone(),
                    site.institution_code.clone(),
                )) {
        ...
        .await;
    if let Err(e) = scan_result {
        tracing::warn!(error = %e, "cleanup_stale_cpms_sites: shard scan failed");
        return;
    }
    if stale_by_province.is_empty() {
        return;
    }

    let total: usize = stale_by_province.values().map(|v| v.len()).sum();
    for (province, sfids) in &stale_by_province {
        let sfids_owned = sfids.clone();
        if let Err(e) = state
            .sharded_store
            .write_province(province, move |shard| {
            ...
            .await
        {
            tracing::warn!(province, error = %e, "cleanup_stale_cpms_sites: write_province failed");
        }
    }
    ...
    // 双写 legacy store
    {
        match state.store.write() {
            Ok(mut store) => {
                for sfids in stale_by_province.values() {
                    for sfid in sfids {
                        store.cpms_site_keys.remove(sfid);
                    }
                }
            ...
            Err(e) => {
                tracing::warn!(error = %e, "dual-write legacy store failed (cleanup stale cpms)");
            }
        }
    }
    ...
    tracing::info!(
        count = total,
        "cleaned up stale CPMS sites (sfid not finalized, generated by old code)"
    );
}
