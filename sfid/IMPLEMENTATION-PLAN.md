...
## 1. 目标
- 基于既定技术栈 `Tauri + React + TypeScript + Vite` 完成 SFID 独立系统上线。
- 完成“人工绑定 + 自动投票凭证”闭环。
- 全程不修改区块链业务代码，仅做接口对接。
- 分阶段密钥策略：`v1.0` 单国家级签名密钥，`v2.0` 升级分层密钥体系。
...
### M4（v2.0 预研与升级窗口）
交付目标：
- 建立国家 Root + 省级签名密钥分层模型
- 对接链上证书链验签能力

任务清单：
1. 设计省级公钥证书格式与签发流程
2. Root 离线签发、吊销、省级应急切换流程
3. 凭证增加 `signer_scope` 与证书链字段（兼容 v1）
4. 双轨兼容窗口与迁移回滚预案
