#!/usr/bin/env python3
"""按公法人(GFR)+监察院(JC)生成国家/联邦机构与各省监察院身份码与账户哈希。"""
...
def build_records(sc, date: str, secret: str):
    records = []
    zs_city_name, zs_city_code = sc.CITY_OPTIONS_BY_PROVINCE["ZS"][0]
    zs_orgs = [
        "国家监察院",
        "联邦廉政署",
        "联邦调查署",
    ...
    for seq, org_name in enumerate(zs_orgs):
        records.append(
            make_record(
                sc,
                org_name,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate national+provincial supervision codes by GFR+JC")
