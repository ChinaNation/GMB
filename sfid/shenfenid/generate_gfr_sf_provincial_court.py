#!/usr/bin/env python3
"""按公法人(GFR)+司法院(SF)生成每省司法院身份码与账户哈希。"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate provincial court codes by GFR+SF")
    ...
    print("province\torg\tcity\tcode\taccount")
    for r in records:
        print(f"{r['province_name']}\t{r['org_name']}\t{r['city_name']}\t{r['code']}\t{r['account']}")
    return 0
