...
## 1. 目标与范围
- 目标：建设独立 SFID 系统，支持人工绑定、自动投票资格凭证、全量审计。
- 范围：SFID 桌面端、SFID API、签名服务、数据库、权限与容灾。
...
2. `archive_bindings`：索引号、公钥、sfid_code 绑定关系
...
- `archive_bindings.sfid_code` 唯一（建议加）

## 8. 对接边界（与区块链）
- SFID 输出：`identity_hash/sfid_code + nonce + signature + key_id + expired_at`
- 区块链接收并验签，负责链上防重放
- SFID 不写链，不依赖链内私有实现
...
- [ ] DB 约束冻结
- [ ] 容灾拓扑确认
- [ ] 前端组件库与主题规范冻结（Ant Design + SFID 自定义主题）
