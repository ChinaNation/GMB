...
- SFID 为独立联网系统，只通过标准接口与轻节点/区块链接入。
- 绑定环节人工执行，投票凭证环节自动执行。
- 密钥体系分阶段：
...
## 5. 模块设计
1. `sfid-desktop-admin`
- 工单列表、绑定确认、异常处理、操作审计查询。
- 录入校验：索引号格式、重复提示、公钥校验。
- UI 统一采用 `Ant Design` 组件，禁止默认主题直出；必须使用 SFID 主题 token。
...
2. `archive_bindings`
- `binding_id`, `archive_index`, `account_pubkey`, `sfid_identity_hash`, `status`, `bound_by`, `bound_at`, `updated_at`
