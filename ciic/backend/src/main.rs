...
async fn main() {
    ...
    let state = AppState {
        ...
        admin_user: std::env::var("CIIC_ADMIN_USER").unwrap_or_else(|_| "admin".to_string()),
        admin_password: std::env::var("CIIC_ADMIN_PASSWORD")
            .unwrap_or_else(|_| "admin123".to_string()),
        signing_key,
        key_id: std::env::var("CIIC_KEY_ID").unwrap_or_else(|_| "ciic-master-v1".to_string()),
    ...
    info!("ciic-backend listening on http://{}", addr);
    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .expect("bind ciic backend listener");
    axum::serve(listener, app)
        .await
        .expect("run ciic backend server");
}
...
async fn admin_list_citizens(
    ...
    for b in store.bindings_by_pubkey.values() {
        rows.push(CitizenRow {
            ...
            ciic_code: Some(b.ciic_code.clone()),
            is_bound: true,
        });
    }
...
async fn admin_query_by_pubkey(
    ...
    Json(ApiResponse {
        ...
        data: AdminQueryOutput {
            ...
            ciic_code: binding.map(|b| b.ciic_code.clone()),
        },
    })
    .into_response()
}
...
async fn admin_bind_confirm(
    ...
    if let Some(existing) = store.bindings_by_pubkey.get(&input.account_pubkey) {
        ...
        let payload = BindingPayload {
            ...
            ciic_code: existing.ciic_code.clone(),
    ...
    let ciic_code = deterministic_ciic_code(&state, &input.archive_index, &input.account_pubkey);
    let seq = store
        .pending_by_pubkey
    ...
    let binding = BindingRecord {
        ...
        archive_index: input.archive_index.clone(),
        ciic_code: ciic_code.clone(),
        ciic_signature: proof.signature_hex.clone(),
        bound_at,
        bound_by: admin_user,
    };
    ...
    if let Some(binding) = store.bindings_by_pubkey.get(&query.account_pubkey) {
        Json(ApiResponse {
            ...
            data: BindResultOutput {
                ...
                is_bound: true,
                ciic_code: Some(binding.ciic_code.clone()),
                ciic_signature: Some(binding.ciic_signature.clone()),
                message: "ciic bind success".to_string(),
            },
        })
        .into_response()
    } else {
...
async fn verify_vote_eligibility(
    ...
    if let Some(binding) = store.bindings_by_pubkey.get(&input.account_pubkey) {
        ...
        let vote_payload = VotePayload {
            ...
            version: "v1",
            account_pubkey: input.account_pubkey.clone(),
            ciic_code: binding.ciic_code.clone(),
...
fn seed_demo_record(state: &AppState) {
    ...
    for seq in 1..=total {
        ...
        if seq <= bound_total {
            ...
            let ciic = deterministic_ciic_code(state, &archive, &pubkey);
            let binding_payload = BindingPayload {
                kind: "bind",
                version: "v1",
                account_pubkey: pubkey.clone(),
                archive_index: archive.clone(),
                ciic_code: ciic.clone(),
            ...
            store.bindings_by_pubkey.insert(
                ...
                BindingRecord {
                    ...
                    archive_index: archive.clone(),
                    ciic_code: ciic,
                    ciic_signature: proof.signature_hex,
                    bound_at: now,
...
fn deterministic_ciic_code(state: &AppState, archive_index: &str, account_pubkey: &str) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"ciic-code-v1|");
...
fn load_signing_key() -> SigningKey {
    let raw = std::env::var("CIIC_SIGNING_SEED_HEX").unwrap_or_else(|_| {
        // Dev fallback seed. Replace with secure offline-generated seed in production.
        "ciic-dev-master-seed-v1".to_string()
    });
