...
struct ImMlsProvider {
    crypto: RustCrypto,
    storage: MemoryStorage,
}

impl OpenMlsTraitsProvider for ImMlsProvider {
...
#[no_mangle]
pub unsafe extern "C" fn gmb_im_mls_create_key_package_json(
...
/// - 返回字符串必须由 `smoldot_free_string` 释放。
#[no_mangle]
pub unsafe extern "C" fn gmb_im_mls_two_party_smoke_json(
    request_json: *const c_char,
    error_out: *mut *mut c_char,
) -> *mut c_char {
...
/// - 返回字符串必须由 `smoldot_free_string` 释放。
#[no_mangle]
pub unsafe extern "C" fn gmb_im_mls_encrypt_json(
    request_json: *const c_char,
    error_out: *mut *mut c_char,
) -> *mut c_char {
...
/// - 返回字符串必须由 `smoldot_free_string` 释放。
#[no_mangle]
pub unsafe extern "C" fn gmb_im_mls_decrypt_json(
    request_json: *const c_char,
    error_out: *mut *mut c_char,
) -> *mut c_char {
...
fn create_key_package_json(request_json: *const c_char) -> Result<String, String> {
    ...
    require_non_empty("wallet_chat_account", &request.wallet_chat_account)?;
    ...
    let (key_package_hex, cipher_suite, device_public_key_hex) =
        if let Some(dir) = request.state_store_dir.as_deref() {
            ...
            (
                key_package_hex,
                format!("{:?}", GMB_IM_MLS_CIPHERSUITE),
                device_public_key_hex,
            )
        } else {
            let provider = OpenMlsRustCrypto::default();
            let (credential, signer) = generate_credential(
                format!("{}:{}", request.wallet_chat_account, request.device_id).into_bytes(),
                GMB_IM_MLS_CIPHERSUITE.signature_algorithm(),
                &provider,
            )?;
            let bundle = generate_key_package(&provider, &signer, credential)?;
            let key_package_hex = hex::encode(
    ...
    let response = json!({
        "protocol_version": 1,
        "owner_wallet_account": request.wallet_chat_account,
...
fn two_party_smoke_json(request_json: *const c_char) -> Result<String, String> {
    ...
    let group_config = MlsGroupCreateConfig::builder()
        .ciphersuite(GMB_IM_MLS_CIPHERSUITE)
    ...
    let response = json!({
        ...
        "cipher_suite": format!("{:?}", GMB_IM_MLS_CIPHERSUITE),
...
fn encrypt_json(request_json: *const c_char) -> Result<String, String> {
    ...
    require_non_empty("recipient_chat_account", &request.recipient_chat_account)?;
    ...
    let response = json!({
        ...
        "recipient_chat_account": request.recipient_chat_account,
        "cipher_suite": format!("{:?}", GMB_IM_MLS_CIPHERSUITE),
...
fn load_provider(state_dir: &Path) -> Result<ImMlsProvider, String> {
...
fn save_provider(state_dir: &Path, provider: &ImMlsProvider) -> Result<(), String> {
    fs::create_dir_all(state_dir).map_err(|error| format!("创建 MLS 状态目录失败: {error}"))?;
...
fn ensure_device_signer(
    provider: &ImMlsProvider,
    state_dir: &Path,
    wallet_chat_account: &str,
    device_id: &str,
) -> Result<(CredentialWithKey, SignatureKeyPair), String> {
    let record_path = device_record_path(state_dir);
    let signature_algorithm = GMB_IM_MLS_CIPHERSUITE.signature_algorithm();
    if record_path.exists() {
        ...
        if record.wallet_chat_account != wallet_chat_account || record.device_id != device_id {
        ...
        let credential = credential_with_public_key(wallet_chat_account, device_id, public_key);
        return Ok((credential, signer));
    }

    let (credential, signer) = generate_credential(
        format!("{wallet_chat_account}:{device_id}").into_bytes(),
        signature_algorithm,
        provider,
    )?;
    let record = DeviceRecord {
        wallet_chat_account: wallet_chat_account.to_string(),
...
fn credential_with_public_key(
    ...
    CredentialWithKey {
        credential: BasicCredential::new(format!("{wallet_chat_account}:{device_id}").into_bytes())
            .into(),
...
mod tests {
    ...
    fn creates_real_openmls_key_package() {
        let request =
            CString::new(r#"{"wallet_chat_account":"alice-wallet","device_id":"alice-phone"}"#)
                .expect("request should be valid");
        let response =
            create_key_package_json(request.as_ptr()).expect("key package should be created");
        let json: serde_json::Value =
            serde_json::from_str(&response).expect("response should be json");
        assert_eq!(json["owner_wallet_account"], "alice-wallet");
