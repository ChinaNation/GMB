...
| 注册局                  | Registry                                      | 每市自治节点托管的链外系统，内嵌身份识别码（CID）能力，负责身份 ID、投票资格、人口快照、公民护照与机构营业执照颁发 |
| 身份识别码               | CID / Identity Identification Code            | 注册局内嵌能力，签发统一身份 ID、投票资格和人口快照凭证 |
...
* 选举投票模块是独立模块，用于按公民宪法选举各类公职人员，既包括公民普选，也包括公权机构成员的内部互选；选举投票只接收身份绑定凭证或哈希，不接收明文 CID，同一提案同一身份绑定只能投一次，选民集视职位而定（普选取全体认证公民快照，互选取特定机构现任成员快照）；<br><span class="whitepaper-en">The election-vote module is an independent module used to elect public officials according to the Citizen Constitution, covering both general citizen elections and internal mutual-elections among the members of public-authority institutions. Election voting accepts only identity-binding credentials or hashes, not plaintext CID, and the same identity binding may vote only once on the same proposal. The electorate depends on the position: a general election takes a snapshot of all certified citizens, while a mutual-election takes a snapshot of the current members of the specific institution.</span>
...
### 5.9.1.身份识别码校验<br><span class="whitepaper-heading-en">5.9.1. cid-system</span>
...
* 注册局（Registry）是每市自治节点托管的链外系统，去中心化部署，每市独立自治运行；注册局内嵌身份识别码（CID）能力，负责为公民、自然人、组织机构等签发统一身份 ID，并产出投票资格凭证和人口快照凭证。如此，可鉴权的统一身份 ID 由注册局提供，让个人可使用实名参与互联网与链上投票。<br><span class="whitepaper-en">The Registry is an off-chain system hosted by each city's self-governing node. It is deployed in a decentralized manner, with each city running independently and autonomously. The Registry has embedded identity identification code (CID) capability and is responsible for issuing unified identity IDs to citizens, natural persons, organizations, institutions, and other entities, and for producing voting-eligibility credentials and population-snapshot credentials. Accordingly, verifiable unified identity IDs are provided by the Registry, enabling individuals to participate in the Internet and on-chain voting under verified real-name identities.</span>
