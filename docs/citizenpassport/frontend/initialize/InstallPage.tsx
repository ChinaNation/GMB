...
export default function InstallPage() {
  ...
  const handleQr1Scanned = async (qrContent: string) => {
    ...
    try {
      ...
      if (res.data) {
        setMsg(`机构 CID: ${res.data.cid_number}`);
      }
      await load();
    } catch (e) {
        ...
        <div className="login-card__body">
          ...
          {currentStep === 1 && (
            <div className="card" style={{ boxShadow: 'none', border: '1px solid var(--color-border)' }}>
              <div className="card__title" style={{ textAlign: 'center', borderLeft: 'none', paddingLeft: 0 }}>扫描 CID 安装授权二维码</div>
          ...
          {currentStep === 3 && (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <div style={{ color: 'var(--color-text-secondary)', fontSize: 13, marginBottom: 4 }}>
                机构 CID: <strong>{status?.cid_number}</strong>
              </div>
