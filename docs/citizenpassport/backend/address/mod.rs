...
pub(crate) async fn sync_installed_city_address(db: &sqlx::PgPool) -> Result<(), String> {
    let cid_number: Option<String> =
        sqlx::query_scalar("SELECT cid_number FROM system_install WHERE id = 1")
            .fetch_optional(db)
            .await
            .map_err(|e| format!("query install failed: {e}"))?
            .flatten();
    let Some(cid_number) = cid_number.filter(|v| !v.trim().is_empty()) else {
        return Ok(());
    };
    sync_city_address_by_cid(db, &cid_number).await
}

pub(crate) async fn sync_city_address_by_cid(
    db: &sqlx::PgPool,
    cid_number: &str,
) -> Result<(), String> {
    let mut tx = db
    ...
    sync_city_address_by_cid_in_tx(tx.as_mut(), cid_number).await?;
    tx.commit()
        .await
...
pub(crate) async fn sync_city_address_by_cid_in_tx(
    conn: &mut sqlx::PgConnection,
    cid_number: &str,
) -> Result<(), String> {
    let (province_code, city_code, _) = find_install_city(cid_number)?;
...
pub(crate) fn validate_install_area(
    ...
    let (province_code, city_code, area) = find_install_city(cid_number)?;
...
fn find_install_city(cid_number: &str) -> Result<(String, String, china::CityArea), String> {
    let (province_code, city_code) = parse_cid_area_codes(cid_number)?;
...
fn parse_cid_area_codes(cid_number: &str) -> Result<(&str, &str), String> {
    let r5 = cid_number
        .trim()
        .split('-')
        .next()
        .ok_or_else(|| "install cid r5 segment missing".to_string())?;
    if r5.len() != 5 {
        return Err("install cid r5 segment invalid".to_string());
    }
    let (province_code, city_code) = r5.split_at(2);
    if city_code == "000" {
        return Err("cpms install cid must bind to a city public security bureau".to_string());
    }
