...
if [[ -z "${CPMS_KEY_ENCRYPT_SECRET:-}" ]]; then
  CPMS_KEY_FILE="${ROOT_DIR}/.citizenpassport_key"
...
echo "=== 清理残留 CPMS 进程 ==="
STALE_PIDS="$(pgrep -f 'citizenpassport-back|target/debug/citizenpassport|citizenpassport/backend' || true)"
...
if [[ "$CPMS_RESET" == "1" ]]; then
  echo "=== CitizenPassport 显式重置：重建数据库 ==="
  ...
  echo "=== CitizenPassport 保留现有数据库（如需重置请执行：CPMS_RESET=1 ./citizenpassport.sh 或 ./citizenpassport.sh --reset）==="
...
wait_backend_ready() {
  ...
  for ((i=1; i<=retries; i++)); do
    if ! kill -0 "$BACKEND_PID" 2>/dev/null; then
      ...
      echo "CitizenPassport 仍处于开发期，可执行：./citizenpassport.sh --reset 重建开发库。"
      return 1
    fi
