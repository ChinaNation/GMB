...
7. 进入执行阶段后，由当前主聊天入口按需自动调度专业工作线程，不要求用户手工指定 `Blockchain Agent`、`OnChina Agent`、`Mobile Agent` 或 `Wallet Agent`
...
- 使用中文沟通
- 不清楚逻辑时必须先和用户沟通
- 禁止猜测硬规则：任何 AI 线程不得在未读取仓库代码、文档、任务卡或真实运行输出前，对现有实现、运行状态、字段命名、扣费、分账、权限、存储、部署结果作推断式结论；对不了解或未复查的实现，必须先全仓搜索、读取相关代码和文档，必要时执行只读检查，再回复用户。无法确认时只能明确说明“尚未检查/无法确认”，不得用“如果当前实现没有……”等假设口径暗示事实。
...
- `Blockchain Agent / OnChina Agent / Mobile Agent / Wallet Agent` = 由当前主聊天入口按任务需要调度的专业工作线程
