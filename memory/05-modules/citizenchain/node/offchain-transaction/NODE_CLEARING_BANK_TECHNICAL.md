...
管理员记录按目标机构类型分流：公权为 `admin_account + cid_number + family_name + given_name`，私权为 `admin_account + family_name + given_name`。账户用于人员识别和签名，但账户本身没有机构业务权限；公权非空公民 CID 只引用 `citizen-identity` 真源。
