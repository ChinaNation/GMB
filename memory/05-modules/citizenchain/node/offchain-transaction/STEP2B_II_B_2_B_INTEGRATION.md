...
- `FullDeps` 加字段 `offchain_clearing_rpc: Option<Arc<crate::transaction::offchain_transaction::rpc::OffchainClearingRpcImpl>>`
...
- 2026-04-19:Step 2b-ii-β-2-b 完整落地,清算行节点端到端链路打通(citizenapp RPC → ledger → packer → pool → extrinsic → runtime)。offchain/ 子树 + service.rs + rpc.rs + cli.rs + command.rs + node_runner.rs 零编译错误。
