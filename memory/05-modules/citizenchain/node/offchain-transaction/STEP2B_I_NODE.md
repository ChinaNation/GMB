...
| 职责 | 后端路径 | 前端路径 |
|---|---|---|
| 功能入口/状态机 | `citizenchain/node/src/transaction/offchain_transaction/commands.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/section.tsx` |
| OnChina 查询 | `citizenchain/node/src/transaction/offchain_transaction/cid.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/cid.tsx` |
| 清算行节点声明 | `citizenchain/node/src/transaction/offchain_transaction/signing.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/register.tsx` |
| 链上节点查询 | `citizenchain/node/src/transaction/offchain_transaction/chain.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/detail.tsx` |
| 管理员解密 | `citizenchain/node/src/transaction/offchain_transaction/decrypt.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/admin.tsx` |
| 节点端点信息 | `citizenchain/node/src/transaction/offchain_transaction/health.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/node.tsx` |
| DTO 类型 | `citizenchain/node/src/transaction/offchain_transaction/types.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/types.ts` |
| 清算行命令 API | `citizenchain/node/src/transaction/offchain_transaction/commands.rs` | `citizenchain/node/frontend/transaction/offchain-transaction/api.ts` |
| 清算行页面样式 | 暂无后端 | `citizenchain/node/frontend/transaction/offchain-transaction/styles.css` |
| 清算行密钥容器 | `citizenchain/node/src/transaction/offchain_transaction/keystore.rs` | 暂无桌面前端页面 |
| 清算行启动接线 | `citizenchain/node/src/transaction/offchain_transaction/bootstrap.rs` | 暂无桌面前端页面 |
| 结算引擎 | `citizenchain/node/src/transaction/offchain_transaction/settlement/*` | 暂无桌面前端页面 |
...
- `commands.rs`:清算行页面的 OnChina 查询、节点查询、端点自测、扫码签名、管理员解密命令。
...
(中间需要先把 offchain-transaction 的 OffchainBatchItem 加 DecodeWithMemTracking 派生,本步已修)
```
...
3. `packer::pack_and_submit` 实现:
   - `ledger.take_pending_for_batch(MaxBatchSize)` → 构造 `OffchainBatchItem` 列表
...
- 2026-04-29:二次目录收口。`offchain_keystore.rs` 迁入 `offchain/keystore.rs`,
  ...
  `frontend/transaction/offchain-transaction/api.ts` / `styles.css`。
