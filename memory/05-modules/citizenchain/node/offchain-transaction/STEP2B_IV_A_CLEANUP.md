...
Step 2b-iii-b 完成后,新清算行节点(citizenapp RPC → ledger → packer → pool → runtime → listener → reserve_monitor)已完整闭环并在共存模式下可运行。**本步**把旧省储行清算在节点端的 3 个源文件、`service.rs` 的全局配置 + gossip worker、`rpc.rs` 的 3 个 RPC 端点和 2 个 helper、`home/process/mod.rs` 的老 ledger 切入,全部删除。
...
- citizenapp 侧 `offchain.dart` RPC 客户端 + `offchain_pay_page.dart` 页面(Step 2c 重写时一并删除)
...
- 2026-04-29 二次目录收口时,根级 `node/src/offchain_keystore.rs` 已迁入
  `node/src/transaction/offchain_transaction/keystore.rs`,引用统一改为 `crate::transaction::offchain_transaction::keystore::*`
...
| 删除 | 说明 |
|---|---|
| `use crate::transaction::offchain_transaction_ledger::*;` | 顶部 import |
...
### 2.8 `citizenapp/lib/trade/onchain/onchain_trade_page.dart`

- 删除 `import 'package:citizenapp/trade/offchain/offchain_pay_page.dart';`
...
(零 error,仅 Tauri `frontend/dist` 门禁与 pre-existing settlement/submitter.rs
 测试 `sp_core::sr25519` unused import 警告,均与本步无关)

$ cd citizenapp && flutter analyze lib/trade/onchain/onchain_trade_page.dart
No issues found!
```
...
| Runtime 侧老 Calls 仍存在占 `RuntimeCall` 枚举槽位 | **P3** | 历史金额提取分类当时仍在；后续已删除 |
...
- 2026-04-20:Step 2b-iv-a 完整落地。删除 3 个节点旧文件(736 行)+ `service.rs` 9 块(约 50 行)+ `rpc.rs` 378 行(3 RPC + 2 helper + 字段 + import)+ `home/process/mod.rs` 58 行 + 头注释净化若干处。citizenapp 老入口在 `onchain_trade_page.dart` 下架。`cargo check -p node --tests` 零新 error,`flutter analyze` 零 issue。
