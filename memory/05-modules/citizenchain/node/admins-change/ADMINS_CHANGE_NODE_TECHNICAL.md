...
Node 桌面端只负责按 pallet 分流读取管理员事实、展示人员资料和岗位任职、激活本机管理员账户。所有机构记录统一为 `account_id + cid_number + family_name + given_name`。Node 不提供机构管理员集合直接变更入口；机构岗位或任职变化由 `entity` 独立更新，不得派生或覆盖 `admins`。
...
├── mod.rs          # 模块导出与边界
├── account_id.rs   # AccountId 严格格式校验
...
- `governance` 和 `transaction/offchain` 只消费统一的管理员账户与岗位任职 DTO。
...
1. 从 `PublicAdmins` 或 `PrivateAdmins::AdminAccounts` 读取机构码和管理员集合；storage key 的 CID 是机构 CID，两类 value 中的 `cid_number` 都是该管理员的公民 CID 引用。
2. 从对应 `PublicManage` 或 `PrivateManage::InstitutionRoles` 读取有效岗位。
3. 从对应 `InstitutionRoleAssignments` 读取有效任职。
4. 按管理员账户聚合全部有效岗位任职；同一账户在同一机构只显示一张卡片。
5. 管理员允许没有岗位，此时保留管理员人员记录并返回空 `assignments`；只有有效任职引用不存在或停用岗位时才返回链上状态不一致错误。

Node 对公权、私权统一使用 `InstitutionAdmins<Vec<Admin<[u8; 32]>>>` 精确 SCALE 解码，拒绝尾随字节、旧纯账户数组、旧三字段、重复账户和非法 UTF-8，不保留旧布局兼容。
...
| 字段 | 中文说明 |
|---|---|
| `account_id` | 管理员唯一账户 ID，固定为小写 `0x` 加 64 位十六进制 |
| `family_name` | 管理员姓，只用于展示 |
| `given_name` | 管理员名，只用于展示 |
| `assignments` | 该账户在本机构的全部有效岗位任职 |
...
机构管理员 DTO 不包含公民 CID、`creator`、`created_at` 或 `updated_at`。Node 只显示链上管理员记录已经保存的姓、名，不从账户反推公民身份资料。
...
- 激活前必须在 finalized 链上状态中确认签名公钥对应的账户属于该机构当前管理员集合；管理员没有岗位也不影响管理员授权。
- 验签后写入 `{app_data}/activated-admin-accounts.json`；记录按 `account_id`、机构 CID、机构码和类型归档。
- 每次读取已激活管理员时重新与链上当前管理员集合交叉校验；从 `admins` 移除后本地激活立即失效。
- 所有机构必须提供 CID；Node 按链上真实管理员 pallet 和机构记录读取账户，不接受旧 `accountHex` 参数。

## Tauri 命令

- `get_institution_admins_state`：读取机构管理员账户及岗位任职。
- `get_account_balances`：在同一 finalized 块批量读取管理员账户余额。
- `build_activate_admin_request`：验证链上任职后构建本地激活签名请求。
- `verify_activate_admin`：验证冷钱包签名并保存本地激活凭证。
- `get_activated_admins`：读取仍与链上管理员集合一致的本机激活管理员。
...
`InstitutionAssignmentCard` 固定按一个管理员账户展示：

- 顶部：序号和激活/投票操作状态。
- 账户区：按中文顺序合并的管理员姓、名，管理员账户的 SS58 展示地址和 finalized 余额。
- 任职区：逐条显示岗位名称、岗位代码、任期、来源和来源依据。

卡片不展示公民 CID。同一账户有多个岗位时在同一卡片内分条展示，没有岗位时显示“暂无岗位”；投票资格仍按 `account_id` 唯一计算，不能按姓名或岗位重复生成同一机构内的票。
