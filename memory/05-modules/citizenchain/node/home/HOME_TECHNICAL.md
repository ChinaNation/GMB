...
- 桌面端白皮书由 `docs/《白皮书》.md` 经 `citizenchain/scripts/generate-local-docs.mjs` 内置到前端 bundle，显示样式由 `node/frontend/app/styles/global.css` 的 `.local-doc-shell.doc-whitepaper` 负责
- `docs/《白皮书》.md` 是白皮书排版结构唯一真源：中文标题与英文标题必须写在同一 Markdown 标题内并用 `<br>` 换行；列表项中文与对应英文必须写在同一个列表项内并用 `<br>` 换行，不能依赖桌面端 tab 或官网二次规整段落结构
