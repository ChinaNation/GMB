# citizenchain/node/src/offchain/packer.rs · Step 2b-ii-α 技术说明
...
- `packer.rs` 完整业务逻辑(组 batch → 签名 → 提交 → 失败回滚)
...
**明确不做**(Step 2b-ii-β 接):
- `KeystoreBatchSigner`(用 `offchain_keystore::SigningKey` 签)
- `PoolBatchSubmitter`(拼 `RuntimeCall::OffchainTransactionPos(submit_offchain_batch_v2 {...})` 外包 `UncheckedExtrinsic` + 调 `TransactionPool`)
...
字段**逐字段对齐** runtime `offchain_transaction_pos::batch_item::OffchainBatchItemV2`,SCALE 编码字节完全一致。单测 `node_batch_item_encodes_deterministically` 断言编码长度 268 字节 + round-trip 成功。
...
Step 2b-iv 链上 `settlement::execute_clearing_bank_batch` 接入 batch_signature 校验时,**必须与本函数产生的消息逐字节一致**。
...
3. batch_seq = batch_seq_counter.fetch_add(1)  (内存,Step 3 持久化)
...
1. **`KeystoreBatchSigner`**
   - 从 `offchain_keystore::SigningKey` 拿到清算行管理员 sr25519 私钥
   - `sign_batch(message) -> [u8; 64]` 调 `sp_core::Pair::sign(&key, message)`
   - 放在 `offchain/keystore_signer.rs`(新文件)
...
2. **`PoolBatchSubmitter`**
   ...
   - `submit` 内:
     ...
     - 构造 `RuntimeCall::OffchainTransactionPos(submit_offchain_batch_v2 {...})`
     - 外包 `UncheckedExtrinsic`(需要签名,或 unsigned extrinsic 走特殊路径)
     - 调 `pool.submit_one(...)` 拿 `TransactionHash`
   - 放在 `offchain/pool_submitter.rs`(新文件)
