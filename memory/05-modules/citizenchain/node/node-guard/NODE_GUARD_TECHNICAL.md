...
| `citizenchain/node/src/core/node_guard/governance_skeleton.rs` | 90 个保护机构（89 个公权 + 1 个私权创世技术公司）管理员、固定岗位、固定岗位权限与任职的精确 RAW key、共享类型解码、固定制度/席位不变式与按机构触发条件；ADR-039 目标允许额外动态岗位 |
...
90 个创世机构（89 个公权 + 1 个私权技术公司），保护身份必须同时匹配机构码、CID 和主账户。普通区块只在 delta 触及受保护
...
`NodeGuard::new` 使用 block#0 状态执行启动自检并建立后续导入所需的节点基准。公权治理骨架规格来自编译进节点二进制的 `primitives::governance_skeleton`，私权创世技术公司规格来自 `primitives::cid::china::citizenchain`；节点直接使用 `admin-primitives::Admin { admin_account, family_name, given_name }`、`InstitutionAdmins` 和 `entity-primitives` 共享类型精确解码，不再维护手写 SCALE 镜像或纯账户兼容布局。姓名必须非空且为 UTF-8，但只用于展示；任职账户必须属于同机构 admins，同一岗位内不得重复占席；同一账户可以担任多个不同岗位。启动只枚举 90 个保护机构 CID 下的岗位、任职和权限子树，不扫描全部机构。技术公司的 `PrivateManage::Institutions` 中文全称必须逐字等于共享常量；法定代表人三字段允许同时为空，存在时账户必须与唯一 `LR` 任职一致。FRG 是一个 215 人管理员集合，43 个省专员岗位及每岗 5 席由 entity 任职真源表达。任职来源、引用与任期由 runtime 和投票引擎判定，不在原生组织结构守卫重复执法。全节点发行要求创世累计块数、累计金额均为 0 且不存在最近奖励审计；公民认证发行只允许 FRAME 规范空状态；GenesisPallet 要求三个创世事实逐字等于 primitives 且阶段/开发者开关使用规范缺省形态；省储行固定发行要求 43 个 `stake_account` 的完整账户状态逐户等于 `stake_amount` 且三项年度审计为空；CID 策略建立不可改写的创世账户索引基准。全部策略都不读取可被 runtime 升级改变的 metadata。
...
- 2026-07-19 最终校正后，固定岗位任职按岗位分别校验：普通固定岗位保持精确席位，LR 为 0..=1；账户必须属于 admins，同一岗位内去重，但允许同一账户兼任多个岗位。技术公司法定代表人三字段可与 LR 任职同时空缺。当前源码 fresh 创世哈希为 `0xa96d68d95c9f05f6ad893c93e4738e993bf031eb9eebb64506f0d7174418c805`，state root 为 `0x630708630485316c372a182a0ede4670c3d487353f8462719f58dc03ec4ec5c6`。

- 保护清单精确包含国家储委会 1 个、省储委会 43 个、省储行 43 个、国家司法院 1 个、联邦注册局 1 个和中国公民链技术股份有限公司 1 个，共 90 个；
- 前 89 个机构的 `PublicAdmins::AdminAccounts` 与技术公司的 `PrivateAdmins::AdminAccounts` 必须存在，且机构码、CID、主账户三者共同匹配；
...
- 管理员记录必须完整使用 `admin_account + family_name + given_name`，姓名非空且为 UTF-8；姓名变化不改变成员身份或权限；
...
- `LR` 岗位本身属于固定骨架且任职只能为 0 或 1；公权固定机构与技术公司均允许岗位空缺，技术公司法定代表人三字段存在时必须与 `LR` 任职一致。
...
|---|---|---|
| 固定治理骨架 | 89 个公权保护身份使用 `PublicAdmins/PublicManage`，私权创世技术公司使用 `PrivateAdmins/PrivateManage`；对应管理员、岗位、岗位权限、任职表均使用规范 key | 机构码/CID/协议主账户/管理员总数；强制 LR 与创世固定岗位的代码/名称/固定权限/所属 CID/精确席位；固定岗位任职不变量。额外普通动态岗位允许存在。技术公司额外校验法定代表人账户与 `LR` 任职一致。主账户只校验协议账户完整性，不作授权 key |
...
## 20. 私权创世技术公司接入验收（2026-07-19）

- 保护清单从 89 个公权机构扩展为 90 个机构；新增项固定从 `primitives::cid::china::citizenchain` 读取，不在 NodeGuard 复制公司 CID、账户、管理员或岗位常量；
- 技术公司使用 `PrivateAdmins::AdminAccounts` 与 `PrivateManage::Institutions/InstitutionRoles/InstitutionRoleAssignments`，其 RAW key 路由、完整状态分区和触发式普通区块复核与原 89 个公权机构共用同一策略；
- 守卫额外检查技术公司三个固定岗位的代码、名称和固定权限不变；产品经理、程序员各保持一席，`LR` 保持唯一岗位并允许 0/1 任职。任职账户必须属于三名 admins，同一账户可兼任不同岗位；法定代表人三字段与 `LR` 任职必须同时空缺或账户一致。公司机构记录、固定岗位、任职或管理员缺失及公私权错误路由均 fail-closed；普通动态岗位允许存在；
