...
| `citizenchain/node/src/core/node_guard/governance_skeleton.rs` | 90 个保护机构（89 个公权 + 1 个私权创世技术公司）管理员、岗位与任职的精确 RAW key、共享类型解码、目录/席位不变式与按机构触发条件 |
...
- 保护清单精确包含国家储委会 1 个、省储委会 43 个、省储行 43 个、国家司法院 1 个、联邦注册局 1 个和中国公民链技术有限公司 1 个，共 90 个；
...
|---|---|---|
| 固定治理骨架 | 89 个公权保护身份使用 `PublicAdmins/PublicManage`，私权创世技术公司使用 `PrivateAdmins/PrivateManage`；对应管理员、岗位、任职表均使用 `Blake2_128Concat` | 机构码/CID/协议主账户/管理员总数；岗位代码/名称/所属 CID/精确席位；任职 CID/岗位；任职钱包去重集合必须等于 admins。技术公司额外校验法定代表人账户与 `LR` 任职一致。主账户只校验协议账户完整性，不作管理员 key |
...
- 守卫额外检查技术公司三项固定岗位各一席、岗位任职账户并集精确等于三名 admins、`LR` 任职账户等于法定代表人账户；公司机构记录、岗位、任职或管理员缺失及公私权错误路由均 fail-closed；
