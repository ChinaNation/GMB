...
- 本次真实网络部分没有手工注入伪造坏块；坏块“不委派内层、不入库”的证据来自包装器矩阵，网络证据
  负责证明三节点合法链推进和同步一致。临时 chainspec、base-path、keystore、签名器和日志已删除。
- P2P 恶意候选块注入专项探测结论：当前节点 RPC 没有 `engine_*`、manual-seal 或任意 block submit
  ...
  不能代表 NodeGuard 永久规则坏块。若要补齐该项，需要另行确认临时恶意块生产器或专用导入 harness，
  生成结构完整且 state root 可验证的坏候选块。
- 已在 `citizenchain/crates/blockchain-test-harness/` 创建专用区块链测试 harness crate，并加入 workspace。
  ...
  5/5 通过；真实 `import-blocks` 基线中，合法 block#0 文件可导入，篡改 stateRoot 后的 block#0 文件
