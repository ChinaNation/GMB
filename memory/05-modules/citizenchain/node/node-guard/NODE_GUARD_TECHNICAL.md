...
`NodeGuard::new` 使用 block#0 状态执行启动自检并建立后续导入所需的节点基准。公权治理骨架规格来自编译进节点二进制的 `primitives::governance_skeleton`，私权创世公民链基金会规格来自 `primitives::cid::china::citizenchain`；节点分别使用 `admin-primitives::PublicAdmin { admin_account, cid_number, family_name, given_name }` 和私权 `Admin { admin_account, family_name, given_name }`，并通过共享 `InstitutionAdmins`、`entity-primitives` 类型精确解码，不再维护手写 SCALE 镜像或纯账户兼容布局。公权管理员的公民 CID、姓、名当前允许为空；一旦公民 CID 非空，runtime 必须按 `citizen-identity` 的双向索引校验 CID 与钱包绑定。私权管理员姓名保持非空 UTF-8 展示字段。任职账户必须属于同机构 `admins`，同一岗位内不得重复占席；同一账户可以担任多个不同岗位。启动只枚举 90 个保护机构 CID 下的岗位、任职和权限子树，不扫描全部机构。公民链基金会的 `PrivateManage::Institutions` 中文全称必须逐字等于共享常量；其一名私权管理员同时担任 `LR`、创世产品经理和创世程序员三个固定岗位。FRG 是一个 215 人管理员集合，43 个省专员岗位及每岗 5 席由 entity 任职真源表达。任职来源、引用与任期由 runtime 和投票引擎判定，不在原生组织结构守卫重复执法。全节点发行要求创世累计块数、累计金额均为 0 且不存在最近奖励审计；公民认证发行只允许 FRAME 规范空状态；GenesisPallet 要求三个创世事实逐字等于 primitives 且阶段/开发者开关使用规范缺省形态；省储行固定发行要求 43 个 `stake_account` 的完整账户状态逐户等于 `stake_amount` 且三项年度审计为空；CID 策略建立不可改写的创世账户索引基准。全部策略都不读取可被 runtime 升级改变的 metadata。
...
- 公权管理员记录必须使用 `admin_account + cid_number + family_name + given_name`；公民 CID、姓、名当前允许为空，非空公民 CID 由 runtime 对照 `citizen-identity` 唯一真源校验。私权管理员继续使用 `admin_account + family_name + given_name` 且姓名非空 UTF-8；展示字段变化不改变成员身份或权限；
