...
| `citizenchain/node/src/core/node_guard/governance_skeleton.rs` | 90 个保护机构（89 个公权 + 1 个私权创世公民链基金会）管理员、固定岗位、固定岗位权限与任职的精确 RAW key、共享类型解码、固定制度/席位不变式与按机构触发条件；ADR-039 目标允许额外动态岗位 |
...
90 个创世机构（89 个公权 + 1 个私权公民链基金会），保护身份必须同时匹配机构码、CID 和主账户。普通区块只在 delta 触及受保护
机构的精确管理员、岗位或任职 key 时检查对应机构；一般机构治理变化不触发本策略。
只有 `:code` 变化时才全量复核 90 个机构，并枚举复核所有存量内部阈值快照；因此新 WASM
不能通过不改旧 storage key 绕过 NRC/PRC/PRB/NJD/FRG 的固定阈值。

若未命中，依据上一状态已经通过守卫的归纳前提走快路径。若命中，则使用“本块 delta 优先、
父状态补齐”的后置状态视图检查管理员集合、固定岗位及其权限和任职席位。ADR-039 迁移后，额外动态岗位不属于固定目录违规。违规或无法完成检查时
...
- 当前一次扫描同时抽取 90 个保护机构治理状态、NSN/NRP/NED 三张精确组成状态、内部阈值快照及其提案、`FullnodeIssuance`、`CitizenIssuance`、`GenesisPallet`、`ProvincialBankInterest`、手续费状态、相关 `System::Account`、`Balances::TotalIssuance` 及 CID 规范表；后续策略必须继续复用同一份完整导入态；
- 普通机构允许依法删除，CID 策略不再伪造“所有机构永久存在”的历史单调性；任意高度完整状态均可校验，但 block#0 精确机构集合仍必须与启动时建立的创世基准一致。

完整态实现使用一次共享分区扫描：输入 key 只遍历一次，并只抽取 90 个受保护机构的治理状态以及全节点发行/账户、
公民发行、创世模块、省储行固定发行、手续费制度和 CID 所需状态。共享分区测试证明输入 key 只扫描一次，
并覆盖统一 `KnownBad` 返回和拒绝路径内层导入零调用；真实 warp 导入和峰值内存仍必须在任务关闭前完成。

## 6. 启动锚定

`NodeGuard::new` 使用 block#0 状态执行启动自检并建立后续导入所需的节点基准。公权治理骨架规格来自编译进节点二进制的 `primitives::governance_skeleton`，私权创世公民链基金会规格来自 `primitives::cid::china::citizenchain`；节点对公权、私权统一使用 `admin-primitives::Admin { account_id, cid_number, family_name, given_name }`，并通过共享 `InstitutionAdmins`、`entity-primitives` 类型精确解码，不再维护手写 SCALE 镜像、三字段或纯账户兼容布局。非空公民 CID 必须按 `citizen-identity` 的双向索引校验 CID 与账户绑定。任职账户必须属于同机构 `admins`，同一岗位内不得重复占席；同一账户可以担任多个不同岗位。启动只枚举 90 个保护机构 CID 下的岗位、任职和权限子树，不扫描全部机构。公民链基金会的 `PrivateManage::Institutions` 中文全称必须逐字等于共享常量；其一名私权管理员以完整四字段记录同时担任 `LR`、创世产品经理和创世程序员三个固定岗位。FRG 是一个 215 人管理员集合，43 个省专员岗位及每岗 5 席由 entity 任职真源表达。任职来源、引用与任期由 runtime 和投票引擎判定，不在原生组织结构守卫重复执法。全节点发行要求创世累计块数、累计金额均为 0 且不存在最近奖励审计；公民认证发行只允许 FRAME 规范空状态；GenesisPallet 要求三个创世事实逐字等于 primitives 且阶段/开发者开关使用规范缺省形态；省储行固定发行要求 43 个 `stake_account` 的完整账户状态逐户等于 `stake_amount` 且三项年度审计为空；CID 策略建立不可改写的创世账户索引基准。全部策略都不读取可被 runtime 升级改变的 metadata。
...
## 7. 当前策略与 ADR-039 目标：固定治理骨架

本策略冻结固定制度结构，不冻结依法轮换的成员身份：

- 2026-07-19 第 4 步已改为只精确保护强制 LR、创世固定岗位及其固定权限；保护机构依法新增的普通动态岗位由 runtime 治理，不会被节点误判为额外固定岗位。
- 2026-07-20 管理员与机构阈值重构后，固定岗位任职按岗位分别校验：普通固定岗位保持精确席位；账户必须属于 `admins`，同一岗位内去重，但允许同一账户兼任多个岗位。公民链基金会固定为一名私权管理员同时担任三个固定岗位，机构内部治理阈值仍为 2；阈值与管理员人数、岗位数解耦。前一版本的 fresh 创世哈希已因本次源码与存储布局变化失效，当前哈希以本节后续真实启动验收记录为准。

- 保护清单精确包含国家储委会 1 个、省储委会 43 个、省储行 43 个、国家司法院 1 个、联邦注册局 1 个和公民链技术发展基金会 1 个，共 90 个；
- 前 89 个机构的 `PublicAdmins::AdminAccounts` 与公民链基金会的 `PrivateAdmins::AdminAccounts` 必须存在，且机构码、CID、主账户三者共同匹配；

### 7.1 国家级单例与成员组成

- CID 生命周期策略额外冻结 PRS、NLG、NSN、NRP、NSP、NED 六个 block#0 精确身份；同码新 CID、删除、关闭或替换主账户均拒绝。
- NSN、NRP、NED 始终有空缺默认 `LR` 岗位；成员组成规范状态另看 `AdminAccounts`、指定成员岗位和指定岗位任职。成员岗位与任职全空是未组成状态，组成前 admins 必须独立建立。
...
- 所有机构管理员记录必须使用 `account_id + cid_number + family_name + given_name`；非空公民 CID 由 runtime 对照 `citizen-identity` 唯一真源校验。展示字段变化不改变成员身份或权限；
- NRC/PRC/PRB/NJD/FRG 的管理员总数分别固定为 19/9/9/15/215；
- FRG 是一个机构管理员集合，不存在 43 个虚拟省组账户；
- 固定岗位代码、名称、所属 CID、Active 状态、席位数和完整权限数组不得改变；允许依法增加清单外的普通动态岗位；
- NJD 固定 7/1/2/5，FRG 固定 43 个省专员岗位且每岗 5 席；
- 固定岗位任职账户必须属于同机构 `admins`，同一岗位内同一账户不得重复占席；`admins` 是可任职人员名册，不要求每个管理员恰好占一个岗位；
- 管理员允许原子轮换；任职来源、来源引用和任期由 runtime 与投票引擎负责，一个钱包可在多个机构、同一机构的多个不同岗位任职；
- `LR` 岗位本身属于固定骨架且每个机构绝对唯一；公民链基金会的 `LR`、创世产品经理和创世程序员各固定一席，由同一名私权管理员兼任，法定代表人原子结构必须与 `LR` 任职一致。
- **基金会创世管理员身份永久冻结（2026-07-23）**：三个创世岗位永久由**同一位公民**担任；该管理员的**公民 CID、姓、名**必须逐字节等于创世常量（CID 复用 `LEGAL_REPRESENTATIVE_CITIZEN_CID_NUMBER` 真源，姓名取 `CITIZENCHAIN_GENESIS_ADMINS`），任一漂移判 `FoundationGenesisIdentityChanged`。**唯一可变的是钱包账户**——换钱包不换人；换钱包须同步更新三条任职与机构信息中的法定代表人账户。「只能一个人」由既有 `expected_len = 1` + 每岗 `seats = 1` + 任职账户必须来自名册集合共同锁死，将来拆成三个人过不了守卫。
- **换钱包必须同步换绑**：若该公民 CID 已在 `CitizenIdentity::AccountIdByCid` 登记绑定，则基金会名册账户必须与之逐字节相等，否则判 `FoundationWalletBindingMismatch`；**未登记则跳过**——创世不播种该绑定（citizen-identity 无 `genesis_config`），无条件校验会否掉创世状态本身。与 `cid_lifecycle` 的绑定正反闭环校验互补，不重复。
...
| 机构 CID | `PublicManage/PrivateManage::{Institutions,InstitutionAccounts,AccountRegisteredCid}` | CID 不删除、不跨 namespace 复用；`town_code/institution_code/created_at` 不变；法定代表人整个结构存在或为 `None`；协议账户集合按机构制度完整且不可关闭，正反索引闭环；自定义账户允许依法删除 |
...
- 固定治理骨架覆盖机构缺失、CID/机构码/状态/总人数变化，固定岗位缺失、改名、停用或固定权限变化；ADR-039 迁移后不再拒绝额外普通动态岗位，
...
- 该历史验收中的“额外岗位拒绝”已被 ADR-039 取代；第 4 步已改测“额外动态岗位允许、修改固定权限拒绝”。其余岗位缺失/改名、席位变化、admins 不一致、任期、畸形 key、尾随字节
...
- 历史桌面进程曾在既有数据库上通过管理员布局修复；但 ADR-039 第 3 步的 fresh `--dev --tmp` 实测再次报告 `AdminAccountDecodeFailed([NRC])`，随后仍正常开放 RPC 并运行到 best #9。第 4 步必须以当前 fresh genesis 原始 storage 对拍共享 `InstitutionAdmins<BoundedVec<Admin>>`、固定岗位权限和 NodeGuard 读取路径，验收标准是 fresh 启动不再出现该错误；不得用“启动后仍可服务”替代守卫通过。

## 19. 国家级单例与成员组成验收（2026-07-14）

- 六个国家级单例完整身份和三类法定成员组成约束统一来自 `primitives::institution_constraints`；固定治理码及六类单例码无法用非规范 CID 预占登记。
- runtime 真实创世上六个国家单例均只有空缺默认 LR 岗位，无成员岗位、任职、admins 和动态阈值；管理员须先独立登记，再以治理结果组成成员岗位和任职。
...
- 保护清单从 89 个公权机构扩展为 90 个机构；新增项固定从 `primitives::cid::china::citizenchain` 读取，不在 NodeGuard 复制基金会 CID、账户、管理员或岗位常量；
- 公民链基金会使用 `PrivateAdmins::AdminAccounts` 与 `PrivateManage::Institutions/InstitutionRoles/InstitutionRoleAssignments`，其 RAW key 路由、完整状态分区和触发式普通区块复核与原 89 个公权机构共用同一策略；
