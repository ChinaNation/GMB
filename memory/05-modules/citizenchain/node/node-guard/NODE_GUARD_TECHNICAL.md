...
- 保护清单精确包含国家储委会 1 个、省储委会 43 个、省储行 43 个、国家司法院 1 个、联邦注册局 1 个和中国公民链技术发展基金会 1 个，共 90 个；
...
- `LR` 岗位本身属于固定骨架且每个机构绝对唯一；公民链基金会的 `LR`、创世产品经理和创世程序员各固定一席，由同一名私权管理员兼任，法定代表人三字段必须与 `LR` 任职一致。
...
- 守卫额外检查基金会三个固定岗位的代码、名称、固定权限和一席席位不变；一名私权管理员程伟以同一钱包同时担任 `LR`、创世产品经理、创世程序员。法定代表人三字段必须与 `LR` 任职一致。基金会机构记录、固定岗位、任职或管理员缺失及公私权错误路由均 fail-closed；普通动态岗位允许存在；
