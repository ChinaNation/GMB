...
- 公权管理员记录必须使用 `account_id + cid_number + family_name + given_name`；公民 CID、姓、名当前允许为空，非空公民 CID 由 runtime 对照 `citizen-identity` 唯一真源校验。私权管理员继续使用 `account_id + family_name + given_name` 且姓名非空 UTF-8；展示字段变化不改变成员身份或权限；
