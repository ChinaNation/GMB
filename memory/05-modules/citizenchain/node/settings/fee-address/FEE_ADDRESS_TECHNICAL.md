...
- 路径：`node/src/settings/fee-address/mod.rs`
- 前端路径：`node/frontend/settings/fee-address/WalletSection.tsx`
- 前端共享依赖：`node/frontend/shared/ss58.ts` 校验 SS58/0x 地址，`node/frontend/shared/qr/AddressScanModal.tsx` 负责扫码填入地址
- 对外命令：
  - `get_reward_wallet`
  - `set_reward_wallet`
...
1. 校验设备登录密码非空。
2. 通过 `node/src/settings/device-password/mod.rs` 执行设备密码验证（macOS/Linux/Windows）。
