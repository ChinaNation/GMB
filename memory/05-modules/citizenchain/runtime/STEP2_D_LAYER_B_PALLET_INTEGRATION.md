...
- **日期**:2026-04-20
- **范围**:在 `offchain-transaction-pos` pallet 内建 `tests::` 模块,构造完整
  mock runtime(`frame_system + pallet_balances + OffchainPos` + mock SFID),
...
impl offchain_transaction_pos::Config for Test {
    ...
    type InstitutionAssetGuard = ();     // fail-open(institution-asset-guard 自带)
