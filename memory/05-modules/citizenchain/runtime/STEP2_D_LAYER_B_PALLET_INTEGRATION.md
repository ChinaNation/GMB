...
- **范围**:在 `offchain-transaction` pallet 内建 `tests::` 模块,构造完整
  mock runtime(`frame_system + pallet_balances + OffchainTx` + mock CID),
...
- **后续**:E2E Layer C(dev chain 进程级,需 chain_spec 预置清算行 + CID mock)
...
### 2.1 `MockCid` — `CidAccountQuery` 实现
...
| `BANK_FEE_BYTES = [0xAB; 32]` | 清算行费用账户(同 CID,`费用账户`,Active) |
| `BANK_ADMIN_BYTES = [0xAC; 32]` | 唯一管理员 |
| `OTHER_BANK_BYTES = [0xBA; 32]` | 故意不注册,用于负路径 |

`MockCid` 按这张表实现 4 个方法;费用账户在 `settlement.rs` 内部
...
- **跨行 fixture**:加第二家 `BANK2_MAIN`(另一个 CID + main/fee 对),扩
  `MockCid` 与 `new_test_ext` 预置;补跨行 settlement 测试(需 Step E bug 修复先落地)
