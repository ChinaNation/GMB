...
- `VoterSnapshot[(proposal_id, RoleSubject)]` 保存每个岗位主体的有效任职账户。当前 `EffectiveVoterSnapshot[(proposal_id, cid_number)]` 仍把同一 CID 的参与账户合并去重，这是尚待删除的中间态：同一钱包兼任多岗时必须按 `RoleSubject + admin_account` 分别行使岗位席位票权，不能降低机构阈值或复制钱包。
...
- 固定创世机构的阈值由 genesis 写入 entity，不是岗位阈值。公民链基金会只有一个程伟管理员钱包，但三个固定岗位各一席、机构阈值仍为 2；结构可建案和记票必须在下一步岗位席位票据完成后验收。
...
- `population_data(scope)`：一次读取作用域、人口分母、资格 revision 和护照判定日期。该数据只能由 `citizen-identity` 产生。
- `can_vote_at(who, population_data)`：按人口数据中的 revision 和日期查询账户身份历史，不读取投票时的当前身份。

`citizen-identity` 是人口数据和身份历史唯一真源，但不生成、不编号、不保存提案快照。`votingengine::create_population_snapshot(proposal_id, scope)` 调用 `population_data` 后，把完整不可变数据写入 `ProposalPopulationSnapshots[proposal_id]`；投票、分母读取和 90 天清理都只围绕该 proposal_id 快照进行。
...
- 联合公投资格由 `CitizenIdentityReader::can_vote_at(who, population_data)` 判定；提案创建后新增、迁居或被撤销的当前身份均不能改变已有提案的成员集合。
...
- `ProposalExecutionOutcome::RetryableFailed` 继续进入手动重试宽限期；机构提案只有创建时 `EffectiveVoterSnapshot` 中的有效岗位选民可重试/取消，个人多签才读取 `AdminSnapshot`。执行成功或失败终态仍统一触发互斥释放、业务终态通知和 90 天延迟清理。
...
- 普选必须使用 `citizen-identity` 的 `PopulationScope`、`can_be_candidate`、`population_data` 和 `can_vote_at`；投票引擎按 proposal_id 保存不可变人口数据，不接收、不枚举、不保存全国/省/市/镇完整选民列表。
...
- 普选人口作用域写入 `ElectionMeta`，完整人口数据写入核心 `ProposalPopulationSnapshots`；互选不写公民作用域，按 VotePlan 中的一个或多个目标机构 `RoleSubject` 写入核心 `VoterSnapshot` 和 CID 去重后的 `EffectiveVoterSnapshot`。`MutualVoters`、调用方选民参数和 `MaxMutualVoters` 已删除。
...
- election-vote 现有测试文件内建立完整 mock runtime，覆盖普选/互选创建、人口/岗位快照、资格拒绝、写票、超时、结果回调与分块清理；当前为 13 项。
