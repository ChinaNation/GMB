...
2026-07-21 冻结的最终公民主体为 `CitizenSubject { cid_number, wallet_account }`。后续接口迁移完成后，资格读取必须返回完整主体；联合公投、立法公投和 Popular 选举的公民票据不得继续以裸钱包作为唯一主体。人口快照仍只保存作用域、有效总数、资格 revision、判定日期和创建区块，不枚举公民名单。
...
- 无具体规则的通用 `election-campaign` 骨架已决定删除，不得改名或扩展成所有选举规则的集中模块。具体业务模块本身就是该类选举的规则真源。
...
- 普选人口作用域写入 `ElectionMeta`，完整人口数据写入核心 `ProposalPopulationSnapshots`；互选不写公民作用域，按 VotePlan 中的一个或多个目标机构 `RoleSubject` 写入核心 `VoterSnapshot`，并以 `MutualElectionVotesByTicket` 保存完整岗位票据。`MutualVoters`、调用方选民参数和 `MaxMutualVoters` 已删除。
