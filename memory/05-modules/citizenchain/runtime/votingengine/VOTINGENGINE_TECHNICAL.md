...
- 普选终态映射为 `PopularElection`，互选终态映射为 `MutualElection`；`proposal_id` 的 SCALE 编码写入 `assignment_source_ref`。
- runtime 只负责按机构码路由结果；entity 校验机构、岗位、固定席位和任期，并从有效任职派生 admins 钱包集合。
