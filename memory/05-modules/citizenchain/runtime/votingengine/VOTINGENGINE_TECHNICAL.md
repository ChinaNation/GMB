...
- 业务模块不得在自己的 extrinsic、DTO、ProposalData 或测试 mock 中接收 `eligible_total`、`snapshot_nonce`、`signature`、`province`、`signer_pubkey`。
...
- `citizen_vote` benchmark 走完整 `do_citizen_vote` 逻辑，并传入 ADR-008 step3 的 `province` 与 `signer_pubkey` 双层匹配字段。
