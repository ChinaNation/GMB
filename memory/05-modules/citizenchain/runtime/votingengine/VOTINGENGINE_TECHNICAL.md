...
最新更新：2026-05-08，第 2 步已落地：`internal-vote` 强制账户级管理员快照非空/无重复，注册创建与注销关闭使用全员阈值，普通内部提案与管理员集合变更提案的阈值必须可由快照管理员人数达成。
...
结果：`internal-vote` 86 passed，`votingengine` 0 tests passed，`joint-vote` 5 passed，`admins-change` 34 passed，`personal-manage` 16 passed，`organization-manage` 24 passed。
