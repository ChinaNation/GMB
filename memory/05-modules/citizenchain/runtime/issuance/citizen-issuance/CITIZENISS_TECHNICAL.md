...
- 在 CID 绑定成功后，按链上固定规则自动发放一次性“公民轻节点认证奖励”。
- 奖励发放逻辑必须与 `cid-system` 解耦，由回调触发而非独立用户交易触发。
- 奖励结果必须可审计，成功与跳过都要有事件。

### 0.2 业务规则需求
- 同一 CID 只能领奖一次。
- 同一账户只能领奖一次，即使后续换绑 CID 也不能重复领奖。
...
### 0.3 计重与接线需求
- 模块不提供外部 extrinsic，全部逻辑由 `OnCidBound` 回调驱动。
- 模块必须向上游暴露 `on_cid_bound_weight()`，供 `cid-system::bind_cid` 叠加申报 weight。
- weight 的单一真相源必须来自本模块自身，而不是由上游模块手写估算。
- 上游只有在 CID 绑定真正成功后才能触发奖励回调，模块自身不负责认证流程与身份校验。

---

## 1. 模块定位
`citizen-issuance` 是一个 FRAME pallet，用于在 CID 绑定成功后发放“公民轻节点认证奖励”。

核心目标：
- 与 `cid-system` 解耦：本模块只负责奖励发行逻辑。
...
接线方式：
- `cid-system::bind_cid` 成功后调用 `T::OnCidBound::on_cid_bound(&who, binding_id)`。
- Runtime 中配置 `type OnCidBound = CitizenIssuance`：
  - `/Users/rhett/GMB/citizenchain/runtime/src/configs/mod.rs`
  - `type WeightInfo = citizen_issuance::weights::SubstrateWeight<Runtime>`

Weight 集成：
- `cid-system` 在 `bind_cid` 的 weight 中，叠加 `T::OnCidBound::on_cid_bound_weight()`。
- 本模块实现 `OnCidBoundWeight` 并公开估算值，避免回调开销遗漏。
...
## 6. 核心流程
入口：
- `OnCidBound::on_cid_bound(who, binding_id)`

执行逻辑：
1. 先进行 CID 去重检查（`RewardClaimed`）。
...
## 7. Weight 策略
本模块暴露：
- `Pallet::on_cid_bound_weight() -> Weight`
- `OnCidBoundWeight::on_cid_bound_weight()`（委托到上述方法，单一真相源）
- `WeightInfo::on_cid_bound()`（由 runtime 注入）

当前状态：
- `WeightInfo::on_cid_bound()` 已由 Substrate benchmark CLI 自动生成（见 `src/weights.rs` 文件头，日期 2026-03-17）。

说明：
- 该值用于上游 `bind_cid` 申报 weight 叠加。
...
## 8. Benchmark 设计
- benchmark 入口：`on_cid_bound`
- 覆盖口径：首次成功发奖路径（含 `deposit_creating`、计数写回、双重去重标记与事件）
- 未单独覆盖：重复领取、达到上限、零奖励配置等跳过路径
- 目的：校准上游 `bind_cid` 叠加的回调 weight，而不是测用户交易本身
- Cargo feature：`runtime-benchmarks` 会向 `pallet-balances` 与 `cid-system` 传播；`primitives` 当前不暴露 benchmark feature，不在传播列表中。
...
- 成功发放事件断言
- 跳过事件断言（重复 CID / 达上限 / 账户已领奖）
- 不同账户不同 CID 独立发放
- 同一账户不同 CID 仅首笔发放

### 10.2 集成测试
`cargo test -p citizen-issuance --test integration_bind_cid` 覆盖：
- `bind_cid` extrinsic → `OnCidBound` → 奖励发放完整链路
- 同一账户换绑时奖励被跳过但绑定成功
- 不同账户独立绑定独立领奖
- 达到发放上限后绑定成功但奖励被跳过
- `bind_cid` weight 声明包含回调 weight（非零）
