# SHENGBANK Interest Technical Notes

## 0. 功能需求
`shengbank-interest` 的功能需求是：按年度向 `CHINA_CH` 中固定 43 家省储行的多签账户发放质押利息，并把结算结果、失败原因和补救动作全部体现在链上。
...
## 1. 模块定位
`shengbank-interest` 是一个 FRAME pallet，用于按年度向 `CHINA_CH` 省储行账户发放质押利息。
...
代码位置：
- `/Users/rhett/GMB/citizenchain/runtime/issuance/shengbank-interest/src/lib.rs`
