...
- `propose_create_public_institution`（call 5）：只接收目标 CID、全称、简称、镇码、`admins(admin_name + admin_account)` 和注册局授权字段。机构码从 CID 解析；runtime 自动建立完整零余额协议账户、唯一空缺 `LR / 法定代表人` 岗位和严格多数阈值。
