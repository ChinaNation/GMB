...
- 五类固定创世机构禁止运行期修改岗位定义，并逐岗位强制治理骨架席位；依法轮换只改变任职账户及其任期、来源、引用。
- 完整校验后，从机构全部有效岗位任职重新派生 admins；岗位、任职、法定代表人和 admins 同事务提交，失败整体回滚。动态机构保持既有 Active 阈值，固定治理机构保持代码级固定阈值。
