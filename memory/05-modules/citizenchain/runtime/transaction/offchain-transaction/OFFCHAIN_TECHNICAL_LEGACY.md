...
> - `memory/05-modules/citizenchain/runtime/transaction/offchain-transaction/STEP2A_RUNTIME.md`(Step 2a 实现)
> - `memory/05-modules/citizenchain/runtime/transaction/offchain-transaction/STEP1_TECHNICAL.md`(Step 1 骨架)
>
...
- 若 `payer` 或 `fee_address` 命中制度账户白名单边界，还必须通过 `institution-asset` 的资金动作检查。
