# offchain-transaction-pos · 扫码支付 Step 1 技术说明
...
链上**不新增** SFID 枚举,而是直接对 `duoqian-manage-pow::AddressRegisteredSfid` 存的 `sfid_id` 字符串做 A3 前缀匹配(前 3 字节)。
...
`bank_check` 不直接依赖 `duoqian-manage-pow`,而是通过 trait 抽象:
...
**runtime 层实现**(`citizenchain/runtime/src/configs/mod.rs` 的 `DuoqianSfidAccountQuery`):委托给 `duoqian-manage-pow` 的三张 Storage:
- `AddressRegisteredSfid` → `account_info`
- `SfidRegisteredAddress` → `find_address`
- `DuoqianAccounts` → `is_active`

**好处**:
- offchain-transaction-pos 的 Cargo.toml 不新增 duoqian-manage-pow 依赖
- pallet 的 `Config` trait 不强制 `T: duoqian_manage_pow::Config`
...
费用归类在 `citizenchain/runtime/src/configs/mod.rs::PowTxAmountExtractor` 的 `OffchainTransactionPos` 分支显式分类,不走兜底。
...
## 10. `institution-asset-guard` 新增 4 个 Action
...
`()` 默认 fail-open,runtime 的 `RuntimeInstitutionAssetGuard` 会按"清算行主账户是否合法"严格裁决。

## 11. 与现有模块的边界

| 模块 | 动向 |
|---|---|
| `duoqian-manage-pow` | **不动**(清算行注册复用现有机制) |
| `duoqian-transfer-pow` | **不动** |
| `onchain-transaction-pow` | **不动** |
| `sfid-code-auth` | **不动** |
| `institution-asset-guard` | **扩展 4 枚举**(代码改动见上) |
| `offchain-transaction-pos` 现有省储行清算逻辑 | **不动**,Step 2 替换 |

## 12. 编译验证

```
$ cargo check -p offchain-transaction-pos
$ cargo check -p institution-asset-guard
