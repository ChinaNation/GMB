...
2. `InstitutionAssetGuard::can_spend` 检查（action = `OffchainFeeSweepExecute`）
...
8. 手续费通过 `FeeRouter`（即 `TransferFeeRouter` -> `PowOnchainFeeRouter`）按 80/10/10 分账
9. 设置提案状态为 `STATUS_EXECUTED`
10. 触发 `SweepToMainExecuted` 事件（含 `reserve_left` 余额）

## 手续费分账路径

`TransferFeeRouter` 将旧 `NegativeImbalance` 转换为新 `Credit`，传递给 `PowOnchainFeeRouter`：
- 80% -> 当前区块矿工（PoW 全节点）
- 10% -> 国储会费用账户
- 10% -> 国储会安全基金账户
