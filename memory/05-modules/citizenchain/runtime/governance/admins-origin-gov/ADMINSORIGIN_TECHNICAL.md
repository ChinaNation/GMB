...
`admins-origin-gov` 是链上管理员主体的统一真源，同时负责管理员等长替换治理。
...
5. 调 `voting-engine-system` 创建内部投票。
...
- `RuntimeInternalAdminProvider` 从 `admins-origin-gov::Pallet::is_subject_admin` 读取。
