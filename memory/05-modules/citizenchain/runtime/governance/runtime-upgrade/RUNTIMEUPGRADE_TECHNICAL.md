...
- WASM CI 在编译前会读取链上 `state_getRuntimeVersion.specVersion`；优先使用已有的无密码部署密钥 `CID_DEPLOY_SSH_KEY` 通过 SSH 登录服务器后访问本机 `127.0.0.1:9944`，没有该密钥时才使用 `CITIZENCHAIN_SSH_KEY`，源码版本不足时只在 CI 工作区临时提升到 `链上版本 + 1` 再编译 artifact
