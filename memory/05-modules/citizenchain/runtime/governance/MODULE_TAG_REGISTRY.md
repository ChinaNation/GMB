...
| `b"org-mgmt"` | `[100,113,45,109,103,109,116]` | `organization-manage` | `MODULE_TAG + ACTION_CODE (1 byte) + payload (SCALE)` |
...
| `b"adm-set-v1"` | `[97,100,109,45,115,101,116,45,118,49]` | `admins-change` | `AdminSetChangeAction<AdminsOf<T>> (SCALE)` |
...
说明：`admins-change` 的管理员集合变更提案只把 `MODULE_TAG` 写入 `ProposalOwner`，`ProposalData` 直接保存 `AdminSetChangeAction`，不再重复嵌入 tag 前缀。
...
3. `organization-manage` 在 TAG 后增加 1 字节 ACTION_CODE 区分 create/close/其他操作
