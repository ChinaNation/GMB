...
### 4.3 已废弃: `execute_destroy`（原 call index = 1）
2026-05-02 unified voting entry 整改后，本模块的 `execute_destroy` wrapper extrinsic 物理删除。手动重试统一走：

- `VotingEngine::retry_passed_proposal(proposal_id)`

签名账户必须是提案快照管理员，权限和重试次数由 `voting-engine` 统一校验；仅当提案已 `STATUS_PASSED` 且存在 retry state 时可重试执行。用于"提案已通过但自动执行失败（如余额不足）"后的后续重试。
...
注：原 `execute_destroy()` 已于 2026-05-02 废弃，手动重试走 `VotingEngine::retry_passed_proposal`，权重由投票引擎统一计入。
...
- 已执行提案不阻塞新提案
- 重复投票由投票引擎拒绝
- 非管理员不能触发投票引擎 retry（原 execute_destroy 已于 2026-05-02 废弃）
- 无效机构返回 None
...
## 10. 运维建议
1. 监控 `DestroyExecutionFailed` 事件，出现后优先补齐机构余额，再由快照管理员调用 `VotingEngine::retry_passed_proposal`（`execute_destroy` 已于 2026-05-02 废弃）。
