...
| `china/citizenchain.rs` | 私权非营利创世机构“公民链技术发展基金会”的身份、协议账户、法定代表人、一名程伟管理员、同一钱包的三个固定岗位任职和机构阈值 2 的唯一常量真源 |
...
- `china_zb.rs` 中的 639 个保留地址由 `RuntimeReservedAccountGuard` 注入 `public-manage`、`private-manage`、`personal-manage`，在注册和创建账户时统一校验，防止抢注制度地址；其中公民链基金会主账户和费用账户与其它制度账户同等保护。
- `governance_skeleton.rs` 继续只表达既有 89 个公权固定治理机构；私权公民链基金会的固定治理真源独立位于 `china/citizenchain.rs`，NodeGuard 合并后保护总数为 90，避免把私权机构伪装进公权清单。
...
- 国家码是两字节值 `CN`,类型 `CountryCode = [u8; 2]`,由常量 `COUNTRY_CN` 承载(常量名不是国家码);元数据携带 `country_full_name / country_full_name_en / country_short_name / country_short_name_en`(中华民族联邦共和国 / Federal Republic of the Chinese Nation / 中华联邦 / Chinese Federation)。
