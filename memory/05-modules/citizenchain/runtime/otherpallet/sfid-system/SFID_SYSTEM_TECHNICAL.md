# SFID System Technical Notes

## 1. 模块定位

`sfid-system` 是公民链 runtime 中负责 SFID 绑定与投票资格消费的 FRAME pallet。
...
- 不保存任何特殊管理员名册。
- 不保存签发机构私钥或链下缓存。
- 凭证签发人身份统一由 runtime 验签桥接到 `admins-change::AdminAccounts` 判断。

代码位置：

- `/Users/rhett/GMB/citizenchain/runtime/otherpallet/sfid-system/src/lib.rs`
- `/Users/rhett/GMB/citizenchain/runtime/otherpallet/sfid-system/src/weights.rs`
- `/Users/rhett/GMB/citizenchain/runtime/otherpallet/sfid-system/src/benchmarks.rs`
- `/Users/rhett/GMB/citizenchain/runtime/otherpallet/sfid-system/src/tests/`
...
- `issuer_sfid_number`：签发机构 SFID 号。
- `issuer_main_account`：签发机构主账户，也是 `admins-change::AdminAccounts` 的查询键。
...
1. 用 `issuer_main_account` 读取 `admins-change::AdminAccounts`。
...
- `SfidVerifier`：验证绑定凭证。
- `SfidVoteVerifier`：验证投票凭证。
- `OnSfidBound`：绑定成功后通知发行模块。
- `SfidEligibilityProvider`：给投票模块使用的资格查询和凭证消费接口。
...
### 5.1 `bind_sfid(origin, credential)`
...
4. `T::SfidVerifier::verify(&who, &credential)` 必须通过。
...
5. 调用 `T::OnSfidBound::on_sfid_bound(&who, binding_id)`。
6. 发出 `SfidBound` 事件。

### 5.2 `unbind_sfid(origin, target)`
...
3. 拒绝重复 nonce。
4. 调用 `T::SfidVoteVerifier::verify_vote(...)`。
...
- `RuntimeSfidVerifier`：验证 SFID 绑定凭证。
- `RuntimeSfidVoteVerifier`：验证公民投票凭证。
- `RuntimePopulationSnapshotVerifier`：验证联合提案人口快照凭证。
- `RuntimeSfidInstitutionVerifier`：验证机构注册凭证。

以上验签器全部使用相同签发身份规则：

1. 读取 `issuer_main_account` 对应的 `admins-change::AdminAccounts`。
...
- `issuer_sfid_number` 用于审计和展示；链上管理员真值以 `issuer_main_account` 下的 `admins` 为准。
- 所有机构和个人多签管理员集合的唯一真源是 `admins-change`。

## 9. 验收要求

本模块改动后必须执行：

- `cargo test --manifest-path /Users/rhett/GMB/citizenchain/Cargo.toml -p sfid-system`
