...
> node 其余路径不持私钥:治理投票/链上转账是"构造 payload → wumin 冷钱包扫码签 → node 验签广播"的中继(`governance/*/signing.rs`、`onchain_transaction/`);外部导入的用户钱包只存地址公钥(`onchain_transaction/wallet_store.rs`)。
