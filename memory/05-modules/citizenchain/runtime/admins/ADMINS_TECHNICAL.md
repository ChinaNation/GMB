...
最新更新：2026-07-19。`citizenchain/runtime/admins/` 由四个 crate 组成，管理员链上状态按管理员类型分别保存在各自 pallet。ADR-039 已冻结目标授权模型，代码迁移尚待任务卡后续步骤。
...
- 所有 `admins` 每项统一为 `Admin { admin_account, family_name, given_name }`。姓、名只展示，钱包账户用于人员识别与签名，但账户本身没有机构业务权限；不保存公民 CID、岗位、权限、任期和任职来源。
...
姓名缺失时，runtime 在进入签名载荷、投票数据或 storage 前分别补为 `family_name="管理"`、`given_name="员"`；前端按中文顺序组合显示“管理员”。人员去重读取 `admin_account`；机构业务授权和投票资格必须继续校验完整 `RoleSubject`、岗位权限与有效任职。
