...
| `genesis-admins` | 创世管理员：国储会、43 个省储会、43 个省储行、联邦注册局、国家司法院管理员治理。只保存管理员集合与换届治理，不保存创世机构本体封存表。 |
...
- 注册个人多签和注册机构账户的动态阈值由 `votingengine/internal-vote` 的动态阈值表保存；NRC/PRC/PRB/FRG/NJD 创世治理阈值来自代码级固定阈值。
...
- 创世管理员只由 `genesis-admins` 维护；国储会、省储会、省储行、国家司法院固定人数，联邦注册局按 43 个省级 5 人组治理。
...
| 模块 | 管理员更换 call |
|------|----------------|
| `genesis-admins` | `12.0 propose_admin_set_change`（NRC/PRC/PRB/NJD） |
