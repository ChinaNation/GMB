...
- `onchina_proc` 用 env 把资源/数据路径告诉 OnChina(`CID_PG_BIN_DIR`/`CID_PG_DATA_DIR`/`CID_PG_PORT`/`CID_TLS_DIR`/`CID_PG_WAL_ARCHIVE_DIR`/`ONCHINA_FRONTEND_DIST`/`CID_CHINA_DB`/`CID_CHAIN_WS_URL`/`CID_EMBEDDED_PG=1`/`CID_ENABLE_TLS=1`)。
...
- OnChina 内网 API 走 HTTPS(rcgen 自签,证书持久化 `CID_TLS_DIR`);证书 SAN 为 `onchina.local`。
