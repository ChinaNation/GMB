...
Step 1 在 wuminapp 端**只增量、不破坏**:新增 8 个文件 + 改造 2 个,与旧 `lib/wallet/ui/bind_clearing_page.dart`(43 省储行硬编码 + `bind_clearing_institution`)和 `lib/trade/offchain/offchain_pay_page.dart` 共存,Step 2 切换后删除旧入口。
...
| 文件 | 职责 |
|---|---|
| `lib/rpc/onchain_clearing_bank.dart` | 链上 4 个新 extrinsic(call_index 30/31/32/33):`bindClearingBank` / `deposit` / `withdraw` / `switchBank`。复用 polkadart `SigningPayload` + `ExtrinsicPayload` 模式 + `NonceManager`,与现有 `OnchainRpc` 行为一致 |
...
| `lib/rpc/offchain_clearing.dart` | 清算行节点 RPC 客户端:`offchain_queryBalance` / `queryNextNonce` / `queryPendingCount`(WSS) |

### 2.2 页面层(5 个,均在 `lib/trade/offchain/`)
...
| `clearing_payment_entry_page.dart` | 4 入口聚合页(简化 wallet 详情页改造量) |

## 3. 改造文件(2 个)

| 文件 | 改动 |
|---|---|
| `lib/wallet/ui/wallet_page.dart` | 顶部 `import 'clearing_payment_entry_page.dart'`;PopupMenu 加 `clearing_bank_v2`/`扫码支付(清算行,Beta)` 项;`_onMenuAction` 加 case;新增 `_openClearingPaymentEntry()` 方法,从 `String.fromEnvironment('SFID_BASE_URL'|'CLEARING_NODE_WSS')` 读取地址 |
...
| 文件 | 原因 |
|---|---|
| `lib/trade/offchain/offchain_pay_page.dart`(576 行) | 旧扫码支付页,仍走 `bind_clearing_institution` + 省储行清算路径,Step 2 整体重写,本步不破坏 |
