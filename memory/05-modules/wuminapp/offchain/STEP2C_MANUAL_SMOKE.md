...
flutter run -d <device> \
  ...
  --dart-define CLEARING_NODE_WSS=wss://<清算行节点 host>:9944
```

### 期望

- App 正常启动,钱包页可见
- 如 `CLEARING_NODE_WSS` 未传或为空,后续扫码付款页会提示"未配置清算行节点 WSS"(保护)
...
| 现象 | 原因 | 处置 |
|---|---|---|
| 扫码付款页 `请先绑定清算行` | `UserBank[A]` 空 / `CLEARING_NODE_WSS` 连错节点 | 回步骤 4 确认绑定成功;检查 WSS URL 前缀 |
| `收款方清算行 ... 未在 SFID 系统查到` | QR 的 `bank` 字段与 SFID 已注册机构不匹配 | 收款方换绑定 → 重生成 QR |
| `Step 1 仅支持同行扫码支付` | QR 的 `recipient_bank.main_account` != 付款方 `payer_bank` SS58 | 收款方绑到同一家清算行 |
