...
`wuminapp/lib/proposal/` 不再保存多签转账页面、service、模型字段或投票实现；只保留提案通用元数据、上下文、缓存和投票引擎共享能力。多签转账业务详情通过 `ProposalWithDetail.businessDetails` 的不透明键值挂载，键名由本模块定义。

`wuminapp/lib/organization-manage/` 不再实现多签转账按钮和跳转；多签账户详情页只允许挂载 `duoqian-transfer` 提供的 `DuoqianTransferEntryCard`，入口自身逻辑在 `duoqian-transfer` 内部。

`wuminapp/lib/institution/` 和 `wuminapp/lib/vote/` 只允许调用 `DuoqianTransferProposalAdapter` / `DuoqianTransferProposalFeed`，不得直接判断 `TransferProposalInfo`、`SafetyFundProposalInfo`、`SweepProposalInfo`，也不得直接打开 `DuoqianTransferDetailPage`。
