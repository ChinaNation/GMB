...
- **历史说明**:本文记录 Step 1 当时的增量路径。2026-04-30 后当前源码目录以
  `OFFCHAIN_DIRECTORY.md` 为准,链下扫码支付业务统一位于 `wuminapp/lib/transaction/offchain-transaction/`。
...
Step 1 在 wuminapp 端**只增量、不破坏**:新增 8 个文件 + 改造 2 个,与旧 `lib/wallet/ui/bind_clearing_page.dart`(43 省储行硬编码 + `bind_clearing_institution`)和 `lib/transaction/offchain-transaction/pages/offchain_pay_page.dart` 共存,Step 2 切换后删除旧入口。
...
| 文件 | 职责 |
|---|---|
| `lib/transaction/offchain-transaction/rpc/onchain_clearing_bank_rpc.dart` | 链上 4 个新 extrinsic(call_index 30/31/32/33):`bindClearingBank` / `deposit` / `withdraw` / `switchBank`。复用统一 `SignedExtrinsicBuilder`，交易 nonce 每次签名前实时读取 runtime，不使用客户端本地 nonce 管理器 |
...
| `lib/transaction/offchain-transaction/rpc/offchain_clearing_rpc.dart` | 清算行节点 RPC 客户端:`offchain_queryBalance` / `queryNextNonce` / `queryPendingCount`(WSS) |

### 2.2 页面层(5 个)

2026-04-30 目录收口后,仍保留的清算行页面已迁入 `lib/transaction/offchain-transaction/pages/`。
...
| 文件 | 改动 |
|---|---|
| `lib/wallet/ui/wallet_page.dart` | 顶部接入清算行入口;PopupMenu 加 `clearing_bank_v2`/`扫码支付(清算行,Beta)` 项;`_onMenuAction` 加 case;SFID 查询统一走 `SfidApiConfig`,清算节点端点由链上 `ClearingBankNodes` 读取 |
...
| 文件 | 原因 |
|---|---|
| `lib/transaction/offchain-transaction/pages/offchain_pay_page.dart`(576 行) | 旧扫码支付页,仍走 `bind_clearing_institution` + 省储行清算路径,Step 2 整体重写,本步不破坏 |
...
- WS 订阅清算行节点的到账推送
- 删除旧省储行清算入口文件，不再恢复历史清算行硬编码和旧 RPC。
