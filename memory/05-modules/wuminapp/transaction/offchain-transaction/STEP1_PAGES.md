...
| 文件 | 职责 |
|---|---|
| `lib/transaction/offchain-transaction/rpc/onchain_clearing_bank_rpc.dart` | 链上 4 个新 extrinsic(call_index 30/31/32/33):`bindClearingBank` / `deposit` / `withdraw` / `switchBank`。复用 polkadart `SigningPayload` + `ExtrinsicPayload` 模式 + `NonceManager`,与现有 `OnchainRpc` 行为一致 |
