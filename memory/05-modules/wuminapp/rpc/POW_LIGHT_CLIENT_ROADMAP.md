...
现状：`wallet_page.dart` 已从逐个钱包调用 `fetchBalance()` 切换为批量 finalized 余额查询，N 个钱包 = 1 次 finalized storage proof 请求。

方案：在 Dart 层构建所有钱包的 System.Account storage key，用 `fetchFinalizedStorageBatch()` 一次查完。
...
- `ChainRpc.fetchBalance()` 与 `fetchConfirmedNonce()` 的轻节点分支已切到原生 `System.Account`；页面金额展示改走 `fetchFinalizedBalance()` / `fetchFinalizedBalances()` / `fetchFinalizedTotalBalance()`
