...
- 目录内不保留 `.git`
- 增加来源说明文档（如 `UPSTREAM.md`）
- `wuminapp/rust/Cargo.toml` 仅依赖这个固定快照路径
- 同步 GitHub fork 时走“显式更新快照”的流程，不能偷偷漂移
...
本方案不推荐 Git submodule。

原因：

- 手机端与 CI 构建需要更强的可复现性
- submodule 容易造成递归克隆遗漏
- 当前团队已经因嵌套 Git 目录出现提交干扰，继续沿用会放大风险
