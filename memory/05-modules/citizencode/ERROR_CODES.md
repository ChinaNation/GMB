...
| `CID_BIND_ARCHIVE_ALREADY_BOUND` | 409 | 档案号已绑定 |
| `CID_BIND_PUBKEY_ALREADY_BOUND` | 409 | 公钥已绑定档案 |

### CPMS ARCHIVE 验真

| error_code | HTTP | 含义 |
|---|---:|---|
| `CID_CITIZEN_ARCHIVE_SIGNATURE_BAD` | 422 | ARCHIVE 的 CPMS 签名无效 |
| `CID_CITIZEN_ARCHIVE_GEO_SEAL_INVALID` | 422 | `geo_seal` 不能用已授权 CPMS 解密 |
| `CID_CITIZEN_ARCHIVE_SCOPE_MISMATCH` | 422 | `geo_seal` 中的授权范围不匹配 |
| `CID_CITIZEN_ARCHIVE_PUBKEY_MISMATCH` | 422 | CPMS 本机公钥与已绑定授权不一致 |
| `CID_CITIZEN_QR_EXPIRED` | 410 | CPMS 状态 QR 已过期 |
| `CID_CITIZEN_QR_HEADER_INVALID` | 400 | CPMS 状态 QR 头部字段非法 |
