...
| `CID_ADMIN_REPLACEMENT_NOT_ONCHAIN` | 403 | 联邦注册局管理员更换目标账户不是链上有效管理员 |
| `CID_ADMIN_CITY_REGISTRY_CITY_LIMIT_REACHED` | 409 | 本市市注册局管理员已达到 30 人上限 |
| `CID_STORE_PERSIST_FAILED` | 500 | 写操作持久化失败，接口不得返回业务成功 |

管理员公钥全局唯一。新增市注册局机构管理员或更换联邦注册局机构管理员时，后端必须先按规范化公钥查全局
`admins` 账号表；命中后按已有角色返回上述稳定错误码，前端按目标角色展示中文提示。
联邦注册局机构管理员不走本地新增/删除;更换目标必须已经在链上 Active 管理员集合中。市注册局机构管理员每省每市最多 30 人，达到上限时必须在后端拒绝新增。
