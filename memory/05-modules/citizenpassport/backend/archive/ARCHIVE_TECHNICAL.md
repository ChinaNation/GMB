...
## 1. 模块定位
`backend/archive/` 负责 CPMS 档案业务：档案创建/查询、列表游标分页、`CID_CPMS_V1 / ARCHIVE` 档案二维码构建与签名、公民状态校验、有效期计算、公民资料库、档案操作记录聚合、档案生命周期硬删除和年度状态导出。

本模块不保存实名归属判断逻辑；CPMS 所属省市来自 `initialize` 保存的 INSTALL 授权材料。公民居住地由该省市 + 镇 + 地址段 + 详细地址输入段组成；出生地由全国省、市、镇组成，来自 CPMS 随包的 CID 行政区唯一真源 `china.sqlite` 只读拷贝，保存后不得在编辑接口中修改。
...
- `export::build_and_record_cpms_status_export(...)`：生成 CPMS 给 CID 手工导入的年度状态更新文件，并记录导出批次。

档案号和护照号生成统一归属 `memory/05-modules/citizenpassport/backend/number/NUMBER_TECHNICAL.md`
与 `citizenpassport/backend/number/`。
...
二维码明文字段不得出现 `cid_number / province_code / city_code / town_code` 或中文行政区名称。归属密文 `geo_seal` 加密 CPMS 机构 `cid_number`、居住地代码、出生地代码和 `election_scope_level`，由 CID 根据安装授权中的 `install_secret` 解密。
ARCHIVE 不包含 `code_id` 或使用次数；重复绑定由 CID 的 `archive_no / cid_number / wallet_pubkey` 三者唯一关系约束。
...
`geo_seal` 明文结构只在 CPMS 签发端与 CID 解密端存在：
...
- `geo_seal` AAD：`cid-cpms-v1|geo-seal|{archive_no}|{cpms_pubkey}`。
...
- 出生地在创建档案时必填 `birth_province_code / birth_city_code / birth_town_code`，从 CPMS 随包的 CID 行政区真源只读接口选择；保存后编辑档案接口不得接收或改写出生地字段。
...
- 前端列表搜索和创建出生日期统一使用 `citizenpassport/frontend/components/DateInput.tsx`，
  页面内不得散落直接的日期输入实现。
...
- CID 号不在 CPMS 回收；CPMS 通过年度导出向 CID 更新档案号状态、公民状态和投票状态。

## 8. 公民资料库

- 公民资料库命名为“公民资料库”，在公民档案详情页“资料库”左侧导航 tab 展示。
- 后端入口固定在 `backend/archive/materials.rs`，路由由 `archive::router()` 挂载；不得把资料库存储和生命周期逻辑放进角色模块。
...
- 开发默认文件正文保存到 `data/archive-materials/<archive_id>/`；正式离线安装包固定设置
  `CPMS_MATERIALS_DIR=/var/lib/citizenpassport/materials`，备份脚本必须同步备份该目录。
...
- 公民档案详情页固定提供“操作记录”tab，前端入口为 `citizenpassport/frontend/archive/ArchiveDetail.tsx`，后端入口为 `GET /api/v1/archives/:archive_id/audit-logs`。
...
- 导出文件类型固定为 `CID_CPMS_V1 / CPMS_STATUS_EXPORT`。
...
- `citizen_binding_records` 是当前仍有钱包绑定的档案快照，包含 `archive_no / wallet_address / wallet_pubkey / wallet_sig_alg / wallet_bound_at / citizen_status / voting_eligible / status_updated_at`，用于 CID 按档案号覆盖本地绑定状态；`voting_eligible` 导出前按公民状态和 16 周岁年龄线重新计算。
- `binding_release_records` 只包含 `archive_no / released_at / release_reason`，用于表达 100 年硬删除后 CID 可以释放该档案号与身份 ID、钱包地址的绑定关系。
- 导出文件不得包含姓名、出生日期、地址、护照号等实名或 CPMS 内部号码。
- 导出文件使用 CPMS ARCHIVE 签发密钥签名，签名原文为 `cid-cpms-v1|cpms-status-export|{cid_number}|{cpms_pubkey}|{export_batch_id}|{exported_at}|{records_hash}`。
