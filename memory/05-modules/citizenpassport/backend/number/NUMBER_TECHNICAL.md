...
`citizenpassport/backend/number/` 是 CPMS 编号工具模块，负责在创建公民档案时同步生成一对编号：

- `archive_no`：档案号，供 CID 绑定和 ARCHIVE 载荷使用。
...
- 本机 `archives.archive_no` 唯一约束拒绝重复；CID 绑定时继续以 `archive_no` 做全局唯一验收。
...
- 总长度：11 位。
- 示例：`GD7K3M9X2QH`。
- 省代码来自 CPMS 安装码内 `cid_number` 的 R5 段前两位，必须是 CID 省级代码表中的两位省代码。
