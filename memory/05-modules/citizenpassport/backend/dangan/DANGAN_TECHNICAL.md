...
## 1. 模块定位
`backend/dangan/` 负责 CPMS 档案业务：档案创建/查询、列表游标分页、`CID_CPMS_V1 / ARCHIVE` 档案二维码构建与签名、公民状态校验、有效期计算、公民资料库、档案操作记录聚合、档案生命周期硬删除和年度状态导出。
...
- 公民档案详情页固定提供“操作记录”tab，前端入口为 `citizenpassport/frontend/dangan/ArchiveDetail.tsx`，后端入口为 `GET /api/v1/archives/:archive_id/audit-logs`。
