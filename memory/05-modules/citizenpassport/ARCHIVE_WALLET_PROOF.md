...
- 行政区数据唯一源是 CID 系统 `citizencode/backend/china/china.sqlite`，CPMS 源码树不得保存行政区第二份源码。
- CPMS 后端只允许通过 CID 行政区真源导出的只读数据使用省、市、镇和地址段信息；
  通用发行版只内置编译后的只读行政区数据。
- 一个 CID INSTALL 安装码只对应一个市公安局；CPMS 初始化后只启用安装码 R5 段对应城市的
  镇和地址段。
- 公民档案创建/编辑时，出生日期、性别、身高均为必填；出生日期使用 `YYYY-MM-DD`，
  身高必须在 `30-260 cm`。
- 选举资格必须同时满足公民状态 `NORMAL` 和已满 16 周岁；未满 16 周岁不得保存为有选举资格。
- CPMS 前端所有日期输入统一使用 `citizenpassport/frontend/components/DateInput.tsx`；出生日期类输入
...
钱包私钥控制权验证统一放在 CID 绑定阶段：CID 扫描 CPMS 出具的档案码后，要求 CitizenWallet 对 CID
...
```text
创建档案
→ 用户在 CitizenWallet 电子护照页选择投票账户
→ CitizenWallet 展示投票账户地址二维码（QR_V1 / user_contact）
...
公民档案删除必须走 CitizenWallet 签名确认，不能物理删除。CPMS 创建 `QR_V1 / sign_request`
...
后端只接受当前登录管理员本人公钥签出的签名响应。完成接口先锁定删除 challenge 和档案行；
...
delete_reason = CitizenWallet signed archive delete
```
