...
当前官网由 Cloudflare Pages 项目 `citizenweb` 承载，正式域名为 `https://www.crcfrcn.com`；`/membership` 与同源 `/api` 共同组成官网订阅入口。CitizenConsole 不再提供独立“公民网”卡片或测试部署，唯一发布入口是“公民云 → 官网部署”。

官网部署由公民云已有的 production `CF_ACCOUNT_ID` 与 `CF_DEPLOY_TOKEN` 在一次 Touch ID 后注入当前动作，不保存第二份官网账户或令牌。动作必须按以下顺序完成：

1. `npm ci` 安装 `citizenweb/package-lock.json` 锁定依赖。
2. 验证 Cloudflare Pages 最小权限部署令牌。
3. 执行官网 lint 和 production build。
4. 通过 JSON 接口确认现有 `citizenweb` Pages 项目。
5. 使用 `npm exec -- wrangler pages deploy dist --project-name citizenweb --branch main` 发布。
6. 真实访问 `https://www.crcfrcn.com`，确认正式官网响应正常。

官网不保留本地预览端口、测试 PID、测试日志、测试 Pages 项目、SSH/Nginx 发布路线或其它影子部署流程。
