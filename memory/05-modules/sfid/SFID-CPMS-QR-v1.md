...
2. SFID 能知道档案号属于哪个公安局机构 `sfid_number`，并按 `geo_seal` 中的代码记录居住地投票范围、出生地参选范围和市/镇精度。
...
- `citizen_status=NORMAL / voting_eligible=false` 表示该公民当前无投票资格；SFID 导入年度报告时同样删除对应本地公民记录。
- `citizen_status=NORMAL / voting_eligible=true` 但钱包字段缺失、算法不为 `sr25519`、地址和公钥不匹配或时间字段无效时，SFID 也按无效公民删除本地记录，不允许继续保留旧投票账户。
- CPMS 100 年硬删除后，使用 `binding_release_records` 告知 SFID 删除该档案号与身份 ID、钱包地址的绑定关系；护照号属于 CPMS 内部号码，不导出给 SFID。
...
5. 从 `sfid_number` 解码 CPMS 授权分区 `province_code / city_code`；该分区只表示档案码来自哪个公安局授权安装。
6. 校验 CPMS 本机签名 `sig`。
