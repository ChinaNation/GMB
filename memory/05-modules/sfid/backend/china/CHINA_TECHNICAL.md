...
- `address_units`:只由 CPMS 随包只读快照消费,不进入 SFID 公权机构目录。
...
行政区变更后,`china.sqlite` 的 SHA-256 会进入 `gov_manifest.china_hash`。发布完整数据包和公权机构时,SFID 运行库必须先执行:
...
2026-06-19 东莞拆分后曾完成一次公权目录收口;随后行政区名称归属清理只更新 `china.sqlite`,未重新生成公权目录和客户端包。下一次需要发布 wuminapp / CPMS / SFID 公权机构资产时,必须重新执行本节对账流程。
...
sqlite3 sfid/backend/china/china.sqlite "select count(*) from address_units"
