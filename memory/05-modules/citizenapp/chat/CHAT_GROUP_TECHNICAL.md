...
- 加密 = OpenMLS 群(**单次加密**),投递 = **发送端扇出 N 信封**(同一密文,N 个 `recipient_cid_number`),服务端只瞬时中转、零存储不变。
- 成员变更走 MLS **Commit/Welcome**(前向/后向保密由 MLS 保证)。
- 名册以 **MLS 群成员为唯一真源**,Isar 为镜像,每次 Commit 后对账。
- 门禁沿用会话签发时的 **CID + finalized 绑定版本 + 当前账户** 三元组校验，不放宽；
  MLS 名册、群权限和群消息只认 CID。
...
sendGroupText
 └ group_create_message(1 次加密) ── N×ChatEnvelope ──▶ relay/wake ──▶ group_process
      同一份密文,N 个 recipient_cid_number                              └ 解密 application
...
投递复用 `ChatEnvelope.recipientCidNumber` 和 HTTP `recipient_cid_number`。群消息把同一
`mls_wire_message` 封入 N 个不同收件 CID 的信封，逐个走
`sendEncryptedEnvelope`；Worker 不解析 MLS 密文。
...
- **群 ID**:`group_id = "grp:<creatorCidNumber>:<nonce16>"`,即 `conversation_id`,全成员一致(经 Welcome 里的 `GroupId` 分发)。
...
| `group_remove_members` | `{…, group_id, member_cid_numbers:[…]}` | `{group_id, epoch, commit_wire_hex, removed_identities:[…]}` |
...
// GroupState{epoch, memberIdentities[]};每个 identity 的主身份段是 cid_number
```
...
```dart
ChatGroupEntity           { ownerCidNumber+groupId(唯一), groupName, creatorCidNumber,
                            epoch, memberCount, leftLocally, createdAtMillis, updatedAtMillis }
ChatGroupMemberEntity     { ownerCidNumber+groupId+memberCidNumber(唯一),
                            role(admin|member), joinedAtMillis }
...
- 门禁：群会话签发校验 CID 当前 finalized 绑定三元组；账户只完成签名鉴权。

## 10. 扇出与投递

单密文 → `group_fanout` 生成 N 个 `ChatEnvelope`（逐个设置
`recipient_cid_number`）→ `ChatFlow.deliverWithTransport` 逐个按 CID 投递；离线成员由
`sendChatWake` 唤醒并触发发送端本机队列重试。
...
- **收尾进展(2026-07-16，2026-07-29 CID 收口)**:① **群消息 sender 归属已落地**——`chat_page` 加 `isGroup`,`resolveUser` 群里按 `senderCidNumber` 经 `ProfilePresentation.forIdentityKey` 出稳定昵称,群 text builder 用 flyer `SimpleTextMessage.topWidget` 挂 `Username`(连续同发送者只首条显名);② **群贴纸已落地**——`group_flow.sendGroupSticker`(抽 `_sendGroupUserMessage` 与文本共用)+ `runtime.sendGroupSticker` + `open_group_chat` 接线。
- **群媒体发送已落地(2026-07-16)**:
...
- **验收补测已落地(2026-07-16)**:
  - **权限门控 widget 测**(`group_manage_page_test`):admin 见添加/移除/改群名、非 admin 只见退群；权限主体统一为成员 CID，fake store 覆写 `readGroup`(避 Isar 真异步在 widget 测不 settle)。
...
- **删人=CID 级**:`group_remove_members` 收 `member_cid_numbers`,移除该 CID 在群内**全部设备叶子**。
