...
- ~~Rust 依赖临时指向根目录 `smoldot/`，不利于版本治理与协作提交~~ → 已收编为 `third_party/smoldotpow` submodule
...
- genesis hash 从安装包固定 `#0` checkpoint header 本地计算，不采用远端接口返回值作为缓存信任根
- 启动时严格校验 schema、genesis hash、finalized 高度/哈希和正文大小；旧裸格式、未知字段、损坏或跨链数据直接删除
- finalized database 导出前后读取同一 finalized 锚点，锚点变化时丢弃并有限重试
- addChain 后有界等待 database 异步应用到同步状态机，并反校验真实 finalized 高度/哈希；禁止只相信信封外层声明
- 所有导出/落盘进入同一串行队列，并受 chain 身份和 client 生命周期代际保护
- 只有更高 finalized 能覆盖；同高度同 hash 不重写，更低高度永远丢弃
- 自动化测试已覆盖串行执行、倒退拒绝、导出期间推进、同高度哈希冲突、旧格式/跨链清理和 dispose 后旧任务失效
- Pixel 8a 保留原 App 数据覆盖安装：旧裸缓存被清理，从固定 `#0` 同步至 finalized `#31` 后写入约 7 KB 新信封；最终 profile 包再次冷启动时有界等待 database 异步应用，验证并从 `#31` 恢复成功

效果：保留现有秒级恢复能力，同时保证链高持续增加后本机缓存只前进、不倒退、不串链。

### 优化 B：批量余额查询（减少网络往返）

现状(ADR-017)：`wallet_page.dart` 用批量 finalized 余额查询，N 个钱包 = 1 次 finalized storage proof 请求。全端 finalized 单一口径,不再有 best 视图读取入口。
...
citizenapp/native + citizenapp/rust
    ↓ PoW 轻节点能力层
citizenapp/third_party/smoldotpow
...
建立独立 GitHub 仓库，例如 `gmb-smoldotpow`，作为 PoW 轻节点内核的唯一权威上游。
...
- `citizenapp/pubspec.yaml` 只依赖仓库内 path 包
...
- 用户明确要求只维护一处 `smoldotpow` 源码
- `smoldotpow` 已作为独立 GitHub fork 发布，适合由主仓库只引用提交指针
...
- ADR-017 后已删除 `fetchBalance()/fetchTotalBalance()/fetchBalances()/fetchConfirmedNonce()` 等 best 视图接口；金额展示走 `fetchFinalizedBalance()/fetchFinalizedBalances()/fetchFinalizedTotalBalance()`，通用 storage 读取平名 `fetchStorage()/fetchStorageBatch()` 即 finalized
...
- 已完成第一轮：`ChainRpc` 已改为纯轻节点单通道，`main.dart` 不再读取 `CITIZENAPP_RPC_URL`
...
- 正式 App 永久内置与同一 genesis 绑定的固定 `#0` finalized checkpoint，不随链高更新。新安装用户只要 peer finalized 高于 `#0` 就走 GRANDPA warp；已安装用户从原生验证采用的本机 finalized database 启动，并只对更高 finalized 发起 warp。
- Cloudflare 启动清单只治理 bootnodes，不下发运行时信任的动态 checkpoint；不保留 HTTP 回退或全节点数据库下载分支。
- 该能力属于轻客户端与节点 P2P/GRANDPA proof 服务，不修改 PoW 或 runtime 共识。
...
以下任一项未完成，不应宣称 `citizenapp` 已完成 PoW 轻节点化：
