...
- 正式 App 发布必须附带与同一 genesis 绑定的 finalized checkpoint；新安装用户以安装包签名保护的 checkpoint 为信任锚，再通过 GRANDPA warp 接近链头。
- Cloudflare 启动清单只治理 bootnodes，不下发运行时信任的动态 checkpoint。旧版本 App 仍可从较旧发行锚点 warp，不保留 HTTP 回退或全节点数据库下载分支。
- 该要求属于发布与构建产物治理，不修改 PoW 或 runtime 共识。
