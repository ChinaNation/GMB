...
- `AdminAccountService` 保存管理员主体持久化短缓存；提交投票前仍必须重新读取链上管理员快照、提案状态和对应管理员投票记录。
...
| 管理员集合变更 | `propose_admin_set_change` | `institution_code, subject, admins[], new_threshold` | 目标账户当前管理员 | 内部 |
| 决议销毁 | `propose_destroy` | `institution_code, institution, amount` | 目标联邦注册局机构管理员 | 内部 |
| GRANDPA 密钥更换 | `propose_replace_grandpa_key` | `institution, new_key(32B)` | NRC/PRC 联邦注册局机构管理员 | 内部 |
...
业务模块不接收人口快照材料。联合投票和立法特别案只提交 `PopulationScope`，链端人口分母由投票引擎从链上公民身份读取。

- `PopulationScope`：全国、省、市、镇四级作用域，链端人口分母由 `citizen-identity` 直接读取。
```
...
- `propose_l2_fee_rate(bank, new_rate_bp)`(call_index 40):
  - 签名者必须是该清算行主账户的多签管理员之一(`CidAccountQuery::is_admin_of`)
...
1. 选择业务类型并收集业务字段。
2. 校验当前钱包是否具备该联邦注册局机构管理员权限。
3. 若为联合提案，先按 `PopulationScope` 调用链上人口快照准备入口。
4. 组装链上调用字段并签名提交。
...
- `RuntimeUpgradeDetailPage` 从机构页进入时必须带上：
  - `institution`
  - `adminWallets`
- 页面会先按链上对应管理员 pallet 的 `AdminAccounts` 过滤当前仍有效的管理员钱包。
- 联合投票按钮只在以下条件全部满足时启用：
  - 提案仍处于 `joint` 阶段且状态为 `voting`
  - 当前机构尚未投票
- 当前用户已导入至少一个仍未投票的本联邦注册局机构管理员钱包
...
- 页面还会单独展示“本联邦注册局机构管理员投票进度 / 本机构阈值”，避免把联合权重阈值和机构内部门限混淆。
...
| `InternalVotesByAccount` / `JointVotesByAdmin` / `JointVotesByInstitution` / `CitizenVotesByAccount` | 投票记录 | 投票引擎 |
...
- 清理状态机阶段：`InternalVotes → JointAdminVotes → JointInstitutionVotes → JointInstitutionTallies → CitizenVotes → ProposalObject → FinalCleanup`
...
| `InternalVotesByAccount` / `JointVotesByAdmin` / `JointVotesByInstitution` / `CitizenVotesByAccount` | 投票记录 |
...
6. 阈值不再来自各管理员 `AdminAccounts`；治理机构用固定制度阈值，个人多签/机构账户从 `InternalVote.ActiveDynamicThresholds` 读取。
   `AdminAccounts` 的管理员列表后续字段是创建者和生命周期信息，不得作为阈值解码。
7. 比对当前钱包 `pubkeyHex`（去 0x 前缀、小写）是否在 `admins` 中，确定管理员身份。
8. 查询结果以内存 `accountIdHex` 短缓存；提交管理员更换、投票执行返回、下拉刷新时清除缓存重新查询。

### 7.1.1 管理员更换手机端协议

- 目录边界：公民端管理员更换只在 `lib/citizen/proposal/admins-change/` 内实现；机构/个人注册、注销仍归 `transaction/organization-manage` / `transaction/personal-manage`。
...
2. **管理员身份标识**（仅管理员可见）：绿色提示条"你是本联邦注册局机构管理员，点击上方卡片可发起提案"。
...
institutionBytes / cidFullName / title / subtitle / iconKind /
...
- 公民-广场先读本地全局索引和摘要；本地为空、索引超过 5 分钟、用户下拉刷新、详情返回时，再读取 `ProposalsByCode[institution_code]`（按机构码反向索引）。
- 新区块订阅只做节流检查，当前最短 60 秒读取一次全局治理 ID 索引；不得每个新区块都全量读取三类机构码（`institution_code`）索引。
...
| 本地持久化读库 | `LocalProposalSummary` + 全局/机构索引持久化，App 重启后可先显示本地列表 | `lib/citizen/shared/proposal/proposal_local_store.dart` |
...
**数据流**：首屏先读本机 Isar 全局索引和摘要 → 本地可用则直接显示 → 索引过期/用户刷新/详情返回再读取链上三类机构码（`institution_code`）反向索引 → 首屏 10 条缺失详情批量查询 → 写回本地持久化读库和内存缓存 → 新区块后台节流检查，有新提案再插入顶部。
...
| 公民投票 | 钱包绑定了 CID | ⏭️ 后期 |
| 机构联合投票 | 钱包是当前联邦注册局机构管理员，且该管理员尚未对本机构投票 | ✅ 已实现 |
...
每个机构（`AccountId`）同时最多允许 runtime 配置数量的活跃提案（当前生产值为 **10**），不区分提案类型（转账、销毁、换管理员等），由投票引擎（`votingengine::limit`，原 `active_proposal_limit`）统一管控。
...
- `feeAccount`：费用账户
- `safetyFundAccount`：安全基金账户，仅国储会显示
...
| `lib/citizen/proposal/admins-change/services/institution_admin_service.dart` | 管理员查询门面（委托 `AdminAccountService` 按机构码或 `AdminAccount.kind` 读取对应管理员 pallet 的 `AdminAccounts`） |
...
| `lib/citizen/proposal/runtime-upgrade/runtime_upgrade_detail_page.dart` | 协议升级提案详情页（联合投票/公民投票进度） |
| `lib/citizen/proposal/runtime-upgrade/runtime_upgrade_service.dart` | 协议升级提案链上交互服务 |
| `lib/transaction/organization-manage/institution_admin_list_page.dart` | 机构管理员列表页（SS58 地址展示） |
...
- 机构多签读取 `MultisigInstitutionEntity` 和 `InstitutionMultisigLocalState`。
- 多签列表在首次点击 `多签` Tab 后构建，防止应用启动时提前触发多签账户发现。
- 多签列表首屏只读本机 Isar，不等待链上账户状态查询、不等待对应管理员 pallet 的 `AdminAccounts`
...
- 多签账户详情页必须 local-first：个人详情页读取 `PersonalAccountEntity`、
  ...
  `MultisigInstitutionEntity`、`InstitutionMultisigLocalState` 和
  `institution_multisig_detail:*`。这些都是本机持久化储存，不是内存缓存。
- 详情页本机快照可直接显示名称、地址、本地状态、管理员公钥列表、阈值和余额快照；
  链上只负责更新账户是否存在、Active/Pending 状态、管理员/阈值和 Active finalized 余额。
...
- 个人多签状态刷新由 `PersonalManageService.fetchPersonalAccountsBatch()` 分阶段批量读取
  `PersonalAccounts / PersonalAdmins::AdminAccounts / ActiveDynamicThresholds / PendingDynamicThresholds`。
- 机构多签状态刷新由 `InstitutionManageService.fetchAccountsBatch()` 分阶段批量读取
  `AccountRegisteredCid / InstitutionAccounts / 对应管理员 pallet 的 AdminAccounts / ActiveDynamicThresholds / PendingDynamicThresholds`。
...
| 文件 | 说明 |
| --- | --- |
| `lib/citizen/shared/institution_account_list_page.dart` | 个人 + 机构多签统一账户列表页 |
| `lib/transaction/organization-manage/institution_account_info_page.dart` | 机构多签账户详情页 |
| `lib/transaction/organization-manage/institution_discovery_service.dart` | 机构多签反向索引发现服务 |
| `lib/transaction/organization-manage/institution_manage_models.dart` | 机构关闭提案模型与机构账户状态模型 |
| `lib/transaction/organization-manage/institution_manage_service.dart` | OrganizationManage 机构多签链上交互服务 |
| `lib/citizen/shared/institution_manage_detail_page.dart` | 个人/机构多签管理提案共用投票详情页；业务解码委托对应 manage 服务 |
| `lib/transaction/organization-manage/institution_multisig_create_page.dart` | 机构多签创建表单 |
| `lib/transaction/organization-manage/institution_multisig_close_page.dart` | 机构多签关闭表单 |
...
- `lib/transaction/organization-manage/institution_account_info_page.dart`
- `lib/transaction/organization-manage/institution_discovery_service.dart`
- `lib/transaction/organization-manage/institution_manage_models.dart`
- `lib/transaction/organization-manage/institution_manage_service.dart`
- `lib/citizen/shared/institution_manage_detail_page.dart`
- `lib/transaction/organization-manage/institution_multisig_create_page.dart`
- `lib/transaction/organization-manage/institution_multisig_close_page.dart`
