...
2. 通过 `state_getStorage` 查询链上 `AdminsChange::AdminAccounts(account_id)` 存储。
3. Storage key 格式：`twox_128("AdminsChange") + twox_128("Subjects") + blake2_128(account_id) + account_id`。
4. `account_id` 按主体类型派生：内置治理机构为 `0x01 + cid_number`，个人多签为 `0x03 + AccountId32`，机构账户为 `0x05 + AccountId32`；`0x02 注册机构归属关系` 只用于归属/检索，不进入管理员更换。
