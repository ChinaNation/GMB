...
2. 机构读取 `PublicAdmins / PrivateAdmins::AdminAccounts[cid_number]` 管理员人员集合；公权每项为 `admin_account + cid_number + family_name + given_name`，私权每项为 `admin_account + family_name + given_name`。个人多签独立按 `personal_account` 读取三字段 `PersonalAdmins`。
3. 机构 Storage key 格式：`twox_128(pallet_name) + twox_128("AdminAccounts") + blake2_128(cid_number) + SCALE(cid_number)`。
4. 机构管理员值按 pallet 严格解码为 `institution_code + Vec<PublicAdmin|Admin>`；storage key 中 CID 是机构 CID，公权 value 的 `cid_number` 是管理员公民 CID 引用。两类记录均不含状态、`kind`、岗位、来源或创建时间。
...
| 管理员存储类型 | 公权 `PublicAdmin` | 机构按公权 `PublicAdmin` / 私权 `Admin` 分流；个人为 `Admin` |
