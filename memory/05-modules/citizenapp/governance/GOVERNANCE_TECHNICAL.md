...
3. 组装链上业务调用字段并签名提交；联合公投和立法特别案的人口作用域与快照由
   runtime 在建案事务内按 actor CID 创建和绑定，App 不发送独立快照交易。
...
2. 机构读取 `PublicAdmins / PrivateAdmins::AdminAccounts[cid_number]` 钱包集合；个人多签独立按 `personal_account` 读取 `PersonalAdmins`。
3. 机构 Storage key 格式：`twox_128(pallet_name) + twox_128("AdminAccounts") + blake2_128(cid_number) + SCALE(cid_number)`。
4. 机构管理员值严格解码为 `institution_code + admins`；CID 只在 storage key，不在 value；不含状态、`kind`、岗位、来源或创建时间。
