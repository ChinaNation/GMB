...
2. 机构读取 `PublicAdmins / PrivateAdmins::AdminAccounts(account_id)` 钱包集合；个人多签独立读取 `PersonalAdmins`。
3. Storage key 格式：`twox_128(pallet_name) + twox_128("AdminAccounts") + blake2_128(account_id) + account_id`。
4. 机构管理员账户严格解码为 `cid_number + institution_code + admins + status`；不含 `kind`、姓名、公民 CID、岗位、来源、创建人或创建/更新时间。
5. App 再从对应 entity 的 `InstitutionRoles` 与 `InstitutionRoleAssignments` 读取岗位和有效任职，并按钱包聚合展示。
6. 阈值不来自 `AdminAccounts`；固定治理机构使用制度阈值，动态机构账户从 `InternalVote.ActiveDynamicThresholds` 读取。
7. 当前钱包只有同时属于机构 `admins` 且具有有效岗位任职时，才被识别为机构管理员。
...
- QR call data：`[PersonalAdmins pallet][propose_admin_set_change call][institution_code=PMUL][account_id:32][admins:Compact<Vec<AccountId32>>][new_threshold:u32_le]`。
...
- 机构岗位或任职变化由具体业务模块形成治理结果后写入 entity，并由 entity 派生机构 `admins`；CitizenApp 不提供机构管理员集合直接变更入口。
