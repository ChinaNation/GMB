...
| 决议销毁 | `propose_destroy` | `org, institution, amount` | 目标联邦注册局机构管理员 | 内部 |
...
- 联合投票读取 `JointVote` storage 时，机构参数必须使用统一 `AccountId` 编码；citizenapp 只能调用 `institutionIdentityToPalletId()`，不得在页面内手写 sfid `[u8;48]` 编码。
...
4. `account_id` 按主体类型派生：内置治理机构为 `0x01 + sfid_number`，个人多签为 `0x03 + AccountId32`，机构账户为 `0x05 + AccountId32`；`0x02 注册机构归属关系` 只用于归属/检索，不进入管理员更换。
5. 返回 SCALE 编码的 `AdminAccount { org, kind, admins, creator, created_at, updated_at, status }`。
...
- QR display 字段必须与 citizenwallet 公民钱包 decoder 严格一致：`org`、`subject`、`admins`、`new_threshold`，其中 `subject/admins` 均使用 `0x` 小写 hex。
...
`memory/05-modules/citizenapp/duoqian-transfer/DUOQIAN_TRANSFER_APP_TECHNICAL.md`
管理。
...
`tools/generate_citizenapp_governance_registry.mjs` 从 runtime primitives 生成到
...
`institution_data.dart` 统一输出 D/ADR-015 `AccountId32`：内置治理机构为 `0x01 + sfid_number UTF-8 + 右零填充`；个人多签为 `0x03 + AccountId32 + 15B 零填充`；注册机构账户为 `0x05 + AccountId32 + 15B 零填充`。`0x02 注册机构归属关系` 只保留给同一 SFID 机构下多账户归属/检索，不作为转账支出主体。
