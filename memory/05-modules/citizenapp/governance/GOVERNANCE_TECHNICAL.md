...
7. 当前钱包只要其账户属于机构 `admins` 即被识别为机构管理员；姓名和岗位任职不参与授权。
...
- QR_V1 仍只携带 `b.a + b.d`；CitizenApp 在本步已产生三字段 call data。CitizenWallet 的解码展示与统一 QR 字段注册属于正式创世前统一改造第5步，第5步完成前不将旧冷钱包解码标记为已对齐。
