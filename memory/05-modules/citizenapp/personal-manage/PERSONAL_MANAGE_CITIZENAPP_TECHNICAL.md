...
- 个人/机构多签管理提案投票详情页：共用入口位于 `citizenapp/lib/governance/account_manage_detail_page.dart`，本模块只提供 PersonalManage 解码服务。
...
- 管理员真源是 `AdminsChange::AdminAccounts`，AdminAccountKind 使用 `PersonalAccount AccountId`。
...
- `personal_manage_service.dart` 必须使用 `signAndSubmitInBlock()` 等待入块，并从
  `System.Events` 确认 `PersonalManage.PersonalAccountProposed(event_index=0)`。
...
- 详情页链上明确查不到 `PersonalManage::PersonalAccounts` 时，写入本机
...
- 统一多签列表首屏只读本机 `PersonalAccountEntity` 和 `PersonalAccountLocalState`，
  不等待 `PersonalManage::PersonalAccounts`、`AdminsChange::AdminAccounts` 或 discovery 链上读取。
...
- 个人多签详情页额外使用 `personal_account_detail:*` 本机持久化快照保存管理员公钥、
  ...
  `PersonalManage::PersonalAccounts`、`AdminsChange::AdminAccounts` 或 `InternalVote`
  阈值而全屏等待。
...
- 个人多签列表状态刷新使用 `PersonalManageService.fetchPersonalAccountsBatch()`：
  先批量读取 `PersonalAccounts` 与 `AdminsChange::AdminAccounts`，再按解码出的
...
`citizenapp/lib/governance/organization-manage/` 不再承载 PersonalManage 主业务。当前仅保留：
