# citizenapp duoqian-transfer 技术说明

## 模块边界

`citizenapp/lib/transaction/duoqian-transfer/` 是 citizenapp 端多签转账唯一实现目录。
...
`citizenapp/lib/governance/organization-manage/` 不再实现多签转账按钮和跳转；多签账户详情页只允许挂载 `duoqian-transfer` 提供的 `DuoqianTransferEntryCard`，入口自身逻辑在 `duoqian-transfer` 内部。

`citizenapp/lib/governance/` 和 `citizenapp/lib/citizen/vote/` 只允许调用 `DuoqianTransferProposalAdapter` / `DuoqianTransferProposalFeed`，不得直接判断 `TransferProposalInfo`、`SafetyFundProposalInfo`、`SweepProposalInfo`，也不得直接打开 `DuoqianTransferDetailPage`。
