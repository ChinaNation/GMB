...
- **目录说明**:本文记录收款端基础设计。2026-04-30 后 offchain 业务目录统一为
  `citizenapp/lib/transaction/offchain-transaction/`;若恢复/补齐收款码页面,应落在
  `lib/transaction/offchain-transaction/pages/offchain_receive_page.dart`。
- **范围**:新建清算行**收款 QR 页**(与老 `ReceiveQrPage` 并存),`bind_clearing_bank_page`
  绑定成功写本地 cid_number 缓存,收款页读缓存并轮询余额。
...
付款方无处扫码。本步给:
- 收款方生成含新清算行 `cid_number` 的 QR 码
- 生成 QR 的前置条件:知道收款方自己绑的清算行 `cid_number`
- 实时展示收款方清算行余额(到账后刷新)
...
`clearing_bank_cid_number_{walletIndex}`):
- **写入**:`bind_clearing_bank_page` 链上绑定成功后立即 `save(walletIndex, cidNumber)`
...
| 文件 | 变更 |
|---|---|
| `lib/transaction/offchain-transaction/pages/bind_clearing_bank_page.dart` | 绑定成功后 `await ClearingBankPrefs.save(wallet.walletIndex, widget.bank.cidNumber)`,紧接 SnackBar 提示 |
| `lib/transaction/offchain-transaction/pages/offchain_home_page.dart` | 追加"生成收款码"入口,落 `OffchainReceivePage`;提示文本同步更新(付款入口仍在主页扫码;老省储行页面 ADR-006 已退出) |

### 3.3 不动(保留老路径)

- `lib/wallet/ui/receive_qr_page.dart`:老省储行 `ReceiveQrPage`,`bankCidNumber`
...
```json
{
  ...
  "symbol":  "GMB",
  "memo":    "午餐",
  "bank":    "GD001-SCB05-000000001-2026"      // 收款方清算行 cid_number
}
```

付款方 `offchain_pay_page` 扫到后:
- `toAddress = body.address`
- `recipientBankCidNumber = body.bank`
- `initialAmountYuan = body.amount`
- `memo = body.memo`

CID `searchClearingBanks(keyword=recipientBankCidNumber)` 反查主账户,同行校
验 + 费率查询 + 签名提交。
...
init:
  ClearingBankPrefs.load(walletIndex)
    ├─ null → "请先绑定清算行" + 返回按钮
    └─ "GD001-SCB05-000000001-2026" → 渲染 QR + 启动余额轮询 Timer(5s)
...
| 风险 | 等级 | 缓解 |
|---|---|---|
| `ClearingBankPrefs` 缓存失效(重装 / 清数据 / CLI 绑 / 别机绑) | **P2** | 收款页显示"请先绑定";重走绑定流程会同步写回。Step 3 CID 加反查 API 自动回填 |
