...
| `lib/transaction/offchain-transaction/pages/offchain_receive_page.dart` | 新收款页:WalletProfile + 可选 `clearingNodeWssUrl`;读 prefs → 生成 `CITIZEN_QR_V1 kind=user_transfer` QR(`address` / `name` / `amount` / `memo` / `bank=sfid_number`);每 5 秒轮询 `offchain_queryBalance(user)` 刷新余额 |
...
| 文件 | 变更 |
|---|---|
| `lib/transaction/offchain-transaction/pages/bind_clearing_bank_page.dart` | 绑定成功后 `await ClearingBankPrefs.save(wallet.walletIndex, widget.bank.sfidNumber)`,紧接 SnackBar 提示 |
...
```json
{
  ...
  "symbol":  "GMB",
  "memo":    "午餐",
  "bank":    "GD001-SCB05-000000001-2026"      // 收款方清算行 sfid_number
}
```
