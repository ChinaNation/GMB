...
`lib/qr/` 是 CitizenApp 所有二维码能力的统一收口模块，负责：
...
citizenapp 电子护照页使用 `CITIZEN_QR_V1 / user_contact` 向 CPMS 出示钱包地址。CPMS
只扫描该固定码并保存 `wallet_address / wallet_pubkey`，不要求 citizenapp 在 CPMS 阶段签名。
...
| 协议常量 | 值 | 用途 |
| --- | --- | --- |
| `login` | `CITIZEN_QR_V1` | 登录、绑定签名验证 |
| `sign` | `CITIZEN_QR_V1` | 冷钱包离线交易签名 |
| `user` | `CITIZEN_QR_V1` | 用户信息传输（联系人、付款，通过 purpose 字段区分） |
...
| 类型 | 触发条件 |
| --- | --- |
| `login` | `proto == CITIZEN_QR_V1` |
| `transfer` | `proto == CITIZEN_QR_V1` 且 `purpose == transfer` |
| `contact` | `proto == CITIZEN_QR_V1` 且 `purpose == contact`（或无 purpose） |
| `sign` | `proto == CITIZEN_QR_V1` |
| `legacyAddress` | `gmb://account/...` 或裸 SS58 地址 |
| `unknown` | 无法识别 |

## 5. 登录码协议（CITIZEN_QR_V1）
...
## 6. 收款码协议（CITIZEN_QR_V1 / kind=user_transfer）
...
- `CITIZEN_QR_V1 / kind=user_transfer` JSON 格式 → 完整解析
- `gmb://account/<address>` 格式 → 仅填充收款地址
- 裸 SS58 地址 → 仅填充收款地址

## 7. 用户码协议（CITIZEN_QR_V1）
...
- `signer/`：
  ...
  - `QrSigner` 提供扫码签名协议（`CITIZEN_QR_V1`）
  - 冷钱包离线解码与放行规则统一归 `citizenwallet/lib/signer/`，citizenapp 只展示请求、扫描回执并校验回执一致性
- `wallet/`：
...
- `user/`：
  - `UserQrPayload` 已迁移到 `CITIZEN_QR_V1` 格式
...
离线执行端页面在 citizenwallet 中实现。citizenapp 只负责展示 `sign_request` 二维码并扫描 `sign_response` 回执；citizenwallet 必须按两色规则独立验证，绿色通过才可签名，红色拒签不得生成可用回执。
