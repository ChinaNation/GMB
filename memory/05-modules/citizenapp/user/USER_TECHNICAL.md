...
投票身份字段直接读取链上 `CitizenIdentity::VotingIdentityByAccount`：投票账户、公民身份 CID 号、居住选区、身份状态、投票身份有效期。竞选身份在此基础上读取 `CandidateIdentityByAccount`，增加公民姓名、性别、出生日期和出生地。
...
3. `MyIdService` 只取当前默认热钱包，并读取 finalized `VotingIdentityByAccount` 与 `CandidateIdentityByAccount`
