...
- 当前用户身份唯一取自链上闭环解析出的 `cid_number`；当前绑定 `account_id` 只负责
  签名、鉴权和付款。`account_id` / `ss58_address` 继续保留在资料与通讯录模型中，但
  用户主页、通讯录卡片和通讯录搜索暂不展示或检索账户地址；账户地址只在用户码、钱包、
  转账及绑定核验等明确账户边界使用。
...
默认资料；联系人私人备注只在通讯录显示，账户地址不进入联系人卡片或搜索，也不能充当昵称。

通讯录卡片的身份行统一显示“公民号：`cid_number`”。搜索范围只包含公开昵称、私人备注和
公民号，不匹配 `account_id` 或 `ss58_address`；转账动作仍使用联系人模型中经 finalized
复核的地址快照，不因隐藏展示而删除内部字段。
...
## 9. CID finalized 后页面就地收敛（2026-08-07）

- 首次占号完成 finalized 身份闭环后，服务层先失效 `IdentityAccountCache` 再递增统一身份
  revision；占号不改变 `account_id` 也必须广播，因为 `cid_number` 已从空推进为有效值。
- “我的”、广场、聊天、身份、通讯录、创作者和会员等已挂载页面监听同一 revision，并以
  `account_id + cid_number` 或完整身份重新判定；通讯录/创作者加载使用代际保护，旧的
  “未注册”异步结果不得覆盖新状态。
- 身份缓存的并发合并只允许复用同一 revision 请求；`invalidate()` 后仍在运行的旧链读
  不得回写当前缓存，也不得被 finalized 广播后的新请求复用。
