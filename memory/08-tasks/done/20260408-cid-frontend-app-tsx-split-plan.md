...
> 配套任务卡:`20260408-cid-frontend-app-tsx-split.md`
> 本文件是**执行方案**,列出精确的状态/handler/JSX 归属。

## 当前现状(2026-04-08)

- `citizencode/frontend/App.tsx` = **3431 行 / 1 个 React 组件**
...
- `views/` 目录目前只有:
  ...
  - 任务卡 `20260408-cid-public-security-cpms-embed` 刚新增的 `CpmsSitePanel.tsx` / `CpmsRegisterModal.tsx` / `InstitutionDetailPage.tsx` 也在 institutions/
...
│   ├── useScope.ts                      # 已存在
│   └── useCidMeta.ts                   # 新增:cidMeta + cities 共享
├── utils/
...
- Handler 删:`refreshMultisigCids` / `onDeleteMultisigCid` / `onGenerateMultisigCid` / `openMultisigModal`
- JSX 删:L3155-3245 的"生成机构CID" Modal(整块不可达)
- Import 删:`GenerateMultisigCidResult` / `MultisigCidRow` / `deleteMultisigCid` / `generateMultisigCid` / `listMultisigCids`
- `activeView === 'multisig'` 分支**保留**,继续渲染 `<InstitutionsView category="PRIVATE_INSTITUTION">`

**api/client.ts 连带清理**:`listMultisigCids` / `deleteMultisigCid` / `generateMultisigCid` / `MultisigCidRow` / `GenerateMultisigCidResult`(整仓库 Grep 确认无其他消费者)。
...
**`hooks/useCidMeta.ts`**(新):
```tsx
function useCidMeta(auth: AdminAuth | null) {
  // 懒加载 cidMeta,共享给所有需要的 view
...
8. npm run build 必须绿
9. 手工点一遍该 tab 所有按钮
10. commit:"feat(citizencode/frontend): extract <module> view from App.tsx"
```
...
3. **`cidMeta` 被多处共享** —— operators / keyring / multisig-legacy / institutions 都用。改成 `useCidMeta` hook 后首次加载时机要处理好(懒加载 + 缓存)。
...
配套主任务卡 `20260408-cid-frontend-app-tsx-split.md` 全部 6 步收官完成。
