---
title: 宪法机构主账户字段改名 duoqian_account → main_account
status: done
...
- primitives/china/ 8 文件完成 `duoqian_account → main_account` + `CHINA_RESERVED_DUOQIAN_ADDRESSES → CHINA_RESERVED_MAIN_ADDRESSES` + `is_reserved_duoqian_address → is_reserved_main_address` + 测试函数 `all_china_ch_duoqian_addresses_are_unique → all_china_ch_main_addresses_are_unique`
...
- duoqian-manage 全部（`action.duoqian_account`、`DuoqianAccounts`、`PersonalDuoqianInfo`、`CreateDuoqianAction`、`DuoqianAddressValidator`、`derive_duoqian_address_from_sfid_number`）
- SFID 后端 `MultisigAccount.duoqian_account`、SFID 前端 `deriveDuoqianAddress.ts`
- wuminapp/wumin 的 `duoqianAddress` Dart 字段
- tools/duoqian.py 文件名

# 背景

`primitives/china/` 下 8 个宪法机构常量文件的 `duoqian_account` 字段语义错位：这个字段代表的是机构的**主账户**（与 `fee_account` 费用账户同级），但字面却叫"duoqian（多签）"，而"多签"在本系统里是**更大的范畴**（包含主账户、费用账户、注册机构多签、注册个人多签四类都是多签账户）。

为消除歧义，将宪法机构常量及其直接读取方的 `duoqian_account` 统一改名为 `main_account`。注册多签概念（duoqian-manage / SFID 注册机构 / wuminapp 注册机构）保持 `duoqian_account` 不动。

# 铁律

- 字节值不变，仅字段名变，不触 chainspec 冻结、不需要链重启
- 注册多签（`duoqian-manage::action.duoqian_account`、`DuoqianAccounts`、`PersonalDuoqianInfo`、`DuoqianAddressValidator`、`CreateDuoqianAction`、`derive_duoqian_address_from_sfid_number`）全部保留
- SFID 后端 `MultisigAccount.duoqian_account`、前端 `deriveDuoqianAddress.ts` 保留
...
- [ ] china_cb.rs / china_ch.rs / china_zf.rs / china_jc.rs / china_lf.rs / china_sf.rs / china_jy.rs：字段 `duoqian_account` → `main_account`
...
- [ ] china_cb.rs：顶部注释"duoqian_account 和 fee_account" → "main_account 和 fee_account"

## 第 2 步：Rust runtime 直接读取方

- [ ] `runtime/src/configs/mod.rs`（选择性：保留 duoqian_manage pallet call 参数 `duoqian_account`；rename `n.duoqian_account` + `is_reserved_duoqian_account` 包装 + 注释中的 const 名）
...
- [ ] `runtime/issuance/shengbank-interest/src/lib.rs`（全量）
- [ ] `runtime/transaction/offchain-transaction/src/lib.rs`（1 处）
- [ ] `runtime/transaction/onchain-transaction/src/lib.rs` + benches（全量）
- [ ] `runtime/transaction/duoqian-transfer/src/lib.rs`（选择性：rename primitives 访问，保留 `DuoqianAddressValidator` trait 使用）
- [ ] `runtime/transaction/institution-asset/src/lib.rs`（2 条注释）
...
- [ ] memory/05-modules/citizenchain/runtime/transaction/duoqian-transfer/DUOQIAN_TRANSFER_TECHNICAL.md + SWEEP_TECHNICAL.md（指向 primitives 的段落）
- [ ] memory/00-vision/GMB_WHITEPAPER.md
- [ ] wuminapp 若干 .dart 注释行（"primitives 中的 duoqian_account 字段"）
- [ ] `tools/duoqian.py`：更新扫描正则 + 输出模板（scan `main_account:`，emit `CHINA_RESERVED_MAIN_ADDRESSES` / `is_reserved_main_address`）
...
全仓库 `rg "duoqian_account|CHINA_RESERVED_DUOQIAN_ADDRESSES|is_reserved_duoqian"` 仅保留：注册多签 pallet、SFID、wuminapp、wumin、tools/duoqian.py 内部残留（符合边界设计）。
