...
注册业务字段只使用 `sfid_number / institution_name / account_names[]`,不再把
`subject_property / sub_type / parent_sfid_number` 放进机构注册 payload。区块链软件应能基于
SFID 资格候选查询选择私法人股份公司及其所属非法人,再用注册信息凭证完成链上注册。

## 边界规则

- SFID 查询与注册分开:查询详情仍走 `/api/v1/app/institutions/:sfid_number`,注册信息只走 `/registration-info`。
...
- 节点软件注册信息拉取只调用 `/api/v1/app/institutions/:sfid_number/registration-info`。
- `propose_create_institution` 的业务注册字段只剩 `sfid_number / institution_name / account_names[]`。
- 节点端 call_data 与 runtime 参数顺序一致,包含 `province + signer_admin_pubkey`。
- 前端账户输入行来自 SFID `account_names[]`。
- 旧 `subject_property/sub_type/parent_sfid_number` 不再出现在机构注册 payload/DTO/文档中。
- 相关 Rust/TypeScript 检查通过。

## 执行记录

- `duoqian-manage` runtime:
  - 删除机构注册参数、storage/action 中的 `subject_property/sub_type/parent_sfid_number`。
  - `SfidInstitutionVerifier` 改为校验 `sfid_number / institution_name / account_names[] / nonce / province / signer_admin_pubkey`。
  - 清算行资格不再读取链上机构类型元数据,由 SFID `eligible-search` 负责候选资格。
- 节点软件 Rust:
  - SFID 拉取入口改为 `fetch_clearing_bank_institution_registration_info`。
  - DTO 改为 `InstitutionRegistrationInfoResp { sfid_number, institution_name, account_names, credential }`。
...
- 节点前端:
  ...
  - 提交参数删除 `subject_property/subType/parentSfidId`,新增 `signerAdminPubkey`。
- 文档:
...
- 注册链路 active code 已无 `InstitutionCredentialResp`、`fetch_clearing_bank_institution_credential`、旧 `subject_property/sub_type/parent_sfid_number` 注册参数。
- `eligible-search` DTO 仍保留 `subject_property/sub_type/parent_sfid_number`,这是 SFID 候选资格查询展示字段,不是注册 payload。
