...
注册业务字段只使用 `sfid_number / sfid_full_name / account_names[]`,不再把
...
- `propose_create_institution` 的业务注册字段只剩 `sfid_number / sfid_full_name / account_names[]`。
...
- `duoqian-manage` runtime:
  ...
  - `SfidInstitutionVerifier` 改为校验 `sfid_number / sfid_full_name / account_names[] / nonce / province / signer_admin_pubkey`。
  - 清算行资格不再读取链上机构类型元数据,由 SFID `eligible-search` 负责候选资格。
- 节点软件 Rust:
  - SFID 拉取入口改为 `fetch_clearing_bank_institution_registration_info`。
  - DTO 改为 `InstitutionRegistrationInfoResp { sfid_number, sfid_full_name, account_names, credential }`。
