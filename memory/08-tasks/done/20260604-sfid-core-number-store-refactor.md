# 20260604 SFID core/number/store 目录整改

## 任务目标

- 后端统一目录命名：`app_core` 改为 `core`，`sfid_number` 改为 `number`，`store_shards` 改为 `store`。
...
- `sfid_number` 字段名和业务含义不变；只改编码协议目录名为 `number`。
- 行政区划归 `china`，身份 ID 编码协议归 `number`，不得混用。
- 公权机构归 `gov`，私权机构归 `private`，主体公共能力归 `subjects`。
- 不保留 `models`、`backend/login`、`backend/qr`、`backend/app_core`、`backend/sfid_number`、`backend/store_shards`、`frontend/qr`、`frontend/sfid` 等旧目录残留。
...
- 已更新 SFID 架构文档、模块文档、任务引用和相关注释。
- 已同步 `agent-rules`、`repo-map`、`unified-naming` 和 Store 分片铁律等活跃规则文档。
- 已通过 `cargo fmt --manifest-path sfid/backend/Cargo.toml`。
- 已通过 `cargo check --manifest-path sfid/backend/Cargo.toml`。
