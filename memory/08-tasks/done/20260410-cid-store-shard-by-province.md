...
- 承载能力:从 Phase 1 的 ~500 并发 → **~5000 并发 ADMIN**
...
当前 `citizencode/backend/main.rs` 的 `load_store_postgres`:

```
每次 HTTP 请求中 store 状态变化时触发:
  1. SELECT payload FROM runtime_cache_entries WHERE key='cid_store'  (~1.2MB JSONB)
...
- `runtime_cache_entries(key TEXT, payload JSONB)` —— 单行大 JSON 快照,key = `cid_store`
...
pub(crate) struct StoreShard {
    ...
    // ── 本联邦管理员(SHENG_ADMIN + ADMIN,从 admins 表关联)──
...
切换开关环境变量 `CID_SHARD_SINGLE_WRITE=true`:
...
let institution = store.multisig_institutions.get(&cid_number)?;
...
**province 从哪里来**:
- ADMIN / FederalAdmin 的请求 → `ctx.admin_province`(已有)
- 公开路径(citizen 绑定等)→ 从 cid_number 里解析省份代码(见 `citizencode/province.rs`)
...
- [ ] 本地起后端,跑压测脚本(`cid-load-test`)
- [ ] 对比 baseline:Phase 1 vs Phase 2 的 P50/P99
- [ ] 目标:单省 100 并发时 P99 降低 >50%
- [ ] 环境变量 `CID_SHARD_SINGLE_WRITE=false`(过渡期)/ `true`(完全切换)
- [ ] 更新 `feedback_cid_store_sharded.md`
- [ ] 任务卡归档 done/
...
- Phase 1 任务卡:`memory/08-tasks/done/20260409-cid-sheng-admin-per-province-keyring-impl.md`
