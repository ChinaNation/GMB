...
- 归属：Blockchain Agent（citizenchain runtime · voting-engine-system）

## 背景

Phase 0 分析（见 MEMORY 里的 `project_duoqian_finalize_transfer_step2.md`）发现：
- `VotingEngineSystem::internal_vote` 的公开 call 被历史决策封禁（`Err(NoPermission)`），
...
**只改** `citizenchain/runtime/governance/voting-engine-system/`。业务模块和客户端分别
在 Phase 2/3 改。
...
`voting-engine-system/src/lib.rs`：新增 trait + 空实现 + 手写 tuple impl（覆盖 1~6
...
- ✅ `cargo test -p voting-engine-system --lib` — **49/49 全部通过**
- ✅ `grep -n 'cast_internal_vote' voting-engine-system/` — 空
- ✅ `grep -n 'pub fn create_internal_proposal\|pub fn create_joint_proposal'
  voting-engine-system/src/lib.rs` 只匹配 trait 定义，不再有 `#[pallet::call]`

## 不在本 Phase 范围

- ⚠️ **整链 `cargo check -p node` 不通过**：业务模块（admins-origin-gov / resolution-destro-gov /
  grandpa-key-gov / duoqian-manage-pow / duoqian-transfer-pow）当前仍在引用
