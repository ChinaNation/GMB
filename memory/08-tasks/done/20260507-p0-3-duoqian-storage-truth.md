...
- 注册机构地址反查：`OrganizationManage::AccountRegisteredSfid(duoqian_account)`
...
- 新增 `wuminapp/lib/duoqian/shared/duoqian_storage_codec.dart`，统一维护 `AccountRegisteredSfid`、`Institutions`、`InstitutionAccounts`、`PersonalDuoqians`、`PersonalDuoqianInfo`、`AdminsChange::AdminAccounts` 的 storage key 与 SCALE 解码。
- `DuoqianManageService.fetchPersonalMeta` 改读 `PersonalManage::PersonalDuoqianInfo`。
- `DuoqianManageService.fetchDuoqianAccount` 改成先走注册机构路径 `AccountRegisteredSfid -> Institutions + InstitutionAccounts`，未命中再走个人多签 `PersonalManage::PersonalDuoqians`。
