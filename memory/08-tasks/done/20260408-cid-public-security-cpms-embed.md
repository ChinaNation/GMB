...
- **任务 ID**: `20260408-cid-public-security-cpms-embed`
- **模块**: citizencode-backend + cid-frontend
- **状态**: 已完成
- **完成日期**: 2026-04-08

## 目标

把"给公安局安装 CPMS 系统"的整套 QR1/QR2/QR3 流程,从 cid 前端老入口
...
### 新增 HTTP 路由
- `GET /api/v1/admin/cpms-keys/by-institution/:cid_number`
  - Handler: `sheng_admins::get_cpms_site_by_institution`
  - 输入:`multisig_institutions.cid_number`(机构主键,**不是 CPMS cpms_site_key**)
...
### 关键约束
- **两个 cid_number 不同值**:`multisig_institutions.cid_number` 是两层机构模型主键,
...
### 改动文件
- `citizencode/backend/sheng_admins/institutions.rs`(新增 handler)
- `citizencode/backend/sheng_admins/mod.rs`(re-export)
- `citizencode/backend/main.rs`(路由 + 启动钩子调用)
- `citizencode/backend/app_core/runtime_ops.rs`(新增 `cleanup_orphan_cpms_sites`)

## 前端改动

### 新增文件
- `citizencode/frontend/institutions/CpmsSitePanel.tsx`(~300 行)
  - 公安局详情页上半部分主组件
  ...
  - 复用 `api/client.ts` 里已有的 `generateCpmsInstitutionCid` /
    `reissueInstallToken` / `disableCpmsKeys` / `revokeCpmsKeys` / `deleteCpmsKeys`
  - 调新增 API `getCpmsSiteByInstitution`

- `citizencode/frontend/institutions/CpmsRegisterModal.tsx`(~105 行)
  - QR2 payload 粘贴输入 → `registerCpms` → 展示 QR3 payload + 复制
  - 本次不做摄像头扫码(老 App.tsx 里的扫码实现后续拆分任务再搬)

### 改动文件
- `citizencode/frontend/institutions/api.ts`
  - 新增 `getCpmsSiteByInstitution(auth, cidId) → Promise<CpmsSiteRow | null>`
  - 从 `client.ts` re-export `CpmsSiteRow` 类型

- `citizencode/frontend/institutions/InstitutionDetailPage.tsx`
...
- imports:移除 `CpmsSiteRow`, `GenerateCpmsInstitutionCidResult`,
  `deleteCpmsKeys`, `disableCpmsKeys`, `generateCpmsInstitutionCid`,
  `listCpmsSites`, `registerCpms`(**保留 `scanCpmsStatusQr`**,QR4 citizen
  档案扫描仍在用)
- state:`cpmsSites` / `cpmsSitesLoading` / `institutionCidOpen` /
  `institutionCidLoading` / `institutionCidResult` / `institutionQrPreview`
- forms/refs:`institutionCidForm`, `institutionQrRef`, `institutionQrPreviewRef`
- handler:`refreshCpmsSites`, `onDisableCpmsSite`, `onDeleteCpmsSite`,
  `openRegisterScanner`, `openInstitutionCidModal`,
  `onGenerateInstitutionCid`, `onFinishInstitutionCid`,
  `downloadQrFromRef`, `onDownloadInstitutionCid`, `onDownloadInstitutionPreview`
...
# 前端
cd citizencode/frontend && npx tsc --noEmit   # EXIT=0
...
```
INFO cid_backend::app_core::runtime_ops: cleaned up orphan CPMS sites (no matching institution)
  count=3 sample=["CID-000", "CID-001", "CID-002"]
```
...
- 后续任务:`20260408-cid-frontend-app-tsx-split` — App.tsx 彻底拆分
