---
title: 全仓库统一签名/派生域为 DUOQIAN + op_tag
status: done
...
|---|---|---|
| 机构主账户（含注册多签） | `DUOQIAN + 0x00 + ss58 + sfid_number [+ name]` | SFID 身份码 |
| 机构费用账户 | `DUOQIAN + 0x01 + ss58 + sfid_number` | 机构身份 ID |
| 省储行质押账户 | `DUOQIAN + 0x02 + ss58 + citizens_number_u64_le` | 省人口数 |
| 国储会安全基金 | `DUOQIAN + 0x03 + ss58 + NRC_sfid_number` | NRC 身份 |
| 个人多签账户 | `DUOQIAN + 0x04 + ss58 + creator_32 + name` | 创建人+名称 |
| 公民身份绑定签名 | `(DUOQIAN, 0x10, genesis, who, binding_id, nonce)` | SCALE tuple |
| 公民投票签名 | `(DUOQIAN, 0x11, genesis, who, binding_id, proposal_id, nonce)` | SCALE tuple |
| 人口快照签名 | `(DUOQIAN, 0x12, genesis, who, eligible_total, nonce)` | SCALE tuple |
| SFID 机构登记签名 | `(DUOQIAN, 0x13, genesis, sfid_number, name, nonce [, province])` | SCALE tuple |
...
**统一目标**：一个 domain `DUOQIAN` + 1 字节 `op_tag` 覆盖全部"地址派生 + 签名 payload"。

# 统一方案

```rust
// primitives/src/core_const.rs
pub const DUOQIAN: &[u8; 10] = b"DUOQIAN";

// 地址派生 op_tag (0x00-0x0F)
pub const OP_MAIN:     u8 = 0x00;  // ss58 || sfid_number [|| name]
pub const OP_FEE:      u8 = 0x01;  // ss58 || sfid_number
pub const OP_STAKE:    u8 = 0x02;  // ss58 || citizens_number_u64_le
pub const OP_AN:       u8 = 0x03;  // ss58 || NRC_sfid_number
...
- [ ] `primitives/src/core_const.rs`：新增 `DUOQIAN` + `OP_*` 常量
