...
| pallet | 删除 | 新增/保留 |
|---|---|---|
| `admins_origin_gov` (12) | `vote_admin_replacement` call | `propose_admin_replacement` + `execute_admin_replacement`(call_index 2→1) + `InternalVoteExecutor` |
| `resolution_destro_gov` (14) | `vote_destroy` call | `propose_destroy` + `execute_destroy`(call_index 2→1) + `InternalVoteExecutor` |
| `grandpa_key_gov` (16) | `vote_replace_grandpa_key` call | `propose_replace_grandpa_key` + `execute_replace_grandpa_key`(2→1) + `cancel_failed_replace_grandpa_key`(4→2) + `InternalVoteExecutor` |
| `duoqian_manage_pow` (17) | `finalize_create` + `vote_close` + `CreateVoteIntent` + `signing_hash` + 相关 use | `propose_create`/`propose_close`/`register_sfid_institution`/`propose_create_personal`(5→3)/`cleanup_rejected_proposal`(6→4) + `InternalVoteExecutor`(按 ACTION_CREATE / ACTION_CLOSE 分派) |
| `duoqian_transfer_pow` (19) | `finalize_transfer`/`finalize_safety_fund_transfer`/`finalize_sweep_to_main` + `TransferVoteIntent` + `verify_and_cast_votes` + 相关 use | `propose_X ×3`(0/1/2) + **新增** `execute_X ×3`(3/4/5) 作为任意人重试通道 + `InternalVoteExecutor`(三路分派:transfer 走 MODULE_TAG、safety_fund/sweep 走独立存储键) |
...
另:
- 简化 `PowTxAmountExtractor` 中 `VotingEngineSystem::Call` 的匹配(create_X/finalize_X
  已删,只剩 internal_vote/finalize_proposal 免费,其他付费 1 元)
- 删除 `DuoqianManagePow::Call::finalize_create` / `vote_close` 两条匹配
- 修复 runtime lib.rs 的旧 `ResolutionDestroGov::vote_destroy` 测试 → `internal_vote`
...
| pallet | MODULE_TAG |
|---|---|
| `admins_origin_gov` | `b"adm-rep"` |
| `grandpa_key_gov` | `b"gra-key"` |
| `resolution_destro_gov` | `b"res-dst"` |
| `resolution_issuance_gov` | `b"res-iss"` |
| `runtime_root_upgrade` | `b"rt-upg"` |
| `duoqian_manage_pow` | `b"dq-mgmt"` |
| `duoqian_transfer_pow` | `b"dq-xfer"` |

## 验收

- ✅ `cargo check -p node`(需 WASM_FILE + 占位 frontend/dist)— **全链编译通过**
- ✅ 6 个 pallet 单测:
  - `voting-engine-system`: 49/49
  - `admins-origin-gov`: 20/20
  - `resolution-destro-gov`: 14/14
  - `grandpa-key-gov`: 15/15
  - `duoqian-manage-pow`: 17/17
  - `duoqian-transfer-pow`: 20/20
  - **合计 135/135 通过**
- ✅ 全仓 `grep 'finalize_create\|finalize_transfer\|finalize_safety_fund\|finalize_sweep\|vote_admin_replacement\|vote_destroy\|vote_replace_grandpa_key\|vote_close' citizenchain/runtime/` → 只剩 `runtime_root_upgrade::finalize_joint_vote` / `resolution_issuance_gov::finalize_joint_vote`(联合投票回调,合理保留)
...
测试辅助函数迁移:
- `finalize_with` / `finalize_transfer_n`(duoqian-manage-pow / duoqian-transfer-pow)
  参数签名保留,内部改为循环调 `VotingEngineSystem::internal_vote(...)`。
