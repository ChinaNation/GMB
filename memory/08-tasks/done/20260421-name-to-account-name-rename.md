...
- [duoqian-manage-pow/src/lib.rs](citizenchain/runtime/transaction/duoqian-manage-pow/src/lib.rs)：
...
- [benchmarks.rs](citizenchain/runtime/transaction/duoqian-manage-pow/src/benchmarks.rs)：`bench_name<T>` → `bench_account_name<T>`，添加 `let account_name = bench_account_name::<T>()?` 到 propose_create / propose_close 各 setup（先前 5 处调用缺此参数，遗留的 pre-existing 测试 bug 一并修好）
...
- [duoqian-transfer-pow/src/lib.rs](citizenchain/runtime/transaction/duoqian-transfer-pow/src/lib.rs)：测试 trait impl 参数 + `MaxSfidNameLength` → `MaxAccountNameLength`
- [offchain-transaction-pos](citizenchain/runtime/transaction/offchain-transaction-pos/src/bank_check.rs) + [tests.rs](citizenchain/runtime/transaction/offchain-transaction-pos/src/tests.rs)：`SfidAccountQuery::find_address(sfid_id, name)` trait 方法签名 → `(sfid_id, account_name)`；`ensure_can_be_bound` 局部 `name_bytes` → `account_name_bytes`
...
- [DUOQIAN_TECHNICAL.md](memory/05-modules/citizenchain/runtime/transaction/duoqian-manage-pow/DUOQIAN_TECHNICAL.md)：派生公式、extrinsic 签名、Storage key、Error 名、角色翻译段全部 `name` → `account_name`
...
- [STEP1_TECHNICAL.md](memory/05-modules/citizenchain/runtime/transaction/offchain-transaction-pos/STEP1_TECHNICAL.md)：SfidAccountQuery trait 签名
...
链端 `duoqian-manage-pow` 里代表"SFID 机构下账户名称"的字段从早期叫 `name` / `SfidNameOf<T>` / `MaxSfidNameLength` / `EmptySfidName`，语义含糊（容易和机构名 `institution_name` 搞混）。SFID 后端 `model.rs:95` 的 `MultisigAccount.account_name` 已经用清晰命名，前端 `duoqian_create_proposal_page.dart:179` 也在用 `account.accountName`。本次把链端对齐。
...
1. 链端 duoqian-manage-pow/src/lib.rs（~40 处）
2. duoqian-manage-pow/src/benchmarks.rs（~3 处）
3. configs/mod.rs（~8 处）
4. duoqian-transfer-pow/src/lib.rs 测试（~3 处）
5. offchain-transaction-pos `SfidAccountQuery` trait（~3 处）
