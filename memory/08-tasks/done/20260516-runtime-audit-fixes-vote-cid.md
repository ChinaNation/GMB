# 任务卡：修复 runtime 审计发现的 CID 解绑、联合投票快照、终态清理和 50% 判定问题

- 任务编号：20260516-runtime-audit-fixes-vote-cid
- 状态：done
...
- `citizenchain/runtime/otherpallet/cid-system/`：修正 `unbind_cid` 事件字段和测试。
...
- 任务卡已创建
- `cid-system`：`CidUnbound` 移除不真实的 `admin` 字段，Root 解绑只记录 `who` 与 `binding_id`。
...
- 测试：已覆盖 CID 解绑事件、过期人口快照、极端 `u64` 阈值、终态清理失败入队与重试。

## 验证记录

- `cargo test --manifest-path citizenchain/runtime/Cargo.toml -p cid-system`
