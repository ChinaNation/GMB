...
- 联邦管理员采用同级模型,仅代码内置初始联邦管理员承担不可删除安全根职责。
- 代码内置初始联邦管理员不可删除,且不能删除自己。
- 新增联邦管理员可新增联邦管理员,可管理所属联邦/市管理员,但不能删除联邦管理员。
- 管理员不存在状态字段,前后端删除 `ACTIVE / DISABLED` 与停用/启用逻辑。
- 联邦管理员和市管理员表格操作只保留编辑、删除、更新密钥。
- 编辑只允许修改姓名。
- Passkey 拆成独立工具,只允许当前登录管理员更新自己的 Passkey。

## 固定约束

- 不保留联邦管理员层级描述、类型、接口和 UI。
- 不保留管理员状态字段和停用/启用动作。
...
- 2026-05-31:后端新增联邦管理员安全动作,市管理员编辑收敛为只改姓名。
- 2026-05-31:Passkey 更新改为当前管理员本人替换式生成,完成后只保留一个有效凭据。
- 2026-05-31:前端注册局联邦管理员页改为表格,市管理员表格删除状态列和启用/停用按钮,新增独立 `Passkey.tsx`。
...
- 2026-05-31:前端 Passkey 工具改名为 `admins/Passkey.tsx`,联邦管理员新增账户扫码入口改为图标按钮。
- 2026-05-31:注册局子 tab 改为“联邦管理员列表”,顶部管理员身份展示改为“角色 · 姓名”。
