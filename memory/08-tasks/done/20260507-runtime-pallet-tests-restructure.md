...
| cid-system | 33 | tests.rs 1014 → tests/ 1019 |
| onchain-transaction | 20 | 1448 → 453 |
| admins-change | 31 | 2240 → 1023 |
| duoqian-transfer | 20 | 2331 → 1016 |

**形态最终落地**:小型单 mod.rs(3 个),其余 12 个一律 `mod.rs(mock+helper) + cases.rs(用例)` 双文件,严格对齐 internal-vote 样板。citizen-issuance 根目录原有的 tests/integration_bind_cid.rs 保留不动(跨 pallet OnCidBound 集成测试,单测覆盖不到的契约)。
...
|---|---|
| `otherpallet/cid-system/src/tests.rs` | 1014 |
...
12. `otherpallet/cid-system`(1014,形态 B)
...
| otherpallet/cid-system | (tests.rs 中,需点数) |
...
| organization-manage | 22 用例(+2 frame 自动 = 24 passed) | 441 行(mock + 14 helper) | 716 行 | CID 登记×5 + 创建×8 + 关闭×5 + 边界×4 |
...
| `runtime/src/configs/mod.rs:1485-2553`(1067 行) | 18 | runtime 装配集成测试(CID 双层签名/admins/joint vote/population snapshot) | cases.rs 簇 2 |
