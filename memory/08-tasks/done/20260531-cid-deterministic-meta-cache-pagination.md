...
- 新增 `citizencode/frontend/citizencode/metaCache.ts`,统一承接省份元数据、城市列表和公安局确定性展示缓存。
- 公安局 Tab 改为本地 20 条分页,展示 `共 N 条 / 第 X 页`,上一页/下一页只切本地缓存切片。
- 注册局市列表、公权机构市列表、私权机构市列表、机构新增弹窗和通用元数据 hook 已改为读取缓存工具。
- 已更新 `memory/05-modules/citizencode/frontend/FRONTEND_LAYOUT.md` 的前端缓存与公安局分页规则。
