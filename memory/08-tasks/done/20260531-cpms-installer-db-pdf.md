...
- `cpms/deploy/linux/`：修复安装脚本中的数据库角色、数据库、schema、旧表 owner 权限处理，并安装 PDF 手册。
- `cpms/scripts/`：调整安装包打包脚本，删除正式安装 SQL payload，增加 PDF 打包。
- `cpms/docs/`：新增 CPMS 安装配置手册 Markdown 源文件和 PDF 文件。
- `.github/workflows/`：让手动 CI 产物包含安装手册 PDF，push / PR 仍只跑编译测试。
- `cpms/CPMS_TECHNICAL.md`：更新正式安装数据库初始化、证书信任和 PDF 手册说明。
- `memory/01-architecture/cpms/`、`memory/05-modules/cpms/`：同步模块文档，清理正式安装导入 `schema.sql` 的旧描述。
...
- 安装时停止旧 `cpms-backend`，避免重装过程中旧服务继续占用数据库或旧二进制。
- 安装生成证书后，CPMS 主机自动信任本机 Root CA；局域网客户端仍按 PDF 手册导入。
- 安装手册复制到 `/opt/cpms/docs/CPMS安装配置手册.pdf`，手动 CI artifact 也包含该 PDF。

## 验证

- `bash -n cpms/deploy/linux/install_host.sh cpms/scripts/build_linux_host_installer.sh`：通过。
- `cargo test --manifest-path cpms/backend/Cargo.toml`：通过，32 个测试全部通过。
