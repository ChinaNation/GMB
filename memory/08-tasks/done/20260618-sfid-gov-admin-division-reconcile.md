# 任务卡：SFID 公权机构随行政区变更自动对账

## 任务目标

行政区开发库 `sfid/backend/china/china.sqlite` 变更后，SFID 运行库中的确定性公权机构必须同步变化：
...
- `sfid/backend/gov/`：公权机构确定性目标目录、hash 校验、对账删除边界。
- `sfid/backend/main.rs`：`ensure-gov`、`serve` 启动守卫、维护命令日志。
- `sfid/backend/core/db.rs`：`gov.source` schema 收敛，区分 `GENERATED` 与 `MANUAL`。
- `sfid/deploy/prod/`：部署脚本强制执行 `reconcile-gov --changed-only` 和 `check-gov --strict`。
- `memory/`：更新 ADR、SFID 技术文档、部署文档和任务记录。
...
- 当前本机 SFID 运行库对账后不得残留 `HU/107`、`管理市` 或旧 `gov_manifest.china_hash`。
...
- 2026-06-18：`serve` 启动前校验全局 `gov_manifest`;生产目录过期直接拒绝启动,本地显式设置 `SFID_GOV_AUTO_RECONCILE=1` 才允许启动前自动对账。
...
- 2026-06-18：本机真实 SFID PostgreSQL 已完成运行库对账:
...
- 2026-06-18：真实启动验收:`SFID_BIND_ADDR=127.0.0.1:0 sfid-backend serve`
  启动通过,日志确认 `sfid gov directory manifest matches current china sqlite` 并进入监听。
- 2026-06-18：文档和注释已同步更新:
  - ADR-021 发布流程改为“对账 + strict 校验 + 导出公权机构包”
  - SFID 技术架构补充 `serve` 守卫、`gov.source` 边界和部署命令
...
- `cd sfid/backend && cargo test` 通过:65 passed。
- `python3 sfid/backend/china/check_code_immutable.py` 通过。
- `SFID_BIND_ADDR=127.0.0.1:0 ./target/debug/sfid-backend serve` 通过 manifest 守卫并进入监听,随后手动停止验收进程。
- `git diff --check` 通过。
