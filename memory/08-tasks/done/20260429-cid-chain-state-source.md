...
- CID 系统只负责机构身份和账户名称。
- `account_name` 保持 DUOQIAN 协议字段，不拆分、不改名。
- 机构创建后默认生成 `主账户`、`费用账户`。
- CID 后台不再手动激活账户，账户激活状态只能来自链上注册/注销同步。
...
- `citizencode/backend`
- `memory/05-modules/citizencode/backend/institutions`
- `memory/07-ai`
...
2. CID 状态模型
   ...
   - 创建 CID 机构时，机构为 `NotRegistered`，默认账户为 `NotOnChain`。
...
- 创建 CID 机构后默认账户均为未上链状态。
- CID 后台/API 不再暴露手动激活账户动作。
...
- 已更新 AI 编程系统强制规则。
- 已移除 CID 后台账户手动激活路由与前端激活按钮。
- 已将机构链上状态和账户链上状态改为链上同步真源模型。
- 已新增区块链软件机构搜索、详情、账户列表与链上状态同步接口。
- 已按 `account_name` 保持 DUOQIAN 协议语义,没有拆分字段。
- 已更新 CID 机构技术文档。
