# 任务卡：全面仔细的检查一遍 onchain-transaction-pow 这个模块有没有安全漏洞、有没有需要改进的地方、功能需求是否严格实现、中文注释技术文档是否完整、有没有要清理的残留
...
全面仔细的检查一遍 onchain-transaction-pow 这个模块有没有安全漏洞、有没有需要改进的地方、功能需求是否严格实现、中文注释技术文档是否完整、有没有要清理的残留
...
- 任务卡已创建
- 已检查 `citizenchain/runtime/transaction/onchain-transaction-pow/src/lib.rs`
- 已检查 `citizenchain/runtime/src/configs/mod.rs`
- 已检查 `citizenchain/runtime/transaction/onchain-transaction-pow/benches/transaction_fee_paths.rs`
- 已检查 `memory/05-modules/citizenchain/runtime/transaction/onchain-transaction-pow/ONCHAIN_TECHNICAL.md`
- 已检查 `citizenchain/node/src/rpc.rs`
- 已检查 `citizenchain/nodeui/backend/src/mining/mining-dashboard/mod.rs`
- 已完成 `cargo test -p onchain-transaction-pow`
- 已完成 `cargo check -p citizenchain`
- 已完成 `cargo bench -p onchain-transaction-pow --bench transaction_fee_paths --no-run`

## 审查结论

1. `transfer_all` 的收费口径偏大
   - `PowTxAmountExtractor` 对 `Balances::transfer_all` 直接按“当前可转全部余额”提取金额
   ...
   - 位置：
     ...
     - `citizenchain/runtime/transaction/onchain-transaction-pow/src/lib.rs:134`
...
2. `FeePaid` 事件不包含 tip，但外部链路把它当成“真实手续费”
   ...
   - 位置：
     - `citizenchain/runtime/transaction/onchain-transaction-pow/src/lib.rs:164`
...
3. 全节点手续费分成继承了 PoW 作者身份信任风险
   - `PowOnchainFeeRouter` 通过 `FindAuthor` + `RewardWalletByMiner` 把全节点份额打给作者绑定钱包
   - runtime 当前的 `PowDigestAuthor` 只是从 pre-runtime digest 解码 `AccountId`
   - 这意味着本模块的全节点手续费分成继承了 `fullnode-pow-reward` 同一条作者身份信任边界
   - 位置：
     - `citizenchain/runtime/transaction/onchain-transaction-pow/src/lib.rs:265`
...
4. 技术文档对事件口径不完整
   ...
   - 位置：
     - `memory/05-modules/citizenchain/runtime/transaction/onchain-transaction-pow/ONCHAIN_TECHNICAL.md`
...
- `cargo check -p citizenchain`
  - 结果：通过
- `cargo bench -p onchain-transaction-pow --bench transaction_fee_paths --no-run`
