...
- `runtime/src/configs/mod.rs`:
  - 4 个签名 payload domain tag 统一为 `b"GMB_SFID_V1"`（2026-04-20 统一为 `DUOQIAN + OP_SIGN_*`）(代码里的 verifier 共享常量,
...
- `submit_register_sfid_institution_extrinsic`:
  ...
  - 签名 payload 追加 `province` 字段,使用 `b"GMB_SFID_V1"`（2026-04-20 统一为 `DUOQIAN + OP_SIGN_*`） domain
