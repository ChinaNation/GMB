...
- memory/01-architecture/citizenpassport/README.md
...
3. 技术文档不完整
   - `memory/05-modules/citizenpassport/` 目前只有 `initialize/login/dangan` 三份模块技术文档
   - `super_admin`、`operator_admin`、`authz` 没有独立技术文档
   - `CPMS_TECHNICAL.md` 的模块索引直接指向源码文件，不是模块文档

4. 部署与清理还有残留
   - 安装脚本仍写入 `CPMS_INSTALL_FILE=/var/lib/citizenpassport/runtime/cpms_install_init.json`
...
- 敏感数据加密要求：`memory/01-architecture/citizenpassport/README.md`
- 敏感档案明文入库：`citizenpassport/backend/operator_admin/mod.rs`
- 二维码签名私钥明文入库：`citizenpassport/backend/initialize/mod.rs`、`citizenpassport/backend/db/schema.sql`
- 登录二维码复用 `qr_sign_keys`：`citizenpassport/backend/login/mod.rs`
- 模块文档缺失：`memory/01-architecture/citizenpassport/CPMS_TECHNICAL.md`、`memory/05-modules/citizenpassport/`
- 安装残留变量：`citizenpassport/deploy/linux/install_host.sh`
