...
- 已确认手机端不是“没网”，而是“连到了节点传输层，但链定义不一致”：
  - 真机日志存在稳定 `pong`，`prczss.wuminapp.com` 与 `nrcgch.wuminapp.com` 的 `ping_time` 约 `41ms` 至 `89ms`
...
- 额外发现：bootnode 列表里大量 `prc*.wuminapp.com` 子域名在真机上会报 `failed to lookup address information: No address associated with hostname`，说明 bootnode 名单里还夹杂了大量当前不可解析节点，进一步压缩了可用 peer 范围
- 已确认根因集中在 App 侧：
