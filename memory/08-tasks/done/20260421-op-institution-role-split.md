...
- **验证**：10 个 pallet `cargo check` 全通过；`cargo test -p primitives` 7/7 通过；全仓库活代码 `rg derive_duoqian_address_from_sfid_number` 零残留

# 最终账户派生公式对齐

| 账户类型 | op_tag | preimage |
|---|---|---|
| 宪法机构主账户（NRC/PRC/PRB） | `OP_MAIN = 0x00` | `ss58 \|\| sfid_number` |
| SFID 机构主账户（清算行/公安局/其他） | **`OP_MAIN = 0x00`** | **`ss58 \|\| sfid_number`**（与宪法公式完全一致，仅 sfid_number 命名空间不同） |
| 宪法机构费用账户 | `OP_FEE = 0x01` | `ss58 \|\| sfid_number` |
| SFID 机构费用账户 | **`OP_FEE = 0x01`** | **`ss58 \|\| sfid_number`**（与宪法公式完全一致） |
| 省储行质押账户（仅 PRB） | `OP_STAKE = 0x02` | `ss58 \|\| citizens_number_u64_le` |
| 国储会安全基金（仅 NRC） | `OP_AN = 0x03` | `ss58 \|\| NRC_sfid_number` |
| 个人多签 | `OP_PERSONAL = 0x04` | `ss58 \|\| creator_32 \|\| name` |
| **SFID 机构自定义命名账户**（临时/工资/运营/备用金...） | **`OP_INSTITUTION = 0x05`** | **`ss58 \|\| sfid_number \|\| name`** |

每个 op_tag 单一派生公式，无"if name empty"分支。保留名铁律：`"主账户"` / `"费用账户"` 链端强制走 `Role::Main` / `Role::Fee`，不能作为 `Role::Named` 参数（返回 `ReservedAccountName` 错误）。

# 背景

统一 DUOQIAN 方案落地后，仍有一处语义不一致：`OP_MAIN = 0x00` 同时背负两个职责：
- name 空 → 宪法机构主账户（input = sfid_number）
- name 非空 → SFID 登记机构的任意命名账户（input = sfid_number + name）
...
|---|---|---|---|
| OP_MAIN | 0x00 | 所有机构主账户（宪法 + SFID） | ss58 ‖ sfid_number |
| OP_FEE | 0x01 | 所有机构费用账户（宪法 + SFID） | ss58 ‖ sfid_number |
| OP_STAKE | 0x02 | 仅 PRB 质押账户 | ss58 ‖ citizens_number_u64_le |
| OP_AN | 0x03 | 仅 NRC 安全基金 | ss58 ‖ NRC_sfid_number |
| OP_PERSONAL | 0x04 | 个人多签（creator + name） | ss58 ‖ creator_32 ‖ name |
| **OP_INSTITUTION** | **0x05** | **SFID 机构自定义命名账户** | **ss58 ‖ sfid_number ‖ name** |
