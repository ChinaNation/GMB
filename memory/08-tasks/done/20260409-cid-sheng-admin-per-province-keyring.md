...
当前状态:
- 链端 `cid_system` pallet 的 verifier 只信任这一个账户
...
- `feedback_cid_pow_chain_recipe.md`:显式 nonce + immortal + InBestBlock
...
│  │  Verifier(register_cid_institution 等):              │  │
...
**文件**:`citizenchain/frame/cid-system/src/lib.rs`
...
找到现有 `register_cid_institution` 等 extrinsic 中校验 `origin` 的地方,扩展为:

```rust
fn is_authorized_cid_writer<T: Config>(who: &T::AccountId) -> bool {
...
- 新 signer 推 `register_cid_institution` 成功

---

### 阶段 2 — citizencode-backend(1 天)

**2.1 Store 新增字段**(`citizencode/backend/models/mod.rs`):
...
当前 `federal-registry/institutions.rs::submit_register_cid_institution_extrinsic` 里:
```rust
```
...
后端环境变量 `CID_SHENG_SIGNER_ENABLED=true|false`(默认 false,灰度期间)
- `true`:按角色路由
...
**4.3 监控**:
- 新增 Prometheus 指标:
  - `cid_chain_submit_by_role{role}` counter
  - `cid_chain_submit_by_province{province}` counter
  - `cid_sheng_signer_cache_hit` counter
- Grafana 面板:每省推链 QPS + fallback 率

**4.4 回滚预案**:
- 任何异常:`CID_SHENG_SIGNER_ENABLED=false` 立即回退
...
4. ✅ **压测工具**:自写 Rust 脚本(`citizencode/backend/tools/load_test.rs` 或独立 crate)
...
```rust
// citizencode/backend/models/mod.rs - 扩展现有 AdminUser
...
- 参考铁律:
  ...
  - `feedback_cid_pow_chain_recipe.md`
