# 任务卡：全面仔细检查一遍 shengbank-interest 模块有没有安全漏洞、有没有需要改进的地方、功能需求是否严格实现、中文注释技术文档是否完整、有没有要清理的残留
...
全面仔细检查一遍 shengbank-interest 模块有没有安全漏洞、有没有需要改进的地方、功能需求是否严格实现、中文注释技术文档是否完整、有没有要清理的残留
...
- 已执行 `cargo test -p shengbank-interest`，结果通过
- 已执行 `cargo test -p shengbank-interest --features runtime-benchmarks`，结果失败，失败原因为 benchmark test suite 引用了私有 `new_test_ext()`
...
2. `runtime-benchmarks` 变体编译失败。`benchmarks.rs` 使用 `impl_benchmark_test_suite!(..., crate::tests::new_test_ext(), ...)`，但 `new_test_ext()` 在测试模块里是私有函数，导致 `cargo test -p shengbank-interest --features runtime-benchmarks` 直接失败。
...
- 完成摘要：完成 shengbank-interest 模块审查：确认 1 个高优先级年度周期偏差、1 个 benchmark 编译问题、1 组失败分支测试缺口，以及 1 个上下文装载/文档登记缺口；普通单元测试通过，未发现直接权限绕过或任意收款漏洞。
