# 任务卡:cid 前端 App.tsx 彻底拆分

- **任务 ID**: 20260408-cid-frontend-app-tsx-split
- **模块**: cid-frontend
- **优先级**: 中(技术债,阻塞后续功能迭代效率)
- **前置依赖**: `20260408-cid-public-security-cpms-embed`(CPMS 流程搬走之后再拆,减少合并冲突)
- **状态**: 待启动

## 背景

任务卡 3 的前端模块化只完成了 20% —— 只把"私权/公权/公安局"三个 tab 的内容抽到了 `views/institutions/`,其他功能(登录、注册局、业务员绑定、操作员管理、密钥轮换、Dashboard)原封不动留在 `citizencode/frontend/App.tsx`,导致该文件长期维持在 **3769 行**。每次动这个文件都战战兢兢,极易误伤不相关模块。
...
- `citizencode/frontend/App.tsx` ≤ 300 行
...
- 回写 `feedback_cid_frontend_modular_structure.md` 记录目录约定:**以后所有新 cid 前端功能必须放到 `views/<module>/`,禁止直接写进 App.tsx**
...
- 删除未用 import:`CidCityItem`(仅被 dead helper 引用)、未使用的 `loginBg` 常量
- 合并 Tab onClick 里三份重复的 `getCidMeta` 逻辑到单个 `loadCidMetaForInstitutions` helper
- 新增顶部中文文件头注释,明确 App.tsx 的路由壳子职责与"严禁新增业务代码"铁律
- 新增 `ActiveView` 类型别名替代行内长 union

### 新增文档/feedback
- `citizencode/frontend/<feature>/README.md`(追加铁律段)
- `memory/feedback_cid_frontend_modular_structure.md`(新建)
