...
- `primitives/src/derive.rs`:
  ...
  - **删除**:`account_to_institution_id` / `sfid_number_to_institution_id` 函数定义
  - 新增 ≥6 个协议正确性测试 case
...
- 任何调用 `account_to_institution_id` / `sfid_number_to_institution_id` / `sfid_number_to_fixed48` 的位置,grep 替换为新名
...
### 2.7 测试 mock
- `duoqian-transfer/src/lib.rs` 测试 mock 内 `MaxSfidNumberLength` 同步 `ConstU32<47>`(若有引用)
...
```bash
# 旧函数名零残留
grep -rn "account_to_institution_id\|sfid_number_to_institution_id\|sfid_number_to_fixed48" \
...
# MaxSfidNumberLength=96 不再出现
grep -rn "MaxSfidNumberLength.*ConstU32<96>" citizenchain/ --include="*.rs" 2>/dev/null
```
...
| ID | 风险 | 缓解 |
|---|---|---|
| R1 | sfid_number 长度 47B 约束 | MaxSfidNumberLength=47 强制守门;BoundedVec 入链时校验失败拒绝 |
...
| R5 | china_cb/china_ch 创世构建时 institution_id 字节内容变化(头加 0x01) | runtime 创世逻辑全部经 institution_id_from_sfid_number;创世测试自动覆盖 |
...
### 残留扫描(全零)
1. `account_to_institution_id` / `sfid_number_to_institution_id` / `sfid_number_to_fixed48`:零残留
2. `MaxSfidNumberLength.*ConstU32<96>`:零残留
...
### 风险闭环
- R1 sfid_number 长度收紧 47B:MaxSfidNumberLength 强制守门 ✓
...
- R5 china_cb/china_ch 创世 institution_id 字节内容变化:全部经 institution_id_from_sfid_number 派生,创世测试自动覆盖 ✓
- R6 nrc_pallet_id_bytes 等下游硬编码:Step 5 切换 + Step 7 测试守门 ✓
