...
- D1：square_memberships、square_uploads、square_posts、square_follows(owner=A)、square_user_signals(owner=A)、chat_devices、chat_keypackages、chat_device_binding_nonces、chat_turn_credentials、square_browse_days、square_media_assets(owner=A)、square_login_challenges。
...
- Chat 附件边界：只删 A 的、保留 B 未 ack 引用的附件。
...
- **编排服务**（新 [square_account_deletion_service.dart](../../citizenapp/lib/8964/services/square_account_deletion_service.dart)）：先服务端硬删(失败即抛、绝不清本地)→成功后清本地(资料缓存/会话/Chat私信历史/原生设备子钥)。
- **Chat 本地清**：[chat_store.dart](../../citizenapp/lib/chat/storage/chat_store.dart) 加 `clearAllForOwner`(清会话/消息/出入站队列;路由缓存非owner归属不清)。
...
- **app**：`signer/signing.dart` +3 op_tag 常量 + `scaleString`/`u64Le`(chat_device_binding 去重复用)；`device_subkey.dart` buildDeviceBindingPayload→`buildDeviceBindingSigningMessage`(digest)；`SquareLoginSigner`/`DeviceBindingSigner`/`ChatSquareLoginPayloadSigner` 改收 `Uint8List`，op_tag+hex 解码集中在 `_establishSession`/registrar(客户端钉死 op_tag,不采信服务端)；signer 闭包只签字节；**删** `signUtf8WithWallet`/`WalletSignResult` 死码。金标 fixture 同步 + 新增 `device_binding_golden_test.dart`(0x1C 字段编码 App↔Worker 同 hex `e9e25d…`)。`analyze` 净(2 处 pre-existing info) + signer26/im+publish11/wallet+api24 例绿。
...
- `cloudflare/src/account/purge.ts`（新）`purgeAccount(env, owner)` 顺序：
  ...
  5. D1 批删所有 owner=A 引用：memberships/uploads/posts/user_signals/media_assets/follows/chat_devices/chat_keypackages/chat_device_binding_nonces/chat_turn_credentials/square_browse_days/device_subkeys/login_challenges；
