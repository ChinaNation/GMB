...
- 清理当前 CID 代码、注释、文档、任务索引里的旧角色残留。
- 将旧 ADR、旧任务卡文件名和正文改为当前联邦管理员三槽自治表述。
- 数据库迁移改为当前二角色基线，不再保留旧对象创建或刷新逻辑。

## 影响范围

- `citizencode/`
- `citizenchain/runtime/otherpallet/cid-system/`
- `memory/04-decisions/`
...
- 已执行 `cargo fmt`、`cargo check --manifest-path citizencode/backend/Cargo.toml`、
