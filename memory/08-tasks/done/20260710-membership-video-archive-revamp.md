...
1. **保留四档会员**（freedom/democracy/voting/candidate），身份档继续是 visitor/voting/candidate；荣誉/紫色档作废。
2. **移除「账户总储存上限」维度**（四档均不设，删 D1 两列）。对齐 YouTube/推特（储存不作限制维度）。
3. **视频时长/体积按档（Plan B·1080p）**：长度沿用现值，体积新设。
   - 自由：**60s / 512 MB / 标清**
   - 民主 / 投票：**30min(1800s) / 2 GB / 高清(1080p)**
...
- `MembershipLevel` 保持四档（不加 honor）。
- `DynamicQuota` **新增 `max_video_bytes: number`**。
- 四档数值（Plan B）：
  - freedom：`max_video_seconds 60`、`max_video_bytes 512*mib`、`video_quality 'sd'`
  - democracy / voting：`1800`、`2*gib`、`'hd'`
  - candidate：`10800`、`10*gib`、`'hd'`（时长/清晰度=现值不变，仅确认体积）
- **删 `legacy_storage_quota_bytes`**（interface + 四档），及唯一读点 `service.ts:233`。
...
## 9. Stripe / 环境
- 四档价格与 Price ID 不变（无新增 Stripe 价格）。
...
- `index.ts`：加 `scheduled()`（waitUntil 跑 sweep）；`wrangler.toml`：三环境 `[triggers] crons=["0 3 * * *"]` + `ARCHIVE_ENABLED` / `ARCHIVE_LAPSE_DAYS="90"`；`types.ts Env` 加两项。
...
- 完成摘要：完成四档视频额度、退订满 90 天冷归档、production 开关启用与远端定时扫描验收
