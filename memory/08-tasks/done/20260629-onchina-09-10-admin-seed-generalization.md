...
- 状态:**完成(done,2026-06-29)**。P0-P6 实现 + R4 对抗审计(29 agent)确认无回归。
  ...
  - citizenapp follow-up:`kFixedGovernanceInstitutions` 的 FRG 单 mainAccount 条目读空,需按 province_code 读省组。
...
## 创世机构新模型(已核实,链上真相)
- **5 固定治理码** `is_fixed_governance_code` = NRC/PRC/PRB/FRG/**NJD**(全 `kind=PublicInstitution`,`source=Genesis`)。阈值 NRC13/PRC6/PRB6/FRG3/NJD8。
- **NRC/PRC/PRB/NJD** → `PublicAdmins::AdminAccounts`(键=机构主账户)。NJD 新增(china_sf `NATIONAL_JUDICIAL_YUAN_ADMINS` 13 人,带 `admin_role` 护宪/首席/次席/大法官)。
...
- **修复**:① 新增镜像读 `PublicAdmins::FederalRegistryProvinceGroups[province_code]`(value 同 `OnChainAdminAccount`);② `node_institution_identity` 为 FRG 解析 province_code(由 `ONCHAIN_CREDENTIAL_SCOPE_PROVINCE_NAME` → `primitives::cid::code::PROVINCE_CODE_INFOS` 映射);③ `fetch_active_admins_onchain` 对 FRG 分流到省组读,非 FRG 仍读 AdminAccounts。
- **关联**:citizenapp `kFixedGovernanceInstitutions` 的 FRG 单 mainAccount 条目同样读空 → 列 citizenapp 侧 follow-up(不在本卡)。
...
- **不碰链端**:china_zf/china_sf/固定治理链端/cid code.rs 冻结不动;onchina + citizenapp(本卡仅 onchina,citizenapp follow-up)。
