# 任务卡：机构全称/简称单一字段 = cid_full_name / cid_short_name + 两 UI 显示简称 + 清理第二源
...
机构只有两个名字字段:全称 `cid_full_name`、简称 `cid_short_name`。系统里却"到处再造机构全称/简称真源",
导致同一个"联邦注册局"被多处硬编码/派生制造。要求:① 单一真源就是这两个字段;② 管理员列表页头部 +
右上角徽标都显示**简称**;③ 清理所有再造机构全称/简称的第二源头。联邦注册局:全称=`总统府联邦注册局`,简称=`联邦注册局`。
...
3. `admins/login/model.rs`：`AdminAuthContext`/`AdminAuthOutput`/`AdminIdentifyOutput` 曾加旧登录简称字段；当前任务已统一为 `cid_short_name`。
4. `admins/repo.rs`：曾新增旧登录简称解析函数；当前任务已统一为 `resolve_home_cid_short_name_conn`(+ `&Db` 非 conn 包装),按 registry_org_code 查 subjects.cid_short_name(联邦=org_code='FEDERAL_REGISTRY' 单行;市=org_code='CITY_REGISTRY' AND province_name AND city_name)。
...
7. 徽标(place 2)`App.tsx resolveHeaderAdminIdentity`：删 registry_org_code 三元硬编码,当前读 `auth.cid_short_name`(右段仍 admin_display_name)。`auth/api.ts`(AdminAuthCheck/AdminIdentifyResult)+`auth/types.ts`(TokenAdminAuth)同步目标字段;`App.tsx` bootstrap 合并 + `LoginView.tsx` 两处 nextAuth 透传。
...
- 数据流终态：place 1 头部读 `federalRegistryDetail.institution.cid_short_name`(联邦注册局 · 中枢省)；place 2 徽标读 `auth.cid_short_name`(联邦=联邦注册局 / 市={市}注册局,右段仍 admin_display_name,简称空时整段隐藏不显伪造名)。
