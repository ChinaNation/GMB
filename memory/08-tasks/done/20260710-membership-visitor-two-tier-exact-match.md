...
- 状态：**已上线（2026-07-11）**；worker 生产部署（版本 `c2959b09`）+ 四档实测通过 + Stripe 4 价 + 官网 www.crcfrcn.com
...
## 9. 上线记录（2026-07-11，已完成）

- Stripe LIVE 建 4 个月度 price，写入 `[env.production.vars]`（price ID 非密钥，可入库）：
  - `STRIPE_PRICE_VISITOR = price_1TrrMkHSzSYWD2rFsabKgbxa`（自由 $2.99）
