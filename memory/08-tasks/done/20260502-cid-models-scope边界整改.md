...
- 公民 DTO 迁入 `citizens/model.rs`,CPMS DTO 迁入 `citizenpassport/model.rs`,CID 元信息 DTO 迁入 `citizencode/model.rs`。
- 机构链状态迁入 `institutions/model.rs`。
- `scope` 收敛为 `mod.rs / rules.rs / filter.rs / admin_province.rs`。
- 审计查询迁入 `audit.rs`,公民查询迁入 `citizens/handler.rs`,CPMS scope 谓词迁入 `citizenpassport/scope.rs`,pubkey 工具迁入 `crypto/pubkey.rs`。
- 已更新 backend layout、models、scope、citizens、cpms、cid、audit、crypto 文档。
