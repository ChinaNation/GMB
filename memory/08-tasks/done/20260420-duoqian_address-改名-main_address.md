...
为消除歧义，将宪法机构常量及其直接读取方的 `duoqian_address` 统一改名为 `main_address`。注册多签概念（duoqian-manage / SFID 注册机构 / wuminapp 注册机构）保持 `duoqian_address` 不动。
...
- [ ] `runtime/src/configs/mod.rs`（选择性：保留 duoqian_manage pallet call 参数 `duoqian_address`；rename `n.duoqian_address` + `is_reserved_duoqian_account` 包装 + 注释中的 const 名）
