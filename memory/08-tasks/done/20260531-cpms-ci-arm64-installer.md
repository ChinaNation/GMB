...
- `cpms-ubuntu24-amd64.run`
- `cpms-ubuntu24-arm64.run`
...
- `.github/workflows/cpms-ci.yml`：增加 amd64 / arm64 matrix；手动运行才打包并上传正式安装包。
- `cpms/scripts/build_linux_host_installer.sh`：增加 `--arch amd64|arm64`，按架构输出不同 `.run` 和 `.sha256`。
- `cpms/deploy/linux/install_host.sh`：读取包内 `payload/manifest.env`，按包架构校验目标机。
- `cpms/CPMS_TECHNICAL.md` 与 CPMS 模块文档：更新双架构正式安装规则。
...
- `bash -n cpms/scripts/build_linux_host_installer.sh cpms/deploy/linux/install_host.sh`
- `git diff --check`
- `npm run build`
