# 任务卡：新增 institution-asset-guard 公共模块，并接入机构账户资金操作白名单边界
...
新增 institution-asset-guard 公共模块，并接入机构账户资金操作白名单边界
...
- 任务卡已创建
- 已新增 `citizenchain/runtime/transaction/institution-asset-guard` 公共 crate
- 已将 `duoqian-manage-pow` 接入关闭流程资金白名单检查
...
- 已更新现有交易模块文档与目录说明
- 验证通过：
  - `cargo test -p institution-asset-guard`
  - `cargo test -p duoqian-manage-pow`
  - `cargo test -p duoqian-transfer-pow`
  - `cargo test -p offchain-transaction-pos`
