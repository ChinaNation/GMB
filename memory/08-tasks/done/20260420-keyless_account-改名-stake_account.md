---
title: 省储行质押地址字段改名 keyless_address → stake_account
status: done
...
- [primitives/china/china_ch.rs](citizenchain/runtime/primitives/china/china_ch.rs) 44 处 `keyless_address → stake_account`
...
| 角色 | 字段名 | 适用机构 |
|---|---|---|
| 主账户 | `main_account` | 所有多签机构 |
| 费用账户 | `fee_account` | 所有多签机构 |
| 质押账户 | `stake_account` | 仅省储行 |
...
`primitives/china/china_ch.rs` 的 `keyless_address` 字段描述的是**机制**（无私钥），但在新的机构账户三元模型下，其**角色**是质押账户。与 `duoqian_account → main_account` 对齐，把 `keyless_address` 统一改名为 `stake_account`，使省储行三个账户语义清晰：

- 主账户 `main_account`（已完成）
- 费用账户 `fee_account`（原名保留）
- 质押账户 `stake_account`（本次改名）
