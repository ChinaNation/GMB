...
- **任务 ID**: `20260410-cid-store-shard-by-province`
- **版本**: v1 完整实施版
- **日期**: 2026-04-09
- **工作量**: 4 天(Day1 结构 + Day2 PG 后端 + Day3 handler 改造 + Day4 文档收官)
- **前置**: Phase 1 `cid-sheng-admin-per-province-keyring` 已完成
- **架构上下文**: `memory/01-architecture/citizencode/CID_50K_CONCURRENT_FRAMEWORK.md`
- **状态**: **已完成** (2026-04-09)
...
| 文件 | 说明 |
|---|---|
| `citizencode/backend/store_shards/mod.rs` | 模块入口,StoreShard / GlobalShard / ShardedStore / ShardBackend trait 定义 + 8 单测 |
| `citizencode/backend/store_shards/shard_types.rs` | StoreShard / GlobalShard 类型定义与序列化 |
| `citizencode/backend/store_shards/backend.rs` | ShardBackend trait + ShardedStore 核心实现(DashMap 分片 + read/write/for_each_province) |
| `citizencode/backend/store_shards/pg_backend.rs` | PostgresShardBackend 实现(store_shards 表 CRUD + 乐观并发 version) |
| `citizencode/backend/store_shards/migration.rs` | 启动时幂等迁移:从 runtime_cache_entries 拆分灌入 store_shards 表 |

### 改动文件(7 个)

| 文件 | 改动内容 |
|---|---|
| `citizencode/backend/main.rs` | AppState 接入 ShardedStore,启动时初始化 + 迁移 + 预加载 |
| `citizencode/backend/Cargo.toml` | 新增依赖(dashmap 等) |
| `citizencode/backend/institutions/handler.rs` | cpms_site_keys 读写迁移到 sharded_store |
| `citizencode/backend/sheng_admins/institutions.rs` | multisig_institutions + multisig_accounts 读写迁移到 sharded_store |
| `citizencode/backend/scope/rules.rs` | 状态查询迁移到 sharded_store |
| `citizencode/backend/app_core/runtime_ops.rs` | 启动路径保留 legacy store(迁移 + backfill + orphan cleanup) |
| `citizencode/backend/main_tests.rs` | store_shards 8 个单测 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `CID_SHARD_SINGLE_WRITE` | false | 双写过渡期,设 true 切单写 |
| `CID_SHARD_PRELOAD_ALL` | true | 设 false 跳过启动预加载(全部懒加载) |
| `CID_SHARD_MIGRATION_SKIP` | false | 设 true 跳过迁移(紧急情况) |
...
`citizencode/backend/models/mod.rs::BackendStore` 是一个 single big struct,包含:
- 43 省所有 citizen 记录
- 43 省所有机构/账户
- 全局 admin_users_by_pubkey(KEY + SHENG + SHI 混合)
- 全局 sessions / challenges / audit_log
- CID 工具状态(cid_meta / cities / 等)
...
AppState.store: Arc<RwLock<BackendStore>>
    ↕ serde_json
runtime_cache_entries(key='cid_store', payload JSONB)  ← 1.2MB 大 JSON
                                                        ← 每请求 load 一次
...
3. **全量 persist**:写入时 DELETE `runtime_cache_entries` WHERE key='cid_store' + INSERT 全量 JSON
...
pub(crate) struct StoreShard {
    ...
    // ── 本省 CID 生成历史 ──
    pub(crate) generated_cid_by_pubkey: HashMap<String, String>,
...
过渡期 `CID_SHARD_SINGLE_WRITE=false`(默认),完全切换后设置为 `true`,跳过 legacy 同步。
...
let institution = store.multisig_institutions.get(&cid_number).cloned();
```
...
let institution = state.store_shards.read_province(province, |shard| {
    shard.multisig_institutions.get(&cid_number).cloned()
}).await?;
```
...
store.multisig_institutions.insert(cid_number.clone(), inst);
drop(store);
...
state.store_shards.write_province(province, |shard| {
    shard.multisig_institutions.insert(cid_number.clone(), inst);
}).await?;
...
pub(crate) async fn migrate_legacy_store_if_needed(
    ...
    for (cid_number, inst) in &legacy_store.multisig_institutions {
        ...
        shard.multisig_institutions.insert(cid_number.clone(), inst.clone());
    }

    // 1.2 按 cid_number 关联 multisig_accounts
    for (key, acc) in &legacy_store.multisig_accounts {
        // key 格式 "cid_number|account_name"
        if let Some(cid_number) = key.split('|').next() {
            if let Some(inst) = legacy_store.multisig_institutions.get(cid_number) {
...
fn derive_province_from_citizen(record: &CitizenRecord) -> String {
    ...
    if let Some(archive_no) = &record.archive_no {
        if let Some(code) = archive_no.get(..2) {
            return crate::cid::province::province_name_from_code(code)
...
let double_write = std::env::var("CID_SHARD_SINGLE_WRITE")
    .map(|v| v != "true")
    .unwrap_or(true);  // 默认双写
...
2. 运行 baseline 对比:
   ...
   - Phase 2 状态(`CID_SHARD_ENABLED=true`,走新分片):50/100/200 并发 × 1 省
   - Phase 2:10 省 × 50 并发 = 500 并发
3. 验证目标:
   - 单省 100 并发 P99 < 100ms(比 Phase 1 降 >50%)
   - 10 省 500 并发 P99 < 200ms
4. 更新运维文档:
   - 环境变量 `CID_SHARD_SINGLE_WRITE`(默认 false 双写,过渡 2 周后切 true)
   - 启动迁移日志格式
   - 回滚步骤(环境变量切回 + 重启老二进制)
5. 创建 `feedback_cid_store_sharded.md` 铁律
6. 任务卡归档 `done/`
7. 更新 `MEMORY.md` 索引
...
| `submit_register_cid_institution_extrinsic` | 从 `write_province(province, |s| s.multisig_institutions / accounts)` 读写,签名 signer 继续从 sheng_signer_cache 按 province 取 |

### 9.2 不影响的 Phase 1 模块

- 链端(完全不动)
- sheng_signer_cache / signer_router / chain_sheng_signing(独立于分片)
- 4 个 payload DOMAIN 常量(`GMB_CID_V1`，2026-04-20 彻底退役为 `DUOQIAN + OP_SIGN_*`)
...
```bash
# 1. 停老 citizencode-backend
systemctl stop citizencode-backend

# 2. 备份 PG
pg_dump -t runtime_cache_entries -t admins -t operators ... > backup.sql

# 3. 部署新二进制(含 store_shards 模块)
cp target/release/citizencode-backend /usr/local/bin/
# 默认 CID_SHARD_SINGLE_WRITE 未设置 → 双写开启

# 4. 启动
systemctl start citizencode-backend

# 5. 观察日志
journalctl -u citizencode-backend -f
# 期待看到:
...
# 7. 压测:./cid-load-test --concurrency 100 --provinces 1 --duration 60s
```
...
```bash
# 修改 env
echo "CID_SHARD_SINGLE_WRITE=true" >> /etc/citizencode-backend.env
...
cp /backup/citizencode-backend-phase1 /usr/local/bin/citizencode-backend
...
**不能回滚的场景**:如果已经切到 `CID_SHARD_SINGLE_WRITE=true` 超过 24 小时,`runtime_cache_entries` 过期 → 无法回滚。这种情况下只能前滚修复。
...
- [ ] 注册机构、替换 sheng admin、CID MAIN 轮换、CPMS 激活 — 手工各走一遍
...
- Phase 1 任务卡(完成):`memory/08-tasks/done/20260409-cid-sheng-admin-per-province-keyring-impl.md`
- 架构框架:`memory/01-architecture/citizencode/CID_50K_CONCURRENT_FRAMEWORK.md`
- 铁律:
  - `feedback_cid_sheng_signing_keyring.md`(Phase 1 产出)
  - `feedback_scope_auto_filter.md`(省级 scope 过滤)
  - `feedback_cid_module_is_single_entry.md`
  - `feedback_cid_three_roles_naming.md`
...
cd /Users/rhett/GMB/citizencode/backend
...
| 变量 | 默认 | 作用 |
|---|---|---|
| `CID_SHARD_SINGLE_WRITE` | false(双写)| true 时跳过 legacy 同步,只写新分片 |
| `CID_SHARD_PRELOAD_ALL` | true | false 时跳过启动预加载,全部走懒加载 |
| `CID_SHARD_MIGRATION_SKIP` | false | true 时跳过迁移(紧急情况,不推荐) |
