# 任务索引

## open
- `memory/08-tasks/open/20260507-173627-账户级内部投票管理员模型第0步设计-所有可操作账户使用内部投票-质押账户永不可操作-治理机构账户共.md`
- `memory/08-tasks/open/20260507-201558-第4步改造-organization-manage-机构账户级管理员主体.md`
- `memory/08-tasks/open/20260507-onchain-issuance-plain-ft.md`
- `memory/08-tasks/open/20260508-clean-run-fresh-genesis.md`
- `memory/08-tasks/open/20260509-admins-change-node-citizenapp.md`
- `memory/08-tasks/open/20260509-frontend-home-mining-flatten.md`
- `memory/08-tasks/open/20260509-node-transaction-duoqian-transfer.md`
- `memory/08-tasks/open/20260509-node-transaction-offchain-transaction.md`
- `memory/08-tasks/open/20260509-node-transaction-onchain-transaction.md`
- `memory/08-tasks/open/20260509-org-manage-split.md`
- `memory/08-tasks/open/20260510-runtime-upgrade-step1-runtime.md`
- `memory/08-tasks/open/20260510-runtime-upgrade-step2-node-runtime-upgrade-fix.md`
- `memory/08-tasks/open/20260510-runtime-upgrade-step3-citizenapp-citizenwallet-fix.md`
- `memory/08-tasks/open/20260511-runtime-fee-model-five-kinds.md`
- `memory/08-tasks/open/20260511-citizenapp-transfer-proposal-success.md`
- `memory/08-tasks/open/20260516-cpms-cid-two-code.md`
- `memory/08-tasks/open/20260516-cid-cpms-install-archive.md`
- `memory/08-tasks/open/20260517-citizenchain-ci-manual-release-desktop-update.md`
- `memory/08-tasks/open/20260517-node-constitution-csp.md`
- `memory/08-tasks/open/20260517-node-nsis-installer-icon.md`
- `memory/08-tasks/open/20260517-node-sync-guard.md`
- `memory/08-tasks/open/20260517-citizenapp-duoqian-create-confirm.md`
- `memory/08-tasks/open/20260517-citizenapp-permissions.md`
- `memory/08-tasks/open/20260517-citizenapp-square-contact-address.md`
- `memory/08-tasks/open/20260517-citizenapp-vote-chain-truth.md`
- `memory/08-tasks/open/20260520-citizenapp-wallet-tx-history.md`
- `memory/08-tasks/open/20260521-finalized-amount-display.md`
- `memory/08-tasks/open/20260522-citizenchain-data-dir-isolation.md`
- `memory/08-tasks/open/20260524-myid-identity-status-display.md`
- `memory/08-tasks/open/20260524-myid-validity-cid-citizenapp.md`
- `memory/08-tasks/open/20260524-network-cards-cleanup.md`
- `memory/08-tasks/open/20260524-cid-archive-code-ui.md`
- `memory/08-tasks/open/20260524-citizenapp-cid-two-paths.md`
- `memory/08-tasks/open/20260528-myid-bind-cleanup.md`
- `memory/08-tasks/open/20260530-cpms-admin-archive-simple-fixes.md`
- `memory/08-tasks/open/20260530-cpms-admin-wallet-step2.md`
- `memory/08-tasks/open/20260530-cpms-annual-export-rule.md`
- `memory/08-tasks/open/20260530-cpms-archive-material-library.md`
- `memory/08-tasks/open/20260530-cpms-db-schema-hardening.md`
- `memory/08-tasks/open/20260530-cpms-frontend-structure.md`
- `memory/08-tasks/open/20260530-cpms-p26-number-revoke.md`
- `memory/08-tasks/open/20260530-cpms-reset-install-redirect.md`
- `memory/08-tasks/open/20260530-cpms-security-deploy-hardening.md`
- `memory/08-tasks/open/20260530-cpms-cid-annual-report-import.md`
- `memory/08-tasks/open/20260530-cpms-status-export-cid-fields.md`
- `memory/08-tasks/open/20260530-cpms-status-export.md`
- `memory/08-tasks/open/20260530-cpms-validity-date-align.md`
- `memory/08-tasks/open/20260530-cid-admins-module-unify.md`
- `memory/08-tasks/open/20260530-cid-cpms-used-install-ui.md`
- `memory/08-tasks/open/20260530-cid-province-admin-governance-passkey.md`
- `memory/08-tasks/open/20260601-cid-official-institutions.md`
- `memory/08-tasks/open/20260603-cid-gov-private-subjects.md`
- `memory/08-tasks/open/20260604-cid-tabs-notice-admins.md`
- `memory/08-tasks/open/20260606-reconciliation-fund.md`
- `memory/08-tasks/open/20260606-cid-gov-directory-police.md`
- `memory/08-tasks/open/20260607-pqc-step1-chain-runtime-node.md`
- `memory/08-tasks/open/20260607-pqc-step2-wallet-citizenapp-citizenwallet.md`
- `memory/08-tasks/open/20260607-cid-cpms-pqc-signing.md`
- `memory/08-tasks/open/20260607-cid-number-protocol.md`
- `memory/08-tasks/open/20260607-wallet-pqc-passkey.md`
- `memory/08-tasks/open/20260608-pow-difficulty-fork-storm-txpool-desync.md`
- `memory/08-tasks/open/20260608-cid-registry-detail-tab.md`
- `memory/08-tasks/open/20260609-cid-citizen-list-refill-cpms-key-align.md`
- `memory/08-tasks/open/20260610-cid-education-tab-jy-unify.md`
- `memory/08-tasks/open/20260610-cid-gov-create-uninorg-unify.md`
- `memory/08-tasks/open/20260610-citizenwallet-payload-decoder-signing-tail.md`
- `memory/08-tasks/open/20260611-citizenapp-institution-registry-regen.md`
- `memory/08-tasks/open/20260611-citizenapp-keyspaged-pin-finalized.md`
- `memory/08-tasks/open/20260611-citizenapp-nrc-hefund-account-row.md`
- `memory/08-tasks/open/20260611-citizenapp-proposaldata-stale-48-guard.md`
- `memory/08-tasks/open/20260612-adr017-card1-chain-voting-rule.md`
- `memory/08-tasks/open/20260612-adr017-card2-citizenapp-finalized.md`
- `memory/08-tasks/open/20260612-adr017-card3-node-finalized.md`
- `memory/08-tasks/open/20260612-adr018-card1-proposal-feed-cache.md`
- `memory/08-tasks/open/20260612-adr018-card2-multisig-discovery.md`
- `memory/08-tasks/open/20260612-adr018-card3-nplus1-batch.md`
- `memory/08-tasks/open/20260612-adr018-card4-poll-to-sub.md`
- `memory/08-tasks/open/20260612-adr018-card5-chain-read-cache.md`
- `memory/08-tasks/open/20260612-adr018-card6-http-cache.md`
- `memory/08-tasks/open/20260612-adr018-card7-rules-doc.md`
- `memory/08-tasks/open/20260612-node-compact-u128-test-value.md`
- `memory/08-tasks/open/20260612-cid-audit-detail-structured.md`
- `memory/08-tasks/open/20260612-citizenapp-run-adb-preflight.md`
- `memory/08-tasks/open/20260613-android-16kb-page-alignment.md`
- `memory/08-tasks/open/20260613-cpms-archive-detail-nav.md`
- `memory/08-tasks/open/20260613-cpms-archive-list-detail-polish.md`
- `memory/08-tasks/open/20260613-cid-institution-detail-nav.md`
- `memory/08-tasks/open/20260613-cid-institution-list-audit-accounts.md`
- `memory/08-tasks/open/20260613-cid-citizenapp-bff-public-catalog.md`
- `memory/08-tasks/open/20260613-citizenapp-public-institution-card0-derivation-base.md`
- `memory/08-tasks/open/20260613-citizenapp-public-institution-cardA-data-sync.md`
- `memory/08-tasks/open/20260613-citizenapp-public-institution-cardB-nav-ui.md`
- `memory/08-tasks/open/20260613-citizenapp-public-institution-cardC-detail.md`
- `memory/08-tasks/open/20260614-im-p2p.md`
- `memory/08-tasks/open/20260614-cid-education-classification.md`
- `memory/08-tasks/open/20260614-citizenapp-public-institution-detail-align-governance.md`
- `memory/08-tasks/open/20260615-cpms-cid-birthplace-election-scope.md`
- `memory/08-tasks/open/20260616-admin-district-dedup-cleanup.md`
- `memory/08-tasks/open/20260618-citizenapp-admin-division-incremental-sync.md`
- `memory/08-tasks/done/20260619-admin-district-fresh-genesis-hk-zs.md`
- `memory/08-tasks/open/runtime-full-audit.md`

## done

- `memory/08-tasks/done/20260621-root-tools-to-scripts.md`
- `memory/08-tasks/done/20260620-cid-startup-chain-config-boundary.md`
- `memory/08-tasks/done/20260619-citizenapp-public-all-gov.md`
- `memory/08-tasks/done/20260618-version-reset-v1.md`
- `memory/08-tasks/done/20260618-prefecture-city-residual-cleanup.md`
- `memory/08-tasks/done/20260320-193702-完善-ai-编程系统的启动验收-任务卡硬规则-细粒度文档校验和轻量中文注释门禁.md`
- `memory/08-tasks/done/20260320-194945-删除已无业务代码的-citizenchain-node-目录-并将桌面节点功能统一收口到.md`
- `memory/08-tasks/done/20260320-220031-修复-citizenchain-node-backend-对-runtime-primitives-的错误路径引用.md`
- `memory/08-tasks/done/20260320-221924-更新-ai-编程系统中的技术语言基线-citizenchain-区块链与-node-使用-rust-tauri.md`
- `memory/08-tasks/done/20260320-223441-调整-citizenchain-启动脚本语义-run-sh-改为不清库继续启动开发链-clean-dev-sh.md`
- `memory/08-tasks/done/20260320-225846-把-codex-主窗口自动调度-blockchain-agent-cid-agent-cpms-agent.md`
- `memory/08-tasks/done/20260320-231550-修复手机端-runtime-升级联合投票提案在投票列表和投票事件列表不显示的问题.md`
- `memory/08-tasks/done/20260320-runtime-integration-retroactive.md`
- `memory/08-tasks/done/20260321-074809-生成-cid-env-dev-local-本地开发环境文件-并确保与当前启动脚本和后端必需环境变量一致.md`
- `memory/08-tasks/done/20260321-082100-将-citizenapp-手机启动脚本补充-citizenapp-api-base-url-并指向云服务器-147-224-14.md`
- `memory/08-tasks/done/20260321-133021-统一机构页在手机端的双列显示规则-并确认手机访问-rpc-的联网条件.md`
- `memory/08-tasks/done/20260321-133855-按链上要求实现手机端-runtime-升级提案联合投票-补齐投票按钮-机构上下文-管理员签名收集.md`
- `memory/08-tasks/done/20260321-161014-统一-gmb-ai-编程系统支持-codex-与-claude-双聊天窗口入口.md`
- `memory/08-tasks/done/20260321-180000-整理热钱包详情页扫码入口.md`
- `memory/08-tasks/done/20260321-190000-分离离线签名功能到独立-citizenwallet-app-软件硬件钱包.md`
- `memory/08-tasks/done/20260322-123405-补齐-china-机构常量模块接入并纳入-primitives-模块树.md`
- `memory/08-tasks/done/20260322-170838-检查-citizenapp-切换-smoldot-轻节点后能连接节点但钱包余额无法更新的问题并定位故障点.md`
- `memory/08-tasks/done/20260322-175545-整理-citizenwallet-正式发布状态并替换logo与应用名版本号.md`
- `memory/08-tasks/done/20260322-200357-移除-citizenwallet-与-citizenapp-导入钱包后误触发剪贴板提示.md`
- `memory/08-tasks/done/20260322-smoldot-步骤1-集成smoldot替换chainrpc底层provider.md`
- `memory/08-tasks/done/20260322-smoldot-步骤2-重写区块订阅用smoldot原生订阅替换websocket.md`
- `memory/08-tasks/done/20260322-smoldot-步骤3-适配服务层和交易提交移除rpc节点残留.md`
- `memory/08-tasks/done/20260323-094248-修复-node-挖矿页面收益统计口径-按区块当时的真实到账规则显示矿工实际收益.md`
- `memory/08-tasks/done/20260323-102718-制定-citizenapp-轻节点模式在-citizenchain-保持-pow-不改前提下的长期落地方案-不保留-http.md`
- `memory/08-tasks/done/20260323-173653-将-tools-zhujichi-py-从输出-json-改为输出-txt-并验证生成结果.md`
- `memory/08-tasks/done/20260324-chain-phase-control.md`
- `memory/08-tasks/done/20260324-node-governance-tab.md`
- `memory/08-tasks/done/20260324-skip-empty-blocks.md`
- `memory/08-tasks/done/20260325-125830-runtime-citizenapp-citizenwallet-双端联动强制规则.md`
- `memory/08-tasks/done/20260325-130439-新增-institution-asset-公共模块-并接入机构账户资金操作白名单边界.md`
- `memory/08-tasks/done/20260325-133531-统一-duoqian-manage-命名-superseded.md`
- `memory/08-tasks/done/20260325-150802-为-cid-机构生成流程新增独立机构名称字段并用于-citizenapp-与-citizenwallet-显示.md`
- `memory/08-tasks/done/20260325-232827-排查出块记录中区块高度-时间-手续费到账收益-铸块奖励-区块作者不显示的原因.md`
- `memory/08-tasks/done/20260325-genesis-bootstrap.md`
- `memory/08-tasks/done/20260325-remove-dev-chain.md`
- `memory/08-tasks/done/20260325-最后一次开发者直升-从开发期切换到运行期.md`
- `memory/08-tasks/done/20260326-011500-收敛-grandpa-voter-启动条件并停止-concluded-rounds-追加持久化.md`
- `memory/08-tasks/done/20260326-103500-清库后节点必须先同步网络再允许出块-禁止本地先起分叉.md`
- `memory/08-tasks/done/20260326-104800-删除前10块自动引导出块逻辑-普通节点只允许同步现网后出块.md`
- `memory/08-tasks/done/20260326-112500-修复普通节点关闭-grandpa-voter-后网络连接被-grandpa-协议空接收端打断.md`
- `memory/08-tasks/done/20260327-083033-全面检查-grandpa-为什么不工作-不验证区块-基于代码-配置-日志和运行行为定位根因.md`
- `memory/08-tasks/done/20260327-fix-smoldot-submit-and-vote-display.md`
- `memory/08-tasks/done/20260327-fix-try-execute-transfer-atomicity.md`
- `memory/08-tasks/done/20260328-073618-review-shengbank-interest.md`
- `memory/08-tasks/done/20260328-075127-review-fullnode-issuance.md`
- `memory/08-tasks/done/20260328-095223-review-citizen-issuance.md`
- `memory/08-tasks/done/20260328-100300-review-pow-difficulty.md`
- `memory/08-tasks/done/20260328-101548-review-cid-system.md`
- `memory/08-tasks/done/20260328-112224-review-grandpakey-change.md`
- `memory/08-tasks/done/20260328-112311-审查-admins-change-模块安全与质量.md`
- `memory/08-tasks/done/20260328-113654-审查-resolution-destro.md`
- `memory/08-tasks/done/20260328-120407-审查-runtime-upgrade.md`
- `memory/08-tasks/done/20260328-121809-审查-duoqian-manage.md`
- `memory/08-tasks/done/20260328-123104-review-institution-asset.md`
- `memory/08-tasks/done/20260328-123709-审查-onchain-transaction.md`
- `memory/08-tasks/done/20260328-124050-review-voting-engine.md`
- `memory/08-tasks/done/20260328-130310-review-duoqian-transfer.md`
- `memory/08-tasks/done/20260328-131513-review-genesis.md`
- `memory/08-tasks/done/20260328-132030-审查-node.md`
- `memory/08-tasks/done/20260328-140327-review-node.md`
- `memory/08-tasks/done/20260328-141519-审查-primitives-模块安全与质量.md`
- `memory/08-tasks/done/20260328-145337-审查-runtime-src.md`
- `memory/08-tasks/done/20260328-164356-审查-cid-系统.md`
- `memory/08-tasks/done/20260328-164533-review-cpms-system.md`
- `memory/08-tasks/done/20260329-100000-citizenwallet-citizenapp-扫码登录-cid-cpms.md`
- `memory/08-tasks/done/20260329-124242-统一切换-libp2p-wss-30333-清理-litep2p-并修复全链节点连接问题.md`
- `memory/08-tasks/done/20260329-150008-统一-cid-城市代码与省级占位码规则.md`
- `memory/08-tasks/done/20260329-160355-收口-cid-绑定阶段-cid-兜底生成路径.md`
- `memory/08-tasks/done/20260331-092013-排查本地电脑节点与两台服务器节点无法连通的问题-确认根因并修复相关配置或文档.md`
- `memory/08-tasks/done/20260401-runtime-noamount-fee-无金额交易收取最低手续费.md`
- `memory/08-tasks/done/20260402-125842-citizenapp-light-node-connect-fix.md`
- `memory/08-tasks/done/20260402-governance-admin-activation.md`
- `memory/08-tasks/done/20260405-offchain-deposit-model-redesign.md`
- `memory/08-tasks/done/20260407-cid-runtime-meta-清理-与-pow-推链修复.md`
- `memory/08-tasks/done/20260408-cid-frontend-app-tsx-split-plan.md`
- `memory/08-tasks/done/20260408-cid-frontend-app-tsx-split.md`
- `memory/08-tasks/done/20260408-cid-public-security-cpms-embed.md`
- `memory/08-tasks/done/20260408-cid-三角色命名统一-任务卡0.5.md`
- `memory/08-tasks/done/20260408-cid-公安局市清单对账-任务卡6.md`
- `memory/08-tasks/done/20260408-cid-前端三机构视图-任务卡4.md`
- `memory/08-tasks/done/20260408-cid-前端最小脚手架-任务卡3.md`
- `memory/08-tasks/done/20260408-cid-机构详情页多账户-任务卡5.md`
- `memory/08-tasks/done/20260408-cid-机构账户两层模型-任务卡2.md`
- `memory/08-tasks/done/20260408-cid-模块补全-任务卡1.md`
- `memory/08-tasks/done/20260409-cid-sheng-admin-per-province-keyring-impl.md`
- `memory/08-tasks/done/20260409-cid-sheng-admin-per-province-keyring.md`
- `memory/08-tasks/done/20260410-cid-store-shard-by-province-impl.md`
- `memory/08-tasks/done/20260410-cid-store-shard-by-province.md`
- `memory/08-tasks/done/20260415-governance-nrc-back-nav-fix.md`
- `memory/08-tasks/done/20260415-proposal-action-resolver.md`
- `memory/08-tasks/done/20260415-citizenwallet-institutions-unified-registry.md`
- `memory/08-tasks/done/20260419-cid-机构两步式创建.md`
- `memory/08-tasks/done/20260420-duoqian_account-改名-main_account.md`
- `memory/08-tasks/done/20260420-keyless_address-改名-stake_account.md`
- `memory/08-tasks/done/20260420-unified-DUOQIAN_V1-domain.md`
- `memory/08-tasks/done/20260421-154308-多签账户注册改造-离线QR聚合签名-step1.md`
- `memory/08-tasks/done/20260421-162425-多签账户转账改造-离线QR聚合签名-step2.md`
- `memory/08-tasks/done/20260421-name-to-account-name-rename.md`
- `memory/08-tasks/done/20260421-op-institution-role-split.md`
- `memory/08-tasks/done/20260421-cid-默认账户两步激活.md`
- `memory/08-tasks/done/20260422-unified-voting-entry-phase1.md`
- `memory/08-tasks/done/20260422-unified-voting-entry-phase2.md`
- `memory/08-tasks/done/20260422-unified-voting-entry-phase3.md`
- `memory/08-tasks/done/20260423-citizenapp-wallet-detail-redesign.md`
- `memory/08-tasks/done/20260424-node-tab-restructure.md`
- `memory/08-tasks/done/20260424-step1-cid-clearing-bank-eligibility.md`
- `memory/08-tasks/done/20260424-citizenapp-wallet-detail-redesign-v2.md`
- `memory/08-tasks/done/20260424-citizenapp-wallet-detail-redesign-v3.md`
- `memory/08-tasks/done/20260424-citizenapp-wallet-detail-redesign-v4.md`
- `memory/08-tasks/done/20260424-citizenapp-wallet-list-redesign-v6.md`
- `memory/08-tasks/done/20260424-citizenapp-wallet-list-redesign.md`
- `memory/08-tasks/done/20260425-083200-ui-app.md`
- `memory/08-tasks/done/20260425-node-cleanup-preview-html-and-identity-doc.md`
- `memory/08-tasks/done/20260427-step2-citizenchain-clearing-bank-nodes.md`
- `memory/08-tasks/done/20260428-123621-检查清算行功能完成度并输出完成度报告.md`
- `memory/08-tasks/done/20260428-135057-本地开发节点默认查询本机cid服务正式节点默认查询147正式cid服务.md`
- `memory/08-tasks/done/20260429-082049-治理模块改名-admins-change-grandpakey-change.md`
- `memory/08-tasks/done/20260429-084207-resolution-issuance-merge.md`
- `memory/08-tasks/done/20260429-085144-修复-admins-change-grandpakey-change-改名残留.md`
- `memory/08-tasks/done/20260429-093954-rename-issuance-modules.md`
- `memory/08-tasks/done/20260429-094308-rename-pow-difficulty-and-cid-system.md`
- `memory/08-tasks/done/20260429-100032-duoqian-modules-full-rename.md`
- `memory/08-tasks/done/20260429-104102-重命名本地-libp2p-websocket-patch-目录.md`
- `memory/08-tasks/done/20260429-104514-node-src-ui-flatten.md`
- `memory/08-tasks/done/20260429-111541-node-src-and-frontend-layout-cleanup.md`
- `memory/08-tasks/done/20260429-113130-收敛-citizenchain-node-frontend-前端目录结构.md`
- `memory/08-tasks/done/20260429-114553-禁止-node-frontend-根层-vite-配置生成物产出.md`
- `memory/08-tasks/done/20260429-ai-frontend-backend-same-feature-path-rule.md`
- `memory/08-tasks/done/20260429-duoqian-manage-step1-institution-refactor.md`
- `memory/08-tasks/done/20260429-governance-rename-resolution-runtime.md`
- `memory/08-tasks/done/20260429-node-offchain-dir-align.md`
- `memory/08-tasks/done/20260429-node-offchain-second-consolidation.md`
- `memory/08-tasks/done/20260429-cid-chain-state-source.md`
- `memory/08-tasks/done/20260429-step3-runtime-node-rename-cleanup.md`
- `memory/08-tasks/done/20260429-task-card-filename-byte-limit.md`
- `memory/08-tasks/done/20260429-voting-engine-rename.md`
- `memory/08-tasks/done/20260429-wasm-ci-checkout-and-version-zero.md`
- `memory/08-tasks/done/20260430-092239-citizenapp-duoqian-split.md`
- `memory/08-tasks/done/20260430-095017-citizenapp-duoqian-pure-migration.md`
- `memory/08-tasks/done/20260430-100427-citizenapp-duoqian-create-close-only-from-account.md`
- `memory/08-tasks/done/20260430-111018-梳理-citizenapp-公民-公民治理机构-治理-投票-提案等功能的实现目录与关键文件.md`
- `memory/08-tasks/done/20260430-115117-删除公民提案多钱管理预留目录.md`
- `memory/08-tasks/done/20260430-115508-清理管理员更换模块旧投票残留并修复-admins-change-benchmark-中硬编码-proposal-id-的问题-暂不实现-pending-主体-ap.md`
- `memory/08-tasks/done/20260430-120456-修复我的钱包拖拽排序松手后回到原位置的问题-原因是-ui-层对-fixed-length-钱包列表执行-removeat-insert-需改为.md`
- `memory/08-tasks/done/20260430-120900-把官网的更新部署到服务器-64-181-239-233.md`
- `memory/08-tasks/done/20260430-124640-恢复-citizenapp-交易-tab-四个独立入口.md`
- `memory/08-tasks/done/20260430-131327-修复交易页恢复原-ui-仅新增个人多签和机构多签入口.md`
- `memory/08-tasks/done/20260430-132721-p2-pending-api-active-snapshot.md`
- `memory/08-tasks/done/20260430-140612-p2-admins-change-proposal-consistency.md`
- `memory/08-tasks/done/20260430-145704-p1-admins-change-mutex.md`
- `memory/08-tasks/done/20260430-154307-p2-p3-voting-finalized-transaction.md`
- `memory/08-tasks/done/20260430-161713-p1-voting-status-state-machine.md`
- `memory/08-tasks/done/20260430-165708-核验-voting-engine-unix-seconds-to-year-元旦附近年份错分问题.md`
- `memory/08-tasks/done/20260430-170233-检查-voting-engine-中-set-status-and-emit-store-proposal-data-set-proposal-passed-是否为无校验-pub-fn-并评估是否存在跨-palle.md`
- `memory/08-tasks/done/20260430-174943-检查-joint-vote-rs-联合投票阈值是否动态读取而未使用快照-确认是否存在一致性裂缝.md`
- `memory/08-tasks/done/20260430-182001-按正确边界改造联合投票固定治理阈值-注册多签继续动态阈值并创建时快照-暂不升级-runtime.md`
- `memory/08-tasks/done/20260430-185906-检查-governance-proposal-cleanup-schedule-cleanup-连续队列满后是否导致提案存储永久不清理.md`
- `memory/08-tasks/done/20260430-200621-修复-voting-engine-权限和重试残留.md`
- `memory/08-tasks/done/20260430-204134-修复-admins-change-close-subject-禁止关闭内置治理机构.md`
- `memory/08-tasks/done/20260430-213218-修复-voting-engine-m4-m5-与-admins-change-module-tag-版本化问题.md`
- `memory/08-tasks/done/20260430-213345-只读分析-voting-engine-internal-vote-与-active-proposal-limit-以及-admins-change-回调中的-l1-l5-审查问题是否存在.md`
- `memory/08-tasks/done/20260430-215740-修复-voting-engine-internal-vote-provider-fallback-threshold-provider-默认实现-active-proposal-上限硬编码与主体合法性语义.md`
- `memory/08-tasks/done/20260430-citizenapp-offchain-move.md`
- `memory/08-tasks/done/20260501-081728-审查-resolution-issuance-双-finalize.md`
- `memory/08-tasks/done/20260501-081838-检查-shengbank-interest-自动补-8-年单块权重风险.md`
- `memory/08-tasks/done/20260501-082729-删除-resolution-issuance-双-finalize-入口.md`
- `memory/08-tasks/done/20260501-085322-清理-resolution-issuance-无效-cleanup-和错误码.md`
- `memory/08-tasks/done/20260501-090259-修复-issuance-常量死分支和区块号耦合.md`
- `memory/08-tasks/done/20260501-090444-检查-runtime-issuance-模块权重占位-cargo-迁移和-shengbank-利率溢出事件问题.md`
- `memory/08-tasks/done/20260501-091136-修复-issuance-权重占位-cargo-冗余-迁移兜底与利率溢出审计.md`
- `memory/08-tasks/done/20260501-091450-修复-onchain-transaction-手续费公式入口和销毁审计事件.md`
- `memory/08-tasks/done/20260501-094149-新增首页交易矿工热钱包.md`
- `memory/08-tasks/done/20260501-102629-检查-resolution-destro-p0-p1-p2-残留权重错误码机构约定与-citizenapp-空目录.md`
- `memory/08-tasks/done/20260501-103245-修复-grandpakey-change-p1-p5-问题.md`
- `memory/08-tasks/done/20260501-node-clearing-bank-institution-detail-and-create.md`
- `memory/08-tasks/done/20260501-cid-chain-folder-restructure.md`
- `memory/08-tasks/done/20260502-134646-在-gmb-ai-编程系统中增加强制规则-每次输出技术方案时-必须输出本次预计修改的目录-并为每个目录附中文.md`
- `memory/08-tasks/done/20260502-191731-检查-voting-engine-benchmark-权重和-admins-change-duoqian-双入口风险.md`
- `memory/08-tasks/done/20260502-192535-治理-runtime-问题核查.md`
- `memory/08-tasks/done/20260502-193144-修复-voting-engine-benchmark-internal-vote-权重和-duoqian-治理边界.md`
- `memory/08-tasks/done/20260502-195022-治理-runtime-核查问题修复.md`
- `memory/08-tasks/done/20260502-201823-清除治理runtime五项已完成清单.md`
- `memory/08-tasks/done/20260502-220000-治理-runtime-审计-4-项后续修复.md`
- `memory/08-tasks/done/20260502-230000-duoqian-manage-按个人机构子目录拆分.md`
- `memory/08-tasks/done/20260502-check-admins-voting-api-retry-cleanup.md`
- `memory/08-tasks/done/20260502-duoqian-registration-info-align.md`
- `memory/08-tasks/done/20260502-fix-admins-lifecycle-voting-retry-cleanup.md`
- `memory/08-tasks/done/20260502-node-offchain-clearing-dir-refactor.md`
- `memory/08-tasks/done/20260502-node-offchain-duplicate-cleanup.md`
- `memory/08-tasks/done/20260502-citizencode-backend-src平移根目录.md`
- `memory/08-tasks/done/20260502-cid-cleanup残留整改.md`
- `memory/08-tasks/done/20260502-cid-cpms-sheng目录整改.md`
- `memory/08-tasks/done/20260502-cid-duoqian-info-layout.md`
- `memory/08-tasks/done/20260502-cid-frontend-api归并功能模块.md`
- `memory/08-tasks/done/20260502-cid-institution-filing-fields.md`
- `memory/08-tasks/done/20260502-cid-institutions粗粒度整合.md`
- `memory/08-tasks/done/20260502-cid-legacy-role-text-cleanup.md`
- `memory/08-tasks/done/20260502-cid-models-scope边界整改.md`
- `memory/08-tasks/done/20260502-cid-sheng-backup-admin-ui.md`
- `memory/08-tasks/done/20260502-cid-sheng-tabs.md`
- `memory/08-tasks/done/20260503-000000-duoqian-manage-创建路径重构-8项.md`
- `memory/08-tasks/done/20260503-110000-费率规则常量统一到-fee-policy.md`
- `memory/08-tasks/done/20260503-114604-检查注册机构多签模块完成度.md`
- `memory/08-tasks/done/20260503-150000-个人多签模块边界测试补全.md`
- `memory/08-tasks/done/20260503-160000-citizenapp-个人多签详情页UI改造.md`
- `memory/08-tasks/done/20260503-180000-citizenapp-多签反向索引发现.md`
- `memory/08-tasks/done/20260503-200000-citizenapp-多签4bug合并修复.md`
- `memory/08-tasks/done/20260503-220000-citizenapp-多签5bug合并修复.md`
- `memory/08-tasks/done/20260504-100000-citizenchain-votingengine-提级与目录拆分.md`
- `memory/08-tasks/done/20260504-160000-客户端接入双层ID与反向索引.md`
- `memory/08-tasks/done/20260505-100000-投票引擎与机构管理模块重构.md`
- `memory/08-tasks/done/20260505-184900-votingengine-3-sub-pallet-拆分.md`
- `memory/08-tasks/done/20260505-215047-rename-org-manage-to-organization-manage.md`
- `memory/08-tasks/done/20260506-admins-change-institutions-subjects-migration.md`
- `memory/08-tasks/done/20260506-rename-institution-id-to-subject-id.md`
- `memory/08-tasks/done/20260506-split-personal-manage-from-organization-manage.md`
- `memory/08-tasks/done/20260506-unified-subject-id-protocol.md`
- `memory/08-tasks/done/20260506-citizenapp-公民三tab扁平化重构.md`
- `memory/08-tasks/done/20260507-185958-第1步改造-admins-change-新增-institutionaccount-账户级主体-保留-cidinstitution-仅作机构归属-实现统一动态阈值工.md`
- `memory/08-tasks/done/20260507-192407-第2步改造-internal-vote-账户级快照与全员生命周期阈值.md`
- `memory/08-tasks/done/20260507-194843-第3步破坏式改造-personal-manage-删除个人创建阈值入参与管理员镜像.md`
- `memory/08-tasks/done/20260507-202521-第5步改造-duoqian-transfer-citizenapp-citizenwallet-接入机构账户级主体.md`
- `memory/08-tasks/done/20260507-203644-修复-gmb-仓库私有化后-公民链桌面节点-ui-的白皮书与公民宪法-tab-因依赖-github-pages-外链而显示-404-的问题.md`
- `memory/08-tasks/done/20260507-205335-修复公民链桌面节点白皮书-tab-外层-html-已本地加载但内部仍-fetch-私有-gmb-raw-markdown-导致-http-404-的问题-改.md`
- `memory/08-tasks/done/20260507-ai-required-reading.md`
- `memory/08-tasks/done/20260507-ai-unified-naming.md`
- `memory/08-tasks/done/20260507-ai-unified-protocols.md`
- `memory/08-tasks/done/20260507-cargo-pubspec-todo-cleanup.md`
- `memory/08-tasks/done/20260507-clean-old-path-text.md`
- `memory/08-tasks/done/20260507-clear-open-task-cards.md`
- `memory/08-tasks/done/20260507-h1-node-admin-subject-storage-key.md`
- `memory/08-tasks/done/20260507-p0-2-propose-create-institution.md`
- `memory/08-tasks/done/20260507-p0-3-duoqian-storage-truth.md`
- `memory/08-tasks/done/20260507-p0-4-immortal-era.md`
- `memory/08-tasks/done/20260507-p0-5-step2d-fixture.md`
- `memory/08-tasks/done/20260507-p0-6-cpms-cid-path.md`
- `memory/08-tasks/done/20260507-p1-6-qr-protocol-source.md`
- `memory/08-tasks/done/20260507-p2-5-citizenapp-isar-dir.md`
- `memory/08-tasks/done/20260507-personal-manage-risk-cleanup.md`
- `memory/08-tasks/done/20260507-pr-c-organization-cleanup.md`
- `memory/08-tasks/done/20260507-pr-c-storage-naming-cleanup.md`
- `memory/08-tasks/done/20260507-pr-d-memory-freeze.md`
- `memory/08-tasks/done/20260507-pr-e-registry-freeze.md`
- `memory/08-tasks/done/20260507-root-cargo-member-cleanup.md`
- `memory/08-tasks/done/20260507-runtime-pallet-tests-restructure.md`
- `memory/08-tasks/done/20260507-rust-warning-and-marker-cleanup.md`
- `memory/08-tasks/done/20260507-cid-format-d8-to-d4-and-1000-retry.md`
- `memory/08-tasks/done/20260507-cid-step1-rename-shenfen-to-cid-number-and-city-001.md`
- `memory/08-tasks/done/20260507-cid-step2-regenerate-derived-addresses.md`
- `memory/08-tasks/done/20260508-164118-docs-and-governance-registry.md`
- `memory/08-tasks/done/20260508-170114-restore-doc-style.md`
- `memory/08-tasks/done/20260508-172933-fix-doc-tab-binding-title.md`
- `memory/08-tasks/done/20260508-174253-governance-detail-inline-accounts.md`
- `memory/08-tasks/done/20260508-175514-只读排查-citizenapp-交易-多签交易中创建个人多签提案时-发起人可投票并出块-但其他管理员投票后前端显示.md`
- `memory/08-tasks/done/20260508-181814-修复-citizenapp-个人多签投票确认状态和进度条阈值.md`
- `memory/08-tasks/done/20260508-admins-change-lifecycle-events.md`
- `memory/08-tasks/done/20260509-093707-检查一遍-admins-change-管理员更换模块的完成度-输出完成度-缺口-风险和建议修复顺序-本轮先不修改业务代.md`
- `memory/08-tasks/done/20260509-131106-检查-runtime-upgrade-模块完成度-输出代码-测试-文档-兼容性和残留风险的只读审计报告.md`
- `memory/08-tasks/done/20260509-135931-personal-manage-完成度检查.md`
- `memory/08-tasks/done/20260509-144425-personal-manage-个人多签目录拆分.md`
- `memory/08-tasks/done/20260509-161649-citizenapp-模块边界整改-institution-shared-common.md`
- `memory/08-tasks/done/20260509-165919-citizenapp-admins-change-命名统一-单文件子目录扁平化.md`
- `memory/08-tasks/done/20260509-182958-调整国储会详情页-runtime-升级入口-状态升级改为协议升级-并新增独立开发升级按钮.md`
- `memory/08-tasks/done/20260509-193214-citizenapp-4模块统一进lib-governance.md`
- `memory/08-tasks/done/20260509-195358-citizenapp-清空-proposal-目录.md`
- `memory/08-tasks/done/20260509-201921-citizenapp-3模块进lib-transaction.md`
- `memory/08-tasks/done/20260509-205303-治理提案-3张转账卡复原.md`
- `memory/08-tasks/done/20260509-duoqian-transfer-boundary.md`
- `memory/08-tasks/done/20260509-duoqian-transfer-strict-boundary-cleanup.md`
- `memory/08-tasks/done/20260509-citizenapp-duoqian-unified-entry.md`
- `memory/08-tasks/done/20260510-citizenapp-my-user-util-move.md`
- `memory/08-tasks/done/20260510-citizenapp-myid-electronic-passport.md`
- `memory/08-tasks/done/20260511-votingengine-boundary-fix.md`
- `memory/08-tasks/done/20260511-wasm-ci-auto-spec-version.md`
- `memory/08-tasks/done/20260511-citizenapp-citizenwallet-duoqian.md`
- `memory/08-tasks/done/20260514-citizenapp-lib-directory-consolidation.md`
- `memory/08-tasks/done/20260516-runtime-audit-fixes-vote-cid.md`
- `memory/08-tasks/done/20260517-200620-citizenapp-runtime-nonce-only.md`
- `memory/08-tasks/done/20260517-citizenapp-android-app-update.md`
- `memory/08-tasks/done/20260517-citizenapp-android-release-signing.md`
- `memory/08-tasks/done/20260517-citizenapp-duoqian-tab.md`
- `memory/08-tasks/done/20260517-citizenapp-isar-read-queue-wallet-governance.md`
- `memory/08-tasks/done/20260517-citizenapp-isar-wallet-chain-status.md`
- `memory/08-tasks/done/20260517-citizenapp-smoldot-android-abi.md`
- `memory/08-tasks/done/20260518-121925-citizenapp-wallet-tx-records.md`
- `memory/08-tasks/done/20260518-citizenapp-cold-wallet-scan.md`
- `memory/08-tasks/done/20260518-citizenapp-duoqian-detail-balance.md`
- `memory/08-tasks/done/20260518-citizenapp-duoqian-detail-local-first.md`
- `memory/08-tasks/done/20260518-citizenapp-duoqian-list-ttl.md`
- `memory/08-tasks/done/20260518-citizenapp-governance-detail-loading.md`
- `memory/08-tasks/done/20260518-citizenapp-governance-order.md`
- `memory/08-tasks/done/20260519-citizenapp-ci-chain-progress-test.md`
- `memory/08-tasks/done/20260519-citizenapp-local-chain-optimization.md`
- `memory/08-tasks/done/20260519-citizenapp-proposal-local-cache.md`
- `memory/08-tasks/done/20260519-citizenapp-wallet-tx-three-status.md`
- `memory/08-tasks/done/20260523-000011-在区块链软件设置页新增全节点模式入口-默认归档全节点-普通全节点和通信全节点作为待完成模式展示-当.md`
- `memory/08-tasks/done/20260523-001849-调整区块链软件设置页全节点模式显示-删除待完成说明文案和已保存提示-归档全节点不显示已支持图标-普.md`
- `memory/08-tasks/done/20260523-121946-保存-citizenapp-p2p-im-技术选型方案-平台原生近场模块采用-citizenapp-android-im-与-citizenapp-ios-im-两个浅层目录-im-目录.md`
- `memory/08-tasks/done/20260524-citizenchain-ci-five-packages.md`
- `memory/08-tasks/done/20260524-update-red-dot-indicators.md`
- `memory/08-tasks/done/20260525-175008-cid绑定签名回执与citizenapp启动anr修复.md`
- `memory/08-tasks/done/20260525-cid-bind-copy-myid-scan-square.md`
- `memory/08-tasks/done/20260525-cid-bind-sign-request-citizenwallet-scan.md`
- `memory/08-tasks/done/20260525-cid-bind-upload-qr.md`
- `memory/08-tasks/done/20260525-cid-cpms-archive-simplify.md`
- `memory/08-tasks/done/20260525-cid-cpms-store.md`
- `memory/08-tasks/done/20260526-cpms-wallet-address-only.md`
- `memory/08-tasks/done/20260526-cpms-wallet-proof.md`
- `memory/08-tasks/done/20260526-error-codes.md`
- `memory/08-tasks/done/20260527-cpms-admin-permissions.md`
- `memory/08-tasks/done/20260527-cpms-archive-list-detail-display.md`
- `memory/08-tasks/done/20260527-cpms-archive-soft-delete-citizenwallet.md`
- `memory/08-tasks/done/20260527-cpms-last-first-name-fields.md`
- `memory/08-tasks/done/20260527-cpms-name-split-display.md`
- `memory/08-tasks/done/20260527-cpms-cid-address.md`
- `memory/08-tasks/done/20260527-cpms-vote-account-scan-ui.md`
- `memory/08-tasks/done/20260527-cid-myid-archive-driven-bind.md`
- `memory/08-tasks/done/20260529-162348-cpms-编号工具与护照号规则.md`
- `memory/08-tasks/done/20260529-164449-cpms-删除签名请求与弹窗修复.md`
- `memory/08-tasks/done/20260529-170516-cpms档案详情打印样式修复.md`
- `memory/08-tasks/done/20260529-171139-cpms投票账户单行显示修复.md`
- `memory/08-tasks/done/20260529-cpms安全与残留整改.md`
- `memory/08-tasks/done/20260530-193458-cid-本地启动端口与数据库残留修复.md`
- `memory/08-tasks/done/20260530-202322-cid统一冷钱包签名组件与passkey注册流程.md`
- `memory/08-tasks/done/20260530-203838-passkey更新密钥首次登录红色角标.md`
- `memory/08-tasks/done/20260530-204022-passkey更新密钥红色角标放大.md`
- `memory/08-tasks/done/20260530-ci-node24-unification.md`
- `memory/08-tasks/done/20260530-cpms-archive-list-search-align.md`
- `memory/08-tasks/done/20260530-cpms-archive-list-town-column.md`
- `memory/08-tasks/done/20260530-cpms-dangan-cursor-pagination.md`
- `memory/08-tasks/done/20260530-cpms-migration-baseline-cleanup.md`
- `memory/08-tasks/done/20260530-cid-admin-permission-step2.md`
- `memory/08-tasks/done/20260531-092106-统一-cid-管理端权限命名为-login-state-passkey-passkey-challenge-新增-admins-operation-auth-调整删除机构账户-文档为通.md`
- `memory/08-tasks/done/20260531-citizenpassport-ci-arm64-installer.md`
- `memory/08-tasks/done/20260531-citizenpassport-ci-clean-ubuntu-debs.md`
- `memory/08-tasks/done/20260531-cpms-installer-db-pdf.md`
- `memory/08-tasks/done/20260531-citizenpassport-ubuntu24-offline-installer.md`
- `memory/08-tasks/done/20260531-cpms日期输入统一组件.md`
- `memory/08-tasks/done/20260531-cpms档案码生成完整性校验.md`
- `memory/08-tasks/done/20260531-cid-admin-duplicate-role-error.md`
- `memory/08-tasks/done/20260531-cid-admin-model-no-status.md`
- `memory/08-tasks/done/20260531-cid-admin-ui-closeout.md`
- `memory/08-tasks/done/20260531-cid-city-cache-public-security-table.md`
- `memory/08-tasks/done/20260531-cid-deterministic-meta-cache-pagination.md`
- `memory/08-tasks/done/20260531-cid-large-data-exact-query-pagination.md`
- `memory/08-tasks/done/20260531-cid-permission-closeout.md`
- `memory/08-tasks/done/20260531-cid-public-security-exact-query.md`
- `memory/08-tasks/done/20260531-cid-remove-migrations-province-admin-source.md`
- `memory/08-tasks/done/20260531-cid-sheng-admin-limit-and-persistence.md`
- `memory/08-tasks/done/20260531-cid-shi-admin-city-limit.md`
- `memory/08-tasks/done/20260531-cid-signature-modal-stack.md`
- `memory/08-tasks/done/20260531-cid-signature-scan-loading.md`
- `memory/08-tasks/done/20260531-统一签名协议复查残留修复.md`
- `memory/08-tasks/done/20260531-统一签名协议整改.md`
- `memory/08-tasks/done/20260603-cid-remove-institutions-china-sqlite.md`
- `memory/08-tasks/done/20260604-cid-core-number-store-refactor.md`
- `memory/08-tasks/done/20260604-cid-gov-init.md`
- `memory/08-tasks/done/20260604-cid-structured-db.md`
- `memory/08-tasks/done/20260606-citizen-constitution-committee-titlecase.md`
- `memory/08-tasks/done/20260606-citizen-constitution-naming-fix.md`
- `memory/08-tasks/done/20260606-citizen-constitution-pass3.md`
- `memory/08-tasks/done/20260606-cpms-address-china-converge.md`
- `memory/08-tasks/done/20260606-citizenpassport-backend-flatten-src-china-recouple.md`
- `memory/08-tasks/done/20260606-cpms-china-db-path-dev-default.md`
- `memory/08-tasks/done/20260606-cpms-common-module.md`
- `memory/08-tasks/done/20260606-cpms-common-shared-dto.md`
- `memory/08-tasks/done/20260606-cpms-install-upload-qr.md`
- `memory/08-tasks/done/20260606-merge-citizen-constitution-into-genesis.md`
- `memory/08-tasks/done/20260606-node-constitution-nav-article52.md`
- `memory/08-tasks/done/20260606-cid-legal-rep.md`
- `memory/08-tasks/done/20260606-cid-schema-converge-order.md`
- `memory/08-tasks/done/20260607-duoqian-accountid-governance.md`
- `memory/08-tasks/done/20260607-merge-traits-types-into-multisig.md`
- `memory/08-tasks/done/20260608-genesis-ssot-redesign.md`
- `memory/08-tasks/done/20260608-purge-dead-votingengine-migrations.md`
- `memory/08-tasks/done/20260608-cid-admin-district-unify-zhen.md`
- `memory/08-tasks/done/20260609-silent-failure-submit-path-fixes.md`
- `memory/08-tasks/done/20260610-node-dryrun-future-friendly-hint.md`
- `memory/08-tasks/done/20260611-citizenwallet-institutions-name-sync.md`
- `memory/08-tasks/done/20260612-181650-重构-cid-私权机构架构-保留身份id格式-私权机构按个体经营-合伙企业-股权公司-股份公司-公益组织-注册协.md`
- `memory/08-tasks/done/20260612-194131-cid-private-real-module-refactor.md`
- `memory/08-tasks/done/20260612-cid-admin-naming-federal-city.md`
- `memory/08-tasks/done/20260612-cid-no-compat-cleanup-acceptance.md`
- `memory/08-tasks/done/20260614-im-direct-network.md`
- `memory/08-tasks/done/20260614-im-keypackage-protobuf-mls-boundary.md`
- `memory/08-tasks/done/20260614-im-network-spike.md`
- `memory/08-tasks/done/20260614-im-node-pairing-settings.md`
- `memory/08-tasks/done/20260614-im-persistent-mailbox-transport.md`
- `memory/08-tasks/done/20260614-im-two-node-smoke.md`
- `memory/08-tasks/done/20260615-im-chat-ui-contact-flow.md`
- `memory/08-tasks/done/20260615-im-contact-runtime-flow.md`
- `memory/08-tasks/done/20260615-im-envelope-isar-flow.md`
- `memory/08-tasks/done/20260615-im-mls-session-state.md`
- `memory/08-tasks/done/20260615-im-node-qr-ui.md`
- `memory/08-tasks/done/20260615-im-openmls-protobuf-ffi.md`
- `memory/08-tasks/done/20260615-im-remove-rpc.md`
- `memory/08-tasks/done/20260615-im-wallet-routing-node-mode.md`
- `memory/08-tasks/done/20260615-cid-village-unify-cun-lu-classify.md`
- `memory/08-tasks/done/20260616-admin-district-single-source.md`
- `memory/08-tasks/done/20260618-admin-district-dev-db-authority.md`
- `memory/08-tasks/done/20260618-fresh-genesis-yl-cleanup.md`
- `memory/08-tasks/done/20260618-cid-admin-district-management.md`
- `memory/08-tasks/done/20260618-cid-gov-admin-division-reconcile.md`
