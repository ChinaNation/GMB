...
- 删除 `votingengine::snapshot_institution_admins`、`InternalAdminProvider::get_institution_admins` 及两项旧机构管理员快照测试；同步清理 runtime 生产接线和各 crate 测试 mock。`AdminSnapshot` 不再存在机构写入入口，只服务个人多签。
