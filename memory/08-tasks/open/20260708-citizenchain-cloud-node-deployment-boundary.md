...
- 2026-07-10：完成步骤 3 的 Cloudflare 控制面配置。创建 `CitizenChain RPC` 自托管 Access 应用和仅包含指定 Service Token 的 `Service Auth` 策略；既没有 `Everyone`，也没有 `Allow` / `Bypass` 分支。现有远程管理 Tunnel `nrcgch-rpc` 的唯一业务 hostname 固定为 `nrcgch-rpc.crcfrcn.com -> http://127.0.0.1:9944`，catch-all 继续返回 `404`，没有为 Oracle 主机增加 `9944` 入站规则。
- 2026-07-10：staging Worker 已配置 `CHAIN_URL`、`CHAIN_ID`、`CHAIN_SECRET` 三项 Secret，并删除废弃的 `SQUARE_CHAIN_RPC_URL`。本地类型检查和 18 个测试文件共 103 项测试通过，随后部署 staging 版本 `cb89091f-c393-4e66-a3ad-703bdca91844`；`/health` 与 `/v1/chain/bootstrap` 均返回 200，bootstrap 明确 `exposes_rpc_url=false`、`rpc_proxy=false`、`validator_rpc_public=false`，relay 因 `CHAIN_EXTRINSIC_RELAY_ENABLED=0` 保持关闭。
...
- 2026-07-12：重新实查控制面确认 `nrcgch-rpc` Tunnel 已健康并运行 1 个 connector，但唯一发布路由已变为 `chain.crcfrcn.com -> http://127.0.0.1:18080`；原 `nrcgch-rpc.crcfrcn.com` 无 DNS，当前不存在到 `127.0.0.1:9944` 的私有 RPC 路由。App 交易由 smoldot 轻节点通过 P2P 提交，与 Worker 私有 RPC 是不同链路；因此 App 能发交易不能替代步骤 5、6 的 Worker 链读取和广场入块确认验收。
