...
- [ ] 提案列表:复用 `DuoqianTransferProposalFeed.currentYearProposals`(卡①),按 institutionBytes==派生主账户 过滤。
...
- [ ] 提案过滤 institutionBytes 命中本机构;管理员列表渲染。
- [ ] 订阅 toggle 写入/移除;非管理员隐藏发起入口。

## 验收
- [ ] flutter analyze 0 + flutter test 全过。
- [ ] 真机:国储会外某市公权机构详情显示名称/ID/主+费余额/更多账户/提案/管理员;订阅后出现在关注;余额走 finalized,无轮询。
...
## 改动目录(中文注释)
- 新增 `wuminapp/lib/citizen/public/public_institution_detail_page.dart` + 更多账户/管理员列表子页,代码。
- 改 `wuminapp/lib/governance/shared/`:抽取共享详情 widget(账户卡/余额行/管理员/提案列表),代码。
