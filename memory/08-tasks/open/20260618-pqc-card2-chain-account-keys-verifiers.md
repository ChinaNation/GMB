...
3. **bootstrap**:
   - 🔴 **(B1)challenge 钉死**：bootstrap 先登记独立 `op_tag`，再构造 `sr25519_sign(signing_message(op_tag,SCALE(genesis,spec,tx_version,account,pqc_pubkey_hash,key_version,nonce,call_hash,following_extensions_hash)))`；sr25519 **必须覆盖 pqc_pubkey_hash**；ML-DSA 反向覆盖 `blake2_256(sr25519_sig)`(双向交叉绑定)。
