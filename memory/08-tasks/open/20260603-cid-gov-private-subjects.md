任务需求：
CID 机构架构按精简命名拆分，前后端统一使用 `gov` 表示公权机构，保留现有 `admins` 权限模块，不新增 `registry_admins`。公民继续使用 `citizens` 命名，智能人当前不上线，不建设智能人表。数据库从第一版目标结构开始按省分区设计，不再写“数据大了以后再分区”。

所属模块：
CID

必须遵守：
- 机构唯一身份只认 `cid_number`，不得新增 `identity_key`、`generation_key` 或第二身份键。
...
预计修改目录：
- `citizencode/backend/subjects/`：身份主体索引与分区表结构说明，涉及代码。
- `citizencode/backend/gov/`：公权机构模块外壳与接口归属，涉及代码。
- `citizencode/backend/private/`：私权机构模块外壳与接口归属，涉及代码。
- `citizencode/backend/accounts/`：机构账户模块外壳与接口归属，涉及代码。
- `citizencode/backend/docs/`：机构资料库模块外壳与接口归属，涉及代码。
- `citizencode/backend/admins/`：保留现有权限能力，不新增注册局管理员目录。
- `citizencode/backend/subjects/`：身份主体共享模型、公共详情、非法人能力和残留清理，涉及代码。
- `citizencode/frontend/subjects/`、`citizencode/frontend/gov/`、`citizencode/frontend/private/`、`citizencode/frontend/accounts/`、`citizencode/frontend/docs/`：前端功能目录拆分，涉及代码。
- `memory/05-modules/citizencode/`：同步架构、数据库分区、前后端目录边界，涉及文档。
...
- 已更新 `citizencode/frontend/tsconfig.json`，显式覆盖新前端目录。
...
- 已将 `gov/private/accounts/docs/subjects` 重复的 HTTP helper 抽到 `citizencode/backend/subjects/http.rs`,并清理旧复制函数。
- 已将公权/私权新增弹窗表单抽到 `citizencode/frontend/core/institution/CreateInstitutionForm.tsx`,`gov` 与 `private` 仅保留 API 包装。
- 已修复注册局页签中“联邦注册局机构管理员列表”首次点击被加载流程重置回市列表的问题。
- 已更新 CID 后端布局、前端布局、技术总览、统一命名、统一协议和相关活跃任务/决策文档。
