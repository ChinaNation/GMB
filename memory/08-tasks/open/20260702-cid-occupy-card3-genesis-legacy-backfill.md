...
> 设计真源:`memory/04-decisions/ADR-031-cid-occupy-registry.md`(D5/D9/D4/D0)。2026-07-04 部署口径更新:**市/镇级同样创世直铸,与 NRC/PRC/PRB/FRG/NJD 同一模式,全程零交易**;全节点使用 plain spec + 官方创世状态包,正式安装包首启复制 `genesis-state/` 链数据库;GenesisBuilder 仅开发/排障兜底。CitizenApp 使用 `stateRootHash` 轻形态 + 公权机构创世快照缓存 + 链投影增量。
...
4. **部署形态改造(必做)**:起始 state ≈0.6-0.8GB,raw chainspec(~1.5GB)不可行——节点从「raw include_bytes! 全量 state」改「plain spec + 官方创世状态包」;正式安装包内置 `genesis-state/`,首启复制链数据库并等待 RPC ready;CitizenApp/smoldot 侧 chainspec 用 `stateRootHash` 轻形态。
...
- **剩余 = 部署操作(用户 重新创世 步)**:①chainspec 形态改造(plain spec + genesis-state 创世状态包;smoldot 侧 stateRootHash)②重新创世部署(6 节点 mesh)③onchina 链上机构册只读投影 ④citizenapp 公权机构快照包重跑 ⑤旧 sfid 库删除。genesis-pallet 自身 test mock 为上一轮 runtime 重构预留断链(缺 public_manage/public_admins 等 impl),故派生断言落 primitives(编译/测试均绿);全量 state 物化在 bake 阶段发生,正式用户首启复制创世状态包。
...
- **体积实测(按真源逐条、按存储项真实编码)**:596,799 机构创世 state 原始 KV ≈ **542MB**(每机构 ≈0.95KB:Institutions 1 条+主/费账户各 3 条索引);RocksDB 落盘含 trie/压缩 ≈ **0.8-1.4GB**;这部分在 bake 阶段一次性物化并打进 genesis-state,正式用户首启不再重复全量构建。
...
1. **node 部署形态改造(D5 必做)**:chain_spec.rs 仍为 raw include_bytes(59.7 万机构下 raw JSON 会到 GB 级)→ 改 plain spec + genesis-state 创世状态包;CitizenApp/smoldot 侧 stateRootHash 轻形态。
...
- A3 bake 行为:`bake-chainspec.sh` 启动临时节点经 WASM `GenesisBuilder_build_state` 物化 596,799 机构(596k 号派生 + ~420 万存储项),导出 `target/chainspec/genesis-state/`。
...
①`bake-chainspec.sh --finalize --wasm <CI_WASM>` 重生 plain SSOT、CitizenApp 轻形态和 genesis-state → 提交;②prepack 内置 genesis-state、6 节点部署、`chain_getBlockHash(0)` 全网一致核对;③citizenapp 真机 smoldot 验证 + 公权机构快照包重跑;④onchina 各节点链投影同步通过 + `audit-chain-catalog` 全量比对;⑤删本机与各节点旧 `sfid` 库;⑥公民建档占号端到端 walkthrough。
