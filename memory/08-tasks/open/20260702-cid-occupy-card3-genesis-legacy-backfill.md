...
- genesis 直铸 282 + 49,299 = 49,581,构建期断言全过;genesis 测试断言数量与「常量数组 + 国家/省/市×码表」推导值逐一一致。
...
- 创世后链上 Institutions 总数 = 49,581(常量 282 + 模板派生 49,299),与推导值一致(genesis 测试断言);内置号 100% 通过 parse 校验。
...
- 2026-07-03:**代码核心完成并测试通过**(primitives 45 测试全绿、genesis lib + runtime + onchina 编译过、runtime 30 lib 测试过、onchina 134 测试过):
  ...
  - **测试**:primitives 断言派生数 =49,299、每号 parse 合法+公权家族+全局唯一、名称组装、国家两院国名前缀、区划计数 43/2872/39087。终态链上 = 282 + 49,299 = **49,581**,零交易。
...
- **体积口径更新**:镇级机构不进创世,移出曾导致首启重物化的 547,218 条;正式 bake 仍产 genesis-state,但创世规模降为 49,581 机构。
...
3. **测试补课**:genesis-pallet 自带测试 mock 既有断链(Config 现要求 public_manage/public_admins 全栈);runtime 级 49,581 总数断言需随本轮更新。
...
- A3 bake 行为:`bake-chainspec.sh` 启动临时节点经 WASM `GenesisBuilder_build_state` 物化 49,581 机构,导出 `target/chainspec/genesis-state/`。
...
### D. 测试补课
- D1 runtime 级总数断言:runtime tests 调 `genesis_pallet::institution::build::<Runtime>()` 后 `Institutions::iter().count()==49,581` + 抽查(市级名称组装、账户派生、NJD/FRG 管理员在位;镇级走运行期注册)。
...
5. 终态验收:链上创世 Institutions=49,581、创世哈希一致、onchina 全量比对 CLI 通过、CitizenApp 机构册可读、公民建档占号端到端(真链)walkthrough。
...
- **旧首启物化冒烟实测已废弃**:旧方案的首启重物化数据不再作为验收依据;本轮改为 49,581 机构后必须用 CI WASM 重新 bake。
...
- **D1 runtime 全量断言抓到真缺口并修复**:旧断言曾抓出常量机构漏铸;本轮总数应更新为 49,581(含派生首条逐字节抽查+NJD 管理员在位),plain spec 需用 CI WASM 重生。
...
①`bake-chainspec.sh --finalize --wasm <CI_WASM>` 重生 plain SSOT、CitizenApp 轻形态和 genesis-state → 提交;②prepack 内置 genesis-state、6 节点部署、`chain_getBlockHash(0)` 全网一致核对;③citizenapp 真机 smoldot 验证 + 49,581 创世公权机构快照包重跑;④onchina 各节点链投影同步通过 + `audit-chain-catalog` 全量比对;⑤删本机与各节点旧 `sfid` 库;⑥公民建档占号端到端 walkthrough。
