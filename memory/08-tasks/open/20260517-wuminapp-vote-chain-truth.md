任务需求：
修复 wuminapp 投票提交后的等待状态：投票是否成功必须以 runtime 投票引擎链上记录为准，不能由 txHash、交易池 watch 或 nonce 超时直接判定。
...
预计修改目录：
- wuminapp/lib/votingengine/internal-vote/：修复待确认投票存储的确认口径，属于 Flutter 投票引擎客户端代码。
- wuminapp/lib/rpc/：修正 nonce-only 交易状态注释，避免继续把 `checkTxStatus` 误读成 txHash 级成功确认，属于 Flutter 链 RPC 注释清理。
- wuminapp/lib/governance/：修复多签管理和 runtime 升级投票详情页的 pending / watch 状态处理，属于 Flutter 治理 UI 与服务调用代码。
- wuminapp/lib/transaction/duoqian-transfer/：修复多签转账提案投票详情页的 pending / watch 状态处理，属于 Flutter 交易治理代码。
- memory/01-architecture/wuminapp/：同步 wuminapp 总体投票状态真源规则，属于文档更新。
- memory/05-modules/wuminapp/：同步 wuminapp 治理和多签转账技术文档，属于文档更新。
...
新增命名说明：
- 中文名：待确认投票链上查询回调；English name：PendingVoteChainLookup；类型：Dart typedef；使用位置：`wuminapp/lib/votingengine/internal-vote/pending_vote_store.dart`；简介：允许不同投票类型传入对应的 runtime 投票记录查询逻辑。
...
- `cd wuminapp && dart analyze lib test`：通过，No issues found。
