...
- `citizencode/backend/main.rs`：DB 层新增 `get_federal_registry_with_accounts`（按 `org_code='FEDERAL_REGISTRY'` 查）；路由注册 `GET /api/v1/institution/registry/federal`。
- `citizencode/backend/subjects/admin.rs`：新增 handler `get_federal_registry`（require_admin_any，不做 scope，返回 InstitutionDetailOutput）。
- `citizencode/frontend/gov/api.ts`：新增 `getFederalRegistry`。
- `citizencode/frontend/gov/GovDetailPage.tsx`：新增可选 props `adminListSection` / `loadDetail` / `backLabel`，onBack 改可选；普通机构零行为变化。
- `citizencode/frontend/gov/GovListTable.tsx`：GOV_INSTITUTION 列表过滤掉注册局 org_code。
- `citizencode/frontend/subjects/labels.ts`：导出 `REGISTRY_ORG_CODES` 单一源。
- `citizencode/frontend/admins/RegistryAdminsView.tsx`：解析市注册局 cid、加载联邦注册局 detail。
- `citizencode/frontend/admins/adminUtils.ts`：RegistryAdminsSharedState 增字段。
- `citizencode/frontend/admins/ProvinceDetailView.tsx`：子 tab 改名 + leaf 换成机构详情页（避免 Card 套 Card）。
...
- 后端 `main.rs` DB 层新增 `get_federal_registry_with_accounts()`：按 `org_code='FEDERAL_REGISTRY'` 定位唯一机构 cid 后复用 `get_institution_with_accounts`。
...
- 路由注册 `GET /api/v1/institutions/federal-registry`（放在 `/institutions/` 段，结构上避开 `:cid_number` 动态段冲突）。
...
- 前端 `admins/adminUtils.ts`：`RegistryAdminsSharedState` 增 4 字段（federalRegistryDetail/Loading、cityRegistryCid/Loading）。
- 前端 `admins/RegistryAdminsView.tsx`：新增两个 effect——挂载时加载联邦注册局 detail（scope-bypass）；按活动省市解析市注册局 cid（从 official 目录筛 CITY_REGISTRY）。
...
- `cd /Users/rhett/GMB/citizencode/backend && cargo check` 通过（exit 0）。
- `cd /Users/rhett/GMB/citizencode/backend && cargo test` 通过（52 passed / 0 failed）。
- `cd /Users/rhett/GMB/citizencode/frontend && npm run build`（tsc -b + vite build）通过，0 type error。
...
### 修复 + 改动
1. **后端联邦注册局按 cid_number 定位**（robust，不依赖 org_code / 不需重新对账）：
   - `gov/service.rs` 新增 `federal_registry_cid_number()`：从 china_zf 常量找「总统府联邦注册局」取 cid_number。
...
- 注意:联邦注册局机构在 subjects 表 org_code 仍是 `PUBLIC_ORG`(本次按 cid_number 定位绕过),故 GovListTable 的 FEDERAL_REGISTRY 过滤对它不生效——中枢联邦注册局机构管理员的「公权机构」列表里它仍会以"公权机构"出现(仅影响中枢省;5 个总统府联邦局 org_code 缺映射是既有问题,未扩边)。如需一并清掉,后续单列任务补 `org_code_for_constant_name` 的 5 个联邦局分支 + reconcile 重跑。
...
### 背景
user 发现不止注册局，连「中华民族联邦共和国住房与城镇建设部」等**所有国家级机构**都显示「政府/公权机构」。根因升级：`org_code_for_constant_name` 靠机构名字猜 org_code，但常量存的是**全名**（且经历改名 省政府→省联邦政府、省储备委员会→省公民储备委员会），简称/旧后缀全对不上 → 国家级 + 部分省级全落 `PUBLIC_ORG`。能正常显示细类的（公民安全局、省立法院…）都是区划模板写死 org_code 或后缀恰好对上的。私权机构不受影响（不用 org_code，走 机构代码+企业类型）。user 选**方案 A**（只动 CID，不碰链）。
...
2. **`gov/service.rs` 升 `GOV_TEMPLATE_VERSION` v3→v4**：使 `ensure-gov` 不再 skip（manifest_ready 看 template_version）→ 跑 `check_gov_catalog_db`（[:1065](citizencode/backend/gov/service.rs) `org_code != target.org_code` 会判定 mismatch）→ ok=false → 走 `reconcile_official_institutions_explicit(All, force_row_sync=true)` 全量重写 org_code。catalog hash 也含 org_code（[:828](citizencode/backend/gov/service.rs)）。
