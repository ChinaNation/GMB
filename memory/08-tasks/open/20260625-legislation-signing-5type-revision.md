...
2. **否决/超时救济**:
   ...
   - 特别案例外:特别案经立法公投通过即直接生效、不通过即否决,**任何人不再签署**。
...
5. **提案机构→表决院路由**(委员/admin 直接提案):
   | 提案机构 | 层级 | 可发起类型 | 表决院序列 |
   |---|---|---|---|
   | 国家立法院众议会 | 国家 | 常规/重要/特别 | [国家立法院众议会→国家立法院参议会] |
   | 国家教委会 | 国家 | 常规教育/重要教育 | [国家教委会→国家立法院参议会] |
   | 省联邦立法院众议会 | 省 | 常规/重要/特别 | [省联邦立法院众议会→省联邦立法院参议会] |
   | 市立法会/市自治会 | 市 | 常规/重要/特别 | [市立法会] 单院 |
   | 市教委会 | 市 | 常规教育/重要教育 | [市立法会] 单院 |
   - 链上校验:提案机构 ⟺ 可发起类型一一对应;表决院由提案机构推导;市级提案方≠表决院(直接进市立法会单院)。
6. **选区**:仅特别案有立法公投,按立法机构 全国/省/市。提案两字段=`title`(提案名称,发起人输入)+`vote_type`(提案类型),链端均已有。
7. **法定代表人（2026-07-13 修订）**：法定代表人是机构公开信息，不强制属于该机构 admins；姓名、CID、钱包账户由 entity `InstitutionInfo` 保存，签署人账户从该唯一真源查询。
...
- [x] **B 链端(2026-06-25 全部完成,测试通过零回归)**:
  - [x] **B1 五类法律业务 + 三类数学规则**：`legislation-yuan::VoteType` 保留五类业务，映射到 `RepresentativeVoteRule::{Regular,Major,Special}`；教育变体只决定路由。签署阶段保留 `STAGE_LEG_SIGN=12`/`STAGE_LEG_OVERRIDE=13`。
  - [x] **B3 法定代表人（2026-07-13 已重构）**：删除 admins 的旧法定代表人副本存储、setter/getter 和首位管理员回退；法定代表人姓名、CID、账户迁入 public/private entity `InstitutionInfo`，runtime 签署查询统一读取 entity。
...
2. **法定代表人=机构公开字段**：`InstitutionInfo` 保存姓名、CID、钱包账户三字段，不要求属于 admins；签署时查询该机构当前公开的法定代表人账户。
3. **立法院为上级机构**(宪法第71条:国家立法院设院长,由参议员互选):立法院下设参议会和众议会,自身有法定代表人(院长)。三人会签=立法院(院长)+立法院参议会(参议长)+立法院众议会(众议长)三方法定代表人。

B2/B3 实现要点:
- 签署状态机:非特别案内部全过 → `STAGE_LEG_SIGN`(executive 机构法定代表人签署,30天)。市级:签署=EXECUTED / 否决=REJECTED / 30天超时=EXECUTED(通过)。省国级:签署=EXECUTED / 否决或30天超时 → `STAGE_LEG_OVERRIDE`(立法院+立法院参议会+立法院众议会三方法定代表人,30天):三签同意=EXECUTED / 任一否决或超时=REJECTED。特别案:公投通过即 EXECUTED,不进签署。
...
- 提案携带签署机构：`create_legislation_vote` 接收 `LegislationProcedureConfig`，Law/summary 保存 executive/legislature 业务字段。
...
- [~] **E 收尾**(2026-06-25 重新拆分):
  ...
  - **[x] 统一重生 scale 批(2026-06-25 完成并验证)**:
    ...
    - **储委会节标题改简称(2026-06-25)**:第四章(储委会)第二/三/四节目录由全称改简称——国家公民储备委员会→**国家储委会**、省公民储备委员会→**省储委会**、省公民储备银行→**省储行**(与第一节「储委会联合会议」一致);中英+TOC+节标题同步,正文全称保留;不在第一章故不可修改条款不受影响;scale 重生 **218995 字节**;legislation-yuan 23 测试过。
  - 全链端到端真机 QA(重新创世后)。
