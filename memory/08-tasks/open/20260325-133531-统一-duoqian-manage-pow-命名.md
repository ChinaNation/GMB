# 任务卡：统一 duoqian-manage-pow 命名

- 任务编号：20260325-133531
- 状态：open
- 负责人：Codex

## 1. 任务目标

把 GMB 仓库中的多签机构管理模块相关命名统一为 `duoqian-manage-pow`、
`duoqian_manage_pow`、`DuoqianManagePow`，并同步更新 runtime、外部系统、
文档和任务记录，避免出现旧命名残留和新旧混用。

## 2. 影响范围

- `citizenchain/runtime/transaction/duoqian-manage-pow/`
- `citizenchain/runtime/src/lib.rs`
- `citizenchain/runtime/src/configs/mod.rs`
- `citizenchain/runtime/src/benchmarks.rs`
- `citizenchain/runtime/governance/voting-engine-system/src/internal_vote.rs`
- `citizenchain/runtime/transaction/duoqian-transfer-pow/`
- `sfid/backend/src/super-admins/institutions.rs`
- `memory/05-modules/`
- `memory/08-tasks/`
- `tools/duoqian.py`

## 3. 关键约束

- 不改变现有业务语义，只做统一命名。
- 需要同步更新 runtime pallet 名，保持代码、后台与文档命名一致。
- 改代码后必须同步更新模块文档和任务卡。

## 4. 执行计划

1. 重命名目录、crate 名和 Rust 模块引用。
2. 更新 runtime pallet 别名与相关 call/type 引用。
3. 更新 SFID 后台固定 pallet 名字符串。
4. 更新模块文档、白皮书、工具说明和任务卡引用。
5. 运行最小验证并回写结果。

## 5. 验证要求

- `cargo check -p citizenchain`
- 关键 crate 至少能通过编译
- `git diff --check`

## 6. 执行结果

- 已把 `citizenchain/runtime/transaction/duoqian-manage-pow/` 目录、crate 名和所有 Rust 引用统一为新命名。
- 已把 runtime pallet 别名统一为 `DuoqianManagePow`。
- 已同步更新 SFID 后台固定 pallet 名字符串，避免后台继续提交到旧 pallet 名。
- 已同步更新模块文档、白皮书、工具说明、任务卡和索引中的旧命名引用。
- 已确认主工作树源码与 `memory/` 下不再残留旧命名字符串。

## 7. 验证结果

- `cargo check -p citizenchain`：通过
- `cargo test -p duoqian-manage-pow`：通过
- `git diff --check -- citizenchain sfid memory tools`：通过
