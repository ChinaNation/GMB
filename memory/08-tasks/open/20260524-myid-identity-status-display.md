任务需求：
wuminapp 电子护照页面展示身份ID号码与身份ID状态，并将“绑定账户”改为“投票账户”；SFID 状态接口补充身份ID状态。当前任务只完成 wuminapp 与 SFID 的直接绑定展示闭环，不做投票拦截。

状态：
已执行

所属模块：
wuminapp 电子护照 / SFID 公民身份绑定
...
- 当前任务不做 wuminapp 投票拦截、SFID 公民数量拦截、SFID 投票凭证拦截、链上投票引擎快照拦截。
- 改代码后同步更新文档、完善必要中文注释、清理残留。

输出物：
- SFID 公民记录保存身份ID状态。
- SFID wuminapp 状态接口返回身份ID号码与身份ID状态。
- wuminapp 电子护照页展示身份ID、投票账户、身份ID状态。
- 相关测试、文档和残留清理。
...
- 已新增 wuminapp 页面测试覆盖身份ID、投票账户和正常状态展示。
