...
- 前端：调整 `citizenpassport/frontend/dangan/ArchiveList.tsx` 列结构和整行点击。
- 前端：调整 `citizenpassport/frontend/dangan/ArchiveDetail.tsx` 导航图标、操作记录列、编辑按钮位置和资料库上传弹窗。
- 前端：增强 `citizenpassport/frontend/common/http.ts` 对非 JSON 响应的错误提示。
- 后端：完善 CPMS API 兜底和档案操作记录返回字段。
- 文档：更新 CPMS 技术文档与 dangan 模块文档。
- 前端：按当前确认，仅同步 CID 机构详情共享导航的“返回列表”图标，其他 CID 业务不在本卡范围内。
- 残留：清理旧操作列、旧详情按钮、资料库常驻上传表单和重复数字展示。

## 不做

- 不修改 CID 业务流程、接口、数据结构和详情 tab 组织方式。
...
- 2026-06-13：CPMS 公民详情左侧 tab 图标按 CID 机构详情共享导航语义对齐为房子、文件夹、历史记录；CID 共享导航“返回列表”图标统一为 CPMS 当前返回图标。
...
- 2026-06-13：`cd citizenpassport/frontend && npm run build` 通过。
- 2026-06-13：`cd citizenpassport/backend && cargo fmt --check` 通过。
- 2026-06-13：`cd citizenpassport/backend && cargo check` 通过。
- 2026-06-13：`cd citizenpassport/backend && cargo test` 通过，32 个测试全部通过。
- 2026-06-13：临时启动 `CPMS_BIND=127.0.0.1:18080` 后端并服务 `citizenpassport/frontend/dist`，`GET /api/v1/health` 返回 200。
...
- 2026-06-13：再次执行 `cd citizenpassport/frontend && npm run build` 通过。
- 2026-06-13：执行 `cd citizencode/frontend && npm run build` 通过；仅有既有 chunk 体积提示。
...
- 2026-06-13：调整有效期与选举资格排版后，再次执行 `cd citizenpassport/frontend && npm run build` 通过。
