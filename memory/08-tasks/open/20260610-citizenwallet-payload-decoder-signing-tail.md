# 任务卡：citizenwallet 公民钱包扫码解码器签名扩展尾校验修复

## 背景(根因已查实)

桌面端发起国储会转账提案(`propose_transfer`, 19.0, org=0),citizenwallet 扫码红色"无法独立验证交易内容,禁止签名"。
...
- QR 的 `payload_hex` 是**完整 SigningPayload**:call_data 后永远跟 ≥77 字节扩展尾
  ...
  (节点端 `citizenchain/node/src/governance/signing.rs:628-677`;citizenapp 走 polkadart `signingPayload.encode` 同布局)。
...
- [ ] 端到端:桌面端国储会转账提案 → citizenwallet 扫码绿色 → 签名回传上链(user 真机验证)
