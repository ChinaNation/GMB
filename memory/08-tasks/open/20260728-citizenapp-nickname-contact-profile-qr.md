...
- 通讯录模型同时保留公开昵称索引、私人备注、`ss58_address`、`cid_number` 与内部
  `account_id`；卡片和搜索只呈现公开昵称、私人备注与公民号，账户地址不进入展示或搜索。
- 联系人 `cid_number` 必须进入端到端加密载荷并支持跨设备恢复。
- 用户主页头像下依次展示公开昵称、公民号、签名和三项计数；账户地址只保留在用户码等
  明确账户边界，主页不展示或复制。
...
### Step 4：用户主页身份信息布局收口（2026-07-28，展示边界于 2026-08-07 更新）

- 用户主页头像下收口为公开昵称、独立“公民号”行、关注/关注者/
  帖子三项计数；公开昵称继续只消费 `display_name` 或按 CID 稳定生成的本地兜底，
  不读取钱包名。
- 主页不展示或复制原始 `account_id` / SS58；资料模型继续保留规范 AccountId，供签名、
  授权和用户码使用。
- 公民号固定使用页面路由的 `cid_number` 身份真源，不提供复制按钮；资料尚未加载时显示
  “公民号：暂不可用”。
- 展开头部高度调整为 344，为昵称、公民号、签名、计数和可选订阅按钮预留空间；
  320px 宽度 Widget 验证无布局溢出。
- 主页专项 Widget 测试覆盖公开昵称兜底、公民号、账户地址零展示/零复制、资料模型仍保留
  AccountId、窄屏布局及原有主页交互。
