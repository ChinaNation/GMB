...
护宪大法官终审(F)链端状态机已落地。本卡当前目标态是:管理员集合统一为 `AdminProfile{account, admin_cid_number, name, admin_role, term_start, term_end, source}`;护宪生产成员由 `RuntimeInternalAdminProvider::constitution_guard_members()` 从国家司法院 `NJD` 的 active admins 中过滤 `admin_role=护宪大法官` 取得,且成员数必须恰好 5 人。国家司法院进入创世机构,创世治理账户固定 13 人,角色为 5 名护宪大法官、1 名首席大法官、2 名次席大法官、5 名大法官,内部治理固定阈值为 8/13。

## 已确认范围

- **E1 管理员字段扩展**:机构 admins 记录从「账户/SS58 单值」扩为 `AdminProfile` 结构;个人多签仍为 `AccountId`。
- **E2 护宪生产成员解析**:用 `AdminProfile.admin_role` 实现 `constitution_guard_members()` 生产体——读国家司法院 `NJD` admins,过滤 `admin_role=护宪大法官`,成员数必须恰好 5 人。
- 同一 `admin_role` 字段后续可复用于权责划分(如 NJD 全部 admins 中划出 5 名护宪大法官)与 B3 法定代表人(职务=机构首脑)对齐。
...
5. **B3 法定代表人对齐**:`LegalRepresentatives` 仍表达签署人/机构首脑资格;`admin_role` 表达管理员公开职务,二者不互相派生。
...
2. 逻辑:取 NJD 机构 admins → `filter(admin_role == 护宪大法官)` → 必须恰好 5 人。
3. NJD 机构码常量走单一源(china 常量库);护宪大法官职务取值走 `ADMIN_ROLE_CONSTITUTION_GUARD` 单一源。
4. 测试:mock 注 `admin_role` 字段,验证生产解析正好取出 5 名护宪大法官;非护宪职务 admins 不入选;数量异常按硬约束处理。
...
- 整 runtime check 与真机:留 E3(重新创世与 `genesis-manage` 线程合流后)统一验,本卡不跨线程代改。

## 已关闭问题

1. **字段名**:管理员公开职务字段统一为 `admin_role`,不得再使用 `title`。
2. **护宪人数策略**:NJD 中 `admin_role=护宪大法官` 必须恰好 5 人;不足、超出或重复均视为无效成员集。
3. **legal_representative 与 admin_role**:`LegalRepresentatives` 表达签署资格;`admin_role` 表达公开职务,当前不互相派生。
...
- [x] E2 护宪 5 人 + 3 名及以上赞成/反对规则
- [x] NJD 创世机构 + 13 名创世治理账户角色 + 8/13 固定阈值
- [x] 聚焦编译 + 单测 + 残留扫描
