...
- 2026-07-19：正式创世前统一改造第5步完成。统一 QR 字段注册表删除旧合并姓名字段并登记 `admin_account / family_name / given_name`；个人多签创建、公权/私权机构创建、注册局登记管理员和个人多签管理员更换均按同一三字段 SCALE 顺序严格解码，机器值使用三个精确字段，确认页才按中文顺序合并姓名。旧纯账户数组、旧合并姓名、重复账户、空姓名和非法 UTF-8 全部拒签。
