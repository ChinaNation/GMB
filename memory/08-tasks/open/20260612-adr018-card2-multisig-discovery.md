...
属 ADR-018 §九(2026-06-13 架构修订)。原标题"listCidAccounts 长前缀整表化"已废弃。
...
- [x] 机构账户命名走 `AccountRegisteredCid` **精确批量反查**(新增 `fetchRegisteredInstitutionRefsBatch`),不正向枚举。
- [x] 个人多签元数据同样批量反查(新增 `fetchPersonalMetasBatch`),消除个人侧 N+1。两处批量均加"整体失败不误删孤儿"保护。
- [x] 删除死代码:`listCidAccounts` + 孤儿 `_utf8Decode` + 无用 `smoldot_client` import;`fetchRegisteredInstitutionRef`(单)/`fetchPersonalMeta`(单)被批量取代后亦删除。
- [x] 列表页 `_runBackgroundDiscovery` 改调协调器一次(替代两服务各调一次);进度文案合并为"多签扫描"。
- [ ] **(顺延卡⑤)** 列表余额批量化 + ChainReadCache:发现路径只填 Isar 实体(name/citizencode/admins),余额读属卡⑤ 范畴,本卡不引入缓存层。
...
- [x] 旧代码/文档/注释清理无残留(`listCidAccounts`/`discoverByMyWallets` 零引用)
