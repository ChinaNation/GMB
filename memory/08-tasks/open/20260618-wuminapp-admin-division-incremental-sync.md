# 任务卡:wuminapp 行政区+公权机构 版本驱动增量同步

让 wuminapp「公民-公权」的行政区/机构数据随真源(china.sqlite)版本更新而增量刷新:**变的换、删的清、没变的不动**,零旧数据残留;wuminapp 无服务端,靠 assets 包分发。

## 现状(用户已建的源头侧,勿重造)
- `sfid/backend/china/china.sqlite`(已从 china/data/ 上移到 china/):`metadata.admin_division_version`(全局单调版本,现=3)+ `admin_division_versions`(版本登记)+ `admin_division_change_log`(每版精确变更 MERGE/RESTORE/RENAME,带 old/new 名+码)+ city/town_tombstones。
...
- reconcile(事务内,按主键集合):先读取本省现有实体,逐条比字段,只 upsert 新增/字段变化的行;删「包里没有、Isar 还在」的 key(divisionKey / sfidNumber)。没变的行不动。
- store 补:`divisionsOfProvince/institutionsOfProvince` + `deleteByKeys/deleteBySfids`。
...
- **改动**:18 改 + 3 新(全在 `wuminapp/lib/citizen/public/data` + test + tools + 1 manifest),零外溢。
- **资产包**:已用真实 SFID 本地接口重跑 `generate_public_institution_bundle.mjs --version 3`;公开包 43 省、243520 条可见公权机构。城市级 44115 条、镇级 198655 条名称前缀扫描错位 0。
