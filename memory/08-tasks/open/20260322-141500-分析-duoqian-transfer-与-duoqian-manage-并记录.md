# 任务卡：分析 duoqian-transfer 与 duoqian-manage 并记录 Pending 残留风险
...
基于源码全面分析 `duoqian-transfer` 与 `duoqian-manage` 的实际功能实现，澄清它们与内部投票引擎的关系，并把 `propose_create` 预写 `Pending` 状态但缺少拒绝/超时清理路径的问题固定到任务卡和模块文档中。
...
- 新补充的内置机构 `ZF / LF / JC / JY / SF` 后续是全部接入 `duoqian-transfer`，还是只接其中一部分。

## 实施记录

- 已确认：`duoqian-transfer` 当前只支持内置治理机构 `NRC / PRC / PRB` 转账，不支持注册型 `ORG_DUOQIAN`，也尚未接入 `ZF / LF / JC / JY / SF`。
- 已确认：`duoqian-transfer` 的转账审批实际就是复用 `voting-engine` 的内部投票能力；只要机构类型被本模块识别，并接入 runtime 管理员/阈值提供器，就可以走内部投票转账，不需要新增转账模块。
