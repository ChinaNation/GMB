# 任务卡：CID 公权机构按行政区划名录重构与公安局业务入口校正

## 任务需求

按最新边界重构 CID 公权机构设计与实现：公权机构不做上下级机构，国家/部/省/市/镇只作为目录分类和行政区范围；公安局 tab 只作为市公安局业务入口，因为公安局需要承接 CPMS 安装码和身份码业务；CPMS 安装码只能由联邦注册局机构管理员签发；旧省域管理员对外改为联邦注册局机构管理员，市注册局机构管理员保持原名；机构列表只展示简要名称，详情页展示全称和简称；代码完成后更新文档、补中文注释、清理残留。
...
- CPMS 安装码只能由联邦注册局机构管理员签发。
- `citizencode/backend/citizenpassport` 核心业务不重构。
- 旧省域管理员对外改为联邦注册局机构管理员；市注册局机构管理员不改名。
...
“生成 CPMS 安装码”按钮只给联邦注册局机构 admins 显示。市注册局机构 admins 不能签发，只能按权限查看或办理本市业务。

## 管理员权限

- `registry_org_code=FEDERAL_REGISTRY`：联邦注册局机构 admins。
- `registry_org_code=CITY_REGISTRY`：市注册局机构 admins。
...
说明：底层只允许使用注册局机构码和 `admins` 真源，不得恢复旧管理员权限值或旧省域管理员名称。
...
- `citizencode/backend/gov`
- `citizencode/backend/admins`
- `citizencode/backend/subjects`
- `citizencode/backend/core`
- `citizencode/frontend/gov`
- `citizencode/frontend/admins`
- `citizencode/frontend/china`
- `citizencode/backend/citizenpassport`
- `memory/01-architecture/citizencode`

## 预计修改目录

- `citizencode/backend/china`：读取镇行政区划，作为 `town/town_code` 和镇目录公权机构生成输入，涉及代码。
- `citizencode/backend/gov`：公权机构初始化、对账、列表、详情和公安局聚合接口，涉及代码。
- `citizencode/backend/admins`：旧省域管理员权限对外改名为联邦注册局机构管理员，市注册局机构管理员不改名，涉及代码和用户可见文案。
...
- 不得改变既有机构或个人的 `cid_number`。
- 列表不得同时显示全称和简称。
- 不得把初始化来源作为机构业务字段。
- 不得重构 `citizencode/backend/citizenpassport` 核心业务。
...
- 2026-06-06：完成联邦注册局机构管理员文案、角色值、CPMS 权限说明和 CID 文档同步；残留扫描未发现旧角色名、旧公安局方案、旧初始化来源业务字段残留。
...
- 2026-06-06：验证通过 `cargo fmt --manifest-path citizencode/backend/Cargo.toml`、`cargo check --manifest-path citizencode/backend/Cargo.toml`、`cargo check --tests --manifest-path citizencode/backend/Cargo.toml`、`npm run build --prefix citizencode/frontend`。
