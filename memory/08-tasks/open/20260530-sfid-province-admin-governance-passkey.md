# SFID 联邦管理员治理 Passkey 与公民钱包签名第 1 步任务卡
...
- 查看市管理员列表：保留登录态访问。
- 新增、删除市管理员：必须登录态 + Passkey + 当前联邦管理员公民钱包签名。
- 编辑市管理员姓名：只需要登录态，后端仍校验角色和省域。
- 新增、删除联邦管理员：必须登录态 + Passkey + 当前联邦管理员公民钱包签名。
- 编辑联邦管理员姓名：只需要登录态，后端仍校验角色和省域。
- 删除新增联邦管理员：必须由本省初始联邦管理员通过 Passkey + 公民钱包签名提交。
- 删除联邦管理员云端代签逻辑。
