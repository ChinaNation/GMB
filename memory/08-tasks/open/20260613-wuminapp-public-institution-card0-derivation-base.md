...
- [ ] 两种 preimage 形态(与 SFID `accounts/derive.rs` / 链端一致):
  - 机构账户:`DUOQIAN || op_tag || ss58_le(2027) || sfid_number || (account_name 仅 0x06)`
  - 个人多签:`DUOQIAN || 0x05 || ss58_le || creatorPubkey(32B) || account_name`
  - 末端统一 `blake2b256` → 32B,prefix=2027 SS58。
- [ ] API:`deriveInstitutionAccount({sfidNumber, opTag, accountName?})` + 保留 `derivePersonalAccount({creatorPubkey, accountName})`。
...
## 不做(边界)
- 不改链端、不改 SFID;只动 wuminapp 派生底座。
- 不引入公权机构 UI(卡 B/C)。

## 改动目录(中文注释)
- 新增 `wuminapp/lib/governance/shared/account_derivation.dart`:统一派生原语,代码。
- 删 `wuminapp/lib/governance/personal-manage/personal_duoqian_derive.dart`:归位后清残留。
