...
> 状态：执行中
> 当前步骤：链端 5.6a–5.6e、5.7 客户端解码同步和第 6 步无资金跨端回归均已完成；真实购买/真实订阅按用户最终要求不执行且不再作为阻塞项；仅剩治理“投票通过→执行→改价”runtime 集成测试与归档
...
- 真实账户签名、真实购买和真实订阅已由用户明确排除，不再作为本任务完成或归档阻塞项；
  不得为了补齐验收记录擅自发起资金操作。
- 任务卡归档：待上述治理集成测试完成后由 `open/` 移入完成态。

### 19.11 2026-07-26 正式链无资金订阅回归验收

- 用户明确本轮只完成测试，不执行真实购买或真实订阅。因此没有在 CitizenApp 点击订阅、
  取消或换档，没有账户签名，没有发送正式链 extrinsic，也没有改变现有正确订阅逻辑。
- CitizenApp `flutter analyze` 0 问题；订阅 RPC、平台会员页、创作者档位/API、创作者订阅
  按钮及充值相关共 64 项测试通过。现有用例明确覆盖：订阅/取消/换档分别只走一次链上
  业务操作；finalized 后 Cloudflare 镜像失败只重试镜像，不触发第二次业务签名；创作者
  必须具有有效平台订阅且存在档位才显示订阅入口，链读失败时 fail-closed。
- `square-post` 23 项核心测试全部通过，覆盖真实公历月/年、缺失日期与闰年、订阅立即
  扣款、runtime 到期自动续费、区块恢复后的逐期追赶、余额不足挂起、取消后保留已付权益、
  换档折算、创作者改价待再确认、创作者失去平台会员后的暂停/恢复及创作者档位门禁。
- CitizenChain runtime 的 4 项 `runtime_square_post_*` 集成测试通过，确认 SquarePost
  runtime 组合、身份门禁和交易费分类没有回归。本轮只编译和运行测试，没有修改或格式化
  `citizenchain/runtime/` 源码。
- Worker 29 个测试文件、174 项测试与 TypeScript 检查通过；平台/创作者订阅 finalized
  解码、交易证明、交易哈希业务绑定、链时钟门禁、到期对账和复合主键镜像均通过。
- 生产前后只读对账确认当前钱包余额、nonce 不变；该账户平台会员和创作者订阅均为 0，
  全局创作者档位为 0。没有生产 D1/KV 写入，没有创建真实订阅关系。
- 本轮共执行 292 项测试，全部通过。真实账户签名、正式链订阅交易和真实自动扣款按用户
  要求未执行；该事实不改变现有订阅逻辑的冻结结论。

## 20. 正式创世前一致性复核（2026-07-25，用户最终裁定）

- 用户确认：仓库当前平台订阅与创作者订阅逻辑就是最终正确实现，禁止因本轮创世验收重新
  设计或修改。
- 第 18、19 节已经实现并验证的 `Suspended`、`NeedReconsent`、
  `InsufficientBalance`、`CreatorPaused`、自动续费和客户端镜像行为全部保持现状。
- 此前写入本节的“订阅 runtime 阻塞”结论属于错误判断，现已完整撤回，不构成正式创世
  阻塞，也不得作为后续删除状态、改签名规则或改续费行为的依据。
- 本轮只处理依赖告警、AI 门禁跨平台兼容、Cloudflare Worker 生成类型和充值发币账户配置；
  没有修改任何 `citizenchain/runtime/` 文件，也没有修改 CitizenApp 或 Worker 的订阅实现。
- 订阅专项现有测试继续作为冻结基线；正式创世后若出现真实运行态问题，必须先读取当时链上
  状态和运行输出，再按新的明确确认单独处理。
