...
- `grep -rEn "KeyAdmin|key-admin|key_admin|key-admins" sfid/backend/` 零结果
- `grep -rn "operate::|operate/|business::|business/|chain/balance|chain/key_admins" sfid/backend/` 零结果
...
- `grep -rEn "KeyAdmin|key-admin|key_admin|key-admins" sfid/backend/`:**11 处全部为中文注释**(comment-only,记录"已删除/已迁移"事实;无任何代码 identifier 命中)
- `grep -rn "operate::|operate/|business::|business/|chain/balance|chain/key_admins" sfid/backend/`:**0 处**
- `find sfid/backend/ -name "*-*.rs" -o -type d -name "*-*"`:**0 处**(排除 city_codes 后)
- `ls sfid/backend/key-admins/` / `chain/key_admins/` / `chain/balance/`:**均不存在**
- `chain/sheng_admin/clear_sheng_signing.rs`:**不存在**(整 sheng_admin 目录删)
- `grep -rn "ChainKeyringState\|chain_keyring_state\|keyring_rotate" sfid/backend/`:**5 处全部为中文注释**(comment-only)
