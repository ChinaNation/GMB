...
- [x] CitizenApp 与 CitizenWallet 归并为“公民”卡片
- [x] 移动端生产签名密钥改由 Keychain 保存并支持逐次 Touch ID 查看
- [x] 四张首页卡片减半、公民云/公民动作按钮减少三分之一并居中
- [ ] Runtime 二次确认与真实按钮验收
...
- 2026-08-02：CitizenConsole 54 项测试、Node/Bash/Swift 检查、ShellCheck 与 diff whitespace
  ...
  真实页面确认模块卡片收口完成；WASM CI 在节点 CI 正下方，开发升级在启动节点正下方；
  三张产品卡片均显示同一个 `SSH_KEY`，未触发任何产品 CI、未提交或推送 GitHub。
- 2026-08-02：用户确认首页模块改为固定一行中文小卡片。首页卡片只显示名称，不再显示图标、
  简介、生产状态或进入标识；技术模块名、详情页、密钥状态和业务动作保持不变。窄窗口保持
  单行并允许横向滚动，不恢复两列或单列卡片布局。
- 2026-08-02：首页小卡片改造完成。CitizenConsole 55 项测试及 Node/Bash/Swift、ShellCheck
  全部通过；已通过 Touch ID 重新签名换包并通过完整性校验。真实页面确认卡片均只有一个
  名称节点，桌面端高度统一为 52px 且纵坐标一致；520px 窄窗口下仍保持同一行，卡片区以
  横向滚动承载。未执行任何产品动作、未修改 runtime、未推送 GitHub。
...
- 2026-08-02：充值发币页布局改造完成。CitizenConsole 56 项测试及 Node/Bash/Swift、
  ...
  均已清理；点击「返回」已回到模块首页。验收未解锁发币密钥、未拉取订单、未执行发币、
  未修改 runtime、未提交或推送 GitHub。
- 2026-08-02：用户确认把官网生产部署合并进公民云，删除公民网首页卡片及官网本地测试部署；
  官网部署复用公民云已有 `CF_ACCOUNT_ID` 与 `CF_DEPLOY_TOKEN`。当时保留的高危全量数据重建
  动作已于 2026-08-03 永久移出日常控制台，公民云现只保留生产部署与官网部署。
- 2026-08-02：WASM CI run `30787255661` 的真实日志确认 Runtime Clippy、WASM 编译和产物校验
  均成功；失败发生在 NodeGuard 探针测试的宿主 Node 编译阶段，`glib-sys` 因 Ubuntu 缺少
  `glib-2.0.pc` 退出，行为探针尚未执行。本步只补齐 WASM workflow 的 Node/Tauri Linux
  依赖，不修改 runtime。WASM 远端 run 被准确 SHA 查询确认后，本地日志标签允许关闭并仅
  断开跟踪，后台等待与 GitHub CI 继续运行；未确认远端 run 前不得伪装成可关闭状态。
- 2026-08-03：官网部署、五卡片首页、三列紧凑操作区和 WASM 可脱离跟踪的源码改造完成；
  旧 `citizenweb.sh`、本地测试日志、测试端口与独立 CitizenWeb 模块均已清理。WASM workflow
  已补齐完整 Node/Tauri Linux 系统依赖。Node/Bash 语法、CitizenConsole 59 项测试、
  ShellCheck、Actionlint 与 diff whitespace 全部通过；未修改 runtime、未推送 GitHub、
  未触发部署或 CI。
- 2026-08-03：真实签名换包尚未完成。终端直接签名被 macOS 证书私钥访问拒绝；随后分别在
  可见内置浏览器和 Chrome 触发控制台打开，两次均由系统立即返回“生物验证未通过”，未进入
  可等待 Touch ID 的状态。现有签名包自身 `start.sh verify` 通过，但仍包含改造前资源，故本轮
  不把源码测试通过冒充为新生产包真实页面验收。构建暂存流程已同时修正失败码吞掉问题：任何
  签名失败都会删除 `next/`、返回非零且保留旧包，测试已覆盖该失败边界；本次失败遗留的
  `build-input/` 已移出工作区，`next/`、旧官网测试日志和旧官网动作脚本均确认不存在。
- 2026-08-03：用户确认把 CitizenApp 与 CitizenWallet 合并为 CitizenConsole 内部 `mobile`
...
- 2026-08-03：用户确认 `GMB_APP_KEY` 改由 CitizenConsole 的 macOS Keychain 保存生产真源，
  ...
  浏览器存储。本阶段不修改 runtime、移动端产品源码或 workflow，不推送 GitHub、不触发 CI。
- 2026-08-03：公民移动端归并实施完成。首页固定为“发币 / 公民链 / 公民云 /
  公民移动端”四张卡片；移动端详情两行各保留三个动作，后端按 `productId`
  调用原 `citizenapp.sh` 或 `citizenwallet.sh`，未新增重复脚本。`GMB_APP_KEY` 与
  `SSH_KEY` 在移动端密钥列表中各保留一项，“查看”位于“配置/更换”左侧。
- 2026-08-03：Node/Bash/Swift 检查和 CitizenConsole 59 项测试全部通过；已重新
  构建、签名、原子换包并通过 `start.sh verify`。真实签名页面确认四卡片顺序、
  移动端两行六动作和去重密钥列表；`SSH_KEY` 查看逐次 Touch ID 成功，只验证
  返回非空，未读取或输出明文，关闭后弹窗和文本域 DOM 均已清除。本机
  `GMB_APP_KEY` 当前尚未配置；旧 GitHub Secret 无法回读，首次需在合并卡片重新配置。
- 2026-08-03：用户确认把 `mobile` 分组的中文卡片和详情标题从“公民移动端”
  改为“公民”，并移到“发币”与“公民链”之间。四张首页卡片宽度按原四等分
  的 50% 显示；公民云和公民详情的动作按钮宽度减少三分之一，保留原宽度的
  三分之二；公民链、公民云和公民的动作按钮主标题与说明文字统一居中。
- 2026-08-03：上述布局改造已写入 CitizenConsole 私有源码，中文首页顺序固定为
  “发币 / 公民 / 公民链 / 公民云”，内部 `mobile` 仅作分组 id。Node、Bash、
  Swift 检查和 CitizenConsole 59 项测试全部通过；最新源码已重建、签名、原子换包并通过
  `start.sh verify`，签名包内 `server.mjs`、`web/app.js` 和 `web/styles.css`
  与当前源码逐字节一致，`next/` 无残留。本次真实页面解锁三次均被 Touch ID
  门禁拒绝，控制台正确失败关闭；未绕过生物识别，也未把本次页面解锁冒充为通过。
