...
> 功能 B（暂缓）。先做前台实时修复卡 `20260705-citizenapp-im-realtime-delivery-fix`，本卡待用户具备 Firebase/APNs 资源后再排。
...
- 设备 token 注册：`citizenapp/cloudflare/`（新增 push token 表 + 注册/注销接口）+ `citizenapp/lib/im/`（客户端注册）。
- Worker 推送发送：`citizenapp/cloudflare/src/chat/`（新 envelope 时向收件方离线设备发推送）。
- 客户端通知：`citizenapp/lib/im/` + `lib/security/`（权限）+ Android/iOS 原生通道。
...
- `citizenapp/lib/im/`：客户端 token 注册、通知展示、点击深链；代码。
- `citizenapp/android/`、`citizenapp/ios/`：FCM/APNs 原生接入与证书配置。
- `memory/05-modules/citizenapp/im/`：架构文档补后台推送。
