...
## 架构决定
- 新建 `sfid/backend/citizenapp/` 作为 **BFF(Backend-For-Frontend)**:只放薄 handler + DTO + 路由组装,挂进现有非 admin 路由组(匿名只读 + 复用 global_rate_limit_middleware + CORS)。
...
## 改动目录(中文注释)
- 新增 `sfid/backend/citizenapp/`:citizenapp BFF 薄层(handler/dto/route),代码。
