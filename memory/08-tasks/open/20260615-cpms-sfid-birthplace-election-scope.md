...
- 新增公民时从 SFID 行政区唯一真源的 CPMS 随包只读拷贝中选择全国省、市、镇作为出生地，保存后不可更改。
- “设置投票账户”弹窗新增“注册市选举公民”和“注册镇选举公民”两个开关；默认关闭，开启镇必须同时开启市。
- 公民详情页投票账户下方只读展示市/镇选举公民注册结果，不允许在详情页直接勾选。
- CPMS 档案码按选举地域精度携带居住地和出生地的省、市、镇代码；SFID 录入档案码后按精度落库，为后续投票区域权限判断准备数据。
- SFID 公民详情不展示居住地代码、出生地代码；投票范围按居住地展示，参选范围按出生地展示。
- SFID 录入档案码和导入年度报告时，只保留正常、有投票资格且已设置投票钱包的公民；无资格、注销或释放绑定的公民记录必须从 SFID 删除。
...
必须遵守：
- CPMS 永不联网；出生地选择数据只读来自 CPMS 随包内嵌的 SFID 行政区唯一真源拷贝。
- ARCHIVE 档案码不得明文暴露中文省市镇名称；只携带行政区代码。
- SFID 不保存原始实名数据，不接管 CPMS 实名信任根。
- 不突破模块边界，不在 CPMS/SFID 业务模块内实现投票流程。
...
- SFID 扫描档案码后按精度落库居住地和出生地代码。
- SFID 公民详情按行政区真源名称展示投票范围和参选范围，不展示行政区代码。
- SFID 档案码录入必须拒绝 `citizen_status != NORMAL`、`voting_eligible=false` 或未携带投票钱包的公民。
- SFID 导入 CPMS 年度报告时，对注销、无投票资格或释放绑定的匹配公民执行删除，保证 SFID 公民库只保留真正可投票公民。
- 现有公民管理省市查询不被破坏。
- 文档已同步，残留旧文案已清理。

执行记录：
- 已新增 CPMS 全国出生地只读接口，数据只来自 CPMS 随包的 SFID `china.sqlite` 行政区唯一真源。
...
- 已在 SFID 扫码验档案码后校验选举地域精度，并把居住地/出生地代码落入公民库。
- 已将 SFID 公民详情从代码展示改为 `投票范围 / 参选范围` 展示：投票范围按居住地，参选范围按出生地，名称来自 SFID 行政区唯一真源。
- 已在 SFID 档案码绑定 challenge 入口增加硬门槛：仅 `NORMAL + voting_eligible=true + 钱包字段齐全` 可进入绑定流程。
- 已将 SFID 年度报告导入改为只保留正常且有投票资格的公民；无资格记录和释放记录会删除 SFID 本地公民绑定。
- 已增加 SFID 启动期历史残留清理，删除注销、无投票资格、未绑定钱包或非完整绑定的旧公民行。
- 已同步 CPMS/SFID 技术文档和 SFID-CPMS-QR-v1 协议文档。

验收记录：
- `cargo fmt`：CPMS 后端、SFID 后端通过。
- `cargo check -q`：CPMS 后端、SFID 后端通过。
- `npm run build`：CPMS 前端、SFID 前端通过；SFID 前端仅保留 Vite 既有 chunk-size 警告。
- `git diff --check`：通过。
- 本轮新增 SFID 年度报告判定测试：`cargo test -q citizens::status_export_import` 通过，5 个测试通过。
- 本轮重新执行 SFID 后端 `cargo check -q` 通过，SFID 前端 `npm run build` 通过，仅保留 Vite 既有 chunk-size 警告，`git diff --check` 通过。
- 本轮临时启动当前 SFID 后端 `127.0.0.1:18899` 成功，`/api/v1/health` 返回 `UP`；启动初始化完成且无无资格公民清理 SQL 错误。
- 本轮真实数据库校验：`citizens` 当前 1 条，不合格残留查询结果为 0。
- 本轮临时启动当前 SFID 前端预览 `http://localhost:5189/`，浏览器打开生产构建页面成功，标题为 `SFID 管理端`，登录页可渲染，控制台无 error。
- CPMS 临时后端 `127.0.0.1:18080` 启动成功，`/api/v1/health` 返回 `ok`。
- SFID 临时后端 `127.0.0.1:18899` 启动成功，`/api/v1/health` 返回 `UP`，并连接本地链节点。
- CPMS 真实数据库 `_sqlx_migrations` 已到版本 `2`，`archives` 已存在出生省/市/镇和选举范围字段。
- SFID 真实数据库 `citizens` 已存在居住地、出生地和选举范围字段，并存在 `idx_citizens_residence_scope`、`idx_citizens_birth_scope` 索引。
