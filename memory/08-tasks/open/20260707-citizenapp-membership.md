...
- 验收：
  ...
  - 真实本地 Worker HTTP 验收通过：`wrangler dev --local --port 8787 --var SQUARE_DEV_UPLOAD_PROXY:1 --var STRIPE_WEBHOOK_SECRET:whsec_test` 启动后，带 Stripe 签名的 visitor subscription webhook 返回 200，D1 查询确认 `sub_http` 写入 `membership_level=visitor`、`subscription_status=active`、`identity_level=visitor`。
...
- 根据 2026-07-08 新增需求，会员套餐统一为美元计价：访客会员 `$2.99/month`、投票公民会员 `$9.99/month`、竞选公民会员 `$99.99/month`。
...
- 验收：
  ...
  - 真实本地 Worker HTTP 验收通过：`sub_usd_validation` 使用 `usd/299` 的 visitor subscription webhook 返回 200 并写入本地 D1；`sub_hkd_reject` 使用 `hkd/2400` 的 visitor subscription webhook 返回 400 `stripe_price_currency_mismatch`，D1 查询确认未入库。
...
- 新增 `citizenapp/cloudflare/src/media/cloudflare_assets.ts`：封装 Cloudflare Images Direct Creator Upload、Cloudflare Stream basic direct upload、Stream tus direct upload、Images / Stream 状态刷新和播放 URL 生成；本地 `SQUARE_DEV_UPLOAD_PROXY=1` 时使用同源 `dev-media` 端点验证完整流程。
...
- 更新 `citizenapp/cloudflare/src/media/cloudflare_assets.ts`：新增 Images / Stream provider asset 删除封装；生产环境调用 Cloudflare API 删除真实媒体，`SQUARE_DEV_UPLOAD_PROXY=1` 本地验收环境跳过外部 API。
- 更新 `citizenapp/cloudflare/src/posts/confirm.ts`：新增 `deletePostRoute` / `deletePostCloudflareData`；仅作者本人可删除，删除顺序为 provider asset、R2 manifest、D1 展示状态。D1 保留最小发布索引，把 `square_posts.post_state` 置为 `deleted` 并清空 `title/text`，删除 `square_media_assets` 行，按上传估算值回收 `square_memberships.storage_used_bytes`；重复删除不重复回收用量。
...
- 验收：
  ...
  - 真实本地 Worker HTTP 删除验收通过：`wrangler dev --local --port 8787 --var SQUARE_DEV_UPLOAD_PROXY:1` 启动后，本地 KV 写入 `sqs_delete_smoke` session，本地 D1/R2 写入 `sqp_delete_smoke` 测试帖子；`DELETE /v1/square/posts/sqp_delete_smoke` 返回 `post_state=deleted`、`deleted_media_assets=1`、`deleted_r2_objects=1`、`reclaimed_storage_bytes=1024`；D1 查询确认 `square_posts.title=NULL/text=''`、`square_media_assets` 计数为 0、`storage_used_bytes` 从 4096 回收至 3072；重复 DELETE 返回 `reclaimed_storage_bytes=0` 且用量保持 3072。
