# 任务卡：清算行功能完成度补齐：区块链侧技术闭环，包括 runtime、node、桌面节点、验收与文档
...
清算行功能完成度补齐：区块链侧技术闭环，包括 runtime、node、桌面节点、验收与文档
...
- 整理 桌面节点 清算行 tab 的剩余禁用项，明确哪些归本卡、哪些归 SFID / wumin / wuminapp。
...
## 2026-04-29 Step 3 实施记录

- node 跨行 pending 补齐：
  - `OffchainLedger::accept_payment_with_chain_state` 支持跨行收款方节点使用链上
    `DepositBalance[payer_bank][payer]` / `L3PaymentNonce[payer]` 校验余额和 nonce。
  - 跨行收款方节点不创建付款方本地账户,避免 ghost account 污染 `confirmed_sum_snapshot`。
  - `queryNextNonce` 取链上 nonce 与本地 pending nonce 的最大值,支持连续跨行 pending。
  - `submitPayment` 跨行时读取链上余额/nonce 后再入 pending。
- runtime 权重收口：
  - `offchain-transaction/src/weights.rs` 从空壳替换为非零保守 `WeightInfo`。
  - `lib.rs` 所有清算行 Call 改走 `T::WeightInfo`。
  - 生产 runtime 配置为 `offchain_transaction::weights::SubstrateWeight<Runtime>`。
  - `benchmarks.rs` 更新说明:正式自动权重需在完整 runtime + benchmarking WASM 下生成。

## 2026-04-29 Step 3 验证

- 通过：`cargo test -p offchain-transaction --lib`（23 passed）
- 通过：`WASM_FILE=/Users/rhett/GMB/citizenchain/target/wasm/citizenchain.compact.compressed.wasm cargo test -p node offchain -- --nocapture`（65 passed）
- 通过：`npm run build`（`citizenchain/node/frontend`）

## 本轮剩余项

- 正式自动 benchmark 尚未生成；当前已用非零保守权重替代空权重。要生成正式
  `weights.rs`,需准备带 benchmarking runtime api 的 WASM。
- 桌面节点 清算行前后端目录已在 2026-04-29 收口到 `node/src/offchain` 与 `node/frontend/offchain`；`dist/*` 属构建产物,不作为业务真源。
