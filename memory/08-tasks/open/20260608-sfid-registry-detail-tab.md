...
- 「注册局」tab 两个子 tab 改名：`市级管理员列表`→`市注册局`，`联邦管理员列表`→`联邦注册局`。
- **市注册局**：联邦管理员 城市网格→点市→该市注册局机构详情页（内嵌本市市级管理员列表，可增删改）；市级管理员 直接进本市注册局机构详情页（管理员列表只读）。
- **联邦注册局**：两角色都进全国唯一联邦注册局机构详情页；内嵌联邦管理员列表只显示登录管理员本省（逻辑不变）；联邦管理员可增删改、市级管理员只读。
- 注册局机构详情页 = 完整机构详情（机构信息+账户列表+资料库+操作记录），管理员列表插在**机构信息卡与账户列表卡之间**。其它普通机构详情页零行为变化。
- 联邦注册局位于中枢省，现有 scope 校验会 403 拦截外省管理员 → 后端新增只读接口，仅返回联邦注册局这一个机构、绕过 scope。
...
- `sfid/frontend/admins/ShengAdminsView.tsx`：解析市注册局 sfid、加载联邦注册局 detail。
- `sfid/frontend/admins/shengAdminUtils.ts`：ShengAdminSharedState 增字段。
...
- 三角色进入路径符合目标表（联邦管理员可增删改、市级管理员只读）。
- 注册局机构详情页：管理员列表位于机构信息卡与账户列表卡之间。
- 联邦注册局：外省管理员可正常读取机构详情（200），管理员列表仅本省。
- 普通机构详情页零行为变化（无管理员列表）。
...
- 前端 `admins/shengAdminUtils.ts`：`ShengAdminSharedState` 增 4 字段（federalRegistryDetail/Loading、cityRegistrySfid/Loading）。
- 前端 `admins/ShengAdminsView.tsx`：新增两个 effect——挂载时加载联邦注册局 detail（scope-bypass）；按活动省市解析市注册局 sfid（从 official 目录筛 CITY_REGISTRY）。
- 前端 `admins/ProvinceDetailView.tsx`：子 tab 改名为「市注册局 / 联邦注册局」；leaf 改为 GovDetailPage + 内嵌管理员列表（CityOperatorsView / ShengAdminSubTab，均包 inner Card）；用 `useCallback` 稳定 `loadFederalRegistry` 复用预加载 detail，避免重复请求；联邦管理员城市网格→点市→详情、市管理员直接进本市详情、返回按钮按角色隐藏。
...
- 待 user 真机手测三角色路径（见任务卡验收表）+ 确认外省管理员可读联邦注册局（200）。
...
2. **注册局子 tab 提升为顶级 tab**（删 `注册局` tab + 两个子 tab）：
   ...
   - `App.tsx`：ActiveView 改 `city-registry`/`federal-registry`；tab 顺序 …公安局 → **市注册局 → 联邦注册局**；passkey 未绑定按角色强制进对应注册局 tab（联邦→federal-registry，市级→city-registry）；两个 ShengAdminsView 实例按 viewResetToken 加 key。
   - `admins/ShengAdminsView.tsx`：mode 改 `list|city-registry|federal-registry`，effect 按新 mode 分流，dispatch 到 CityRegistryView/FederalRegistryView。
   - `admins/ProvinceDetailView.tsx`：拆成 `CityRegistryView` + `FederalRegistryView`（删 SubTabBar/子 tab）。
   - `admins/OperatorsView.tsx`：旧 `mode="system-settings"` → `mode="city-registry"`。
3. **管理员列表表头布局**：CityOperatorsView / ShengAdminSubTab 改为 `Card`，title=「市级管理员列表 / 联邦管理员列表」+ 计数（n/上限）紧随其右，`extra`=新增按钮置于统一行最右；删除原内部表头行 + 去掉标题里的计数括号。

### 迭代二验证
- 后端 `cargo check` + `cargo test`（52 passed）通过。
- 前端 `npm run build`（tsc + vite）通过，0 type error。
- 注意:联邦注册局机构在 subjects 表 org_code 仍是 `PUBLIC_ORG`(本次按 sfid_number 定位绕过),故 GovListTable 的 FEDERAL_REGISTRY 过滤对它不生效——中枢省管理员的「公权机构」列表里它仍会以"公权机构"出现(仅影响中枢省;5 个总统府联邦局 org_code 缺映射是既有问题,未扩边)。如需一并清掉,后续单列任务补 `org_code_for_constant_name` 的 5 个联邦局分支 + reconcile 重跑。
- user 确认（迭代二收尾）：只改注册局即可；另外 4 个总统府联邦局保持现状不动；联邦注册局留在中枢省/锦程市公权列表是可接受的（"正好"）。核实 china.sqlite：中枢省(ZS) 001 市=锦程市，联邦注册局正落于此，故锦程市市级管理员与中枢省联邦管理员都能在公权列表看到它。

## 迭代三（2026-06-08，管理员列表表头微调）

1. **ShengAdminSubTab（联邦注册局）**：title 改「联邦管理员列表 · {province}」（中点 `·` 用 `Space align="center"` 垂直居中）；计数「联邦管理员：n/5」→「用户数：n/5」移到 `extra` 内、置于「新增联邦管理员」按钮左侧。
2. **CityOperatorsView（市注册局）**：title 改回纯「市级管理员列表」；计数「本市市级管理员：n/30」→「用户数：n/30」移到 `extra` 内、置于「新增市级管理员」按钮左侧。
