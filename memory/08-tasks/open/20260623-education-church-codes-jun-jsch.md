...
## 需求
教育组(F)新增:JUN 教会大学(3 位)、JSCH 教会学校(4 位);常量库与 CID 系统同步。码表 90→92,F 教育组 4→6,私法人桶 7→9。
...
## 改动(9 文件)
- `citizencode/backend/number/code.rs`:enum+2、as_code+2、label_zh+2、ALL+2(90→92)、profit_policy(Variable)、is_private_legal、is_education_institution、requires_education_level(仅 JSCH)、头注释/单测 90→92。
- `citizenchain/runtime/primitives/src/code.rs`:PRIVATE_LEGAL_CODES 7→9、ALL_CODES 90→92、单测(all_codes_has_92/private=9)、头注释。
...
- `citizenapp` / `citizenwallet` institution_code:`_privateLegalCodes` +2 + 镜像注释 90→92、= 7→9。
