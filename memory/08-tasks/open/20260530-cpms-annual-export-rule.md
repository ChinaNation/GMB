...
- 超级管理员从每年 1 月 1 日起可导出上一年度数据；未导出时不再在 1 月 10 日后关闭导出窗口。
- 超过 1 月 10 日仍未导出时，操作管理员一直不能登录，直到超级管理员完成导出。
...
- `cpms/backend/src/operator_admin`：新增年度导出状态接口，导出接口调用新规则。
- `cpms/frontend/super_admin`：系统设置页按钮状态、系统设置页签角标、年度导出 API/type。
...
- 超级管理员在每年 1 月 1 日后只要存在未导出年度即可导出最早未导出年度。
- 导出完成后该年度按钮置灰，下一年度 1 月 1 日再提示。
- 1 月 11 日起仍有未导出年度时，操作管理员登录和已有会话被锁定。
- 前端不会在非可导出状态点出后端英文错误。
- 文档、注释、残留和测试同步完成。
...
- 2026-05-30：新增 `GET /api/v1/archives/status-export/state`，返回按钮可用状态、角标状态和操作管理员锁定状态。
- 2026-05-30：操作管理员锁定改为 UTC 1 月 11 日起持续生效，直到所有逾期年度补导完成；超级管理员不受锁定影响。
