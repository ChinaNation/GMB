# 任务卡：将 duoqian-manage-pow 注册型多签地址接入 duoqian-transfer-pow 转账功能
...
让 `duoqian-manage-pow` 注册出来的多签地址也能复用现有
`duoqian-transfer-pow` 的提案、投票和执行转账流程。
...
- 不新增新的转账 pallet，直接扩展现有 `duoqian-transfer-pow`
- 不改现有 `DuoqianTransferPow` pallet index / call index
- 运行时改动后，需要同步更新在线端与冷钱包兼容信息

## 4. 执行计划

1. 扩展链上 `duoqian-transfer-pow` 支持 `ORG_DUOQIAN`
...
- 已扩展 `duoqian-transfer-pow` 支持 `ORG_DUOQIAN`，注册型 Active 多签地址现在可直接复用 `propose_transfer / vote_transfer / execute_transfer`
- 已补链上单测，覆盖注册型多签地址达到阈值后自动执行转账
- 已同步更新 `wuminapp`
  - 统一 institution id 编码：治理机构用 `shenfen_id`，注册型机构用 `duoqian_address(32)+16字节0`
  - 注册型机构管理员与阈值改为从 `DuoqianManagePow.DuoqianAccounts` 动态读取
- 已同步更新 `wumin` 冷钱包
  - `org = 3` 摘要显示为“注册多签机构”
...
- `cargo test -p duoqian-transfer-pow`：通过
