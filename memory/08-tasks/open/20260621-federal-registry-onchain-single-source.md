...
- 联邦注册局只有一个：`总统府联邦注册局`（china_zf `ZS001-GZF0P-249474503-2026`），链上一个机构、一份 admins，链不区分省份；省份划分纯属 CID 业务层给联邦注册局管理员分配的权限范围（联邦注册局管理员负责本省市注册局）。
- 联邦注册局管理员补齐为 43 省 × 5 = 215 个：每省 = 现有 CID 该省注册局管理员 1 个（保留在最前）+ 弃用池补 4 个。
- CID 后端不再内置注册局管理员、不再用 postgres `admins` 表当真源，改为只读链 + 保留省份标签映射。
...
- CID `federal_registry_admins.rs` = 43 省 × 1 管理员；助记词用户已备份，复用其公钥安全。
...
- 顺序 = CID `FEDERAL_REGISTRY_MAINS` 的 43 省序；每省 5 个 = [该省现有 CID 管理员] + [弃用池顺序取 4]。
...
### C. CID 后端：删内置 + 改读链
- `citizencode/backend/admins/federal_registry_admins.rs`：删 `FEDERAL_REGISTRY_MAINS` 内置常量（管理员真源迁链）；保留 `admin→省` 只读 scope 映射（非第二真源，仅权限范围属性）。
- postgres `admins` 表退出真源角色（CHECK 仅 FEDERAL/CITY、无机构号列，承载不了统一模型）。
- 新增按机构 cid_number 读链 admins 的能力（联邦注册局读总统府联邦注册局 admins）。

### D. 重新创世
- 往创世写 zf/sf/jc/jy/lf + 215 联邦注册局管理员属于改创世状态，必须新 WASM 重生 chainspec（开发链重新创世，非 setCode）。
- 重新创世后须重跑 CID 机构注册表 / CitizenApp 公权机构数据包生成器。
...
- `citizencode/backend/admins/`（代码+残留清理：删内置注册局管理员常量、改读链、保留 scope 映射）
...
- [x] C. CID 删内置联邦注册局清单（按用户指令：内置管理员真源已在 china_zf 链上常量，CID 不再内置、不在 CID 读链——读链不在本卡范围）：
  - 删除 `citizencode/backend/admins/federal_registry_admins.rs`（FEDERAL_REGISTRY_MAINS 43×1 常量 + 3 个 helper）。
...
- [ ] D. 重新创世（**用户指令：现在不做**，部署时再由用户执行：构建新 runtime WASM→重生 citizenchain.raw.json→CID/CitizenApp 数据包重生）

## 验收

- china_zf：总统府联邦注册局 admins 恰 215，每省首位 = CID 该省现有管理员，其余 4 来自弃用池；全文件 admins 无新增重复；`cargo check`/`cargo build` 通过。
- 创世：admins-change 内 zf/sf/jc/jy/lf + 总统府联邦注册局均为 Active 内置；国储会等不回归。
- CID：无任何内置注册局管理员常量、无 postgres admins 真源；联邦注册局管理员页读链展示；scope 按省过滤不变。
