...
## 改动目录(中文注释)
- 改 `citizenapp/lib/citizen/public/public_page.dart`:占位页 → 省/市导航,代码。
- 新增 `citizenapp/lib/citizen/public/city_institution_list_page.dart` 及导航 widget,代码。
- 复用 `citizenapp/lib/governance/` 列表样式(只读复用,不改 governance)。
