...
3. 选举走机构自组织提案(Option B):不建选举委员会机构,总统选举由总统府组织,删宪法选举委员会描述;删 `CSLF`/`TSLF`,市/镇自治会改非法人挂政府(ADR-028 决策 4/5)。
   ...
   - 选举保留普选 + 互选:普选按行政区/职位范围组织,互选按机构现任成员快照组织；国家立法院参议员/众议员由各省行政区公民分别在本省联邦立法院参议会/众议会现任成员中普选产生。
...
- （OUT,后续卡/ADR-027 轨道,触 runtime 二次确认）`citizenchain/runtime/{votingengine/election-vote,primitives/src/code.rs,primitives/src/旧宪法 HTML}`、`citizencode/backend/{subjects,gov,number}`。
...
- **公民宪法已迁为结构化链上法律**:`旧宪法 HTML` 是仓库内创世宪法生成源,宪法 = `law_id=0, tier=宪法`(`constitution.scale` 创世注入),`ImmutableManifest` 冻结 8 条不可修改条款 `[1,2,3,17,19,23,33,41]`(=「宪法守卫」)。节点运行态真源是链上 law_id=0;节点展示宪法走 `constitution_getDocument` RAW storage RPC,普通法律浏览可走 `LegislationApi`。
...
- **治理视图**:`citizen/governance/governance_tab.dart`(port 自 GovernanceListPage)——`repo.listByCodes({NRC,PRC,PRB})` 按 orgType 分三组,**保留拖拽排序(SharedPreferences)+ 折叠 + 国家储委会横跨 + 管理员高亮**;详情走统一 `InstitutionDetailPage`。
...
**立法 tab 最终布局(用户拍板 2026-06-24,2026-07-04 口径修正)**:固定顶部 5 卡(公民宪法整行长卡 + [国家立法院 NLG | 国家教委会 NED] + [国家立法院众议会 NRP | 国家立法院参议会 NSN])+「省市立法机构」标签;滚动 body = 公权式省竖导航(去关注组)+ 选中省的 省联邦立法院/省联邦立法院众议会/省联邦立法院参议会 + 该省全部市的市立法会。机构卡 → 统一详情页;详情页加「**法律原文**」入口(仅立法机构 NLG/NRP/NSN/PLG/PRP/PSN/CLEG/NED)→ `list_laws(tier,scope)`。宪法(law_id=0)= 顶部卡直达条款项阅读器。块号→日期接 `target_block_time_ms`。
...
- [x] **旧块高日期换算 + 会话内缓存**:块高日期换算器已随法律生效时间戳改造删除;`legislation_api.dart` 加会话内内存缓存(law/version/manifest,宪法 219KB 不重拉)。Isar 跨会话持久缓存 deferred(ADR-018 R3 后续)。
...
- 任务：公民 → 立法顶部整行宪法卡标题从“公民宪法”改为“《公民宪法》”，并相对整张
  卡片水平居中；点击入口、书本图标、右箭头和法律阅读器路由保持不变。
- 预计修改目录：
  - `citizenapp/lib/citizen/legislation/`：只调整宪法卡标题文案和居中布局。
  - `citizenapp/test/citizen/legislation/`：更新既有测试，断言书名号和几何中心。
  - `memory/08-tasks/open/`：记录目标状态和验收结论。
- 验收完成：既有立法测试 3/3 通过，其中宪法卡几何断言确认标题中心与整卡中心误差
  `≤ 0.01dp`；`dart analyze` 0 问题、`git diff --check` 通过。Pixel 8a profile 真机
  显示“《公民宪法》”，整卡语义边界为 `[42,313][1038,481]`，标题视觉居中，左侧书本、
  右侧箭头和点击进入法律阅读器的交互保持不变。
- 用户追加把“《公民宪法》”标题从 16sp 调大到 18sp；只增大标题字号，居中定位、
  卡片高度、图标、箭头和点击路由保持不变。
- 18sp 追加项验收完成：立法测试 3/3 通过、`dart analyze` 0 问题、profile APK 构建
  成功；Pixel 8a 真机确认标题放大后仍相对整卡居中，无裁切、溢出或卡片高度变化。
