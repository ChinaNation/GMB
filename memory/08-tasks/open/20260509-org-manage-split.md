# 任务卡：将机构多签管理统一收敛为 organization-manage 独立模块：runtime 中 organization-manage 作为机构多签管理模块；citizenchain/node 前后端在 governance 目录下创建 organization-manage 目录；wuminapp/lib 创建 organization-manage 目录并删除旧 duoqian 目录；代码改动后同步更新文档、补中文注释并清理残留。

- 任务编号：20260509-151149
- 状态：open
- 所属模块：citizenchain-wuminapp
- 当前负责人：Codex
- 创建时间：2026-05-09 15:11:49

## 任务需求

将机构多签管理统一收敛为 organization-manage 独立模块：runtime 中 organization-manage 作为机构多签管理模块；citizenchain/node 前后端在 governance 目录下创建 organization-manage 目录；wuminapp/lib 创建 organization-manage 目录并删除旧 duoqian 目录；代码改动后同步更新文档、补中文注释并清理残留。
...
- 已将 wuminapp 机构多签目录从 `wuminapp/lib/duoqian/` 迁入 `wuminapp/lib/organization-manage/`，旧 `lib/duoqian/` 物理目录已删除。
- 已将 wuminapp 机构多签测试目录从 `wuminapp/test/duoqian/` 迁入 `wuminapp/test/organization-manage/`。
- 已将个人/机构共用的提案详情页迁入 `wuminapp/lib/proposal/shared/duoqian_manage_detail_page.dart`，避免放入机构多签专属目录。
- 已将个人多签使用的账户状态轻量模型收回 `wuminapp/lib/personal-manage/personal_manage_models.dart`，避免个人模块依赖机构多签模型。
...
- 旧目录残留扫描：未发现 `wuminapp/lib/duoqian/`、node `offchain/organization_manage`、node 前端 `offchain/organization-manage` 等旧路径残留。
