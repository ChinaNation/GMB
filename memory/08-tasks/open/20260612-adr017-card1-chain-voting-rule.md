...
## 方案
`node/src/core/service.rs:723` voting_rule 从 `VotingRulesBuilder::default().build()`(BeforeBestBlockBy(2)+3/4,最多投到 best−2)改为 `()`(官方无约束实现,允许投到链尾)。不动 runtime/创世;节点二进制重建后滚动重启生效(国家储委会权威节点优先)。
...
- **部署待 user**:重建节点二进制 → 全网滚动重启(国家储委会权威节点优先)→ 实测发一笔交易数秒内 finalized==best。
