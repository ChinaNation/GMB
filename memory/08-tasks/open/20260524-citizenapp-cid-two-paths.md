...
- 不允许保留局域网 IP、`CID_PUBLIC_BASE_URL`、`CID_BIND_ADDR` 拼客户端地址、`CITIZENAPP_API_BASE_URL` 任意注入、`CID_BASE_URL` 任意注入、`127.0.0.1:8787` 旧默认值。
...
- 旧的 `CITIZENAPP_API_BASE_URL`、`CID_BASE_URL`、`CID_PUBLIC_BASE_URL` 客户端路径不再存在。
...
- 已删除 citizenapp 客户端旧的 `CITIZENAPP_API_BASE_URL`、`CID_BASE_URL`、`127.0.0.1:8787` 默认路径。
- 已将 `citizenapp/scripts/citizenapp-run.sh` 固定为 USB 开发路径：注入 `CITIZENAPP_CID_ENV=dev_usb`，并强制建立 `adb reverse tcp:8899 tcp:8899`。
