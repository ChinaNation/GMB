# citizenapp 钱包交易记录与收款流水修复

## 任务需求

修复 citizenapp 钱包交易记录相关问题：
...
- `citizenapp/lib/wallet/`：修复钱包详情页最近交易记录展示、状态标签、点击路径，并复用交易详情页。
- `citizenapp/lib/rpc/`：给 `ChainTxMonitor` 增加未 finalized 区块补扫，保证入块收入先显示为“已出块”。
- `citizenapp/lib/transaction/shared/`：按需补强本地流水去重和状态升级工具。
- `citizenapp/lib/transaction/onchain-transaction/`：修复交易页切换交易钱包后的转出状态刷新，不改变签名、广播和确认逻辑。
- `citizenapp/lib/wallet/widgets/`：调整钱包详情页操作卡第三列文案，明确零钱包余额不是链上余额。
- `citizenapp/test/`：补交易流水与钱包详情页交互测试。
- `memory/05-modules/citizenapp/`：同步更新钱包流水和交互文档。
...
- `dart format citizenapp/lib/wallet/pages/transaction_history_page.dart citizenapp/lib/wallet/pages/wallet_page.dart citizenapp/lib/rpc/chain_tx_monitor.dart citizenapp/test/wallet/transaction_history_page_test.dart`：通过。
