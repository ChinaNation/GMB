...
- [x] private/handler.rs create_institution：放开 G 非 JY（仅 ZF/LF/SF/JC，CB/其它拒绝）；手动 G 统一市注册局机构管理员（教育学校/公权机构分别提示）；F 必传 parent + 存在/属性/代码一致/地域/p1 继承五连校验 + 创建即写 parent；grant payload 加 parent_sfid_number（前后端同步）
