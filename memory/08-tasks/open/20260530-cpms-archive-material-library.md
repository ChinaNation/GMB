...
- 主体业务必须放在 `citizenpassport/backend/dangan` 下实现，不能放进 `operators` 作为核心功能。
- 完成后更新文档、补中文注释、清理旧占位残留。

## 预计修改目录

- `citizenpassport/backend/dangan`：新增资料库模块，负责元数据、文件存储、上传下载删除、硬删除联动。
- `citizenpassport/backend/db`：新增 `archive_materials` 表结构，保存资料元数据，不保存文件正文。
- `citizenpassport/backend/main.rs`：挂载 dangan 资料库路由并登记错误码。
- `citizenpassport/frontend/dangan`：档案详情页新增“公民资料库”区域、资料上传、显示、下载和删除。
- `citizenpassport/frontend/assets/styles`：新增资料卡片样式。
- `citizenpassport/CITIZENPASSPORT_TECHNICAL.md` 与 `memory/05-modules/citizenpassport`：同步资料库边界、接口、存储和审计规则。
