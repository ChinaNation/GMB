...
- `cpms/frontend`：CPMS 前端工程根目录；涉及代码迁移、模块重命名、API/type 拆分和残留删除。
- `cpms/cpms.sh`：CPMS 本地启动脚本；涉及前端启动目录硬编码修正。
...
- [x] 创建任务卡并登记索引。
- [x] 移动 CPMS 前端工程到 `cpms/frontend` 根层。
- [x] 拆分 API 与类型文件。
...
- 旧 `web` 包装层不再作为工程目录存在。
- `cpms/frontend/src` 不再存在。
- 前端不再引用原 `api.ts`、`types.ts` 聚合文件。
- CI、脚本和当前文档不再写死旧前端包装路径。
- `npm run build` 在 `cpms/frontend` 可通过。
