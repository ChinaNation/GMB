...
- Cloudflare Worker 负责钱包会话、广场 API、会员权益强制校验、媒体授权、设备登记、一次性 KeyPackage、通用推送唤醒和短期 TURN 凭证。
...
- 用户注销时先关闭实时连接、撤销 TURN 凭证并硬删除设备登记、推送 Token、KeyPackage、防重放摘要和浏览计数，再清理广场及订阅数据。

## 数据边界

- D1 Chat 表只保留 `chat_devices`、`chat_keypackages`、`chat_device_binding_nonces` 和 `chat_turn_credentials`。
- R2 不保存任何聊天对象。
