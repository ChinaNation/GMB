...
彻底修复 CPMS、SFID、citizenapp 三端电子护照绑定闭环。绑定必须由 citizenapp 对 SFID 绑定签名请求签名，SFID 验签后直接完成电子护照绑定并向 citizenapp 状态查询返回结果；绑定公民电子护照流程到 SFID 验签落库即结束。
...
- `cpms/backend/dangan/`：档案码生成、更新、打印和投票账户保存。
- `cpms/frontend/dangan/`：投票账户与档案码 UI 文案。
...
- 全仓电子护照绑定链路只描述 SFID 验签落库与 citizenapp 状态查询。
- 全仓电子护照绑定链路不再使用历史主动注册、历史双模式字段或历史注册路由。
- CPMS、SFID、citizenapp 使用统一字段：`archive_no`、`citizen_status`、`voting_eligible`、`vote_status`、`identity_status`、`valid_from`、`valid_until`、`status_updated_at`、`wallet_address`、`wallet_pubkey`、`wallet_sig_alg`、`sfid_number`、`bind_status`。
...
- 已完成 SFID `ARCHIVE` 验真解析、绑定 challenge、citizenapp 签名验签、本地绑定落库和 `/api/v1/app/myid/status` 查询。
...
- 已清理 SFID 旧绑定回调 worker、回调环境变量文档和死字段；电子护照绑定结果只由 SFID 本地完成并由 citizenapp 状态接口查询。
...
- 已修正 SFID 公民列表绑定入口文案：无记录入口固定为“新增身份ID绑定”，已有记录操作固定为“更换绑定”，弹窗标题和 citizenapp 签名请求摘要按 `create / replace` 区分。
...
- 已更新协议、命名、SFID/CPMS/citizenapp 模块文档，并删除旧实现计划文档。
- 已验证：
