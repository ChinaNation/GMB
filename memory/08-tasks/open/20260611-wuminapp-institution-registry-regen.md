...
- wuminapp 客户端无机构派生逻辑,地址展示/余额查询(`fetchFinalizedBalance(mainAddress)`)/管理员查询(`AdminAccounts` 以 mainAddress 为 key)全部依赖此表 → 全部机构同时断。
...
- [x] 注册表国储会 mainAddress = `39936ebd…1d14db3d`,名称 = 国家公民储备委员会;87 机构(44 CB + 43 CH)与 china_{cb,ch}.rs 逐字段比对 0 mismatch(name/sfid/main/fee/stake/orgType 全字段脚本核对)
...
- ~~待办:wumin 冷钱包 `lib/chain/institutions.dart` 机构名陈旧~~ → 已完成,见 `done/20260611-wumin-institutions-name-sync.md`。
