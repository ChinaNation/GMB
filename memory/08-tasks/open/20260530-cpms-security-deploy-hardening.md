...
- `cpms/backend/src/login`：调整管理员会话空闲过期时间，删除旧登录字段残留，接入限流。
- `cpms/backend/src/authz`：所有管理员统一执行过期检查和滑动续期。
...
- `cpms/deploy` 与 `cpms/backend`：删除 Docker 残留文件。
- `cpms/CPMS_TECHNICAL.md` 与 `memory/05-modules/cpms`：同步安全、部署、会话、限流、密钥说明。
