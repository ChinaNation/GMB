...
- fee_address 的多签保护不受影响（institution-asset 仍然拦截人为转账）
...
```rust
// 当前（免费）：
offchain_transaction::pallet::Call::submit_offchain_batch { .. } => {
    onchain_transaction::AmountExtractResult::NoAmount
}

// 改为（按链下手续费收费）：
offchain_transaction::pallet::Call::submit_offchain_batch { batch, .. } => {
    ...
    if total_fee == 0 {
        ...
        onchain_transaction::AmountExtractResult::Amount(total_fee.saturated_into())
    }
}
```
    ...
    fn fee_payer(_who: &AccountId, call: &RuntimeCall) -> Option<AccountId> {
        match call {
            RuntimeCall::OffchainTransaction(
                offchain_transaction::pallet::Call::submit_offchain_batch { institution, .. }
            ) => {
                // 从 institution 派生 fee_address
                offchain_transaction::Pallet::<Runtime>::fee_account_of(*institution).ok()
            }
            _ => None,
        }
    }
...
### 3. institution-asset 白名单

文件：`citizenchain/runtime/transaction/institution-asset/src/lib.rs`

当前 fee_address 的合法操作只有 `OffchainBatchDebit` 和 `OffchainFeeSweepExecute`。
OnChargeTransaction 的扣费不经过 institution-asset（它是 runtime 内部余额操作），所以不需要额外改动。

但需要确认：institution-asset 的拦截点是在 `Balances::transfer` 等 extrinsic 层面，而 `OnChargeTransaction::withdraw_fee` 调用的是 `Currency::withdraw`，属于不同的执行路径，不会触发 institution-asset 检查。
...
Step 3: 验证 institution-asset 不拦截 OnChargeTransaction 扣费
