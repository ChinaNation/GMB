...
- [x] **删机构写入口**:删 institution_multisig_create_page/close_page;account_list_page 创建入口改只进个人多签(_openCreateInstitution 删、菜单简化);account_info_page 删关闭(_confirmClose/_openClosePage/关闭菜单项);qr 机构创建、关闭和历史拒绝清理 action 及映射全部删除。
- [x] **死写代码已清(2026-06-29)**：`institution_chain_service` 保持只读；2026-07-15 又删除旧 CID→账户读取，当前只读 `InstitutionAccounts` 正向真源和 `AccountRegisteredCid` 反向索引。
