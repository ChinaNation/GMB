...
1. 重跑 `node scripts/generate_citizenapp_governance_registry.mjs` 重生成注册表(44 国储/省储会 + 43 省储行)。
2. 全仓扫描旧地址残留(`c17a81b8…` 等)与旧机构名「国家储备委员会」,确认无第二处硬编码。
3. citizenapp `flutter analyze` + `flutter test` 回归。

## 验收

- [x] 注册表国储会 mainAccount = `39936ebd…1d14db3d`,名称 = 国家公民储备委员会;87 机构(44 CB + 43 CH)与 china_{cb,ch}.rs 逐字段比对 0 mismatch(name/citizencode/main/fee/stake/orgType 全字段脚本核对)
