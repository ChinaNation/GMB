# 任务卡：分析 duoqian-transfer-pow 与 duoqian-manage-pow 并记录 Pending 残留风险
...
基于源码全面分析 `duoqian-transfer-pow` 与 `duoqian-manage-pow` 的实际功能实现，澄清它们与内部投票引擎的关系，并把 `propose_create` 预写 `Pending` 状态但缺少拒绝/超时清理路径的问题固定到任务卡和模块文档中。
