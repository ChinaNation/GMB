...
- **B3 管理员上链录入**：构造 admin set 录入 extrinsic（FRG→CREG / 公权 / 私权 路由），含 cid/name/admin_role/term/source；回写；验收 = console 创建的管理员能登录（进链上 Active 集合）。
...
- [x] A3 协议登记（随 A1/A2/B2/B3 增量完成 + 2026-06-29 补全）：qr-action-registry.md(用户加公私拆分行 0x2005/0x2105/0x2001.. + 我加 `0x0c01` federal_set + 依赖加 onchina 编码器);unified-protocols.md P-TX-001 改名 PublicManage/PrivateManage + 前缀 0x2005/0x2105、P-QR-002 写明链交易 `a=(pallet<<8)|call` 禁扁平、P-TX-007 加 federal_set + 修旧文档误记 `account_id:48`→`32`(真类型对拍证 AccountId32=32B)。**遗留(属用户身份系统重构的文档债,未代劳)**:unified-protocols.md 仍有 ~15 处 `OrganizationManage`/`runtime/private/organization-manage` 引用(P-STORAGE-002 等)需随重构统一对齐。
...
- [x] B3 管理员上链录入（完成，2026-06-29，直接手写未用子智能体）：
  ...
  - [x] **B3.3 市注册局管理员上链 wiring（CreateCityRegistry/DeleteCityRegistry 完成，2026-06-28）**：新建 `institution/admins/admin_set_call.rs::build_city_registry_admin_set_call_data`——解析 CREG 机构 cid(`official_institution_cid::<Infallible>("CITY", province_code, city_code, "", "CREG", province_name, city_name, |_|Ok(false))`,seed_scope=`"CITY"`,GOV-CITY-省-市--CREG 种子)→`derive_account(cid,"主账户")`=main_account;当前集合 `repo::list_city_registry_admins_by_scope_conn`,按 add/remove 合并 delta,逐人 profile 复用 `resolve_admin_identity_conn`(改 pub(crate)),阈值多数 ⌊n/2⌋+1,`encode_admin_set_call`(B3.2)。`auth/actions.rs::build_city_registry_admin_set_sign_request` 接 prepare(CreateCityRegistry/DeleteCityRegistry)→QR `ACTION_CID_ADMIN_SET=5`,新输出字段 `admin_set_sign_request`。**onchina 零提交**(冷钱包提交)。cargo test -p onchina 73 过+fmt 绿。
  - [x] **B3.3-余 ReplaceFederalRegistry（完成，2026-06-29）**:`build_federal_registry_admin_set_call_data`(FRG `federal_registry_cid_number()`→`derive_account(cid,"主账户")`→全集 old→new 替换→genesis `propose_admin_set_change` call 0,FRG 内部投票,非特权)+`auth/actions.rs::build_federal_registry_replace_sign_request` 接 prepare 的 ReplaceFederalRegistry arm。**B3.3 三动作齐全**。
  - [x] **B3.4 决策不实现(经核实无写入目标，2026-06-29)**:登录验收=链直读(B3.1);`admins` 控制台登录表无 chain_status 列;`institution_admins`(B1)链下资料表当前无任何流程写入;`AdminAccountActivated` 只带 main_account 非 cid(反查需 derive 且无落点)。写回=死代码。机构级 chain_status 已由 B2 `InstitutionCreated` 回写覆盖。
  - [x] **CRITICAL-1 动作码协议（2026-06-29）**:链交易 `b.a` 真源 = `a=(pallet<<8)|call`(qr-action-registry.md)。B2/B3 误用扁平 `ACTION_CID_INSTITUTION_CREATE=4`/`ACTION_CID_ADMIN_SET=5`——**5 与已登记 `activate_admin_account=5` 撞码**、4 未登记→CitizenWallet 必拒/错路由。改:删扁平常量,新增 `chain_action_code(pallet,call)`+`ChainCall{action,call_data}`,**b.a 与 b.d 同源派生**(编码器返回 ChainCall,actions.rs 三调用点用 `.action`/`.call_data`)。CREG=0x0c01、FRG=0x0c00。
  - [x] **CRITICAL-2 机构 pallet 拆分对齐（2026-06-29）**:用户并行重构把 `OrganizationManage(17)` 删→拆 `PublicManage(32)`/`PrivateManage(33)`,`propose_create_{public,private}_institution` 两 call **call 5、15 字段逐字节相同**仅前缀不同。B2 编码器仍指 17(死 pallet)→冷签必被链拒(indexer event_parser 已被用户改对,仅编码器漏网)。改:删 `ORGANIZATION_MANAGE_PALLET_INDEX=17`,新增 `PUBLIC/PRIVATE_MANAGE_PALLET_INDEX`+`create_institution_pallet_index(code)`(经 `is_private_legal_code` 派生,与 call data 内 code 同源);编码器内部派生前缀→公权 0x2005/私权 0x2105。测试锁双分支(SFLP→私权 0x2105、PRC→公权 0x2005)。
  - 已验证:`cargo test -p onchina` **73 过**(镜像对拍+admin-set 对拍[12,1]+创建公私双分支)+fmt 绿+零 `.tx` 提交+零 OrganizationManage/17 残留。
