...
- 清理“全部读写都通过 api.onchina.org”“Matrix 替代现有 IM”“CitizenWallet 参与在线链路”等与目标状态冲突的文档表述。
...
- 如果引入 Matrix 替代当前 OpenMLS + Cloudflare 密文 mailbox，会与现有 IM 技术路线冲突。
...
- 第 4 步已完成真实本地运行态 smoke：本地 mock RPC 只接收 `author_submitExtrinsic`，`wrangler dev --local --port 8787 --var CHAIN_EXTRINSIC_RELAY_ENABLED:1 --var SQUARE_CHAIN_RPC_URL:http://127.0.0.1:19944` 后，`GET /v1/chain/bootstrap` 返回 `signed_extrinsic_relay.enabled=true/path=/v1/chain/extrinsics/relay` 且不泄露 RPC URL；`POST /v1/chain/extrinsics/relay` 返回 202、`schema=citizenapp.chain.extrinsic_relay.v1`、`relay_status=broadcast`、`tx_hash=0x3333...3333`、`chain_success_source=finalized_runtime_storage_or_events`；请求体携带 `private_key` 返回 400；`/v1/chain/rpc` 返回 404；App 侧 `SignedExtrinsicRelayApi` 对真实 Worker 响应解析成功。
