...
- **卡内"joint-vote 公投客户端"措辞不准**:`lib/votingengine/{joint-vote,citizen-vote}` 实为空目录;公投 CID 凭证基础设施实际在 `lib/wallet/capabilities/api_client.dart`(fetchPopulationSnapshot/fetchVoteCredential)+ `lib/signer/signing.dart`。
