...
- **签名零新增 op_tag**：`cast_representative_vote`、公投、签署、会签和护宪全部走标准交易签名。
...
- 立法 propose/cast 均为标准 extrinsic,**无需新 op_tag**;特别案人口快照由 runtime
  在建案事务中按 actor CID 内联创建，客户端只提交公投票。
...
- 代表机构表决：`LegislationVote(26).cast_representative_vote(1)`。
- 特别案公投：`cast_referendum_vote(2)`；call 0 永久留洞，客户端不准备、签名或解码
  人口快照。
- 提案列表/详情接入（读核心 Proposal + `RepresentativeMetas`/`LegislationMetas` 和逐机构计票）。
...
- LegislationVote = 26 → call 0 永久留洞 / cast_representative_vote(1) /
  cast_referendum_vote(2) / executive_sign(3) / override_sign(4) / guard_vote(5)

### 2b. 解码 + 标签(`payload_decoder.dart` + `action_labels.dart`)
- **8 个有效 call** 补 decoder 分支,展示关键字段(法律标题/动作/院/5类表决/赞成反对/law_id 等)供核对；call 0 不注册、不解码。
- 补中文动作标签（发起立/修/废法、代表机构表决、行政签署、三人会签、护宪终审、公投）。
- 守两色严格模式铁律:解析失败或 QR action 与 payload 动作不一致 → 红色拒签。
- **签名零新增 op_tag**：上述纯 extrinsic 走标准交易签名，decoder 只负责字段展示与一致性校验。
...
- `citizenapp/lib/votingengine/legislation-vote/`（代表机构表决 + 公投投票）。
...
- [x] CitizenApp 投票/签署/会签/护宪：调用名和存储镜像已更新为代表机构框架，call index 与二维码动作码保持不变。
- [x] CitizenWallet 有效 call 解码 + 标签 + 两色拒签；2026-07-16 已移除
  LegislationVote call 0 的常量、decoder、QR action 与测试夹具，永久拒绝旧载荷。
...
- **遗留 1:特别案立法公投(referendum 阶段)端到端未做**——legislation_vote_page 在
  referendum 阶段只展示进度 + 提示走立法投票入口；客户端只需接
  `cast_referendum_vote`，人口快照已经由建案事务内联创建。院内/签署/会签/护宪四阶段已完整。
