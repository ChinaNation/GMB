...
- **群媒体口径定案 = A(分级) + >100MB 薪火 Cloudflare 瞬时密文中转**([[project_chat_media_tiered_relay_2026_07_15]]):≤100MB P2P;>100MB(仅薪火)客户端群密钥流式加密→中转→拉完/短 TTL 删(需 R2+过期,反转 Chat 禁 R2,阶段3 前显式认)。会员窗口 ADR-036 已确认该 transport 归本卡阶段3、本卡只用其档位/限额值。文件上限单源来自会员 `MembershipPlan.chat_file_max_bytes`(freedom 10MB/democracy 100MB/spark 5GB)。
...
## 阶段3 大媒体中转落地(2026-07-16,1:1 完成,测试通过)

- **口径钉死**(用户 2026-07-16):**只有 >100MB 走 Cloudflare(R2 瞬时),其余一切不经 Cloudflare 字节路径**;只有薪火能发 >100MB;收发两端 + 服务端三重强制。
- **客户端**:`ChatMediaLimits.needsRelay`(固定 100MB 分界)+ `media/media_relay_crypto.dart`(流式分块 AES-256-GCM,一次性内容密钥 K)+ `media/chat_relay_media.dart`(编排)+ `chat_payload` relay 字段(K 随 E2E 信封)+ `ChatFlow.sendMedia` 路由(>100MB 未配置即拒发,绝不 WebRTC)+ `ChatCloudTransport` relay 方法 + `ChatRuntime` 发送 seam/`downloadAttachment` 门②下载。
- **Worker**:`cloudflare/src/chat/relay.ts`(init/blob PUT·GET/ack,薪火+尺寸门,R2 代理)+ routes + Env `CHAT_RELAY` + wrangler 3 环境绑定;`npm run typecheck` 绿。
- **测试**:`test/chat/media/` 10 绿;`flutter analyze lib/chat` 0。
- **待部署/待做**:R2 桶 `citizenapp-chat-relay(-staging)` 创建 + 24h lifecycle;真机 E2E;群大媒体待群 `sendMedia` 落地(transport 已就绪)。
- **R2 引入 = 反转"Chat 禁 R2",用户 2026-07-16 已明确授权。**

## 阶段2 完成(2026-07-16):协议缺口 + UI

**第 2 段 UI**:`group/ui/{open_group_chat,group_create_page,group_manage_page}` + `chat_tab`(新建群入口 + 群会话行 👥/群头像/点开聊天/长按管理)。建群走通讯录多选,管理页加/删仅 admin、改名、退群。`flutter analyze lib/chat` 0、全量 `test/chat` 142 绿。
**收尾进展(2026-07-16)**:群消息 sender 归属已落地(chat_page isGroup + resolveUser 真名 + Username topWidget);群贴纸已落地(sendGroupSticker + 共用 _sendGroupUserMessage)。test/chat 143 绿。
**群媒体发送已落地(2026-07-16)**:pending 键复合化(attachmentId|recipient)+ `MediaResend` 复合去重;`group_flow.sendGroupMedia`(≤100MB per-member WebRTC 扇出 + 离线按成员补发 / >100MB 中转一次上传 + `recipientCount` 贯通)+ `runtime.sendGroupMedia` + `open_group_chat` 接线;Worker `relay.ts` recipient_count(KV 计数,ack 归零删,TTL 兜底)。全量 test/chat 146 绿、analyze 0、worker tsc 绿。
**sender 归属已全类型一致(2026-07-16)**:文本 + 图片/视频/文件/贴纸群入站消息均显发送者名(`_mediaAligned` 加 senderId/groupStatus,按 isGroup 门控;1:1 不变)。全量 test/chat 146 绿、analyze 0。
**验收补测已落地(2026-07-16)**:权限门控 widget 测(group_manage_page admin/非 admin;ownerAccount 注入 seam + fake store)+ Worker relay vitest(init 门 + recipient_count ack 归零删,3 绿)。**顺带修真 bug**:relay 路由未在 limits/catalog 注册(生产 assertKnownRoute 会 404),补 chat_relay/chat_relay_blob + 路由表。全量 worker vitest 168 绿、test/chat 148 绿。
...
### 第 1 段:协议缺口 + 存储透出(2026-07-16 完成,测试通过)

- **群控制载荷** `group/group_control.dart`(`t=gmb.chat.ctrl`,`rename`/`leave_request`;不改 proto、不进 ChatMessageKind;收端先判别、控制不显示、坏数据退化 null)。
- **群名传播**:`renameGroup`(仅 admin)本机改 + 广播 rename;收端同步群名(补 Welcome 不带名)。
- **退群自动重钥**:`leaveGroup` 发 leave_request + 本机 leftLocally;admin 收到自动 `removeMembers([leaver])` 重钥——补齐阶段1 退群后向保密。
- **存储**:`ChatConversationPreview.conversationKind` 透出;`ChatStore.renameGroup`;`ChatGroupFlow` 构造加 owner。
- **测试**:`test/chat/group/` 13 绿(控制载荷退化 + leave_request 自动移除 + rename 同步);`flutter analyze lib/chat` 0。
- **待做(阶段2 第 2 段)**:建群页 / 成员管理页 / 群聊页(chat_page 群适配 + sender 归属)+ chat_tab 新建群入口与群会话行。
