任务需求：
修复 citizenapp 初次安装启动后的基础权限策略：网络权限只做平台声明，通知权限通过首次启动说明后申请，相机与相册权限保留在用户触发扫码、选图、保存二维码等具体功能时申请，并同步文档说明。
...
预计修改目录：
- citizenapp/android/：补齐 release 主 manifest 的网络权限与 Android 13+ 通知权限声明，属于平台配置。
- citizenapp/ios/：补齐 iOS 通知权限用途说明，属于平台配置。
- citizenapp/lib/：新增首次启动权限说明与通知申请逻辑，属于 Flutter 交互代码。
- memory/01-architecture/citizenapp/：同步权限策略和平台差异说明，属于文档更新。
...
新增命名说明：
- 中文名：App 权限启动策略；English name：app_permission_bootstrap；类型：Dart 文件；使用位置：`citizenapp/lib/security/app_permission_bootstrap.dart`；简介：记录首启权限说明状态并桥接原生通知权限申请。
- 中文名：App 权限入口页；English name：app_permission_gate；类型：Dart 文件；使用位置：`citizenapp/lib/security/app_permission_gate.dart`；简介：应用锁通过后展示一次权限说明，并按用户选择申请通知权限。
- 中文名：citizenapp 权限修复任务卡；English name：citizenapp-permissions；类型：任务卡；使用位置：`memory/08-tasks/open/20260517-citizenapp-permissions.md`；简介：记录本次权限策略修复范围、输出物和验收标准。

执行记录：
- 已补 Android release 主 manifest 的 `INTERNET` 与 `POST_NOTIFICATIONS` 权限声明。
- 已新增 Android/iOS 原生通知权限 MethodChannel：`org.citizenapp/permissions`。
- 已新增 Flutter 首启权限说明页，网络只说明不申请，通知按用户选择申请，相机/相册保持功能触发。
- 已将权限入口接入 `_AppLockGate`，应用锁通过后只展示一次。
- 已更新 `memory/01-architecture/citizenapp/CITIZENAPP_TECHNICAL.md` 的首次启动权限策略。
