...
3. 设置页新增链上中国平台启动行和二次确认。
4. OnChina TLS 证书覆盖 `onchina.local`。
5. 更新文档、完善中文注释、清理旧 HTTP 文案残留。

## 验收标准

- 节点程序启动后 OnChina 默认不启动。
- 设置页显示 `链上中国平台`、`https://onchina.local:8964` 和 `启动` 按钮。
- 点击启动弹出二次确认。
- 确认后 OnChina 子进程启动，状态变为已启动。
...
- 已新增 `citizenchain/node/src/settings/onchina_platform/mod.rs`，提供 `get_onchina_platform` / `start_onchina_platform` Tauri 命令。
- 已新增 `citizenchain/node/frontend/settings/OnChinaPlatformSection.tsx`，设置页在“全节点模式”和“通信节点功能”之间显示固定 HTTPS 入口和启动按钮。
