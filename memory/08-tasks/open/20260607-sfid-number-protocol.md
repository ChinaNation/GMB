...
- 不突破模块边界；SFID 负责身份号码协议生成与校验，citizenchain 负责链端常量、链上登记与 DUOQIAN 地址派生一致性，CPMS/wumin/wuminapp 只消费目标态 `sfid_number`。
- SFID 不保存原始实名数据。
...
- wumin/wuminapp 内置机构映射、签名 payload 样例、MyID 字段名更新。
- SFID 编号协议文档更新。
- 统一协议、统一命名登记与跨系统文档更新。
- 中文注释完善。
- 残留扫描与必要测试结果。
...
- 已将 SFID/citizenchain/CPMS/wumin/wuminapp 的测试样例和 DTO 命名统一到 `sfid_number` 与 `subject_property`。
...
- `flutter test test/signer/payload_decoder_test.dart` 于 `wumin` 通过。
- `flutter test` 于 `wuminapp` 指定相关测试集通过。
- `git diff --check` 通过。
