...
- 问题：普通多签转账提案提交前，`DuoqianTransferService.submitProposeTransfer` 把 `InstitutionInfo.mainAddress` 当成 SS58 地址传给 `Keyring().decodeAddress`，导致 64 位账户 hex 中的字符 `0` 被 Base58 校验拒绝。
- 修复：`mainAddress` 按 32 字节 AccountId hex 严格解码；用户输入的收款地址继续按 SS58 解码。
- 同类修复：普通链上转账从通讯录选择联系人、联系人详情页发起转账时，先把联系人内部 `accountPubkeyHex` 转成 SS58，再写入收款地址输入框。
- 残留清理：离线清算支付页 `toAddress` 删除 `0x` hex 双格式入口，只保留 SS58 扫码/展示边界。
- 文档：已在 `memory/01-architecture/wuminapp/WUMINAPP_TECHNICAL.md` 写明账户格式边界：runtime 为 `AccountId32`，App 内部为无 `0x` hex，用户展示/输入为 SS58。
