...
## 关键语义（2026-07-15 CID 主键模型）

- `cid_number`：机构唯一主键，也是机构管理员集合、机构阈值、岗位和提案主体的唯一键。
- `institution_account`：被操作的具体机构账户，可以是主账户、费用账户、制度专用账户或自定义账户，但不得充当机构身份。
- `admin_account`：管理员自己的链上钱包账户，持有私钥并代表 CID 签名。
