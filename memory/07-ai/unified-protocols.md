...
- 签名/验签规则：
  - CPMS 签名原文固定为 `sfid-cpms-v1|archive|{ano}|{cs}|{valid_from}|{valid_until}|{cpms_pubkey}|{geo_seal_hash}`。
  - SFID 先用授权 `install_secret` 解 `geo_seal`，再用 `cpms_pubkey` 验 `sig`。
  - 正式绑定必须发生在已有钱包地址的待绑定记录上，按 `ano / sfid_code / wallet_pubkey` 三者一对一约束拒绝重复绑定。
