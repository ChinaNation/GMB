...
- 编码：
  ...
  - 前两个字节固定为 `[0x11, 0x05]`
- 签名/验签规则：
  - `register_nonce / signature / province_name / signer_admin_pubkey` 由 SFID 机构注册信息凭证提供
  - runtime 通过 `(province_name, signer_admin_pubkey)` 查省级签名公钥并验签
...
- 签名/验签规则：
  - runtime 用 `(province_name, signer_admin_pubkey)` 查省级签名公钥并验签
...
- 消费者：
  ...
  - `citizenchain/runtime/governance/organization-manage`
- 字段：
  - 外层业务字段：`sfid_number`、`sfid_full_name`、`account_names`
  - 凭证字段：`credential.register_nonce`、`credential.province_name`、`credential.signer_admin_pubkey`、`credential.signature`
- 编码：
  - HTTP JSON 响应
  - runtime 验签 payload 按 SFID 后端 `build_institution_registration_info_credential` 的 SCALE tuple 顺序
- 签名/验签规则：
  - SFID 后端用省级签名密钥签发
  - 链端用 `province_name + signer_admin_pubkey` 查验签公钥
- 禁止兼容：不把 `subject_property / private_type / partnership_kind / parent_sfid_number` 纳入链端注册凭证
- 禁止事项：
  - 禁止用普通机构详情接口替代 `/registration-info`
  - 禁止 wuminapp 自己拼 `register_nonce / signature / province_name / signer_admin_pubkey`
...
- 签名/验签规则：
  - SFID 联邦管理员签发人口快照
  - runtime 按 `(province_name, signer_admin_pubkey)` 查省级签名公钥并验签
...
- 禁止兼容：开发期不兼容缺少 `province_name / signer_admin_pubkey` 的旧载荷
- 禁止事项：
  - 禁止节点或前端自行改写人口快照凭证字段
...
- 字段：
  ...
  - `propose_close(7.1)`：`duoqian_account`、`beneficiary`
- 编码：
  - SCALE call data
  - pallet index：`7`
  - call index：`0 / 1`
...
- 生产者：`wuminapp`、`wumin`　消费者：`GmbPqcAuth` 扩展 + `account-keys`、`wumin` decoder
...
- 规则（验序钉死，hash 全 `blake2_256`）：
  ...
  - ③ `ml_dsa_signature` 验交易 payload；`call_hash == blake2_256(body.call)`
  - 三验过 → origin 转 `Signed(account)` → nonce/扣费/业务 dispatch；**绑定写 `AccountPqcKey` 在 `post_dispatch`**
  - 失败语义：绑定在 post_dispatch（nonce/扣费已跑），**内层 call 失败绑定仍保留、内层失败照常收费**
...
- 详细文档：`memory/04-decisions/ADR-022-unified-pqc-crypto.md`
- 生产者：`wuminapp`、`wumin`　消费者：`GmbPqcAuth` 扩展、`wumin` decoder
...
- 规则（路线 A 定稿）：
  - `GmbPqcAuth` 读 `AccountPqcKey[account].pubkey` 验 ML-DSA 签名 + `call_hash==blake2_256(body.call)` → **把 origin 转 `Signed(account)`** → 后续 `CheckNonce`/`ChargeTransactionPayment` 走系统标准逻辑
  - 🔴 **tuple 12 上限**：嵌套 `(GmbPqcAuth, AuthorizeCall)` 占第一项槽位，不加第 13 项；`GmbPqcAuth` 兼管"已绑定拒 sr25519"
  - txpool `provides=(account,nonce)`；**era 默认 immortal**（链域靠 genesis_hash）
  - `validate` 轻量无副作用
