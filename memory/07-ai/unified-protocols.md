...
### P-SFID-001：SFID_NUMBER_V1

- 状态：当前
- 类型：接口契约 / 编码协议
- 唯一真源：`sfid/backend/number/validator.rs`
- 详细文档：`memory/05-modules/sfid/backend/number/NUMBER_TECHNICAL.md`
- 生产者：`sfid/backend/number/generator.rs`
- 消费者：`sfid/backend`、`sfid/frontend`、`cpms`、`wuminapp`、`citizenchain`
- 字段：
  1. `R5`:省码 2 位 + 市码 3 位
  ...
  5. `N9`:9 位稳定散列序列
  6. `D4`:年份
- 编码：`R5-K3P1C1-N9-D4`,示例 `LN001-GCB05-944805165-2026`
- 签名/验签规则：本协议只定义身份号码格式;链上或二维码签名按对应协议条目执行。
- 禁止兼容：不兼容历史格式,不保留历史字段别名。
- 禁止事项：
  - 禁止在 SFID 内部继续使用身份字段别名
  - 禁止恢复独立历史主体属性段
  - 禁止跳过 `C1` 校验
- 必跑测试：`cargo test --manifest-path sfid/backend/Cargo.toml number::`
...
- 签名/验签规则：
  ...
  - 抗量子迁移(草案,ADR-016):`body.sig_alg` 由写死 `sr25519` 扩为枚举 `sr25519 | ml-dsa-65`;`ml-dsa-65` 时 `body.pubkey` 为 ML-DSA-65 公钥、`signature` 为 ML-DSA-65 签名,冷热两端 decoder 必须按 `sig_alg` 分流。本扩展未实现前,`sig_alg` 仍只接受 `sr25519`。
- 禁止兼容：开发期严格模式，不做别名兼容
- 禁止事项：
...
### P-QR-003：WUMIN_QR_V1 / im_node_pairing

- 状态：当前
- 类型：扫码协议内业务 kind
- 唯一真源：`wuminapp/lib/qr/bodies/im_node_pairing_body.dart`
- 详细文档：
  - `memory/05-modules/wuminapp/im/IM_TECHNICAL.md`
  - `memory/05-modules/citizenchain/node/NODE_TECHNICAL.md`
- 生产者：`citizenchain/node/src/settings/communication-node/mod.rs`
- 消费者：`wuminapp/lib/im/im_node_settings_page.dart`
- 字段：
  ...
  - `body.expires_at_millis`
- 编码：外层 `WUMIN_QR_V1` 临时码；body 为 JSON 对象。
...
- 状态：当前
- 类型：接口契约 / 编码协议
- 唯一真源：`wuminapp/im/proto/im_envelope.proto`
- Dart 生成物：`wuminapp/lib/im/proto/im_envelope.pb.dart`、`wuminapp/lib/im/proto/im_envelope.pbenum.dart`、`wuminapp/lib/im/proto/im_envelope.pbjson.dart`
- 详细文档：
  - `memory/04-decisions/ADR-020-wuminapp-p2p-im.md`
  - `memory/05-modules/wuminapp/im/IM_TECHNICAL.md`
- 生产者：`wuminapp/lib/im/`、`citizenchain/node/src/im/`
- 消费者：`wuminapp/lib/im/`、`citizenchain/node/src/im/`
- 字段：
  ...
  - `ImRouteRecord.display_name`
  - `ImRouteRecord.im_device_id`
  - `ImRouteRecord.device_public_key_hex`
...
- 验收接口：
  ...
  - `im_submitEnvelope`
  - `im_submitDirectEnvelope`
  - `im_fetchPending`
  - `im_ackEnvelope`
  - `im_publishKeyPackage`
  - `im_fetchKeyPackages`
  - `im_consumeKeyPackage`
  - `im_fetchDirectKeyPackages`
  - `im_consumeDirectKeyPackage`
  - 以上 `im_*` 接口随节点 RPC 模块注册；实际调用必须由桌面设置页开启通信节点功能后放行。`GMB_IM_OWNER_RPC=1` 仅作为 headless 验收脚本的直接启用开关。
  - `im_debugGetCapability`
  - `im_debugRegisterOwnerDevice`
  - `im_debugSubmitDirectEnvelope`
  - `im_debugFetchPending`
  - `im_debugAckEnvelope`
  - `im_debugPublishKeyPackage`
  - `im_debugFetchKeyPackages`
  - `im_debugConsumeKeyPackage`
  - `im_debugFetchDirectKeyPackages`
  - `im_debugConsumeDirectKeyPackage`
  - 以上接口仅在节点进程设置 `GMB_IM_DEBUG_RPC=1` 时注册，供双节点运行态 smoke 使用，不属于正式开放 RPC。
- 编码：外层 Protobuf；OpenMLS 标准 wire bytes 放入 `mls_wire_message`；链内 SCALE 不作为 IM 主协议。
- 当前实现状态：Dart Protobuf 生成与 `ImEnvelope` / `ImKeyPackage` / `ImRouteRecord` round-trip 已通过；`ImEnvelope` 已正式承载 MLS message kind 与 Welcome ratchet tree；OpenMLS native 边界通过现有 `libsmoldot` C ABI 调用 Rust OpenMLS，可生成真实 KeyPackage、返回设备签名公钥、完成两方 round-trip smoke、创建持久化 MLS 会话、处理 Welcome、解密 application，并在 App 重启后恢复同一会话；公民端已新增 Isar 消息库、远程消息收发状态机、IM 路由缓存、联系人详情消息入口、信息 Tab 会话列表和“我的 -> 设置 -> 设置通信节点”配对页；桌面节点已把通信节点功能拆成独立开关并生成 `im_node_pairing` 二维码。
- 签名/验签规则：
  ...
  - 通信节点 mailbox 持久化路径固定为 `base-path/im/mailbox.json`，不得落入链数据库或 SFID 目录。
  - 通信节点 KeyPackage 池持久化路径固定为 `base-path/im/keypackages.json`，不得落入链数据库或 SFID 目录。
...
- 必跑测试：`cargo test -p node im::`、`cargo test -p node settings::node_mode`、`cargo test -p node settings::communication_node`、`cargo test`（`wuminapp/rust`）、`flutter test --concurrency=1 test/qr/im_node_pairing_body_test.dart test/im/im_node_settings_page_test.dart`、`flutter test --concurrency=1 test/im/im_route_cache_store_test.dart`、`flutter test --concurrency=1 test/im/im_tab_page_test.dart test/im/im_envelope_proto_test.dart test/im/im_mls_native_test.dart`、`flutter test --concurrency=1 test/im/im_mls_session_test.dart test/im/im_mls_native_session_test.dart`、`flutter test --concurrency=1 test/im/im_envelope_session_test.dart test/im/im_isar_store_test.dart test/im/im_chat_ui_adapter_test.dart`、Protobuf 跨端 round-trip、OpenMLS 加解密测试、KeyPackage 防重放测试、通信节点密文落盘检查。
...
- 禁止事项：
  ...
  - 禁止在本载荷末尾追加 `subject_property / private_type / partnership_kind / parent_sfid_number`
...
- 禁止兼容：不把 `subject_property / private_type / partnership_kind / parent_sfid_number` 纳入链端注册凭证
- 禁止事项：
...
- 生产者：`cpms/backend/dangan/mod.rs`
- 消费者：
...
- 签名/验签规则：
  ...
  - SFID 正式绑定必须以有效 ARCHIVE 为入口，不允许先保存空钱包账户；按 `archive_no / sfid_number / wallet_pubkey` 三者一对一约束拒绝重复绑定。
...
- 详细文档：
  - `memory/05-modules/sfid/SFID-CPMS-QR-v1.md`
  - `memory/05-modules/cpms/backend/dangan/DANGAN_TECHNICAL.md`
- 生产者：`cpms/backend/dangan/export.rs`
- 消费者：SFID 后续导入模块
- 字段：
  - `proto`
  - `type`
...
- 适用范围：
  - 本协议仅约束 **sr25519 外层签名**的 signed extrinsic；PQC(ML-DSA-65)交易不走本协议，见下方 ADR-016 注与 P-TX-009。
- 签名/验签规则：
  ...
  - 抗量子迁移(草案,ADR-016):PQC 交易改走 general-transaction(无外层 sr25519 签名),由 `AccountKeys.pqc_dispatch` 携带 ML-DSA-65 签名,在 `#[pallet::authorize]` 授权阶段验签;canonical AccountId 仍为原 sr25519 派生值。详见 P-TX-009。
...
- 禁止兼容：开发期不兼容缺少 `province / signer_admin_pubkey` 的旧载荷
- 禁止事项：
  - 禁止节点或前端自行改写人口快照凭证字段
...
- 编码：
  ...
  - call index：`0`
  - 前两个字节固定为 `[0x0c, 0x00]`
  - 布局：`org:u8 + account_id:48 + Compact<Vec<AccountId32>> + new_threshold:u32_le`
- 签名/验签规则：
...
- 禁止事项：
  - 禁止 wuminapp 继续生成旧 `[org, account_id, new_admins]` 载荷。
  - 禁止 wumin 公民钱包解码旧载荷或忽略尾部多余字节。
...
- 禁止兼容：不兼容旧 `DuoqianAccounts` mirror
- 禁止事项：
...
- 字段：
  ...
  - `status`
- 生命周期事件：
  - `AdminAccountPendingCreated { subject, org, kind, creator, admin_count, threshold }`
...
- 编码：
  ...
  - 注册个人账户继续映射到既有 `AdminAccountKind::PersonalDuoqian`
  - 注册机构账户使用账户级 `AdminAccountKind::InstitutionAccount = 0x05`，payload 为账户 `AccountId` 前 32 字节并右填零
  - `注册机构归属关系 = 0x02` 保留为 SFID 机构归属/检索 ID，不作为新增账户级管理员主体
- 签名/验签规则：
...
- 禁止事项：
  ...
  - 禁止联邦管理员登录二维码与 CPMS 业务二维码签名密钥混用
  - 禁止新建任何派生协议名
...
### P-TX-008：AccountKeys.bind_pqc_key

- 状态：草案（ADR-016，待实现）
- 类型：交易载荷格式
- 唯一真源：`citizenchain/runtime/otherpallet/account-keys/src/lib.rs`（待实现）
- 详细文档：
  - `memory/04-decisions/ADR-016-account-key-pqc-migration.md`
  - `memory/05-modules/citizenchain/runtime/otherpallet/ACCOUNT_KEYS_PQC_TECHNICAL.md`
- 生产者：`wuminapp`、`wumin`
- 消费者：`citizenchain/runtime` account-keys pallet、`wumin` decoder
- 字段：
  1. `pqc_pubkey`（ML-DSA-65 公钥，1952 字节）
  2. `algo`（`0x02 = ML-DSA-65`）
  3. `pqc_self_sig`（ML-DSA-65 对 challenge 的自签名）
- 编码：
  - SCALE call data
  - pallet index：`27`，call index：`0`
- 签名/验签规则：
  - 外层 extrinsic 由 sr25519 正常签名（证明现账户主人，第一签）
  - `challenge = blake2_256(canonical_account ++ pqc_pubkey ++ account_key_nonce ++ genesis_hash)`
  - `pqc_self_sig` 必须是 `pqc_pubkey` 对 `challenge` 的有效 ML-DSA-65 签名（第二签）
  - 两签皆过才写 `BoundPqcKey`；算法升级（65→87）走同一 extrinsic 换 `algo` 重绑
- 禁止兼容：不做历史兼容；canonical AccountId 永远是 sr25519 派生值，不因绑定 PQC 而改变
- 禁止事项：
  - 禁止把 ML-DSA 公钥纳入 `MultiSignature` 派生新 AccountId
  - 禁止链上存完整公钥（只存 `blake2_256(pqc_pubkey)`）
  - 禁止 SFID 托管助记词 / Root Seed / 私钥 / PQC 私钥
- 必跑测试：`account-keys` pallet 单测（bind 双签成功/失败、nonce 防重放、algo 升级重绑）

### P-TX-009：AccountKeys.pqc_dispatch
...
- 消费者：`citizenchain/runtime` account-keys pallet、`wumin` decoder
- 字段：
  1. `account`（canonical AccountId，sr25519 派生值）
  2. `call`（被授权派发的内层 `RuntimeCall`）
  3. `nonce`（`AccountKeyNonce[account]`）
  4. `sig`（ML-DSA-65 签名；公钥由链端按 account 从 `BoundPqcKey` 读，交易不带公钥）
- 编码：
  - general-transaction（无外层 sr25519 签名）
  - SCALE call data，pallet index：`27`，call index：`1`
- 签名/验签规则：
  - `#[pallet::authorize]` 阶段读 `BoundPqcKey[account]` 的公钥与 `state != Revoked`
  - `nonce` 必须等于链上 `AccountKeyNonce[account]`，授权后递增
  - `msg = blake2_256(call.encode() ++ nonce ++ genesis_hash)`，`verify_ml_dsa_65(BoundPqcKey[account].pubkey, msg, sig)` 必须通过
  - 授权成功后以 `RawOrigin::Signed(account)` 派发内层 `call`
- 禁止兼容：不得为 PQC 派发保留旧交易别名或旧载荷;`PqcOnly` 账户由 `RejectSr25519WhenPqcOnly` 扩展拒收 sr25519
- 禁止事项：
  - 禁止跳过 `nonce` 校验或 `genesis_hash` 链域隔离
  - 禁止 decoder 解码后仍有剩余字节
- 必跑测试：`account-keys` pallet 单测（authorize 成功/拒绝、PqcOnly 拒 sr25519）、`wumin` decoder 测试

### P-STORAGE-005：AccountKeys.BoundPqcKey

- 状态：草案（ADR-016，待实现）
- 类型：storage 契约
- 唯一真源：`citizenchain/runtime/otherpallet/account-keys/src/lib.rs`（待实现）
- 详细文档：`memory/05-modules/citizenchain/runtime/otherpallet/ACCOUNT_KEYS_PQC_TECHNICAL.md`
- 生产者：`account-keys` pallet（`bind_pqc_key` 写入）
- 消费者：`account-keys`（`pqc_dispatch` authorize 读）、`offchain-transaction`（批签取公钥）、`RejectSr25519WhenPqcOnly`
- 字段：
  - key：canonical `AccountId`（sr25519 派生值）
  - value `PqcKeyRecord`：`algo:u8`、`pubkey:BoundedVec<u8,2048>`（ML-DSA-65 公钥 1952B）、`state:KeyState(Active/PqcOnly/Revoked)`、`bound_at:BlockNumber`
  - 配套 `AccountKeyNonce: AccountId -> u32`（PQC 交易防重放）
- 编码：SCALE，`StorageMap<Blake2_128Concat, AccountId, PqcKeyRecord>`
- 签名/验签规则：不适用（storage）；存完整公钥，不存私钥
- 禁止兼容：不得新增 PQC 公钥派生 AccountId、旧字段别名或旁路读取;账户状态机 `Sr25519Only → Bound → PqcOnly`,`pqc_dispatch` 不带公钥、按 account 读此表
- 禁止事项：
  - 禁止把私钥写入 state（只存公钥）
  - 禁止用 PQC 公钥（或其 hash）作为新的 AccountId
