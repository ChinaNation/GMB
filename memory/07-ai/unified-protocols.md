...
- 唯一真源：
  ...
  - 两 call 参数形态完全相同（下列 18 字段），仅 pallet 前缀不同
- 详细文档：
  - `memory/01-architecture/qr/qr-action-registry.md`
- 生产者：
...
- 字段：
  ...
  8. `accounts`
  9. `institution_code`
  10. `roles`（`Vec<InstitutionRole>`）
  11. `assignments`（`Vec<InstitutionAdminAssignment>`，创建只接受 `Registry`）
  12. `threshold`
  13. `register_nonce`
  14. `signature`
  15. `actor_cid_number`
  16. `credential_signer_pubkey`
  17. `scope_province_name`
  18. `scope_city_name`
- 编码：
  - SCALE call data
...
- 签名/验签规则：
  ...
  - CID 注册凭证签名覆盖机构名称、法定代表人三字段、账户名列表、完整岗位/任职 SCALE 载荷和 `town_code`；任一字段不得改包。
...
- 字段：
  - key：`account_id`（机构为确定性主账户；个人多签为 personal account）
  - 机构 value：`cid_number`、`institution_code`、`admins: BoundedVec<AccountId32>`、`status`
  - 个人多签 value：继续使用独立 `AdminAccount`，字段为 `cid_number`、`institution_code`、`kind`、`admins: BoundedVec<AccountId32>`、`creator`、`created_at`、`updated_at`、`status`
...
- 同 pallet 的 `Institutions[cid_number] → InstitutionInfo`（2026-07-13 法定代表人三字段已收口到 entity）：
  - value 字段顺序固定为 `cid_full_name`、`cid_short_name`、`town_code`、`legal_representative_name: Option<_>`、`legal_representative_cid_number: Option<_>`、`legal_representative_account: Option<AccountId>`、`institution_code`、`created_at`、`status`（9 项）
  ...
  - 消费方镜像须按机构码路由 PublicManage/PrivateManage 前缀；node、OnChina、CitizenApp 已按上述 9 字段 SCALE 顺序对齐
