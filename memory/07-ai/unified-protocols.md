...
- 签名/验签规则：
  ...
  - 抗量子升级(ADR-022):PQC 交易改走 general-transaction(无外层 sr25519 签名),由 `account-keys.pqc_dispatch` 携带 ML-DSA-65 签名,在 `#[pallet::authorize]` 阶段从 `AccountPqcKey` 读公钥验签;未绑定账户首次走 `bootstrap_pqc_dispatch` 无感绑定;AccountId 仍为原 sr25519 锚点。详见 P-TX-008/009。
...
### P-TX-008：account-keys.bootstrap_pqc_dispatch（未绑定账户首次无感绑定+执行）

- 状态：草案（ADR-022，待实现）
- 类型：交易载荷格式（general-transaction）
- 唯一真源：`citizenchain/runtime/otherpallet/account-keys/src/lib.rs`（待实现）
- 详细文档：`memory/04-decisions/ADR-022-unified-pqc-crypto.md`
- 生产者：`wuminapp`、`wumin`　消费者：`citizenchain/runtime` account-keys pallet、`wumin` decoder
- 字段：`account`、`pqc_pubkey`(ML-DSA-65,~1952B)、`alg`(0x02)、`key_version`、`call`、`nonce`、`sr25519_bootstrap_signature`、`ml_dsa_signature`
- 编码：general-transaction，pallet index `27`，call index `1`
- payload `GMB_PQC_BOOTSTRAP_V1`：`genesis_hash`、`spec_version`、`account`、`pqc_pubkey_hash`、`sig_alg`、`key_version`、`nonce`、`call_hash`
- 规则：
  - `sr25519_bootstrap_signature` 验 bootstrap challenge（证旧地址主人，`account` = sr25519 公钥派生的当前 AccountId）
  - `ml_dsa_signature` 验交易 payload
  - 两签皆过 → 写 `AccountPqcKey[account]` → 立即以 `RawOrigin::Signed(account)` 派发内层 `call`
  - 已绑定账户拒绝再次 sr25519 覆盖（first-bind-wins）
- 禁止：扩 `MultiSignature`；用 PQC 公钥/hash 派生新 AccountId；SFID 托管助记词/私钥
- 必跑测试：bootstrap 双签成功/拒绝、已绑定拒覆盖、写表+派发原子性

### P-TX-009：account-keys.pqc_dispatch（已绑定账户 PQC 交易）

- 状态：草案（ADR-022，待实现）
- 类型：交易载荷格式（general-transaction）
- 唯一真源：`citizenchain/runtime/otherpallet/account-keys/src/lib.rs`（待实现）
- 详细文档：`memory/04-decisions/ADR-022-unified-pqc-crypto.md`
- 生产者：`wuminapp`、`wumin`　消费者：account-keys pallet、`wumin` decoder
- 字段：`account`、`call`、`nonce`、`sig`(ML-DSA-65；公钥由链端按 account 从 `AccountPqcKey` 读，交易不带公钥)
- 编码：general-transaction，pallet index `27`，call index `2`
- payload `GMB_PQC_TX_V1`：`genesis_hash`、`spec_version`、`transaction_version`、`ss58_format`、`account`、`nonce`、`era_or_deadline`、`tip`、`call_hash`、`sig_alg`、`key_version`、`auth_mode`
- 规则：
  - `#[pallet::authorize]` 读 `AccountPqcKey[account].pubkey` 验签 → 以 `RawOrigin::Signed(account)` 派发内层 `call`
  - `nonce` 用 `frame_system::AccountInfo.nonce`（不另造）；txpool `provides=(account,nonce)`
  - 费用从 `account` 扣；authorize/validate 轻量无副作用
- 禁止：跳过 `nonce`/`genesis_hash` 域隔离；decoder 解码后仍有剩余字节
- 必跑测试：authorize 成功/拒绝、nonce 防重放、`wumin` decoder
...
- 详细文档：`memory/04-decisions/ADR-022-unified-pqc-crypto.md`
- 生产者：account-keys（`bootstrap_pqc_dispatch` 写）　消费者：account-keys（`pqc_dispatch` authorize 读）、`offchain-transaction`（批签取公钥）
- 字段：
  - key：`AccountId`（sr25519 锚点）
  - value：`alg:u8`(0x02)、`key_version:u32`、`pubkey:BoundedVec<u8>`(完整 ML-DSA-65 公钥 ~1952B)、`bound_at:BlockNumber`、`bootstrap_mode`
- 编码：SCALE，`StorageMap<Blake2_128Concat, AccountId, AccountPqcKeyRecord>`
- 规则：存完整公钥（非 hash，每笔不带公钥省体积）；first-bind-wins；换算法由当前有效 PQC 私钥授权 + `key_version++`
