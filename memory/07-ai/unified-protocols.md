...
- 消费者：
  ...
  - `wumin/lib/signer/payload_decoder.dart`
- 字段：
  - `propose_create(7.0)`：`account_name`、`admin_count`、`admins`、`threshold`、`amount`
...
- 必跑测试：
  - `wumin/test/signer/payload_decoder_test.dart`
  - `wuminapp/test/duoqian/personal_pending_create_lookup_test.dart`
  - `wuminapp/test/duoqian/personal_proposal_history_service_test.dart`
...
- 消费者：
  ...
  - `citizenchain/node`
  - `wuminapp/lib/duoqian/shared/admin_institution_codec.dart`
- 字段：
  - key：`SubjectId`
...
- 消费者：
  - `citizenchain/node/src/offchain/organization_manage/chain.rs`
  - `wuminapp/lib/duoqian/shared/duoqian_storage_codec.dart`
- 字段：
  - key1：`sfid_number`
  - key2：`account_name`
...
- 禁止事项：
  ...
  - 禁止把机构账户当个人多签账户读取
- 必跑测试：
  - `wuminapp/test/duoqian/duoqian_discovery_service_test.dart`
...
- 字段：
  ...
  - value：`PersonalDuoqian` 状态信息，以 runtime 类型为准
- 编码：
  - storage map key
  - value：SCALE
...
- 编码：
  ...
  - 注册机构账户执行阶段需新增账户级 `SubjectKind`，推荐 `InstitutionAccount = 0x05`，payload 为账户 `AccountId` 32 字节并右填零
- 签名/验签规则：
