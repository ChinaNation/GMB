...
- 编码：
  ...
  - 前两个字节固定为 `[0x11, 0x05]`
- 签名/验签规则：
  - `register_nonce / signature / issuer_sfid_number / issuer_main_account / signer_pubkey / scope_*` 由 SFID 机构注册信息凭证提供
...
- 字段：
  ...
  - 凭证字段：`credential.register_nonce`、`credential.issuer_sfid_number`、`credential.issuer_main_account`、`credential.signer_pubkey`、`credential.scope_province_name`、`credential.scope_city_name`、`credential.signature`
- 编码：
  - HTTP JSON 响应
...
- 禁止事项：
  ...
  - 禁止 wuminapp 自己拼 `register_nonce / signature / issuer_sfid_number / issuer_main_account / signer_pubkey / scope_*`
...
- 禁止事项：
  - 禁止 wuminapp 继续生成旧 `[org, account_id, admins]` 载荷。
...
- 禁止事项：
  ...
  - 禁止 SFID 管理员登录二维码与 CPMS 业务二维码签名密钥混用
  - 禁止新建任何派生协议名
