...
- 普选/互选终态通过 `InstitutionAssignmentResult` 交给 entity；结果字段为 `institution_code`、`institution_account`、`role_code`、`admin_accounts`、`term_start`、`term_end`、`assignment_source`、`assignment_source_ref`。
- `term_start`、`term_end` 统一为自纪元起 `u32` 天；entity 替换目标岗位任职后从机构全部有效任职派生 admins，并与 admins storage 同事务提交。
...
- 签名/验签规则：
  ...
  - **onchina 控制台链写动作码(`b.d`=裸 SCALE call data,冷钱包解码核对后冷签 origin 由 CitizenWallet 提交)**:链交易统一 `a=(pallet<<8)|call`(禁止扁平小整数,会撞非链动作码 1..9)。机构创建=公权 `0x1e05`(PublicManage 30/call 5)/私权 `0x1f05`(PrivateManage 31/call 5,见 P-TX-001);公民投票身份注册=`0x0a00`(CitizenIdentity 10/call 0,见 P-TX-011);公民参选身份上链=`0x0a01`(CitizenIdentity 10/call 1,见 P-TX-011);机构管理员任职没有独立扫码 call，由创世、注册局创建或选举终态结果写入 entity；个人多签管理员集合变更为 `0x1d00`(PersonalAdmins 29/call 0,见 P-TX-007);CREG 市注册局无独立链动作码；非链文本治理 `a=3 = ACTION_ONCHINA_ADMIN / QR_ACTION_ONCHINA_ADMIN`(onchina_admin_governance JSON)。动作码由 `onchina/src/core/institution_call.rs::chain_action_code(pallet,call)` 与 call data 同源派生,非链常量在 `core/qr/mod.rs`,runtime 注释真源在 `primitives::sign`,均与 `qr-action-registry.md` 同步；动作码 `8` 已取消登记，Chat 设备绑定不得进入 QR。
