...
- 链上交易 call data 字段顺序
- SCALE 编码载荷格式
- SFID / CPMS / citizenapp / citizenchain 之间的 API 契约
...
- 消费者：`sfid/backend`、`sfid/frontend`、`cpms`、`citizenapp`、`citizenchain`
- 字段：
  1. `R5`:省码 2 位 + 市码 3 位
...
- 生产者：`citizenapp`、`citizenchain/node`、`sfid`、`cpms`
- 消费者：`citizenwallet`、`citizenapp`、`sfid`、`cpms`
...
- 详细文档：
  ...
  - `memory/01-architecture/qr/qr-action-registry.md`
- 生产者：
  - `citizenapp/lib/governance/organization-manage/duoqian_manage_service.dart`
...
- 必跑测试：
  - `citizenapp/test/governance/organization-manage/duoqian_manage_service_test.dart`
...
- 禁止事项：
  ...
  - 禁止 citizenapp 自己拼 `register_nonce / signature / issuer_sfid_number / issuer_main_account / signer_pubkey / scope_*`
...
- 签名/验签规则：
  ...
  - SFID 绑定阶段必须要求 citizenapp 对 SFID 绑定 challenge 签名，并校验签名公钥等于 ARCHIVE 中的 `wallet_pubkey`。
  - SFID 绑定成功后直接形成本地电子护照绑定结果，并通过 citizenapp 状态查询接口返回；绑定公民电子护照流程不再设计额外确认步骤。
...
- 禁止事项：
  ...
  - 禁止把 `account_id` 注释成当前 `InstitutionPalletId`
  - 禁止在 `citizenapp/lib/governance/organization-manage/`、`citizenchain/node/src/governance/`、`citizenchain/node/frontend/governance/` 或 `citizenchain/node/src/offchain/` 中实现多签转账业务
- 必跑测试：
  - `citizenwallet/test/signer/payload_decoder_test.dart`
...
- 禁止事项：
  - 禁止 citizenapp 继续生成旧 `[org, account_id, admins]` 载荷。
