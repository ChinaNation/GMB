...
- 字段：
  ...
  13. `register_nonce`
  14. `signature`
  15. `issuer_cid_number`
  16. `issuer_main_account`
  17. `signer_pubkey`
...
- 字段：
  ...
  - `status`
- 编码：
  - 机构账户以机构确定性主账户作为 `AdminAccounts` key；子账户通过 entity 解析到同一主账户，不建立独立管理员集合。
...
- 签名/验签规则：
  ...
  - 动态机构与个人多签读取 `internal-vote` Active 动态阈值；固定治理机构读取代码级固定阈值。
