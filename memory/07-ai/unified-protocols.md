...
- 状态：草案（阶段 3 Worker / R2 本地服务已落地；阶段 5 App 上传与上链闭环已落地；阶段 6 Worker 链上事件确认和正式 feed 入库已落地；阶段 10 已改为链上扣费入块后再上传 R2，并增加本地草稿保护；阶段 11 已完成 staging 部署和 prepare 远端 smoke；阶段 12 已配置 staging `SQUARE_CHAIN_RPC_URL` 并通过 R2 上传 + Worker 链确认负向 smoke；阶段 13 已确认 staging 测试热钱包可签名且余额足够，Cloudflare Tunnel/DNS/RPC 已恢复，但正向发布确认 smoke 阻塞于国储会节点当前 runtime metadata 未包含 `SquarePost/publish_square_post/SquarePostPublished`）
- 类型：接口契约 / storage 契约
- 唯一真源：
...
- HTTP API 字段：
  ...
  - `GET /v1/square/media/{object_key}` 公开读取 R2 媒体/头像/背景对象（只允许 `square/`、`profile/` 前缀，拒 `..`）；长缓存 + CORS，供 `Image.network` 与 CDN。
...
- R2 manifest 字段（阶段 5 App 端实际生成的规范化内容清单）：
  ...
  - `text`（文章正文 ≤19890 字）
  - `media_items[]`（文章：`[0]`=首图，`[1..]`=正文图，≤64）
  - `media_items[].media_kind`: `image` / `video`
  - `media_items[].file_name`
  - `media_items[].content_type`
  - `media_items[].byte_size`
  - `media_items[].sha256`
- 文章分类：链上仍发 Normal（不改 `post_category` 枚举、不重新创世）；`content_format`/`title` 只落链下 R2 manifest + D1（confirm 时从 manifest 写入 `square_posts.content_format`(默认 normal)/`title` 两列）。`content_hash` 覆盖整份 manifest（含 content_format/title），上链防篡改。
- R2 manifest 边界：
...
- Worker 环境变量：
  ...
  - `SQUARE_CHAIN_RPC_URL`：Worker 用于读取指定区块 `System.Events` 的链 RPC 地址；只允许由部署环境提供，不写入仓库和 CitizenApp。staging 已配置该 secret，并已验证 Worker 能读取链 RPC；完整 URL 不写入仓库文档。阶段 13 当前阻塞点不是 RPC 连通性，而是远端 runtime metadata 尚未暴露广场 `SquarePost` pallet。
