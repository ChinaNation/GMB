...
- 字段：
  ...
  7. `signature`
  8. `actor_cid_number`
  9. `credential_signer_pubkey`
  10. `scope_province_name`
  11. `scope_city_name`
- 编码：
  - SCALE call data
...
- 签名/验签规则：
  ...
  - CID 注册凭证签名覆盖机构名称、`admins` 完整 SCALE 载荷和 `town_code`；任一字段不得改包。
...
- 编码：
  ...
  - value：SCALE
- 签名/验签规则：storage 本身不签名；call index 5 的机构创建凭证必须覆盖 `admins(admin_name + admin_account)` 完整 SCALE 载荷，任一姓名或账户被替换均验签失败
