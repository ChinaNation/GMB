...
- 禁止事项：
  ...
  - 禁止在本载荷末尾追加 `subject_property / sub_type / parent_sfid_number`
...
- 兼容策略：不把 `subject_property / sub_type / parent_sfid_number` 纳入链端注册凭证
- 禁止事项：
...
- 兼容策略：不做历史兼容；canonical AccountId 永远是 sr25519 派生值，不因绑定 PQC 而改变
- 禁止事项：
...
- 兼容策略：与 sr25519 通道并行（hybrid 阶段）；`PqcOnly` 账户由 `RejectSr25519WhenPqcOnly` 扩展拒收 sr25519
- 禁止事项：
...
- 兼容策略：账户状态机 `Sr25519Only → Bound → PqcOnly`，三态下 canonical AccountId 不变；`pqc_dispatch` 不带公钥、按 account 读此表
- 禁止事项：
  - 禁止把私钥写入 state（只存公钥）
