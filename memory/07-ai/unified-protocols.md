...
- 签名/验签规则：
  ...
  - **onchina 控制台链写动作码(`b.d`=裸 SCALE call data,冷钱包解码核对后冷签 origin 由 CitizenWallet 提交)**:链交易统一 `a=(pallet<<8)|call`(禁止扁平小整数,会撞非链动作码 1..7)。机构创建=公权 `0x2005`(PublicManage 32/call 5)/私权 `0x2105`(PrivateManage 33/call 5,见 P-TX-001);管理员集合=CREG `0x0c01`(`federal_set_city_registry_admins`)/FRG `0x0c00`(`propose_admin_set_change`,见 P-TX-007);非链文本治理 `a=3 = ACTION_CID_ADMIN`(cid_admin_governance JSON)。动作码由 `onchina/src/core/institution_call.rs::chain_action_code(pallet,call)` 与 call data 同源派生,非链常量在 `core/qr/mod.rs`,均与 `qr-action-registry.md` 同步。
...
- 编码：
  ...
  - 机构布局：`institution_code:[u8;4] + account_id:[u8;32] + Compact<Vec<AdminProfile>> + new_threshold:u32_le`；`AdminProfile = account:[u8;32] + admin_cid_number:Compact<Vec<u8>>(≤32) + name:Compact<Vec<u8>>(≤128) + title:Compact<Vec<u8>>(≤128) + term_start:u32_le + term_end:u32_le + source:u8`(0..=4=创世/注册局/内部投票/互选/普选)。account_id 为 `AccountId32`=32 字节裸(onchina `institution_call.rs::encode_admin_set_call` 跨真类型对拍锁定;旧文档误记 48)
  - **关联调用 `GenesisAdmins.federal_set_city_registry_admins`(pallet 12 / call 1,前缀 `[0x0c,0x01]`)**:联邦注册局向下直设市注册局(CREG)管理员集合,绕内部投票;布局同上"机构布局"(institution_code+account+Compact<Vec<AdminProfile>>+threshold)。onchina 构造器:`institution/admins/admin_set_call.rs`(CreateCityRegistry/DeleteCityRegistry 用 federal_set;ReplaceFederalRegistry 用本 pallet call 0 propose_admin_set_change 改 FRG 自身集合)。
- 签名/验签规则：
