...
- 状态：草案（阶段 3 Worker / R2 本地服务已落地；阶段 5 App 上传与上链闭环已落地；阶段 6 Worker 链上事件确认和正式 feed 入库已落地；阶段 10 已改为链上扣费入块后再上传 R2，并增加本地草稿保护）
- 类型：接口契约 / storage 契约
- 唯一真源：
  ...
  - 落地后实现真源：`citizenapp/cloudflare/square_worker/` 与 `citizenapp/lib/8964/`
- 详细文档：
...
- 生产者：
  ...
  - `citizenapp/cloudflare/square_worker/`：会员、上传回执、链上事件确认、feed、关注关系、推荐信号
- 消费者：
  - `citizenapp/lib/8964/`
...
- HTTP API 字段：
  ...
  - `GET /v1/square/membership` 请求：Bearer `session_token`；响应：`membership`、`active`
  - `POST /v1/square/uploads/prepare` 请求：Bearer `session_token`，`post_category`、`media_items[]`、`manifest_hash`；响应：`upload_id`、`post_id`、`storage_receipt_id`、`expires_at`、`estimated_bytes`、`manifest_object_key`、`manifest_upload_url`、`media_items[]`。本接口只固定链上索引和短期上传授权，不写 R2 对象。
  ...
  - `POST /v1/square/uploads/complete` 请求：Bearer `session_token`，`upload_id`、`manifest_hash`、`content_hash`；响应：`upload_id`、`post_id`、`content_hash`、`storage_receipt_id`、`storage_state`。返回的 `storage_receipt_id` 必须等于 prepare 阶段预生成的回执。
...
- R2 object key：
  ...
  - `square/{owner_account}/posts/{post_id}/cover.{jpg|png|webp}`
  - `square/{owner_account}/posts/{post_id}/media_001.{jpg|png|webp}`
  - `square/{owner_account}/posts/{post_id}/media_002.{jpg|png|webp}`
  - `square/{owner_account}/posts/{post_id}/video_001.{mp4|webm}`
...
- R2 manifest 边界：
  ...
  - `post_id`、`manifest_object_key`、`media object_key` 由 Worker 在 `uploads/prepare` 后生成，真源在 D1 `square_uploads.object_keys_json`，不要求 App 在 prepare 前写入 manifest。
- D1 表字段：
  - `square_login_challenges`: `challenge_id`、`owner_account`、`signing_payload`、`expires_at`、`used_at`
  - `square_memberships`: `owner_account`、`membership_level`、`storage_quota_bytes`、`storage_used_bytes`、`expires_at`、`updated_at`
...
- Worker 环境变量：
  - `CITIZENAPP_SQUARE_API_BASE_URL`：CitizenApp 编译期 define，用于指定广场 Worker HTTPS API 根地址；生产构建必须显式提供，本地调试可省略并使用 `http://127.0.0.1:8787`。
  - `SQUARE_CHAIN_RPC_URL`：Worker 用于读取指定区块 `System.Events` 的链 RPC 地址；只允许由部署环境提供，不写入仓库和 CitizenApp。
...
- Worker 阶段 6 实现真源：
  - `citizenapp/cloudflare/square_worker/src/chain/rpc.ts`
  - `citizenapp/cloudflare/square_worker/src/chain/square_event.ts`
  - `citizenapp/cloudflare/square_worker/src/posts/confirm.ts`
  - `citizenapp/cloudflare/square_worker/src/posts/repository.ts`
- 签名/验签规则：
  - Worker session 必须由钱包账户对 `signing_payload` 签名获得。
  - Worker 只能把钱包签名证明用于登录和上传授权，不得托管钱包私钥。
  - R2 上传授权必须短期有效，且绑定 `owner_account`、`upload_id`、`object_keys` 和容量校验结果。
  - CitizenApp 必须先用 finalized 余额确认钱包至少保留 `2.11 元`（ED 1.11 元 + 发布费 1 元），余额不足不得进入 Worker prepare 或 R2 上传。
  - CitizenApp 必须在链上扣费交易入块后才上传 R2；链上未入块不得占用 R2 存储，只能保存本地草稿。
  - 生产环境 R2 上传必须使用 Worker 基于 R2 S3 凭证签发的短期 PUT URL；本地开发代理 `dev-put` 不得部署为生产上传入口。
  ...
  - Worker 确认发布时必须同时校验 session 钱包、D1 上传记录、链上事件、R2 manifest；任一不一致不得入库为 `published`。
- 禁止兼容：开发期不兼容旧广场入口壳数据格式，不保留旧“提案广场”feed 作为个人动态广场接口。
- 禁止事项：
  - 禁止把 R2 API key、Cloudflare token、D1 凭证写入 CitizenApp 或仓库。
  - 禁止要求 App 用户直接注册 Cloudflare 账户或直接向 R2 付费。
...
- 唯一真源：
  ...
  - 落地后实现真源：`citizenapp/cloudflare/square_worker/src/chat/` 与 `citizenapp/lib/im/transport/`
- 生产者：
  - `citizenapp/lib/im/`：OpenMLS 加密、KeyPackage、密文 envelope、ack 删除确认。
  - `citizenapp/cloudflare/square_worker/`：临时密文 mailbox、KeyPackage 池、WebSocket 新密文通知。
- 消费者：
  - `citizenapp/lib/im/`
  - Cloudflare D1 database `citizenapp-square-db`
  - Cloudflare R2 bucket `citizenapp-square-media`
...
- 必跑测试：
  ...
  - Worker 链上事件解码与发布确认测试：阶段 6 已由 `citizenapp/cloudflare/square_worker/test/chain_confirm.test.ts` 覆盖。
...
- 生产者：
  ...
  - `citizenapp/cloudflare/square_worker/src/chat/`：Cloudflare 密文 mailbox 接口，不生成明文。
  - `citizenapp/android/im/`：Android 近场 transport。
  - `citizenapp/ios/im/`：iOS 近场 transport。
- 消费者：
  - `citizenapp/lib/im/`
  - `citizenapp/cloudflare/square_worker/src/chat/`
