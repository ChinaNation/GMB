...
- 签名/验签规则：
  ...
  - CPMS 签名原文固定为 `sfid-cpms-v1|archive|{ano}|{cs}|{valid_from}|{valid_until}|{cpms_pubkey}|{geo_seal_hash}|{wallet_address}|{wallet_pubkey}`。
  ...
  - SFID 正式绑定必须以有效 ARCHIVE 为入口，不允许先保存空钱包账户；按 `ano / sfid_code / wallet_pubkey` 三者一对一约束拒绝重复绑定。
- 兼容策略：开发期不兼容旧 `ve` 或 `status_updated_at` 字段签名原文。
- 禁止事项：
...
- 字段：
  ...
  - `ARCHIVE`：`proto`、`type`、`ano`、`cs`、`ve`、`cpms_pubkey`、`geo_seal`、`sig`
- 编码：
  - JSON
...
- 签名/验签规则：
  ...
  - `geo_seal` 明文仅包含 `sfid_number`，AES-GCM AAD 为 `sfid-cpms-v1|geo-seal|{ano}|{cpms_pubkey}`
  - `ARCHIVE` 由 CPMS 本机私钥签名
  - SFID 验收 `ARCHIVE` 时先解 `geo_seal`，再验 CPMS 本机签名和 `ano` 全局唯一性
- 兼容策略：开发期不兼容其他 `proto`
- 禁止事项：
