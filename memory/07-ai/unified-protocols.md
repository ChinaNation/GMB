...
- 状态：当前代码待本任务第 6、8 步替换；最终公民主体契约已冻结
...
- 字段：
  - `cast_popular_vote`: `[pallet=29, call=2] + proposal_id + candidate_subject`
  - `cast_mutual_vote`: `[pallet=29, call=3] + proposal_id + voter_role_code + candidate_subject`
- 编码：裸 SCALE call data；`proposal_id` 为 `u64`，`candidate_subject` 固定为 `CitizenSubject { cid_number, wallet_account }`。具体 SCALE 顺序在 runtime 实施步骤完成后以同名结构为唯一真源。
