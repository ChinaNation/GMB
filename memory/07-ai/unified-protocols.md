...
- 禁止事项：
  ...
  - 禁止把 `subject_id` 注释成当前 `InstitutionPalletId`
  - 禁止在 `wuminapp/lib/proposal/transfer/`、`wuminapp/lib/duoqian/`、`citizenchain/node/src/governance/`、`citizenchain/node/frontend/governance/` 或 `citizenchain/node/src/offchain/` 中实现多签转账业务
- 必跑测试：
  - `wumin/test/signer/payload_decoder_test.dart`
