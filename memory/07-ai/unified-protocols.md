...
- 状态：当前
- 类型：接口契约 / 编码协议
- 唯一真源：`citizenchain/registry/src/cid/validator.rs`
- 详细文档：`memory/05-modules/citizenchain/registry/src/cid/NUMBER_TECHNICAL.md`
- 生产者：`citizenchain/registry/src/cid/generator.rs`
- 消费者：`citizencode/backend`、`citizencode/frontend`、`citizenapp`、`citizenchain`
- 字段：
  1. `R5`:省码 2 位 + 市码 3 位
...
- 生产者：`citizenapp`、`citizenchain/node`、`cid`
- 消费者：`citizenwallet`、`citizenapp`、`cid`
...
- 状态：当前
- 类型：交易载荷格式
- 唯一真源：`citizenchain/runtime/private/organization-manage/src/lib.rs`
- 详细文档：
  - `memory/05-modules/citizenchain/node/offchain-transaction/NODE_CLEARING_BANK_TECHNICAL.md`
  - `memory/01-architecture/qr/qr-action-registry.md`
- 生产者：
  - `citizenapp/lib/transaction/organization-manage/institution_manage_service.dart`
  - `citizenchain/node/src/private/organization_manage/signing.rs`
- 消费者：
  - `citizenchain/runtime/private/organization-manage`
...
- 字段：
  ...
  5. `institution_code`
  6. `admins_len`
  7. `admins`（A2 起 = `Vec<AdminProfile>`，逐人 account+admin_cid_number+name+title+term_start+term_end+source；投票快照只取 `.account`，编码同 P-TX-007 机构布局的 AdminProfile）
  8. `threshold`
  9. `register_nonce`
  10. `signature`
...
- 签名/验签规则：
  ...
  - **待 B 阶段补全**：CID 注册凭证签名当前覆盖 `cid_full_name`,尚未覆盖 `cid_short_name`;B2 改 `P-CRED-001` 凭证 payload 时须把 `cid_short_name` 纳入签名,防止简称被篡改
...
- 禁止事项：
  ...
  - 禁止用裸非法人机构码（`SFGT/SFGP/UNIN`）直接创建 OrganizationManage 机构账户；非法人必须由 CID 上层明确归属后走对应管理员模块
  - 禁止 CitizenWallet decoder 解码后仍有剩余字节
- 必跑测试：
  - `citizenapp/test/governance/organization-manage/institution_manage_service_test.dart`
...
- 状态：当前
- 类型：凭证载荷 / 接口契约
- 唯一真源：`citizencode/backend/subjects/chain_multisig_info.rs`
- 详细文档：`memory/05-modules/citizenchain/node/offchain-transaction/NODE_CLEARING_BANK_TECHNICAL.md`
- 生产者：`citizencode/backend/subjects/chain_multisig_info.rs`
- 消费者：
  - `citizenchain/node/src/private/organization_manage/cid.rs`
  - `CitizenApp` 机构创建流程
  - `citizenchain/runtime/private/organization-manage`
- 字段：
...
- 禁止事项：
  ...
  - 禁止把 `account_id` 注释成当前 `InstitutionPalletId`
  - 禁止在 `citizenapp/lib/transaction/organization-manage/`、`citizenchain/node/src/governance/`、`citizenchain/node/frontend/governance/` 或 `citizenchain/node/src/transaction/offchain_transaction/` 中实现多签转账业务
- 必跑测试：
  - `citizenwallet/test/signer/payload_decoder_test.dart`
...
- 状态：当前
- 类型：交易载荷格式
- 唯一真源：`citizenchain/runtime/admins/{personal-admins,genesis-admins,public-admins,private-admins}/src/lib.rs`
- 详细文档：
...
- 消费者：
  - `citizenchain/runtime/admins/{personal-admins,genesis-admins,public-admins,private-admins}`
  - `citizenwallet/lib/signer/payload_decoder.dart`
- 字段：
  1. `institution_code`
  2. `account_id`
  3. `admins`（A2 起:**机构(genesis/public/private)= `Vec<AdminProfile>`**;个人多签 personal = `Vec<AccountId32>`,不带 profile）
  4. `new_threshold`
- 编码：
  - SCALE call data
  - pallet index：个人多签 `7`，创世管理员 `12`，公权机构 `29`，私权机构 `30`
  - call index：个人多签 `3`，创世/公权/私权 `0`
  - 前两个字节按 `AdminAccount.kind` 和主体类型选择，不再固定为 `[0x0c, 0x00]`
  - 个人多签布局：`institution_code:[u8;4] + account_id:48 + Compact<Vec<AccountId32>> + new_threshold:u32_le`
  - 机构布局：`institution_code:[u8;4] + account_id:48 + Compact<Vec<AdminProfile>> + new_threshold:u32_le`；`AdminProfile = account:32 + admin_cid_number:Compact<Vec<u8>>(≤32) + name:Compact<Vec<u8>>(≤128) + title:Compact<Vec<u8>>(≤128) + term_start:u32_le + term_end:u32_le + source:u8`(0..=4=创世/注册局/内部投票/互选/普选)
- 签名/验签规则：
...
- 状态：当前
- 类型：storage 契约
- 唯一真源：`citizenchain/runtime/admins/{personal-admins,genesis-admins,public-admins,private-admins}/src/lib.rs` + `citizenchain/runtime/admins/admin-primitives/src/lib.rs`
- 详细文档：
  - `memory/05-modules/citizenchain/runtime/admins/ADMINS_TECHNICAL.md`
  - `memory/05-modules/citizenapp/admins-change/ADMINS_CHANGE_CITIZENAPP_TECHNICAL.md`
...
- 字段：
  ...
  - `admins`（A2 起）：**机构 genesis/public/private = `BoundedVec<AdminProfile>`**(每人 account+admin_cid_number+name+title+term_start+term_end+source);**personal = `BoundedVec<AccountId32>`**(不带 profile)。`AdminAccountQuery::active_account_admins` 仍出 `Vec<AccountId>`(抽 `.account`)→投票/多签/阈值零改;`active_account_admin_profiles` 出完整资料供展示。创世机构 profile 由 `genesis_build`(CHINA_ZF) 填 account+source=Genesis+空 meta,逐人姓名/职务/任期留后期上链充实。
- 编码：
  - storage key：`twox128(pallet_name) ++ twox128("AdminAccounts") ++ blake2_128_concat(account_id)`
  - `pallet_name` 按 `AdminAccount.kind` 选择：`GenesisAdmins / PublicAdmins / PrivateAdmins / PersonalAdmins`
  - value：SCALE
...
- 消费者：
  - `citizenchain/node/src/private/organization_manage/chain.rs`
  - `citizenapp/lib/transaction/organization-manage/multisig_storage_codec.dart`
- 字段：
  - key1：`cid_number`
  - key2：`account_name`
...
- 同 pallet 的 `Institutions[cid_number] → InstitutionInfo`（A1 精简,2026-06-28）：
  ...
  - 消费方镜像须同步（B0 删 `node/.../organization_manage` 镜像;C2 改 `citizenapp` `decodeInstitutionInfo`）
- 编码：
...
- 禁止事项：
  ...
  - 禁止把机构账户当个人多签账户读取
- 必跑测试：
  - `citizenapp/test/governance/organization-manage/institution_manage_storage_test.dart`
...
- 生产者：
  ...
  - `citizenchain/runtime/admins/{genesis-admins,public-admins,private-admins,personal-admins}`
  - `citizenchain/runtime/private/personal-manage`
  - `citizenchain/runtime/private/organization-manage`
- 消费者：
  - `citizenchain/runtime/votingengine/internal-vote`
