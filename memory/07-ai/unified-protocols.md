...
- 机构法定代表人任免生效后，`InstitutionInfo` 必须公开原子结构 `legal_representative: Option<{ family_name, given_name, cid_number, account }>`；创世时没有真实任免资料的机构保持 `None`，不得伪造法定代表人或回退到 `admins[0]`。
- 所有机构管理员集合 `admins` 统一使用 `Admin { account_id, cid_number, family_name, given_name }`；公权、私权不得再建立不同字段布局。`cid_number` 是管理员本人公民 CID 引用，公民身份与账户绑定继续以 `citizen-identity` 为唯一真源。管理员集合只是可任职人员名册，账户本身没有机构业务权限；个人多签虽复用同一 SCALE 结构，但不属于机构管理员，其公民 CID 和姓名是否必填按个人多签规则处理。
- 机构授权主体固定为 `RoleSubject { cid_number, role_code }`。管理员只有同时属于该机构 admins 且对该主体存在有效任职，才能使用该岗位权限。
- CID 决定机构顶层业务能力；岗位权限由强类型 `RoleBusinessPermission(RoleSubject, BusinessActionId, operation)` 表达，`operation` 至少区分 `Propose` 与 `Vote`。不得建立泛化字符串权限表，也不得按管理员账户授权。
- 机构岗位 `InstitutionRole`、岗位权限、`InstitutionRoleNonce`、永久 `UsedRoleCodes` 与任职归 entity；业务动作、指定投票引擎、VotePlan 和通过后执行归对应业务模块。
- 机构管理员任职 `InstitutionAdminAssignment` 归 entity，字段为 `cid_number`、`account_id`、`role_code`、`term_start`、`term_end`、`assignment_source`、`assignment_source_ref`、`assignment_status`。
...
- 所有已完成业务统一通过 `InstitutionGovernanceResult` 交给 entity；一个结果可同时包含动态岗位定义变化、多个岗位的完整目标任职集合，以及可选的法定代表人原子结构整体设置或清空。
...
- 每个机构默认且唯一存在 `LR / 法定代表人` 岗位；允许空缺，不得从管理员首位推导法定代表人，岗位码和岗位名永久不可修改或删除。
- 全部创世固定岗位的岗位码、岗位名和岗位权限永久固定，包括公民链技术发展基金会的 `LR`、`GENESIS_PRODUCT_MANAGER`、`GENESIS_PROGRAMMER`。创世机构仍可增加普通动态岗位；NodeGuard 不得以“出现额外岗位”为由拒绝。
- 动态 `role_code` 由 runtime 生成，格式固定为 `R_<32 位大写十六进制>`。生成规则为取 `blake2_256(SCALE(GMB_ROLE_V1, cid_number, institution_role_nonce, proposal_id))` 前 16 字节；客户端不得提交岗位码，删除后 `UsedRoleCodes` 仍永久占用且永不复用。
- 动态岗位仅 `role_name` 可依法修改；岗位权限不可原地修改，改变权限必须删除旧岗位并生成新岗位码。
- 机构治理阈值唯一真源位于对应 entity 的 `InstitutionGovernanceThresholds[cid_number]`，与管理员人数、岗位数和席位数独立；固定治理机构仍由创世常量播种其制度阈值。任职结果不得携带或修改阈值。
- 任职不保存 `creator`；来源由 `assignment_source + assignment_source_ref` 唯一表达。
- 一个公民 `cid_number` 只能绑定一个钱包账户，一个钱包账户也只能绑定一个公民 CID。
- 一个管理员钱包账户可在多个机构、多个岗位任职；各完整 `RoleSubject` 权限互不串用。
- 普通机构创建必须原子建立 admins、强制 LR、至少一个初始治理岗位及权限、初始任职与初始投票规则；不允许临时管理员权限或创建后再补岗位的启动窗口。
...
- 字段：
  ...
  - `workspace.workspace_modules[]`: 当前实例被授权的模块；公民链基金会可包含 `platform_membership_price`
...
- 字段：顶层只允许 `p/k/i/e/b`;`k=1/2` 的压缩键 `b.u` 在各端唯一映射为 `signer_public_key`;`k=1` 的 `b.d` 是 `review_payload`,除 Runtime 升级 hash-only 外必须完整可解码和可中文展示;`k=3/4` 的展示地址只允许 `ss58_address`;具体字段以 `qr-protocol-spec.md` 为准
- 编码：紧凑 JSON envelope
...
- 禁止事项：
  ...
  - 禁止在 registry、decoder、生成物或 QR 端侧恢复含糊账户字段；单一账户只用 `account_id`，多账户按 `<role>_account_id` 命名
...
- HTTP API 字段：
  ...
  - `POST /v1/square/creator/subscription/confirm` 请求：Bearer `session_token`，`tx_hash`、`block_hash`、`signed_extrinsic_hex`、`action`(`subscribe`/`cancel`/`change`)、`creator_account_id`，订阅或换档时另带 `tier_id`、`billing_period`；响应：finalized 链上创作者订阅镜像。subscriber_account_id 只从 session 派生，Worker 必须核对完整签名交易和同一区块 `Subscriptions[(subscriber, Creator(creator_account_id))]`。
  ...
  - `POST /v1/square/uploads/complete` 请求：Bearer `session_token`，`upload_id`、`manifest_hash`、`content_hash`；响应：`upload_id`、`post_id`、`content_hash`、`storage_receipt_id`、`storage_state`(`completed`/`processing`)。Worker 读取 R2 manifest 并复核 manifest hash、account_id、`post_category`、`content_format`、标题 / 正文长度、媒体数量和 `square_media_assets` 一致性；返回的 `storage_receipt_id` 必须等于 prepare 阶段预生成的回执；视频上传已接收但 Stream 尚未 ready 时返回 `processing`。
  ...
  - `POST /v1/square/follows` 请求：Bearer `session_token`、`followed_account_id`
  - `DELETE /v1/square/follows/{followed_account_id}` 请求：Bearer `session_token`
  - `POST /v1/square/signals` 请求：Bearer `session_token`、`post_id`、`signal_type`
  - `GET /v1/square/users/{account_id}` 请求：可选 Bearer `session_token`；响应：`profile`（`account_id`、`display_name`、`bio`、`avatar_object_key`、`banner_object_key`、`cid_number`、`is_certified`、`counts{following,followers,posts}`、`is_following`、`updated_at`）。公开可读；带 session 时 `is_following` 反映登录者视角；认证以链上已确认发布携带的 `cid_number` 为真源。
  - `GET /v1/square/users/{account_id}/posts` 请求：可选 `category`（all/normal/campaign）、`content_format`（all/normal/article）、`limit`、`cursor`；响应：`posts[]`、`next_cursor`（按 `created_at` keyset 游标）。帖子 Tab 传 `content_format=normal` 排除文章，文章 Tab 传 `content_format=article`。
  - `GET /v1/square/users/{account_id}/follows` 请求：`type`（following/followers）、`limit`、`cursor`；响应：`accounts[{account_id,created_at}]`、`next_cursor`
  - `GET /v1/square/contacts` 请求：Bearer `session_token` + P-256 设备请求证明，可选 `limit`、`cursor`；响应：`items[{contact_id,ciphertext,nonce,mac,updated_at}]`、`next_cursor`。`account_id` 只从 session 派生，Worker 不接收联系人账户或联系人名称明文。
  ...
  - `PUT /v1/square/profile` 请求：Bearer `session_token`，可选 `display_name`(≤40)、`bio`(≤160)、`avatar_object_key`、`avatar_content_hash`、`banner_object_key`、`banner_content_hash`（头像/背景 key 只能分别为本人固定 `profile/{account_id_hex}/avatar`、`profile/{account_id_hex}/banner`）；响应：与 GET `users/{account_id}` 同构的完整 `profile`。
...
- R2 路径分段 `account_id_hex` 固定取规范 `account_id` 去掉 `0x` 后的 64 位小写十六进制；入口必须先按完整 AccountId 规则校验，不做清洗、大小写归一或 SS58 转换。
- 用户公开资料 R2 契约：`profile/{account_id_hex}/profile.json`（schema `citizenapp.square.profile.v1`：`account_id`、`display_name`、`bio`、`avatar_object_key`、`avatar_content_hash`、`banner_object_key`、`banner_content_hash`、`updated_at`）。计数与认证不入 profile.json，响应时由 D1/链上派生。头像/背景固定对象键分别为 `profile/{account_id_hex}/avatar` 与 `profile/{account_id_hex}/banner`。
- Feed item 字段：
  - `post_id`
  - `account_id`
  - `cid_number`
...
- Feed media item 字段：
  ...
  - `height`
- R2 object key：
  - `square/{account_id_hex}/posts/{post_id}/manifest.json`
  - `profile/{account_id_hex}/profile.json`
  - `profile/{account_id_hex}/{avatar|banner}`
...
- D1 表字段：
  - `square_login_challenges`: `challenge_id`、`account_id`、`signing_payload`、`expires_at`、`used_at`
  ...
  - `square_memberships`: 以钱包 `account_id` 为主键；`membership_level`、`pending_membership_level`、`started_at`、`last_charged_at`、`last_charged_price_fen`、`paid_until`、`subscription_status`、`finalized_block_number`、`finalized_block_hash`、`verified_at`、`entitlement_lapsed_at`、`last_tx_hash`。
  - `square_creator_tiers`: 复合主键 `(creator_account_id,tier_id)`；`name`、`tier_order`、三个公历周期的 finalized 价格镜像、`finalized_block_number`、`finalized_block_hash`、`verified_at`、`last_tx_hash`。名称是展示资料，价格镜像只用于证明与展示审计，链上 `CreatorPlans` 始终是扣款真源。
  - `square_creator_subscriptions`: 复合主键 `(subscriber_account_id,creator_account_id)`；当前及待生效 `tier_id`/`billing_period`、链上时间、最近扣款价格、状态、finalized 锚点、`verified_at`、`last_tx_hash`。
  - `chain_transaction_confirmations`: 以 `tx_hash` 为主键，保存首次绑定的钱包、区块、extrinsic 序号、动作、规范化 `request_hash`、链时间和确认时间；相同请求幂等，任何改绑拒绝。
  - `square_uploads`: `upload_id`、`post_id`、`account_id`、`post_category`、`manifest_hash`、`content_hash`、`storage_receipt_id`、`estimated_bytes`、`object_keys_json`、`status`、`created_at`、`completed_at`
  - `square_media_assets`: `upload_id`、`post_id`、`account_id`、`media_index`、`media_kind`、`provider`、`provider_asset_id`、`upload_method`、`resource_key`、`content_type`、`byte_size`、`asset_state`、`declared_duration_seconds`、`duration_seconds`、`width`、`height`、`error_code`、`created_at`、`updated_at`、`ready_at`、`archive_state`、`archived_at`、`r2_archive_key`
  - `square_posts`: `post_id`、`account_id`、`cid_number`、`post_category`、`content_format`、`title`、`text`、`content_hash`、`storage_receipt_id`、`chain_block`、`created_at`、`post_state`
  - `square_follows`: `account_id`、`followed_account_id`、`created_at`
  - `square_user_signals`: `account_id`、`post_id`、`signal_type`、`weight`、`created_at`
  - `resource_reservations`: `reservation_id`、`account_id`、`resource_key`、`period_start`、`period_end`、`byte_size`、`image_count`、`video_seconds`、`expires_at`、`reservation_state`、`created_at`、`used_at`
  - `resource_usage`: `account_id`、`resource_key`、`period_start`、`period_end`、`byte_size`、`image_count`、`video_seconds`、`updated_at`
...
- CitizenApp 本地缓存字段：
  ...
  - `ContactCache`: 按 `account_id` 隔离保存解密后的 `address`、`contact_name`、`created_at`、`updated_at`，复用 `AppKvEntity`，不是公开资料真源。
  - `ContactPendingOps`: 按 `account_id` 隔离保存待同步的添加、改名、删除操作；云端成功后立即移除，不把普通旧缓存当作待上传操作。
- 通讯录云端密文契约：
  - schema 固定为 `citizenapp.contacts.v1`；明文记录只存在 CitizenApp 端，字段固定为 `account_id`、`address`、`contact_name`、`created_at`、`updated_at`。
  - `encryption_key` 与 `index_key` 必须由同一热钱包 seed 经 HKDF-SHA256 域隔离派生；seed 和派生密钥不得上传 Cloudflare。
  - 单条记录使用 AES-256-GCM；AAD 必须绑定 schema、`account_id` 与 `contact_id`，防止跨账户或跨联系人替换。
  - D1 `square_contacts` 只允许保存 `account_id`、`contact_id`、`ciphertext`、`nonce`、`mac`、`updated_at`；禁止保存联系人 `address`、`contact_name`、公开昵称或关系明文。
...
- 消费者：
  - `citizenapp/lib/chat/`
  - Cloudflare D1 database `citizenapp-square-db`
  - Cloudflare Durable Objects / WebSocket，按 `account_id` 路由到 `ChatRealtimeObject`。
- HTTP API 字段：
  ...
  - `POST /v1/chat/keypackages` 请求：Bearer `session_token`，`account_id`、`device_id`、`device_public_key_hex`、`key_package_id`、`key_package`、`cipher_suite`、`created_at`、`expires_at`
  - `GET /v1/chat/keypackages/{account_id}` 请求：Bearer `session_token`，`limit`
  - `POST /v1/chat/keypackages/consume` 请求：Bearer `session_token`，`account_id`、`key_package_id`、`requester_account_id`
  - `POST /v1/chat/envelopes`：`envelope_id`、`sender_device_id`、`recipient_account_id`、`recipient_device_id`、`envelope`；仅在当前请求内转发。
  - `POST /v1/chat/signals`：`sender_device_id`、`recipient_account_id`、`recipient_device_id`、`signal`；仅转发 SDP/ICE/设备就绪信令。
  - `GET /v1/chat/ws`：Bearer session + `x-chat-device`；收到 `gmb_chat_envelope_v2` 时客户端立即解密，收到 `gmb_chat_signal_v1` 时交给 WebRTC。
  - Durable Object binding：`CHAT_REALTIME`，class `ChatRealtimeObject`，对象名称固定为 `account_id`。
- D1 表字段：
  - `chat_devices`: `account_id`、`device_id`、`device_public_key_hex`、`push_provider`、`push_token`、`expires_at`、`created_at`
  - `chat_device_binding_nonces`: `account_id`、`nonce_hash`、`expires_at`、`created_at`
  - `chat_keypackages`: `account_id`、`device_id`、`key_package_id`、`key_package`、`cipher_suite`、`created_at`、`expires_at`
...
- 签名/验签规则：
  ...
  - 设备绑定必须由同一硬件 P-256 设备子钥对 `signing_message(OP_SIGN_CHAT_DEVICE_BIND=0x1A, SCALE(account_id, device_id, device_public_key_hex, expires_at, nonce))` 签名；Worker 只用 session AccountId 查询 `square_device_subkeys.p256_public_key` 验签。
...
- 派生字段：
  - `account_id`：由 signed origin 派生，App 不得作为参数伪造。
  - `cid_number`：由 runtime 按 `account_id` 读取链上公民身份绑定派生；未认证用户为空。
...
- 链上 storage / event：
  - `SquarePosts[post_id] -> SquarePost`：保存 `post_id`、`account_id`、可空 `cid_number`、`post_category`、`content_hash`、`storage_receipt_id`、`storage_until`、`created_block`。
  - `PublishedPostCountByAccount[account_id] -> u64`：保存账户累计成功发布数量。
  - `SquarePostPublished` 事件：Worker 只能根据入块事件确认发布并写入正式 feed。
- 签名/验签规则：
  - 外层链交易必须由 `account_id` 对应钱包账户签名。
  ...
  - `post_category = campaign` 时 runtime 必须通过 `CitizenIdentity::citizen_subject(account_id)` 确认当前钱包与永久 CID 双向绑定完整，且 `VotingIdentityByCid[cid_number]` 的 `citizen_status = Normal`。
...
- call data 顺序：
  ...
  - 创作者订阅：`22 01 || 01 || creator_account_id[32] || 01 || Compact(tier_id_len) || tier_id || billing_period || expected_price_fen:u128LE`。
  - 平台取消：`22 02 || 00`。
  - 创作者取消：`22 02 || 01 || creator_account_id[32]`。
...
- 字段：
  ...
  - `b.u`:压缩传输键；端内唯一语义名为 `signer_public_key`，值为 32B 签名者公钥的 base64url 无填充编码
...
- 状态：runtime 第 3 步已完成，OnChina、CitizenApp、CitizenWallet 与 QR 编解码待本任务第 8 步同步；下列字段顺序为唯一目标协议
- 类型：交易载荷格式
- 唯一真源：`citizenchain/runtime/misc/citizen-identity/src/lib.rs`
- 详细文档：
  - `memory/01-architecture/qr/qr-action-registry.md`
...
- 编码：
  ...
  - `0x0a01 upgrade_to_candidate_identity` 的最终顺序为投票身份字段 + `birth_province_code / birth_city_code / birth_town_code / family_name / given_name / citizen_sex / birth_date`；其中 `birth_date` 为 `u32` 小端 `YYYYMMDD`，位于性别字段之后且为必填字段；该交易同时写入投票身份和参选身份。
  - pallet index：`10`
  - call index：`register_voting_identity = 0`，`upgrade_to_candidate_identity = 1`
  - 前两个字节分别固定为 `[0x0a, 0x00]`、`[0x0a, 0x01]`
  - 动作码分别为 `a=0x0a00`、`a=0x0a01`
- 签名/验签规则：
...
### P-TX-015：ElectionVote 投票交易载荷

- 状态：已完成 Runtime、CitizenWallet 严格解码和 QR 动作登记的最终载荷同步
- 类型：交易载荷格式 / runtime 调用边界
- 唯一真源：`citizenchain/runtime/votingengine/election-vote/src/lib.rs`
- 详细文档：`memory/05-modules/citizenchain/runtime/votingengine/VOTINGENGINE_TECHNICAL.md`
- 生产者：合法选举活动的投票客户端
...
- 禁止事项：
  ...
  - 禁止跨机构选举；具体业务模块只可绑定本机构 `actor_cid_number + role_code`。
  - 禁止 election-vote 直接写入 entity 岗位、任职或 admins；选举结果必须返回创建该提案的具体选举业务模块复核。
- 必跑测试：
...
- 签名/验签规则：
  ...
  - 签名字节固定为 `signing_message(OP_SIGN_CHAT_DEVICE_BIND=0x1A, payload)`；Worker 必须从 session 派生 owner，并用 `square_device_subkeys.p256_public_key` 验签。
  ...
  - Worker 必须校验 session sender、登记设备和 `ChatEnvelope.recipient_account_id`。
...
- 状态：已删除（2026-07-19；ADR-039 第 3 步关闭）
- 类型：交易载荷格式
- 唯一真源：无当前生产代码；PublicManage/PrivateManage 的 call index 5 永久留洞
- 生产者：无；OnChina 创建 API 在新业务模块落地前固定返回 `501 Not Implemented`
- 消费者：无；CitizenWallet 和 QR action registry 必须拒绝 `0x1e05`、`0x1f05`
- 字段：无当前字段；旧 `cid_number/cid_full_name/cid_short_name/town_code/admins/actor_cid_number` 布局已删除
- 编码：无当前编码；`[0x1e,0x05]` 与 `[0x1f,0x05]` 是永久留洞，不得复用
- 替代：任务卡第 6 步新增独立机构创建业务模块，原子绑定 admins、LR、初始治理岗位、不可变权限、初始任职和投票规则，并由业务模块静态指定投票引擎；新协议必须另行登记，不得恢复本条旧载荷
- 禁止兼容：不得保留旧 decoder、旧 QR 动作、旧 OnChina 构造器、旧 runtime extrinsic 或按旧字段序重建交易
- 必跑测试：QR registry 生成一致性、CitizenWallet 对两个留洞动作的拒绝测试、PublicManage/PrivateManage metadata/call 测试
...
- 字段：
  ...
  - `register_institution_admins(30/31.9)`：`cid_number`、`admins(admin_account + family_name + given_name)`、`register_nonce`、`signature`、`actor_cid_number`、`credential_signer_pubkey`、`scope_province_name`、`scope_city_name`
  - `InstitutionGovernanceAction` discriminant：`ReplaceAdmins=0`、`MutateRolesAndAssignments=1`、`ReplaceAdminsAndMutateRoles=2`。
  - 两个岗位变更分支字段顺序固定为 `role_mutations`、`assignment_changes`、`legal_representative_change`；组合分支在最前增加 `admins`。
  - `InstitutionRoleMutation` discriminant：`Create=0`、`Rename=1`、`Delete=2`。Create 字段为 `role_name`、`term_required`、`permissions`、`assignments`，禁止调用方提交岗位码；Rename 字段为 `role_code`、`role_name`；Delete 只有 `role_code`。
  - `RolePermissionSpec` 字段固定为 `business_action_id`、`operation`；entity 生成岗位码后补齐完整 `RoleSubject` 并写入 `InstitutionRolePermissions`。
- 编码：
  - SCALE 裸 call data
...
- 禁止兼容：
  ...
  - 禁止把 admins 当机构业务授权真源；岗位权限必须按完整 `RoleSubject` 与有效任职校验。
...
- 字段：
  - 外层业务字段：`cid_number`、`cid_full_name`、`cid_short_name`、法定代表人原子结构、`account_names`、岗位与任职
...
- 签名/验签规则：
  ...
  - 当前链端先确认 `credential_signer_pubkey` 属于该机构 admins 后验签；ADR-039 目标还必须由最终业务交易携带或确定签发岗位，并校验完整 `RoleSubject` 的登记权限与有效任职。凭证签名通过不能单独授予业务权限。
...
- 禁止事项：
  ...
  - 禁止把机构首次创建重新接回本凭证；新机构创建业务必须按第 6 步另行登记完整原子协议。
- 必跑测试：OnChina 后端 registration-info 测试；P-TX-001 已删除载荷只做永久留洞拒绝测试
...
- 状态：当前 ABI；机构投票主体语义待 ADR-039 breaking 改造，个人多签继续独立
- 类型：交易载荷格式
...
- 编码：
  ...
  - 前两个字节固定为 `[0x14, 0x00]`
- 签名/验签规则：
  - 当前入口为 `InternalVote::cast`；ADR-039 目标机构票资格必须来自提案绑定的 voter `RoleSubject` 有效任职快照，不是管理员身份或 CID 全体 admins
...
- 状态：当前 ABI；`cast_admin` 名称和机构参与主体语义待 ADR-039 breaking 改造
- 类型：交易载荷格式
...
- 编码：
  ...
  - 前两个字节固定为 `[0x15, 0x00]`
- 签名/验签规则：
  - 当前联合机构阶段走 `JointVote::cast_admin`；投票资格只来自提案 `VotePlan` 绑定的 `RoleSubject` 有效任职快照。完整票据为 `CID + role_code + wallet`，同一 CID 内跨岗位分别投票、同一岗位同一钱包只投一次；不按机构全体 admins 判定
...
- 字段：
  1. `scope`：`Country / Province / City / Town`
  2. `eligible_total: u64`
  3. `eligibility_revision: u64`
...
- 创建/授权规则：
  ...
  - 人口分母只统计永久 CID Active、CID↔钱包闭环、状态正常、护照在判定日期有效且
    居住地匹配作用域的公民；护照生效和到期由身份模块 `on_idle` 有界维护。
  - 联合公投创建提案时内联创建全国快照。
...
- 编码：
  ...
  - 前两个字节固定为 `[0x08, 0x00]`
- 签名/验签规则：
  - runtime 必须按 `RoleSubject(actor_cid_number, proposer_role_code)` 与签名钱包校验 `res-iss/0 + Propose`；管理员身份本身不产生权限。
...
- 编码：
  ...
  - call index：`0 / 1 / 2`
- 签名/验签规则：
  - 业务提案由持有私钥且对目标 `RoleSubject` 有有效任职的管理员签名；仅属于 admins 不得创建
...
- 编码：
  ...
  - call index：`0`
  - 前两个字节：`[0x1d, 0x00]`
  - 布局：`institution_code:[u8;4] + account_id:[u8;32] + Compact<Vec<Admin>> + new_threshold:u32_le`；每个 `Admin` 按账户 32 字节、姓 SCALE bounded bytes、名 SCALE bounded bytes 的顺序编码。
- 签名/验签规则：
  - `new_threshold` 是管理员更换通过后写入投票引擎的目标动态阈值。
  - 个人多签阈值必须满足 `threshold * 2 > admins_len && threshold <= admins_len`。
  - 公权/私权机构不使用本 call；其统一四字段管理员人员集合由对应机构 admins pallet 独立保存，不能由 entity 岗位任职反向派生。
...
- 规则：
  ...
  - 机构签名者必须属于 `AdminAccounts[actor_cid_number].admins`，并对目标业务 `RoleSubject` 有有效任职和权限；管理员只负责签名，不是机构费用的回落付款人。
...
- 字段：
  ...
  - `BusinessActionId`：`module_tag`、`action_code: u32`
  - `RoleBusinessPermission`：`role_subject`、`business_action_id`、`operation(Propose|Vote)`
  - `VotePlan`：`business_action_id`、`proposal_owner`、`proposer_subject`、`voter_subjects`、`voting_engine`、`business_object_hash: [u8; 32]`
  - `AuthorizationSubject`：`Institution(RoleSubject)` 或 `PersonalMultisig(personal_account)`
...
- 动态岗位码：`R_<32 位大写十六进制>`；取 `blake2_256(SCALE(GMB_ROLE_V1, cid_number, institution_role_nonce, proposal_id))` 前 16 字节。客户端不传 role_code，已用码永久不复用
...
- 必跑测试：共享类型与跨端 SCALE fixture；`votingengine`、`joint-vote`、`runtime-upgrade`、`resolution-issuance` 岗位快照/授权回归；目标 crate `no_std`；runtime 集成与 fresh 运行态验收
...
- 字段：
  - 公权/私权机构 key：机构 `cid_number`；两类 value 都为 `institution_code + BoundedVec<Admin>`，每项固定为 `account_id + cid_number + family_name + given_name`，其中项内 CID 是管理员公民 CID 引用。两类均不重复保存机构 CID、主账户或生命周期状态。
...
- 同 pallet 的 `Institutions[cid_number] → InstitutionInfo`（法定代表人已收口为 entity 原子结构）：
  - value 字段顺序固定为 `cid_full_name`、`cid_short_name`、`town_code`、`legal_representative: Option<LegalRepresentative>`、`institution_code`、`created_at`（6 项，不保存重复生命周期状态）
  - `LegalRepresentative` 字段顺序固定为 `family_name`、`given_name`、`cid_number`、`account`；整个结构同时存在或为 `None`，不得保存拼接姓名或分裂可选字段
  - 已删 `main_account`/`fee_account`/`admins`/`admins_len`/`threshold`/`creator`/`account_count`：主/费账户由派生且在 `InstitutionAccounts`；管理员真源在 admins 模块；机构阈值真源在同一 entity 的 `InstitutionGovernanceThresholds`
  - 消费方镜像须按机构码路由 PublicManage/PrivateManage 前缀；node、OnChina、CitizenApp、CitizenWallet 已按上述 6 字段及嵌套法定代表人 SCALE 顺序对齐
- 同 pallet 的岗位与任职 storage：
  - `InstitutionRoles[cid_number][role_code] → InstitutionRole`
  - `InstitutionRolePermissions[cid_number][role_code] → BoundedVec<RoleBusinessPermission>`
...
- 编码：
  ...
  - value：SCALE
- 签名/验签规则：storage 本身不签名；岗位治理交易必须覆盖完整 mutation、权限规格、初始任职和提案上下文，动态岗位码只由 runtime 在执行结果时生成
...
### P-STORAGE-004：Account-level internal admin account（待 ADR-039 机构部分取代）

- 状态：当前实现；机构授权与机构投票主体部分已由 P-AUTH-001 取代并待迁移，个人多签部分继续有效
...
- 字段：
  ...
  - `status`
- 编码：
  - 机构以 `cid_number` 作为 `AdminAccounts` 和 public/private entity `InstitutionGovernanceThresholds` key；主账户、费用账户和制度专属账户都只是该 CID 下的账户，不建立独立管理员集合。
