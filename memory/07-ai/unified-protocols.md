...
- 状态：草案
- 类型：接口契约 / 编码协议
- 唯一真源：`memory/05-modules/wuminapp/im/IM_TECHNICAL.md`
- 详细文档：
...
- 字段：
  ...
  - `ImEnvelope.created_at`
  - `ImEnvelope.ttl`
  - `ImEnvelope.ack_policy`
  - `ImContactBundle.chat_account`
  - `ImContactBundle.display_name`
  - `ImContactBundle.node_peer_id`
  - `ImContactBundle.node_endpoints`
  - `ImContactBundle.device_public_keys`
  - `ImContactBundle.keypackage_endpoint`
  - `ImContactBundle.safety_code`
  - `ImContactBundle.expires_at`
  - `ImContactBundle.wallet_signature`
  ...
  - `ImDirectDeliveryRequest.submit`
...
- 签名/验签规则：
  - 钱包账户对 IM 设备、公钥、私人通信全节点 PeerId、端点、过期时间和 nonce 做绑定签名。
  ...
  - 钱包私钥只用于绑定证明和聊天窗口内链上转账签名，不作为 IM 消息加密密钥。
  - KeyPackage 由 IM 设备密钥管理，必须具备 TTL、一次性消费或租约消费、防重放和撤销清理。
  - 私人通信全节点只接受 `mailbox_owner_chat_account == ImEnvelope.recipient_chat_account` 的密文信封。
  - `/gmb/im/1` outbound 必须使用联系人包或用户自有配置中的显式 `PeerId + multiaddr`，先写入 sc-network 地址簿，再发 request。
- 禁止兼容：开发期不兼容未登记字段、未登记协议名或旧 Matrix / Olm / Megolm 主协议口径。
- 禁止事项：
  - 禁止把 SFID 号码、实名信息、身份档案字段写入 IM 协议。
  - 禁止把聊天内容、KeyPackage 目录或联系人目录上链。
  - 禁止通信全节点作为第三方 Relay、公共 DHT、公共 rendezvous 或第三方 mailbox。
  - 禁止复用钱包私钥作为 IM 端到端加密密钥。
  - 禁止 `/gmb/im/1` 直连投递依赖公共发现或第三方中继。
- 必跑测试：`cargo test -p node im::`、Protobuf 跨 Dart/Rust round-trip、OpenMLS 加解密测试、KeyPackage 防重放测试、通信全节点密文落盘检查。
- 当前运行态 smoke：`citizenchain/scripts/im-two-node-smoke.sh` 必须通过，验证两个真实 headless 节点之间的 `/gmb/im/1` 直连投递、owner 拉取/ack 和第三方 mailbox 拒绝。
