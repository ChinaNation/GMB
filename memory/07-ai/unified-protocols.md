...
- 所有已完成业务统一通过 `InstitutionGovernanceResult` 交给 entity；一个结果可同时包含动态岗位定义变化、多个岗位的完整目标任职集合，以及可选的法定代表人三字段整体更新。
- 每条目标任职独立保存管理员钱包、`term_start`、`term_end`、`assignment_source`、`assignment_source_ref` 和状态；未出现在结果中的岗位保持不变，出现的岗位整体替换目标任职集合。
- `term_start`、`term_end` 统一为自纪元起 `u32` 天；entity 校验完整目标状态后，从机构全部有效任职派生 admins，并把岗位、任职、法定代表人与 admins storage 放在同一事务提交。
...
- HTTP API 字段：
  ...
  - `GET /v1/square/membership` 请求：Bearer `session_token`；响应：`plans[]`、`membership`、`subscription_active`、`active`（ADR-036 解耦后不再返回 `identity`/`eligible_levels`/`frozen`/`inactive_*`）。`plans[]` 中三档会员只按美元计价，字段含 `price_currency = usd`、`price_usd_cents`、`price_usd_monthly`、`chat_file_max_bytes`（聊天单文件上限）；解耦后权益态即订阅态，`active` 与 `subscription_active` 等值。
  - `POST /v1/square/membership/subscribe/challenge` 请求：`owner_account`、`membership_level`(`freedom`/`democracy`/`spark`)；响应：钱包签名挑战。`POST /v1/square/membership/subscribe` 请求同一挑战的 `challenge_id`、`signature` 与账户/档位，验签后响应 `checkout_session_id`、`checkout_url`、`membership_level`。**会员与身份解耦（ADR-036）：任意身份可订任意档，下单无身份资格预检**（已删 `required_identity_level`/`identityEligibleForPlan`/`membership_identity_mismatch`）。卡订阅正式授权只以 Stripe subscription webhook 写入 `square_memberships` 为准；USDC→卡时服务端同时写入 `subscription_data.metadata.payment_switch=usdc_to_stripe` 和以现有预付到期日计算的 `trial_end`，作为允许切回卡路线的唯一标志。
  - `POST /v1/square/membership/prepaid/challenge` 与 `POST /v1/square/membership/prepaid`：真实钱包签名后创建 `mode=payment` 的 Stripe Crypto Checkout，`duration` 只允许 `quarter` / `year`；异档活跃预付会员必须走 change 接口。`POST /v1/square/membership/prepaid/change/challenge` 与 `POST /v1/square/membership/prepaid/change`：升档创建补差价 Crypto Checkout，降档即时按剩余价值折算时长并落 D1，同档返回 `same_membership_level`。
  - `POST /v1/square/membership/cancel/challenge` 与 `POST /v1/square/membership/cancel`：先下发取消签名挑战，再按 `subscription_source` 分派；卡订阅验签后调用 Stripe 设置 `cancel_at_period_end=true`，USDC 预付返回 `cancel_kind=usdc_prepaid` 并自然到期，无活跃会员返回 `no_active_subscription`。
  - `POST /v1/square/membership/webhook` 请求：Stripe webhook 原始 JSON + `Stripe-Signature`；响应：`event_id`、`event_type`、`action`。本接口不需要 Bearer，必须用 `STRIPE_HOOK_SECRET` 校验签名。`square_stripe_webhook_events` 对 `event_id` 原子占位，`square_stripe_payments` 对 `payment_intent` 永久去重；重复事件不得重复授时长。`customer.subscription.created/updated` 根据 subscription metadata 写入卡订阅，写入前必须校验 Stripe Price 为 `usd` 且金额匹配会员等级；当当前 D1 行为 `usdc_prepaid` 时，无服务端 `payment_switch=usdc_to_stripe` 标志的迟到卡事件必须原子拒绝并返回 `subscription_superseded`。`checkout.session.completed` 的 `route=usdc_prepaid` / `usdc_prepaid_upgrade` 分支分别授予预付时长或完成升档，其他 Checkout 只观察。
...
- D1 表字段：
  ...
  - `square_memberships`: `owner_account`、`membership_level`(`freedom`/`democracy`/`spark`)、`expires_at`、`updated_at`、`subscription_source`、`stripe_customer_id`、`stripe_subscription_id`、`stripe_price_id`、`subscription_status`、`current_period_start`、`current_period_end`、`cancel_at_period_end`、`entitlement_lapsed_at`、`prepaid_payment_ref`（ADR-036 已删 `identity_level`/`identity_checked_at`/`frozen_at`/`collection_paused`）
...
- Worker 环境变量：
  ...
  - `FREEDOM_PRICE_ID`($2.99)、`DEMOCRACY_PRICE_ID`($9.99)、`SPARK_PRICE_ID`($99.99)：三档 Stripe 美元月付 Price ID（ADR-036，已删 `VOTING_PRICE_ID`，`CANDIDATE_PRICE_ID`→`SPARK_PRICE_ID`）；可作为 subscription metadata 缺失时的会员等级映射。
...
- 签名/验签规则：
  ...
  - **onchina 控制台链写动作码(`b.d`=裸 SCALE call data,冷钱包解码核对后冷签 origin 由 CitizenWallet 提交)**:链交易统一 `a=(pallet<<8)|call`(禁止扁平小整数,会撞非链动作码 1..9)。机构创建=公权 `0x1e05`(PublicManage 30/call 5)/私权 `0x1f05`(PrivateManage 31/call 5,见 P-TX-001);公民投票身份注册=`0x0a00`(CitizenIdentity 10/call 0,见 P-TX-011);公民参选身份上链=`0x0a01`(CitizenIdentity 10/call 1,见 P-TX-011);机构管理员任职没有独立扫码 call，由创世、注册局创建或完成业务复核的任免/选举业务结果写入 entity，投票引擎不得直写；个人多签管理员集合变更为 `0x1d00`(PersonalAdmins 29/call 0,见 P-TX-007);CREG 市注册局无独立链动作码；非链文本治理 `a=3 = ACTION_ONCHINA_ADMIN / QR_ACTION_ONCHINA_ADMIN`(onchina_admin_governance JSON)。动作码由 `onchina/src/core/institution_call.rs::chain_action_code(pallet,call)` 与 call data 同源派生,非链常量在 `core/qr/mod.rs`,runtime 注释真源在 `primitives::sign`,均与 `qr-action-registry.md` 同步；动作码 `8` 已取消登记，Chat 设备绑定不得进入 QR。
...
- 唯一真源：
  ...
  - 两 call 参数形态完全相同（下列 19 字段），仅 pallet 前缀不同
- 详细文档：
  - `memory/01-architecture/qr/qr-action-registry.md`
- 生产者：
...
- 字段：
  ...
  5. `legal_representative_name`
  6. `legal_representative_cid_number`
  7. `legal_representative_account`
  8. `accounts`
  9. `funding_account`（`Option<AccountId32>`；初始入金合计为零时必须 `None`，非零时必须 `Some`）
  10. `institution_code`
  11. `roles`（`Vec<InstitutionRole>`）
  12. `assignments`（`Vec<InstitutionAdminAssignment>`，创建只接受 `Registry`）
  13. `threshold`
  14. `register_nonce`
  15. `signature`
  16. `actor_cid_number`
  17. `credential_signer_pubkey`
  18. `scope_province_name`
  19. `scope_city_name`
- 编码：
  - SCALE call data
...
- 签名/验签规则：
  - `register_nonce / signature / actor_cid_number / credential_signer_pubkey / scope_*` 由 CID 机构注册信息凭证提供
  - runtime 通过 `actor_cid_number` 查询对应机构 admins pallet 的 `AdminAccounts[cid_number]`，确认 `credential_signer_pubkey` 属于该机构 `admins` 后验签
  - `accounts.account_name` 顺序必须与 CID `/registration-info.account_names` 一致
  - 初始入金合计非零时，`funding_account` 必须是 `actor_cid_number` 下正反索引完全一致、允许 `InstitutionCreateFunding` 支出的明确机构账户；不得从管理员钱包扣本金。初始入金为零时禁止携带出资账户。
  ...
  - CID 注册凭证签名覆盖机构名称、法定代表人三字段、账户名列表、`funding_account`、完整岗位/任职 SCALE 载荷和 `town_code`；任一字段不得改包。
...
- 同 pallet 的 `Institutions[cid_number] → InstitutionInfo`（2026-07-13 法定代表人三字段已收口到 entity）：
  ...
  - 三个法定代表人字段必须同时有值或同时为空；运行期注册创建必须全部有值，创世没有真实任免资料时全部为 `None`
...
- 同 pallet 的岗位与任职 storage：
  ...
  - 普选/互选结果整体替换目标岗位的有效任职；随后从同机构全部有效任职派生 admins，与 admins storage 同事务提交。
- 编码：
  - double map storage key(前缀 = `twox_128(PublicManage|PrivateManage)` ++ `twox_128(InstitutionAccounts|Institutions)`)
  - value：SCALE
- 签名/验签规则：storage 本身不签名；call index 5 的机构创建凭证必须覆盖法定代表人姓名、CID、账户三个字段，任一字段被替换均验签失败
