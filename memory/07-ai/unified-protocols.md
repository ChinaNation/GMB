...
- 所有管理员集合 `admins` 的目标值为 `Admin { admin_account, family_name, given_name }` 列表；姓、名只展示，账户是唯一授权字段，不内嵌公民 CID、岗位、任期和来源。缺失姓、名分别使用“管理”“员”。
...
- 字段：
  ...
  5. `admins`（`BoundedVec<Admin>`；每项字段顺序为 `admin_account + family_name + given_name`，至少两人）
  6. `actor_cid_number`
- 编码：
  - SCALE call data
...
- 签名/验签规则：
  ...
  - `admins` 的钱包账户必须唯一，`family_name`、`given_name` 分别不能为空；钱包能解析公民姓、名时分别使用对应字段，否则分别使用“管理”“员”。姓名不参与授权。
...
- 编码：
  ...
  - value：SCALE
- 签名/验签规则：storage 本身不签名；call index 5 的机构创建最终链交易必须覆盖 `admins(admin_account + family_name + given_name)` 完整 SCALE 载荷，任一姓、名或账户被替换均验签失败
