...
本节冻结 ADR-039 目标协议。2026-07-19 已完成共享类型、entity 岗位/权限/任职/动态岗位生命周期 storage、创世固定权限和联合投票第 5A 切片。协议升级与决议发行已按岗位授权和有效任职快照执法；其他引擎与业务模块仍按任务卡后续步骤迁移。旧“admins 即授权、机构投票快照全体 admins”实现不构成兼容路径。
...
- 全部创世固定岗位的岗位码、岗位名和岗位权限永久固定，包括中国公民链技术发展基金会的 `LR`、`GENESIS_PRODUCT_MANAGER`、`GENESIS_PROGRAMMER`。创世机构仍可增加普通动态岗位；NodeGuard 不得以“出现额外岗位”为由拒绝。
...
- 签名/验签规则：
  ...
  - 第 3 步已提供统一授权查询：同时校验 CID 存在、origin 属于 admins、任职在 UTC 日闭区间内有效、岗位具备目标权限、权限主体与 storage key 一致且 CID 顶层能力允许；业务入口将在第 6/7 步切换到该查询。
...
- 编码：
  ...
  - 前两个字节固定为 `[0x15, 0x00]`
- 签名/验签规则：
  - 当前联合机构阶段走 `JointVote::cast_admin`；投票资格只来自提案 `VotePlan` 绑定的 `RoleSubject` 有效任职快照，同一 CID 内跨岗位去重，不同 CID 可分别投票；不按机构全体 admins 判定
...
- 状态：共享类型与跨端 SCALE 已实现；联合投票已接入 `ProposalVotePlans`、岗位快照和 CID 有效选民快照，协议升级/决议发行已按岗位授权；其他引擎与业务仍在实施
...
- 联合投票 storage：`ProposalVotePlans[proposal_id]`、`VoterSnapshot[(proposal_id, RoleSubject)]`、`EffectiveVoterSnapshot[(proposal_id, cid_number)]`；后者对同 CID 多岗位账户并集去重
...
- 签名/验签规则：
  - 机构目标是一个岗位任职席位一票，票据主体为 `RoleSubject + admin_account`；同一钱包兼任多岗时可用同一私钥分别行使各岗票权。机构投票资格由业务模块 VotePlan 指定的 `RoleSubject` 有效任职快照决定，不能锁定 CID 的全体 admins；个人多签仍按 `personal_account` 管理员账户一人一票。当前钱包去重中间态必须在下一步删除。
