...
- Worker 环境变量：
  ...
  - `SQUARE_CHAIN_RPC_URL`：Worker 用于读取指定区块 `System.Events`，也用于读取 `CitizenIdentity::VotingIdentityByAccount` / `CandidateIdentityByAccount` 以校验投票公民会员和竞选公民会员资格；只允许由部署环境提供，不写入仓库和 CitizenApp。staging 已配置该 secret，并已验证 Worker 能读取链 RPC；完整 URL 不写入仓库文档。阶段 13 当前阻塞点不是 RPC 连通性，而是远端 runtime metadata 尚未暴露广场 `SquarePost` pallet。
...
- 官网会员订阅实现真源：
  - `website/src/pages/Membership.tsx`
- 签名/验签规则：
...
- 禁止事项：
  - 禁止响应中返回 `SQUARE_CHAIN_RPC_URL`、Validator RPC、Archive RPC 或任何私密 RPC 完整 URL。
