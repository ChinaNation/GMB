...
- 字段：
  ...
  4. `algo`（`0x02 = ML-DSA-65`）
  5. `pubkey`（ML-DSA-65 公钥）
  6. `sig`（ML-DSA-65 签名）
- 编码：
  - general-transaction（无外层 sr25519 签名）
  - SCALE call data，pallet index：`27`，call index：`1`
- 签名/验签规则：
  - `#[pallet::authorize]` 阶段读 `BoundPqcKey[account]`，校验 `blake2_256(pubkey)==pubkey_hash` 且 `state != Revoked`
  - `nonce` 必须等于链上 `AccountKeyNonce[account]`，授权后递增
  - `msg = blake2_256(call.encode() ++ nonce ++ genesis_hash)`，`verify_ml_dsa_65(pubkey, msg, sig)` 必须通过
...
- 字段：
  ...
  - value `PqcKeyRecord`：`algo:u8`、`pubkey_hash:[u8;32]`、`state:KeyState(Active/PqcOnly/Revoked)`、`bound_at:BlockNumber`
  - 配套 `AccountKeyNonce: AccountId -> u32`（PQC 交易防重放）
- 编码：SCALE，`StorageMap<Blake2_128Concat, AccountId, PqcKeyRecord>`
- 签名/验签规则：不适用（storage）；只存公钥 hash，不存完整公钥、不存私钥
- 兼容策略：账户状态机 `Sr25519Only → Bound → PqcOnly`，三态下 canonical AccountId 不变
- 禁止事项：
  - 禁止存完整 PQC 公钥（永久 state 膨胀）
  - 禁止用 PQC 公钥 hash 作为新的 AccountId
