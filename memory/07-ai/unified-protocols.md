...
- HTTP API 字段：
  ...
  - `PUT /v1/square/uploads/dev-put` 请求：Bearer `session_token`，query `upload_id`、`object_key`；响应：`object_key`、`byte_size`；仅 `DEV_UPLOAD_PROXY=1` 的本地开发模式可用，生产环境禁用。
  - `POST /v1/square/uploads/dev-media` 请求：Bearer `session_token`，query `upload_id`、`media_index`；响应：`upload_id`、`media_index`、`asset_state`；仅 `DEV_UPLOAD_PROXY=1` 的本地开发模式可用，用于模拟 Images/Stream 直传。
  ...
  - `PUT /v1/square/profile/assets/dev-put` 请求：Bearer `session_token`，query `object_key`（须属本人 `profile/{owner_account}/` 前缀）；响应：`object_key`、`byte_size`；仅 `DEV_UPLOAD_PROXY=1` 本地开发可用。
...
- Worker 环境变量：
  ...
  - `DEV_UPLOAD_PROXY`：本地开发上传代理开关；生产环境不得开启。
  - `CF_ACCOUNT_ID`、`R2_ACCESS_ID`、`R2_SECRET_KEY`、`R2_BUCKET`：Worker 生成 R2 S3 预签名上传 URL 所需配置；Cloudflare 账户 ID 与 Images / Stream 共用，不得另造 R2 账户字段。
  - `CF_API_TOKEN`：Worker 调用 Cloudflare Images / Stream API 生成一次性上传 URL；必须使用 Cloudflare secret，不得下发到 CitizenApp。
...
- 状态：当前（阶段 CHAT-2 已落地 Worker mailbox API、D1 迁移、设备绑定验签 helper、KeyPackage、密文 envelope、pending 拉取与 ack；阶段 CHAT-3 已落地 CitizenApp 自动 Worker session、设备绑定签名、KeyPackage 发布、发送前 KeyPackage 拉取/消费和聊天页打开自动 pending 同步；阶段 CHAT-4 已落地聊天 Tab 15 秒前台轮询、聊天页 8 秒前台轮询和失败 30 秒退避；阶段 CHAT-5 已补齐 mailbox pending 拉取、解密落库和 ack 的端到端状态机回归，并新增 native OpenMLS mailbox 闭环用例；阶段 CHAT-6 已完成 macOS host OpenMLS native 真实执行验收；阶段 CHAT-7 已落地加密附件 prepare/upload/complete、R2 密文对象校验和 App 侧加密附件发送底座；阶段 CHAT-8 已落地聊天页文件选择、附件下载授权、密文下载、本地校验解密和私有缓存保存；阶段 CHAT-9 已落地 WebSocket 新密文通知与 App 轮询兜底；阶段 CHAT-10 已将实时连接迁入账户级 Durable Objects fanout；阶段 CHAT-11 已将 Cloudflare mailbox 收紧为临时密文投递队列，ack 后删除 D1 envelope 和对应 R2 加密附件对象；近场真机和多分片续传待后续阶段）
...
- 生产者：
  - `citizenapp/lib/chat/`：OpenMLS 加密、KeyPackage、密文 envelope、ack 删除确认。
  - `citizenapp/cloudflare/`：临时密文 mailbox、KeyPackage 池、WebSocket 新密文通知。
- 消费者：
  - `citizenapp/lib/chat/`
  - Cloudflare D1 database `citizenapp-square-db`
  - Cloudflare R2 bucket `citizenapp-square-media`
  - Cloudflare Durable Objects / WebSocket。当前 CHAT-10 按 `owner_account` 路由到 `ChatRealtimeObject`，由账户级对象管理在线设备连接。
- HTTP API 字段：
  - `POST /v1/chat/devices/register` 请求：Bearer `session_token`，`device_id`、`device_public_key_hex`、`binding_signature`、`expires_at`、`nonce`；`owner_account` 只取 session，客户端提交该字段必须拒绝。
  ...
  - `PUT /v1/chat/attachments/dev-put` 请求：开发代理上传入口，仅在 `DEV_UPLOAD_PROXY=1` 时启用；生产不得依赖该入口。
  ...
  - `GET /v1/chat/attachments/dev-get` 请求：开发代理下载入口，仅在 `DEV_UPLOAD_PROXY=1` 时启用，仍必须带 Bearer session；生产不得依赖该入口。
  - `GET /v1/chat/ws` 请求：Bearer `session_token`，query `owner_account`、`device_id`；Worker 完成 session 和 active device 校验后转发到账户级 `ChatRealtimeObject`；WebSocket 成功后先返回 `gmb_chat_ws_ready_v1`，随后只推送 `gmb_chat_new_envelope_v1` 新密文通知，字段为 `envelope_id`、`conversation_id`、`recipient_account`、`recipient_device_id`、`mls_message_kind`、`created_at`。
...
- D1 表字段：
  ...
  - `chat_device_binding_nonces`: `owner_account`、`nonce`、`expires_at`、`created_at`；`owner_account + nonce` 为一次性重放闸门。
...
- 签名/验签规则：
  ...
  - Chat 设备绑定、KeyPackage、密文同步和实时连接禁止读取钱包 seed；钱包主私钥不得参与 Chat 设备绑定或 OpenMLS 消息加密。
...
- 禁止兼容：不兼容区块链节点通信节点 mailbox、不兼容 已删除的节点聊天协议 作为正式互联网聊天链路。
- 禁止事项：
  - 禁止保存私聊或群聊明文。
  - 禁止把私密聊天写入广场公开评论表。
...
- 禁止事项：
  ...
  - 禁止恢复通信节点配对、桌面通信节点二维码、节点 Chat mailbox 或 已删除的节点聊天协议。
...
- 字段：
  ...
  - `ChatEnvelope.attachment_manifest_hash`
  - `ChatEnvelope.chunk_refs`
  - `ChatEnvelope.created_at_millis`
  - `ChatEnvelope.ttl_millis`
  - `ChatEnvelope.ack_policy`
  ...
  - `ChatKeyPackage.consumed_at_millis`
...
- 验收接口：
  ...
  - 区块链节点通信节点、已删除的节点聊天协议、节点 mailbox、节点 KeyPackage 池和 `chat_node_pairing` 已删除，不再作为正式验收入口。
- 编码：外层 Protobuf；OpenMLS 标准 wire bytes 放入 `mls_wire_message`；链内 SCALE 不作为 Chat 主协议。
- 当前实现状态：Dart Protobuf、OpenMLS Rust FFI、Isar 本地消息库、Cloudflare 密文 mailbox、附件密文收发和 WebSocket 通知均已落地；`ChatRuntime.ensureReady(ownerAccount)` 对同一账户/设备执行 single-flight，复用有效 session/context；聊天 mailbox 登录与设备登记只使用硬件 P-256 设备子钥，账户由 Worker session 派生，钱包 seed 和 CitizenWallet 均不进入聊天运行态；隐藏聊天 Tab 不初始化，进入、resume、轮询与实时通知统一走页面 coordinator；ack 后删除 D1 envelope 和关联 R2 密文附件。
- 签名/验签规则：
  ...
  - 签名字节固定为 `signing_message(OP_SIGN_CHAT_DEVICE_BIND=0x1A, payload)`；Cloudflare mailbox 必须从已验证 session 派生 owner，并用 `square_device_subkeys.p256_pubkey` 验证 P-256 签名。
  - 客户端不得提交 owner 授权真源；CitizenWallet 不参与 Chat 设备绑定。
  - KeyPackage 由 Chat 设备密钥管理，必须具备 TTL、一次性消费或租约消费、防重放和撤销清理。
  - 首次 MLS 会话发送会产生 Welcome + application 两条 wire message；Welcome 必须通过 `ChatEnvelope.ratchet_tree` 伴随传递 ratchet tree bytes。
  - Cloudflare mailbox 只接受 `recipient_account == ChatEnvelope.recipient_account` 的密文信封。
  - 附件密钥材料只允许出现在 OpenMLS application 载荷中，不得放入 `ChatEnvelope.encrypted_metadata`、D1 字段或 R2 manifest 明文。
...
- 禁止兼容：开发期不兼容未登记字段、未登记协议名、旧 Matrix / Olm / Megolm 主协议口径、旧通信节点 mailbox、旧 已删除的节点聊天协议、旧 `chat_node_pairing`。
- 禁止事项：
  - 禁止把 CID 号码、实名信息、身份档案字段写入 Chat 协议。
  - 禁止把 Chat 路由缓存做成第二套通讯录。
  - 禁止复用钱包私钥作为 Chat 端到端加密密钥。
  - 禁止把私聊或群聊明文写入 Cloudflare、链、节点或广场公开表。
  - 禁止恢复区块链节点通信节点聊天、已删除的节点聊天协议、节点 mailbox 或节点 KeyPackage 池作为正式路线。
- 必跑测试：`cargo test`（`citizenapp/rust`）、`flutter test --concurrency=1 test/chat`、Worker chat API 测试、Worker `/v1/chat/ws` smoke、Protobuf 跨端 round-trip、OpenMLS 加解密测试、P-256 设备绑定重放测试、Cloudflare mailbox 只存密文检查。
- 运行态 smoke：以 CitizenApp + Cloudflare Worker 本地/预发环境验证 P-256 登录和设备登记、密文投递、拉取、ack、附件密文上传/下载授权与 WebSocket 通知；近场能力落地后再增加双真机收发。
