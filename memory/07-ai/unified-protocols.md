...
- 签名/验签规则：
  ...
  - CPMS 签名原文固定为 `sfid-cpms-v1|archive|{archive_no}|{archive_status}|{valid_from}|{valid_until}|{status_updated_at}|{cpms_pubkey}|{geo_seal_hash}|{wallet_address}|{wallet_pubkey}`。
  ...
  - 同一 `archive_no` 已存在绑定记录时，只允许 `status_updated_at` 更新的 ARCHIVE 更新档案状态、有效期或钱包字段；旧时间戳档案码必须拒绝，防止旧码覆盖新状态。
...
- 字段：
  ...
  - `ARCHIVE`：`proto`、`type`、`archive_no`、`archive_status`、`valid_from`、`valid_until`、`status_updated_at`、`cpms_pubkey`、`geo_seal`、`wallet_address`、`wallet_pubkey`、`wallet_sig_alg`、`sig`
- 编码：
  - JSON
...
- 签名/验签规则：
  ...
  - `ARCHIVE` 由 CPMS 本机私钥签名，原文为 `sfid-cpms-v1|archive|{archive_no}|{archive_status}|{valid_from}|{valid_until}|{status_updated_at}|{cpms_pubkey}|{geo_seal_hash}|{wallet_address}|{wallet_pubkey}`
