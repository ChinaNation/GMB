...
- 状态：当前
- 类型：交易载荷格式
- 唯一真源：`citizenchain/runtime/admins/admin-management/src/lib.rs`
- 详细文档：
...
- 消费者：
  - `citizenchain/runtime/admins/admin-management`
  - `citizenwallet/lib/signer/payload_decoder.dart`
- 字段：
  1. `institution_code`
  2. `account_id`
...
- 消费者：
  ...
  - `citizenapp/lib/governance/organization-manage/multisig_storage_codec.dart`
- 字段：
  - key1：`cid_number`
  - key2：`account_name`
...
- 禁止事项：
  ...
  - 禁止把机构账户当个人多签账户读取
- 必跑测试：
  - `citizenapp/test/governance/organization-manage/multisig_discovery_service_test.dart`
