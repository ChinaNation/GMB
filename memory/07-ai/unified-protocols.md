...
- 所有机构管理员集合 `admins` 的值为 `Admin { admin_account, family_name, given_name }` 列表；它只是可任职人员名册，账户本身没有机构业务权限。缺失姓、名分别使用“管理”“员”。
...
- 全部创世固定岗位的岗位码、岗位名和岗位权限永久固定，包括中国公民链技术股份有限公司的 `LR`、`GENESIS_PRODUCT_MANAGER`、`GENESIS_PROGRAMMER`。创世机构仍可增加普通动态岗位；NodeGuard 不得以“出现额外岗位”为由拒绝。
...
- 签名/验签规则：
  ...
  - 公权/私权机构不使用本 call；其三字段管理员人员集合由对应机构 admins pallet 独立保存，不能由 entity 岗位任职反向派生。
...
- 字段：
  - 公权/私权机构 key：`cid_number`；value：`institution_code`、`admins: BoundedVec<Admin>`，每项固定为 `admin_account + family_name + given_name`，不重复保存 CID、主账户或生命周期状态。
...
- 签名/验签规则：
  - 目标一人一票一笔交易；机构投票资格由业务模块 VotePlan 指定的一个或多个 `RoleSubject` 有效任职账户快照决定，不能锁定 CID 的全体 admins；个人多签仍按 `personal_account` 管理员快照决定。
