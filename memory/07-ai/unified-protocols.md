...
- 状态：草案（阶段 3 Worker / R2 本地服务已落地；阶段 5 App 上传与上链闭环已落地；阶段 6 Worker 链上事件确认和正式 feed 入库已落地；阶段 10 已改为链上扣费入块后再上传 R2，并增加本地草稿保护；阶段 11 已完成 staging 部署和 prepare 远端 smoke，链上确认 smoke 仍等待 `SQUARE_CHAIN_RPC_URL`）
- 类型：接口契约 / storage 契约
- 唯一真源：
