...
- 业务规则：
  - CPMS 只允许超级管理员在 UTC 1 月 1 日到 1 月 10 日导出上一年度更新数据。
  - UTC 1 月 6 日到 1 月 10 日仍未导出上一年度报告时，CPMS 锁定操作管理员登录和已有会话。
  - `status_records` 按 `citizen_status_updated_at` 落入 `export_year` 过滤，`number_release_records` 按 `hard_deleted_at` 落入 `export_year` 过滤。
- 兼容策略：开发期不兼容旧字段、旧状态名或无签名导出文件。
- 禁止事项：
  - 禁止导出姓名、出生日期、地址、钱包地址等实名或绑定细节。
  - 禁止把硬删除释放记录当作公民状态更新。
