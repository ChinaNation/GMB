...
1. 全仓第一死规则是 Substrate 官方命名绝对优先。任何设计、编码、协议、字段、文档或
   生成物任务开始前，必须先确认相关概念的 Polkadot SDK 官方类型与官方术语；同一语义
   不得存在第二个基础名称。
2. 新线程首轮必须先读本文件，再按本文件读取首轮必读文档。
3. 进入执行阶段前，必须先读执行前必读文档。
4. 涉及协议、命名、安全、模块边界、代码修改、文档重排时，必须读取对应任务类型的必读文档。
5. 不允许把关键规则只留在聊天历史里；规则必须回写到 `memory/`。
6. 如果本文件和其他入口文件的必读清单冲突，以本文件为准，并同步修正其他入口文件。
...
- 仓库中任何新增文件或目录都必须在当前任务中先列出完整路径、用途、原因、是否会被 Git 跟踪，并得到用户明确确认；历史确认、泛化授权或对其它目标的同意不能替代本次确认。
...
- Polkadot SDK 官方文档或当前仓库锁定 SDK 的官方源码
...
必须先确认 Substrate 官方命名，再全仓搜索统一命名登记；不确定命名必须停止执行并报告
用户确认。
