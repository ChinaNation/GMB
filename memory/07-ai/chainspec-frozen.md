...
> 适用范围：`citizenapp/assets/chainspec.json` 及任何分发给轻节点 / 钱包 / App
...
- `citizenapp/scripts/citizenapp-run.sh` 启动前会跑
  `citizenchain build-spec --raw > citizenapp/assets/chainspec.json`。
- `.github/workflows/citizenapp-ci.yml` 打包 APK 前会从另一个 CI 下载
...
- 修复：删除两处自动重生成逻辑，chainspec.json 回退到 `97086e8`（创世）
  的版本，记录 sha256 到 `citizenapp/assets/chainspec.json.sha256`，加
...
旧的 `citizenapp/assets/chainspec.json.sha256` 冻结常量已删除——守卫改为直接和 SSOT 比对,SSOT 即唯一基准。
