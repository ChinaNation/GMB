...
- 彻底修复硬规则：所有问题必须按目标状态彻底修复，不允许把“兼容旧流程、兼容旧格式、兼容旧数据、兼容旧命名、兼容旧交易载荷”作为默认方案；除非用户明确要求兼容，否则不得设计、实现或建议任何历史兼容、过渡兼容、双轨兼容、保留旧分支或影子旧流程
...
- 涉及 `citizenchain/runtime` 且会影响 `wuminapp` 在线端或 `wumin` 冷钱包二维码签名/验签兼容性的任务，必须作为跨模块任务处理；不得只改单侧 runtime
...
- SFID 系统不得新建或恢复独立 `backend/chain/`、`frontend/chain/` 业务目录；各功能模块如需和区块链交互，必须在所属功能模块目录中新建 `chain_` 开头文件；跨模块链底层工具只允许放 `app_core/chain_*`
