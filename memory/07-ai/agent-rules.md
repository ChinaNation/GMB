...
> ### 最高优先级·四条硬性死规则（每次开工前核对，违反即返工）
>
> **死规则一·Substrate 官方命名绝对优先。** 这是全仓库排名第一、优先于其它所有开发规则的死规则。全仓库所有第一方代码、目录、文件、类型、Trait、结构体、枚举、函数、变量、参数、字段、Storage、Event、Error、Extrinsic、RPC、API、SQL、JSON、Dart、TypeScript、测试、生成物、注释和文档，只要涉及 Substrate / Polkadot SDK 已定义的概念，必须严格使用其官方类型和官方术语。同一语义全仓只能存在一个基础名称；只允许 `snake_case` 与 `lowerCamelCase` 之间由语言语法要求产生的大小写转换，禁止改变语义词根、另造同义词、缩写、别名或兼容字段。Substrate 官方命名与仓库历史实现、旧文档、旧协议或旧字段冲突时，无条件以官方命名为准，并在当前任务中彻底清理旧名称，不保留兼容、别名、双轨或影子字段。Substrate 未定义的 GMB 业务概念，以 `memory/07-ai/unified-naming.md` 登记的唯一命名为准。新增或修改命名前必须先查阅 Polkadot SDK 官方资料、再全仓搜索；无法确认时必须停止执行并询问用户。存在命名残留时，任务不得完成、不得创世、不得推送 CI、不得部署。
>
> **死规则二·所有更改必须彻底，绝不保留兼容。** 链尚未运行、仍在开发期——禁止再就「是否需要 migration / 是否要重新创世 / 是否保留旧方案过渡 / spec_version 是否 bump」反复询问用户。默认答案永远是：直接按目标态一次性彻底改造、需要时直接重新创世、不留任何兼容/过渡/双轨/影子流程。发现旧代码、旧命名、旧目录、旧注释、旧文档、旧测试、旧 UI 文案、旧交易载荷、旧接口分支、旧生成物残留，必须在同一任务内全部清理，不允许有任何残留，不允许以「后续再处理」当作完成口径。
>
> **死规则三·所有目录、文件、字段命名必须全仓库统一且尽量精简。** Substrate / Polkadot SDK 已定义的概念先服从死规则一；其余 GMB 业务概念在 Rust / Dart / TypeScript / SQL / JSON / 文档 / 生成物中必须使用同一个基础名称，禁止另造 `name`/`label`/`display_name`/`type`/`status` 等泛化别名。命名要短、要直接表达语义；不确定是否同义时先全仓搜索并向用户确认，绝不擅自新建。
>
> **死规则四·不允许猜测，现有实现必须先查仓库再回复。** 任何 AI 线程不得在未读取仓库代码、文档、任务卡或真实运行输出前，对现有实现、运行状态、字段命名、扣费、分账、权限、存储、部署结果作推断式结论；对不了解或未复查的实现，必须先全仓搜索、读取相关代码和文档，必要时执行只读检查，再回复用户。无法确认时只能明确说明“尚未检查/无法确认”，不得用“如果当前实现没有……”等假设口径暗示事实。

- 逻辑不清必须先沟通
- 对现有代码不了解时必须先检查仓库实现；未检查前不得给出事实判断、风险判断或实现状态判断
...
- 本机部署目录保密硬规则：仓库根目录 `citizenconsole/` 是部署网页、后端、动作脚本和 Keychain 桥接源码的唯一目录，属于本机私有运维工具，必须整目录由 Git 忽略，不得纳入 Git、提交或推送 GitHub。任何 AI 线程不得反选或强制添加其中源码、`.runtime/`、日志、编译二进制、本机状态及任何 Secret/私钥文件。部署密钥只允许保存在 macOS Keychain 或 GitHub Secrets，禁止恢复明文 Secret 文件或把密钥写入源码。生产、Release 和服务器部署必须逐次通过 Touch ID；测试和 CI 不要求密码。根 `scripts/` 只保留非部署仓库工具，不得恢复旧部署入口。
...
- 管理员命名硬规则：所有机构和个人多签的管理员唯一字段统一为 `admins`；OnChina 登录态只允许用 `institution_code + workspace` 表达当前账户所属机构工作台，授权真源是链上在册管理员与岗位码。注册局与其它机构同走这一条路径，不得恢复 `registry_org_code` 这类注册局专用分支，也不得恢复独立管理员身份表、第二授权真源或授权分支。
...
- 密钥/签名文本编码硬规则（ADR-041）：凡「账户标识 / 公钥 / 签名」的**跨端文本**一律小写 `0x` 加 hex、单一形态、拒裸，覆盖所有密码学体系（sr25519 账户 `^0x[0-9a-f]{64}$`、P-256 设备子钥公钥 `^0x04[0-9a-f]{128}$`、P-256 签名 `^0x[0-9a-f]{128}$`，及未来任何密钥/签名文本），不因子系统不同而分叉。边界一次 require 0x + strip → 内部裸；禁止「两端都收 0x」的兼容层。**不纳入**：sha256 内容哈希 / content_hash、R2 路径段 `account_id_hex`（去 0x）、SCALE 内部字节 / 签名消息 preimage / `device_key_hash` preimage、助记词·seed·私钥·Keychain/Keystore/Secret。完整契约见 ADR-041 与 `unified-protocols.md` §0.3。
- 协议版本标识硬规则：全仓唯一允许的版本化协议标识是 `QR_V1`（扫码协议）。签名域唯一走 `primitives::sign::signing_message(op_tag)` = `blake2_256(GMB ‖ op_tag ‖ SCALE)`；非签名哈希域（岗位码派生、提案数据打包等）一律用所属 pallet 的 `MODULE_TAG`。禁止任何其它带版本号的自定义协议常量、字符串域、包名、消息类型或别名；历史标识不得恢复。
- 中国国旗字符禁止硬规则：全仓代码、测试、夹具、注释、文档、配置和生成物均禁止出现中国国旗字符；测试 Unicode、emoji、区域指示符或多字节编码时必须使用中性字符，不得恢复。
- `citizenchain-ci` 的文档、残留、协议命名与 pallet 注册表门禁必须在 `main` push、PR 和手动 CI 三种入口全部执行；push 以 `github.event.before` 为比较基线，PR 以目标分支为基线，手动 CI 以 `HEAD^` 为基线，禁止恢复为仅 PR 门禁。
...
- 扫码签名统一硬规则：全仓 QR 签名 action registry 的代码真源只能是 `citizenchain/crates/qr-protocol/registry/*`；CitizenApp、CitizenWallet、OnChina、node、citizenweb 不得各自手写第二套 action 常量、中文动作名、字段中文名或签名判定分支。
- 扫码签名两态硬规则：所有扫码签名结果只有“正常/绿色/允许签名”和“拒绝/红色/禁止签名”两种；未知、警告、部分识别、未翻译但继续签名等第三状态一律禁止。action、字段或枚举值没有完整中文翻译时必须红色拒绝。
...
- AI 门禁脚本中的新增行匹配必须使用兼容 macOS BSD grep 与 GNU grep 的 ERE 写法；
  第三方固定版本 bundle、Wrangler 固定生成类型和 OnChina Vite 哈希构建资产可按精确路径
  跳过开发残留/中文注释源码扫描；对应手写源文件仍必须完整检查，且不得跳过依赖审计、
  类型漂移、文件完整性或其它门禁；
  Bash 3.2 的 `set -u` 下遍历空数组前必须先检查数组长度，文档-only 变更也必须能独立通过；
  本地验收必须比较基线到当前工作树以覆盖未提交修改，CI 干净工作树沿用相同口径。
...
- **发布消费**:市镇地址变更必须修改开发库；OnChina 与 citizenapp 发布包都从开发库派生本地只读快照。国家码、省码、机构码变更属于 runtime primitives 变更,必须走 runtime 二次确认。不得恢复行政区管理 tab,不得恢复 `/api/app/admin-divisions/*`,citizenapp 不联网拉取行政区新版。
