...
- 本机部署目录保密硬规则：仓库根目录 `deploy/` 是部署网页、后端、动作脚本和 Keychain 桥接源码的唯一目录，其不含密钥的源码必须被 Git 追踪。`deploy/.runtime/`、日志、编译二进制、本机状态和任何 Secret/私钥文件必须被 Git 精确忽略，任何 AI 线程不得强制添加这些私密项。部署密钥只允许保存在 macOS Keychain 或 GitHub Secrets，禁止恢复明文 Secret 文件或把密钥写入源码。生产、Release 和服务器部署必须逐次通过 Touch ID；测试和 CI 不要求密码。根 `scripts/` 只保留非部署仓库工具，不得恢复旧部署入口。
