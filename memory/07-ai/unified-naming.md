...
|---|---|---|---|
| 自由会员档 | `freedom` | API / D1 / Stripe metadata | 三档 `membership_level` 之一；不得再用身份值 `visitor` 表示会员 |
| 民主会员档 | `democracy` | API / D1 / Stripe metadata | 三档 `membership_level` 之一；不得使用泛化旧值 `visitor_pro` |
| 薪火会员档 | `spark` | API / D1 / Stripe metadata | 三档 `membership_level` 之一（ADR-036 取代旧 `voting`/`candidate` 会员档）；与身份解耦，不绑定 `identity_level` |
...
| 薪火会员 Price ID | `SPARK_PRICE_ID` | Worker var | Stripe 薪火会员月付美元 Price ID（ADR-036 取代 `VOTING_PRICE_ID`/`CANDIDATE_PRICE_ID`） |
