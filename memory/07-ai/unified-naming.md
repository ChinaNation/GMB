...
|---|---|---|---|
| 人员姓 | `family_name` | runtime / OnChina / CitizenApp / CitizenWallet / QR | 所有人的姓统一使用该字段；不得增加 `citizen_`、`admin_` 或法定代表人前缀别名 |
| 人员名 | `given_name` | runtime / OnChina / CitizenApp / CitizenWallet / QR | 所有人的名统一使用该字段；展示时才按语言顺序组合，不保存拼接姓名 |
| 法定代表人 | `legal_representative` | runtime / OnChina / CitizenApp / CitizenWallet | 原子可选结构，固定包含 `family_name + given_name + cid_number + account` |
...
动态岗位码固定显示为 `R_<32 位大写十六进制>`；哈希域分隔符唯一使用所属 pallet 的 `MODULE_TAG`，不得另造版本化域常量。全仓唯一允许的版本化协议标识是 `QR_V1`。
...
| Chat 路由缓存记录 | `ChatRoute` | `citizenapp/lib/chat/storage/chat_store.dart` | 公民端设备本地 Chat 路由缓存模型，保存对方永久 CID、Chat 设备公钥、安全码和近场提示 |
...
| 账户名称列表 | `account_names` | call data / storage | 机构账户名数组 |
