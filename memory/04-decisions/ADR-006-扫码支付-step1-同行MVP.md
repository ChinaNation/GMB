...
- 每个清算行在身份注册局注册时一次性生成:
  - `cid_number`
  - 主账户 = `blake2b_256("GMB" || cid_number || "主账户" || nonce_1)[0..32]`
  - 费用账户 = `blake2b_256("GMB" || cid_number || "费用账户" || nonce_2)[0..32]`
- **管理员数量 N 由链上 `propose_create` 时写入**,阈值 T ≥ ⌈N/2⌉,非固定 9
- 清算行在链上两步注册(CID 颁证 + organization-manage),**不经省储行审批**

### 3.2 L3 绑定

- `bind_clearing_bank` = **绑定即开户,无预存,无业务开户费**
- 链上绑定是普通链上操作，由签名者按统一链上费率支付，当前最低 0.1 元；不按投票费收取。
...
- 二维码遵循 `memory/01-architecture/qr/qr-protocol-spec.md` 的 `QR_V1 + user_transfer`,`body.bank` 填**收款方绑定的清算行主账户 SS58**
...
├── bank_check.rs    # Step 1 新增:K1 ∈ {S,F} + Active 判定
...
- `organization-manage`:**不动**,负责机构多签账户的链上注册
- `multisig-transfer`:**不动**,负责机构多签账户之间的转账
