...
- 状态：**全栈代码完成并验证（2026-06-22）**，未提交未推送，重新创世 gated 待用户部署。
...
**机构分类唯一真源 = CID 号机构码（institution_code，86 码）。** 所有"是不是公权/私权/个人/某档治理机构"一律从机构码派生，绝不另立第二套。

### 86 码体系（双布局，总长 26 不变）
- 后端唯一真源 `citizencode/backend/number/code.rs`（`InstitutionCode` 枚举 + 谓词 `is_person/is_public_legal/is_private_legal/is_unincorporated/is_education_institution/profit_policy/admin_level/…`）。
- 链端唯一真源 `citizenchain/runtime/primitives/src/institution_code.rs`（`type InstitutionCode = [u8;4]` 原始码字节 + 治理谓词 `is_fixed_governance_code/fixed_governance_pass_threshold/is_personal_code/is_institution_code/is_registered_multisig_code` + `institution_code_from_cid_number`）。
- 码段二布局：3 字符码（国家/省部）= `码(3)+盈利位(1,公权恒0)+校验(1,mod-36)`；4 字符码（市/镇/私权/个人）= `码(4)+M1(1)`。靠 seg2 index3 数字/字母分流。K1 主体属性段删除。

### 三阶段消除
- **Phase 1（后端 number/）**：删 `subject_property`/`SubjectProperty` 枚举 + DB 列；机构类别一律 `classify(code, name)` 从码派生。86 码双布局落地。
- **Phase 2（后端 org_code）**：删 `subjects.org_code`/`gov.org_code` 列 + 50+ 消费方；改 `InstitutionCode::admin_level()`/`is_city_police()`/纯码匹配派生。`registry_org_code`（管理员授权范围 FEDERAL/CITY_REGISTRY，与机构分类无关）**保留**。citizencode 前端删 `ORG_CODE_LABEL`/DTO；citizenapp 删 `orgCode` + Isar 重生。
...
- 展示侧 `INSTITUTION_CODE_LABEL` 重建为 86 新码→中文（取自后端 `label_zh`）。
- `OrgType`（node 展示枚举 NRC/PRC/PRB）保留为薄标签，值全部 primitives 单源，非独立分类。

## 备选方案
- **链端用 86 变体 enum 镜像后端**：否决——等于把码表复制一份到链上再维护。选 `[u8;4]` 原始码字节 + 少量谓词，最贴合"直接用机构码"。
