...
- **Phase 3（链端 ORG_xx，重新创世）**：旧 `org: u8` 全替换为 `institution_code: [u8;4]`（~499 引用/48 文件）；阈值 `DoubleMap` **保留结构**只改腿类型（用户"阈值存储键保持"）；固定治理档 NRC/PRC/PRB/FRG/NJD 阈值来自代码级常量。china_*.rs 294 内建 cid_number 重烤为专属码（脚本 `scripts/rebake_china_codes.py`，base36 校验位同后端）+ `scripts/rederive_accounts.py --apply` 重派生账户。客户端线格式 `org` 1 字节 → 机构码 4 字节（冷/热钱包解码器 offset、node TS invoke 参数）。

### 治理档派生（取代 ORG_xx 语义）
| 旧 ORG | 新派生 |
|---|---|
| ORG_NRC/PRC/PRB/FRG/NJD（固定阈值） | `is_fixed_governance_code` + `fixed_governance_pass_threshold(&code)`（china 内建表校验） |
