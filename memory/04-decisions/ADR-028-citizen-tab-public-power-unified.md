...
- 公民 tab 现为三子 tab `公权 / 广场 / 治理`(`citizenapp/lib/citizen/citizen_tab_page.dart`),其中:
  - 公权 = `PublicPage`(finalized 链快照 + Isar 目录,省→市→机构 地理浏览,机构详情页接入统一提案主体能力表;管理员激活复用统一管理员列表)。
...
- 一套目录仓库(finalized 链快照 + Isar,本地优先秒开,链是身份与分类唯一真源);
...
- 市/镇自治会不作为公民自助注册入口；自治会实例由注册流程写入链上机构 storage，机构码真源以 `code.rs` 当前 104 码为准。
- **选举 ≠ 立法(正交)**:选举 = 电选机构的**管理员/法定代表人**(选人);立法 = 立**法律条文**(立法)。机构按功能归立法/治理/公权,其管理员的选举是「选举活动」,不在选举 tab 重复列机构。
- **选举保留普选 + 互选**:普选由公民按宪法/选举法规定的行政区和职位范围选举；互选由机构现任成员在成员快照内选举院长、主席、参议长、众议长等职位。国家立法院参议员/众议员已确定为省行政区公民普选:各省公民分别在本省联邦立法院参议会/众议会现任成员中选出。
