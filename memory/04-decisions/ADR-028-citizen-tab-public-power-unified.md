...
CitizenApp 公民 tab 从三子 tab 重构为五子 tab(广场/立法/选举/治理/公权),公权与治理两套并行实现合并为「一套机构模型 + 一套详情页 + 一套链态服务 + 一套目录仓库」,子 tab 退化为 `institution_code` 过滤视图;选举走直选提案(不建选举委员会机构),自治会改非法人挂政府。
...
- 公民宪法**已迁为结构化链上法律**(ADR-027,2026-06-24 完成):`CitizenConstitution.html` 已删,宪法 = `legislation-yuan` 的 `law_id=0, tier=宪法`(创世注入),`ImmutableManifest` 冻结 8 条不可修改条款。下文引用的条文仍有效,但真源是链上 law_id=0;节点展示走 `constitution_getDocument` RAW storage RPC,普通法律浏览可走 `LegislationApi`。第八条「一府两会三院」:政府 / 公民教育委员会 / 公民储备委员会 / 立法院 / 司法院 / 监察院,相互无隶属、职权独立。
- 第十条/四十四条/七十三条/一百一十八条:国家公民教育委员会负责所有教育类法案草案的起草与初审 → 立法职能 → 归立法 tab。
- 第四十三条二款:总统选举现由「国家立法院选举委员会」组织(本 ADR 决定改为总统府发起,见决策 4)。
- 第六十条/六十六条:市政府设市公民自治委员会(市自治会)、镇政府设镇公民自治委员会(镇自治会),民选监督机构。
- ADR-027(立法院模块):立法/选举的链上投票走 `legislation-vote` sub-pallet,`citizen-vote` 空骨架保留供未来公民选举用 —— 本 ADR 的投票引擎部分与 ADR-027 同轨,不重复造。
...
### 4. 选举走直选提案(Option B),不建选举委员会机构

- 不创建任何「选举委员会」机构。选举 = `citizen-vote` 引擎的提案,由有权机构的管理员直接发起。
- 总统选举改由**总统府(PRS)**发起(PRS 已是 `china_zf.rs` 内置机构,自带管理员);删除宪法全部「选举委员会」描述,改第四十三条二款。
...
- **废除互选,全部改分行政区层级普选**:所有公权机构的管理员/法定代表人由公民按所在行政层级(国家/省/市/镇)普选产生;现行宪法所有「互选」职位(国家参/众议员、各级院长/主席、储委会主席、教委会等)全改普选(候选池可仍限下一级在任成员,但表决由公民普选)。护宪大法官归属**国家立法院(NLG)**。← 大宪法改写,OUT(走 ADR-027 + 后续执行卡,重新创世 bake)。
- 理由:Option A 需凭空建 ~3,229 个委员会机构 + 新增宪法机构条文且与直选条文矛盾;Option B 零新增机构体、契合「投票职责边界硬规则(业务只发起、引擎管投票)」、与全民普选模型一致。

### 5. 选举 tab = 选举活动视图(按行政层级),非机构子集

因所有机构管理员/法定代表人均由公民分级普选(决策 4),选举 tab 不再是机构子集,而是**选举活动视图**:按行政层级(国家/省/市/镇)分组,每条 = 某机构管理员/法定代表人的一场选举(候选人/投票窗口/计票/当选)。机构本体在公权(及立法/治理 功能视图)浏览,不在选举 tab 重复。本窗口选举活动流仅占位(依赖 `citizen-vote` 引擎,OUT)。
...
- `citizen-vote` 选举引擎(多候选/计票/胜选/任期)、能力层 `is_action_allowed` 实现;
- 宪法修改(改第四十三条总统府发起 + 删选举委员会 + 废互选改普选 + 护宪大法官归 NLG)—— **现为对 law_id=0 的 `propose_amend_law`**(经立法投票特别案→公投,受 ImmutableManifest 冻结条款约束),经立法发起流(任务卡 P4/P5)在 app 内落地,不再是改 HTML;
