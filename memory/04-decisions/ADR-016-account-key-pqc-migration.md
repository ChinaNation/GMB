...
`BoundPqcKey` 存**完整 ML-DSA-65 公钥**(1952 字节)+ algo/state/nonce。日常 PQC 交易(`pqc_dispatch`)**只带签名(3309 字节),不再每笔重复携带公钥**——链端按账户从 state 读公钥验签。

权衡(决策 2026-06-07):公钥进 state(永久、全节点持有)换取每笔交易省 1952 字节。对频繁交易的账户更省。Step 1 仅矿工/清算行少数高频账户,state 成本可忽略;Step 2 百万级用户钱包(全存约 2GB state)时再评估低频账户是否改回 hash。
...
- **node 本机热钱包属本 ADR 范围**:① 矿工热钱包(`powr` keystore 密钥,签转账/奖励绑定,公钥暴露在每个区块头,优先级最高)② 清算行结算签名器。两者从各自 keystore 助记词/seed 派生 ML-DSA-65(共用 `gmb-pqc`),账户级签名改走 `pqc_dispatch` general-tx。详见实现蓝图 §4.2。
...
- **只存公钥 hash + 每笔交易带公钥**:省 state 但频繁账户每笔多带 1952 字节;Step 1 高频账户不划算,改为存完整公钥(见上)。Step 2 大规模低频账户可再评估。

## 当前 sr25519 阶段:前向兼容守则(现在不接入 PQC)

现在照常只用 sr25519,**为 PQC 要写的代码 = 零**。迁移是叠加式的(未来给现有 sr25519 账户加挂 ML-DSA 凭证),账户/地址/余额不动。account/balance 不变的硬保证 = 账户仍能用现有 sr25519 钥匙签名授权未来的 `bind_pqc_key`;"不换助记词"的保证 = 留住助记词/种子供未来同源派生。

现在只需守住以下"别改/别丢"(都不是新建):
1. **账户派生不变**:`AccountId = sr25519 公钥`,SS58 prefix=2027,不改派生算法。
2. **助记词→种子算法冻结**:现有 `CryptoScheme.miniSecretFromEntropy` 路径不变,保证未来 ML-DSA 从同一种子可复现。
3. **留住助记词/种子**:钱包与 node `powr` keystore 都保留种子(BIP39),不用一次性无种子密钥。
4. **保留钩子**:runtime `TxExtension` 里的 `frame_system::AuthorizeCall`(`lib.rs:165`,general-tx 入口)与 QR/extrinsic 的 `sig_alg` 字段(现固定 `sr25519`)——只是别删。
5. **链上零创世改动**:未来 `account-keys` pallet 走 `setCode` 添加,空 storage 起步,无 migration。
...
- **范围边界(本 ADR 不覆盖,需另立决策)**:
  - 范围 B(系统签名域):SFID 省级签名 / SFID main signer / CPMS 本机档案密钥 —— 任务卡 `memory/08-tasks/open/20260607-sfid-cpms-pqc-signing.md`,拟 ADR-017。
  - 范围 C(共识签名):矿工 `powr` 密钥的 **PoW 出块 seal 签名**(`SimplePow::verify`)—— 共识规则变更,需全节点二进制硬升级,非 setCode,本 ADR 与范围 A/B 均不覆盖,需单独决策(对标 Polkadot 共识签名用 ML-DSA)。
