...
三类主体的 institution_id 第 1 字节互不相同(0x01 / 0x02 / 0x03),即使后续字节内容偶然相同,kind tag 也强制隔离。撞 key 概率从 2^-32(哈希熵巧合)降到 0(结构性互斥)。
...
pub fn build_institution_id(kind: SubjectKind, payload: &[u8]) -> Option<[u8; 48]>;
pub fn parse_institution_id(id: &[u8; 48]) -> Option<(SubjectKind, &[u8])>;
...
1. 在 `SubjectKind` enum 加新 variant + 分配 0x04..0xFE 中的字节值
