...
- APNs/FCM 无内容唤醒。
- 短期 Cloudflare Realtime TURN 凭证。
...
附件经 WebRTC DTLS DataChannel 设备间传输。直连失败时允许 Cloudflare TURN 中继，但 Worker、D1、KV、Durable Object Storage 和 R2 均不得保存附件字节或引用。

## 删除

用户删除会话时只删除当前设备本地记录。用户注销时，当前设备本地 Chat 数据清空，Worker 同步关闭连接、撤销 TURN 并硬删除所有设备登记、KeyPackage、防重放和 TURN 行。
