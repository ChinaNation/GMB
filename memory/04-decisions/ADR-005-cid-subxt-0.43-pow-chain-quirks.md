# ADR-005 cid 后端在 PoW 链上使用 subxt 0.43 的协议对齐

- 日期：2026-04-07
- 模块：`citizencode/backend` ↔ `citizenchain`
- 状态：已采纳
...
3. **wait_for_finalized 在 PoW 链上不可用**
   cid 业务语义只需要"extrinsic dispatch 成功"，不需要 GRANDPA 最终化。
...
`citizencode/backend` 推链路径上**强制**做以下四件事：
...
- `citizencode/backend/core/chain_runtime.rs` 顶部 `*_DOMAIN` 常量段及注释

## 影响

- ✅ register_cid_institution / rotate_cid_keys 在 PoW 链上稳定工作
- ✅ 任何未来新加的链上 verifier domain 都会被代码顶部注释提醒使用数组类型
- ⚠️ cid 推链不再等 GRANDPA finality；如果未来出现需要"等到不可逆"的业务
  场景（例如跨链桥/重资产托管），需要显式 opt-in `wait_for_finalized`，
  并且业务超时要按 GRANDPA 节奏重新设计
- ⚠️ 显式 nonce 取自 best+pool 视图：如果同一账户被并发推多笔交易，
  cid 必须串行化提交（已通过单写入路径保证）
...
2. **改用 substrate-api-client** —— API 更原始，需要更多手写胶水。
   现有 cid 后端 24 处 subxt 调用全都要改，工作量过大，弃用。
