...
- **账户密钥只派生 sr25519 + ML-DSA-65**(决策3:账户**不派生** ML-KEM;机密性 KEM 在 Chat/TLS 层独立,见 §10)。

## 1. 外部标准依据(锁版本 + 测试向量)

- 签名 = **ML-DSA-65**(NIST FIPS 204,有 errata → **锁库版本 + 钉测试向量/golden vector**)。
- 机密性 KEM = **ML-KEM-768**(NIST FIPS 203),**仅用于 Chat/TLS 层,不进账户密钥体系**。
- Chat/TLS 混合 KEM 参考 X-Wing(draft 且非认证 KEM → **不能当身份认证**)。
...
- **(决策6)card0 不含 Chat**:card0 = 清算行真 AES-GCM + App 锁 KDF + 热钱包 at-rest;Chat 归 card6。
...
| 卡 | 范围 |
|---|---|
| card0 | 卫生修复(清算行真 AES-GCM+Argon2id / App锁KDF / 热钱包at-rest)**不含 Chat** |
...
| card6 | Chat X-Wing(ML-KEM-768,一次到位含AES-256)+ TLS X25519MLKEM768 + **Chat 钱包绑定签名升级** + 群rekey归属 |
...
3. **账户不派生 ML-KEM**——机密性 KEM 仅在 Chat/TLS 层,与账户密钥无关。
4. **PQC 交易 era 钉死 immortal**——PoW 难度无问题,不用 mortal/checkpoint。
6. **card0 不动 Chat**——Chat ciphersuite 一次性由 card6 完成。
