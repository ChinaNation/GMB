...
## 一、问题
- `AdminAccounts: StorageMap<Blake2_128Concat, AccountId(账户地址), AdminAccount{org,kind,admins[],...}>`(`runtime/admins/admin-management/src/lib.rs:223`)。
...
- 读路径:轻节点对每个本地钱包 `getKeysPaged(前缀=AdminAccountsByMember[admin_pubkey])` 直接拿到其管理的账户地址 → 再精确读 `AdminAccounts[addr]` / `AccountRegisteredCid[addr]` 取详情。**全程无全表扫**。
