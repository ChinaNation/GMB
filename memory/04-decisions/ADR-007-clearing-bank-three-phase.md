...
公民区块链节点 UI 需要支持"清算行 tab"，让节点机构方在区块链软件上注册自己为清算行节点。在多轮需求澄清中明确了三个核心事实：

1. **清算行不是新机构类型**：链上现有机构类型枚举（国家储委会/省储委会/省储行 = orgType 0/1/2）不增加新枚举值
...
- `App.tsx` TabKey 加 `'clearing-bank'`，顶部 nav 9 tab：首页 / 挖矿 / 国家储委会 / 省储委会 / 省储行 / **清算行** / 白皮书 / 公民宪法 / 设置
