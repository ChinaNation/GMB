...
- **2026-04-20 起升级到 DUOQIAN + op_tag 统一域**：所有 SFID 系统与链端之间的签名 payload 第一字段为 `DUOQIAN = b"DUOQIAN"`(10 字节),紧跟 1 字节 `op_tag`(`OP_SIGN_BIND=0x10` / `OP_SIGN_VOTE=0x11` / `OP_SIGN_POP=0x12` / `OP_SIGN_INST=0x13`)区分业务,再拼 `genesis_hash` 和业务字段。地址派生也统一到同一域(`OP_MAIN=0x00` / `OP_FEE=0x01` / `OP_STAKE=0x02` / `OP_AN=0x03` / `OP_PERSONAL=0x04`)。所有旧域前缀(`GMB_SFID_V1` / `DUOQIAN_SFID_V1` / `DUOQIAN_PERSONAL_V1` / `FEIYONG_SFID_V1` / `ANQUAN_SFID_V1` / `GMB_SFID_*_V2/V3`)彻底退役,按 `feedback_no_compatibility.md` 不留兼容。常量见 `primitives::core_const::{DUOQIAN, OP_*}`。
- **2026-04-21 起 op_tag 新增 `OP_INSTITUTION=0x05`**：专供 SFID 登记机构的自定义命名账户（临时/工资/运营...），派生 `blake2_256(DUOQIAN || 0x05 || ss58 || sfid_number || account_name)`。`OP_MAIN`/`OP_FEE` 统一为 `ss58 || sfid_number`（无 account_name 后缀）——宪法机构和 SFID 机构的主账户/费用账户派生公式**彻底对齐**。链端以 `InstitutionAccountRole` 枚举三分派（`Main`/`Fee`/`Named(account_name)`），保留名 `"主账户"`/`"费用账户"` 强制走 `Role::Main`/`Role::Fee`，不得作为 `Role::Named` 参数（返回 `ReservedAccountName` 错误）。老函数 `derive_duoqian_address_from_sfid_number` 重构为 `derive_institution_account(sfid_number, role)` + `role_from_account_name(account_name)` 翻译辅助。链端字段名 `name`/`SfidNameOf` 同步重命名为 `account_name`/`AccountNameOf`，与 SFID 后端 `MultisigAccount.account_name` 对齐（2026-04-21 第二轮，见 `20260421-name-to-account-name-rename`）。
