...
- `audit`：结构化审计记录，按省分区。
- `admins`：注册局/机构管理员本地元数据缓存(成员资格真源在链上;`federal_registry_scope`/`provinces` 表已退役,FRG 省维度取节点 env / 链上 `FederalRegistryProvinceGroups`,见 [[project_onchina_registry_tier_chainread_2026_06_29]])。
