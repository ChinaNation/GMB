...
机构首次登记只提交 CID 基础资料和至少两个管理员的 `admin_account + family_name + given_name`。后端分别从公民档案解析姓、名，仍无法解析时分别使用“管理”“员”；岗位、法定代表人任职、治理阈值、协议账户地址和注资金额不属于首次登记输入。runtime 自动创建唯一默认“法定代表人”岗位、严格多数阈值和机构类型要求的零余额协议账户。协会 `SFAS` 的盈利属性由注册局显式选择。
