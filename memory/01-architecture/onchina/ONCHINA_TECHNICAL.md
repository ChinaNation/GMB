...
- `audit`：结构化审计记录，按省分区。
- `admins`：机构管理员本地元数据缓存，字段固定为 `admin_account + family_name + given_name`；成员资格真源在链上 active admin 集合。
...
旧机构直接创建入口已关闭：OnChina 当前创建 API 固定返回 501，前端创建按钮固定禁用，不生成 `0x1e05/0x1f05` 或旧签名会话。第 6 步的新机构创建业务必须原子提交 CID 基础资料、至少两个 `Admin { admin_account, family_name, given_name }`、强制 LR、至少一个初始治理岗位及不可变权限、初始任职、初始投票规则和完整零余额协议账户；注册局管理员仅凭 admins 身份不能发起，必须提交其有效 `RoleSubject` 并通过业务权限校验。
...
- 登录态、管理员缓存、机构管理员投影和链上 `admins` 都固定使用 `admin_account + family_name + given_name`；数据库不保留合并姓名列，页面只在渲染时按中文顺序拼接姓、名。
- 机构管理员列表联合读取链上 `admins` 三字段人员集合与 entity 机构岗位任职；没有岗位的管理员仍必须返回并拥有按账户判断的机构管理员资格。本地联系方式、照片和 Passkey 仅是私密资料，不得成为管理员资格或岗位真源。
