...
pub trait IncomeLedgerWriter<CidNumber, Balance> {
    fn record_income(payee: CidNumber, period: TaxPeriod, amount: Balance) -> DispatchResult;
}
...
runtime/transaction/subscription/            [新增] idx 35  name="subscription"
  └── src/{lib,types,proposal,validation,execution,billing,weights,benchmarks}.rs + tests/{mod,cases,billing}.rs
...
```rust
#[runtime::pallet_index(35)] pub type Subscription = subscription;   // 交易-会员订阅按月自动扣
...
| 创作者档 | `CreatorPlans:StorageMap<CidNumber,BoundedVec<CreatorTier>>` | — | — | 读写 | — | PascalCase |
...
| 目录 | `transaction/subscription`、`public/tax-registry` | `domains/membership`、`domains/tax` | `membership/` | `8964/creator/` | — | kebab |
...
**闭合4 命名闭合（`subscribe` 逐端零漂移）**：runtime fn `subscribe`/call_index=N ↔ App `_subscribeCallIndex=N`+`SubscriptionService.subscribe` ↔ Worker `subscription_source='citizen_coin'`；`IssuerKey::Platform|Creator(CidNumber)` 单源 primitives。防漂移三机制：①金标 fixture 逐字节断言（`primitives/tests/fixtures/*.json` + CitizenWallet 同构对同一 JSON）②onchina 编码器单测反向对拍 Rust `.encode()` ③冷签严格两色白名单（未登记默认拒）。新协议先入 `memory/07-ai/unified-protocols.md` + `unified-naming.md`。
...
## 拍板点（落地前）
1. `[分组归属]` subscription→`runtime/transaction/`（推荐，价值转移，与 multisig/onchain 同域）；tax-registry→`runtime/public/`（推荐，公权壳+投票外包，与 legislation-yuan 同域）。
