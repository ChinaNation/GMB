...
│   SquarePost(idx 34) subscription / tax-registry(idx 36·第3期)     │
...
┌──────────────┴─────────────────────────────┴─────────────────────┐
│ 客户端层                                                          │
│  CitizenApp(热钱包:subscribe/cancel/change/set_creator_plans)     │
...
|---|---|---|---|
| `lib.rs` | `#[pallet]` 本体 Config/Storage/Event/Error/GenesisConfig/`#[pallet::call]` | `subscribe`/`cancel`/`change_subscription_plan`/`set_creator_plans` + `propose_set_platform_price` | `propose_set_tax_rate` |
...
| `billing.rs`/`settlement.rs` | 跨块有界调度 | `billing.rs` 在时间戳写入后按真实公历到期索引自动扣款 | `settlement.rs` tax_period 桶扫结算 |
...
| 功能 | 在哪 | 理由 |
|---|---|---|
| **① 权威状态** `Subscriptions`/`PlatformPrice`/`CreatorPlans`/`RenewalSchedule`/`RenewalIndex`/`IncomeLedger`/`TaxRules`/`TaxAuthorization` | **runtime** | 资金授权、自动续费与税基的唯一真源,必须链上不可篡改 |
| **② 资金动作** 按订阅真实公历周期扣款转账(`billing`)、税结算征收(`settlement`) | **runtime** | 真金转移,必须链上原子执行 |
...
| 订阅、取消、换套餐编排(构造 extrinsic/提交/轮询确认) | App `subscription_service` | 自动续费不经过客户端 |
...
runtime/misc/square-post/                    [改动] 扩展(保持名·零新增 pallet):原广场发帖索引 + 会员订阅
  ...
  ├── subscription.rs [新增] IssuerKey{Platform,Creator(AccountId)}(创作者收款账户不要求CID，但设置档位和扣款时必须有有效平台会员)/SubscriptionState/CreatorTier + subscribe/cancel/change_subscription_plan/set_creator_plans
  ├── billing.rs      [新增] 按共识时间戳和真实 UTC 公历有界调度自动扣款
  └── proposal.rs     [新增] MODULE_TAG + propose_set_platform_price 治理 + InternalVoteExecutor
     新增 Storage: Subscriptions/PlatformPrice/CreatorPlans/RenewalSchedule/RenewalIndex;Config +Currency+治理+护栏;Genesis PlatformPrice 三档
...
3. **`RuntimeCallFilter::contains` 与 `RuntimeFeeRouter::fee_route` 拆开写（并入审查 H1，二者语义相反）**：
   - `fee_route` 穷尽匹配（编译器强制）：`subscribe`/`cancel`/`change_subscription_plan`/`set_creator_plans` → `signer_onchain_route`；`propose_*` → `institution_onchain_route`；`billing` 内部触发，不产生外部交易费。
   - `contains` 结尾是 `_ => true`（fail-open），故**未启用/仅内部驱动的 call 必须显式 `RuntimeCall::TaxRegistry(_) => false`**（仿 `OnchainIssuance(_) => false`），防治理入口被静默暴露为可入块；热轨 `SquarePost(subscribe/cancel/change_subscription_plan/set_creator_plans)` 显式列出放行，防误入 deny。
...
```
my/membership/membership_page.dart      [改动] 直调 SubscriptionService.subscribe;banner 展示 Active/Cancelled/Terminated 与真实日期;删 USDC 入口
...
```
signer/pallet_registry.dart   [改动] // ---- SquarePost(34) ---- / // ---- TaxRegistry(36) ---- + call_index 常量
...
|---|---|---|---|---|---|---|
| pallet_index | `34`/`36` | `SQUARE_POST_PALLET_INDEX=34`/`TAX_REGISTRY_PALLET_INDEX=36` | — | `_squarePostPalletIndex=34` | `squarePostPallet=34` | ★ 五端相等 |
...
| 自动续费索引 | `RenewalSchedule` + `RenewalIndex` | — | finalized 镜像 | 只读 | — | 按共识时间戳和真实公历调度 |
| extrinsic 热签 | `subscribe`/`cancel`/`change_subscription_plan`/`set_creator_plans` | — | — | `subscribe`/`cancel`/`changePlan`/`setCreatorPlans` | — | 每个业务操作只签一次 |
...
| 自动终止态 | `SubscriptionStatus::Terminated` | — | `terminated` | `terminated` | — | ★ |
...
**闭合1 公民币订阅（热钱包）**：`membership_page` 选档 → `subscription_service.subscribe(IssuerKey.platform,level)` 校验 → `subscription_rpc._buildSubscribeCall(34,idx,SCALE(issuer,plan))` → `WalletManager.signWithWallet` → `SignedExtrinsicBuilder.signAndSubmit` 标准 extrinsic → runtime 首扣、按 UTC 真实公历写 `Subscriptions[(acct,Platform)]=Active` 并登记 `RenewalSchedule` → 后续到期由 runtime 自动按最新价格扣款 → App `POST …/citizen-coin/confirm{tx_hash}` 仅触发 `citizen_coin.ts` 链读 → `upsertCitizenCoinMembership`（`tx_hash` PK 幂等）镜像 D1。不变量：链上唯一真源、D1 仅镜像、续费无需签名、confirm 只读不定价。

**闭合2 onchina 调价（冷签）**：`domains/membership/handler` → `capability.can_set_platform_price`（技术公司唯一 CID）→ `operation_auth` `PasskeyColdSign` → `chain_call.build_*` 构造 `propose_set_platform_price` 裸 SCALE(pallet=34) → QR → CitizenWallet `payload_decoder` 命中 34/idx → 三处登记齐→两色核对签 / 缺任一→`decodeFailed` 红拒 → `chain_submit` → 链上建 InternalVote → 机构表决通过 → `InternalVoteResultCallback` 命中 `subscription::Executor` 按 `MODULE_TAG=b"sub-scr"` 认领 → `execution` 写 `PlatformPrice[level]` → onchina 回读确认。**逐字节相等项（并入审查 M）=数字动作码 `(pallet<<8)|call_index` + SCALE 字段序**；**方法名闭合三元组 = Rust `#[pallet::call]` fn 名 ↔ `payload_decoder` action 串 ↔ `qr_protocols` switch key**（`review_title` 是中文展示串，**不参与字节校验**）。
...
**闭合4 命名闭合（`subscribe` 逐端零漂移）**：runtime fn `subscribe`/call_index=N ↔ App `_subscribeCallIndex=N`+`SubscriptionService.subscribe`；`IssuerKey::Platform|Creator(AccountId)` 单源 primitives。创作者收款账户不要求 CID，但创建档位和每次创作者订阅扣款都要求其平台会员权益有效。防漂移三机制：①金标 fixture 逐字节断言（`primitives/tests/fixtures/*.json` + CitizenWallet 同构对同一 JSON）②onchina 编码器单测反向对拍 Rust `.encode()` ③冷签严格两色白名单（未登记默认拒）。新协议先入 `memory/07-ai/unified-protocols.md` + `unified-naming.md`。

**签名与收费闭合**：订阅、取消、换套餐、创作者设置档位分别只提交一笔账户签名交易；链上 finalized 后的 Worker 镜像只用 Bearer 会话与链读复核，失败重试不生成账户签名或设备请求签名。上述非系统交易全部经过统一交易支付扩展收费，业务转账金额为零也不能免交易费；runtime 内部自动续费不是外部交易，不追加用户交易费。
...
关键路径引用：`runtime/src/configs.rs`（回调元组第6槽 + CallFilter/FeeRoute 拆写）、`runtime/src/lib.rs`（construct_runtime 34/36）、`onchina/src/domains/address/chain_call.rs`（chain_call 范本）、`cloudflare/src/membership/service.ts`（门禁落点）、`citizenapp/lib/rpc/transfer_rpc.dart`（subscription_rpc 骨架范本）、`node/src/core/chain_spec.rs`（创世装配）。
