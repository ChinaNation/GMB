...
“公民”详情页固定两行：第一行是公民的“编译软件 / 运行 CI / 正式 Release”，第二行是公民钱包的同三项操作。公民的每个动作按钮保留原宽度的三分之二，主标题与说明文字均居中。公民云页的操作行从左到右为「生产部署、官网部署、平台对账、创作者对账」，「配置」靠右；后两个按钮本身就是对账熔断开关，点击即翻转、状态直接写在按钮上，下方状态区只读地展示两条对账线各自的管辖范围与当前状态。对账的真正执行者是 Worker Cron，控制台不提供手动触发入口——开关只用于熔断，关闭期间链上权益不受影响，仅镜像停止更新。公民链页的 5 个动作与「配置」固定同一行、按钮不带副标题；其 6 个公共密钥（`SSH_KEY`、`GH_TOKEN`、`GMB_TOP_KEY`、`GMB_TOP_PUBKEY`、`NRC_SIGNER_PUBLIC_KEY`、`CHAIN_GENESIS_HASH`）只在「配置」弹窗内维护，页面主体只留权威节点列表；44 个节点各自的三项密钥仍在各自条目内。密钥表的增删改查只有一处绑定实现，弹窗与模块详情共用——每一项都要过 Touch ID，复制第二套必然漂移。两个移动产品的 `GMB_APP_KEY`、`SSH_KEY` 和 `GH_TOKEN` 合并去重后各显示一次；每个动作仍携带精确 `productId` 并调用原 CitizenApp 或 CitizenWallet 脚本。全部动作彻底独立并发，一个功能一个日志标签，互不阻塞——曾经的「生产动作全局串行」已删除，它让任意一个卡住的远端任务锁死整个控制台。唯一保留的限制是同一功能不允许同时开两个任务；44 个权威引导节点的部署各自是独立功能，靠 `nodeId` 参与去重键并在标签标题中带出节点名。日志标签任何状态都可关闭：关闭只断开当前页面 SSE 跟踪，后台任务与远端 GitHub CI 继续运行；要真正终止任务须点「停止」。运行中的任务一律可停，每个动作都在独立进程组内，按进程组终止整棵进程树；远端动作停的是本机等待进程，GitHub 上已启动的 CI 不受影响。同一页面不会自动重新弹出已主动关闭的运行标签，重新打开控制台仍可接管尚未结束的任务。
