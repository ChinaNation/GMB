...
“公民”详情页固定两行：第一行是公民的“编译软件 / 运行 CI / 正式 Release”，第二行是公民钱包的同三项操作。公民的每个动作按钮保留原宽度的三分之二，主标题与说明文字均居中。公民云页的操作行从左到右为「生产部署、官网部署、平台对账、创作者对账」，「配置」靠右；后两个按钮本身就是对账熔断开关，状态直接写在按钮上，下方状态区只读地展示两条对账线各自的管辖范围与当前状态。两个开关**进页面时不自动读取**，初始为「点击读取」：读开关要用 Cloudflare 数据令牌，每读一次就强制一次 Touch ID，进页面就读等于只想看一眼部署按钮也被按指纹，刷新时停在 `#cloudflare` 更会连弹两次。第一次点击只读不写（不知道当前值就翻转等于闭眼改熔断开关），一次读取把两条线一起填好，之后点击才是真正的翻转；读或写失败时按钮退回可点的「点击重试」，不允许卡在禁用态。对账的真正执行者是 Worker Cron，控制台不提供手动触发入口——开关只用于熔断，关闭期间链上权益不受影响，仅镜像停止更新。公民链页的 5 个动作与「配置」固定同一行、按钮不带副标题；其 6 个公共密钥（`SSH_KEY`、`GH_TOKEN`、`GMB_TOP_KEY`、`GMB_TOP_PUBKEY`、`NRC_SIGNER_PUBLIC_KEY`、`CHAIN_GENESIS_HASH`）只在「配置」弹窗内维护，页面主体只留权威节点列表；44 个节点各自的三项密钥仍在各自条目内。密钥表的增删改查只有一处绑定实现，弹窗与模块详情共用——每一项都要过 Touch ID，复制第二套必然漂移。两个移动产品的 `GMB_APP_KEY`、`SSH_KEY` 和 `GH_TOKEN` 合并去重后各显示一次；每个动作仍携带精确 `productId` 并调用原 CitizenApp 或 CitizenWallet 脚本。全部动作彻底独立并发，一个功能一个日志标签，互不阻塞——曾经的「生产动作全局串行」已删除，它让任意一个卡住的远端任务锁死整个控制台。保留的限制只有两条：同一功能不允许同时开两个任务（44 个权威引导节点的部署各自是独立功能，靠 `nodeId` 参与去重键并在标签标题中带出节点名）；以及「编译」这一个全局串行点——它 `mv` 走的正是其它动作正在使用的签名包并删掉旧包，同一个动作前半段在旧包里跑、后半段解析到新包，混版执行在日志里看不出来，所以有任何动作在运行时一律拒绝编译。日志标签任何状态都可关闭：关闭只断开当前页面 SSE 跟踪，后台任务与远端 GitHub CI 继续运行；要真正终止任务须点「停止」。运行中的任务一律可停，每个动作都在独立进程组内，按进程组终止整棵进程树；远端动作停的是本机等待进程，GitHub 上已启动的 CI 不受影响。同一页面不会自动重新弹出已主动关闭的运行标签，重新打开控制台仍可接管尚未结束的任务。
