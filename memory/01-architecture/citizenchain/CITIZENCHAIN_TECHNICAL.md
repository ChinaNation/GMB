...
- 机构管理员集合真源归 `admins`，目标只保存钱包账户集合；机构岗位和机构管理员任职关系真源归 `entity`。岗位的具体职责与授权由对应业务模块依据机构、岗位和现有硬规则判定，不建立通用 `role_permissions`。当前 `AdminProfile` 内嵌姓名、CID、岗位、任期和来源的布局是待清理旧实现，不是目标架构。个人多签保持独立，不纳入机构岗位模型。
