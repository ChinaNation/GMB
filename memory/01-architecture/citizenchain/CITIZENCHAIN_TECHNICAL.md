...
- 创世机构：49,593 个公权机构和私权非营利法人“中国公民链技术发展基金会”统一在创世阶段上链。公民链基金会使用 `PrivateManage/PrivateAdmins`，其 CID、协议账户、一名程伟管理员、同一钱包的三项固定岗位任职、机构阈值 2 和法定代表人引用来自 runtime primitives 单一常量源；OnChina 运行期读取链上机构、admins、岗位权限和有效任职，并用本地投影补齐展示字段，不生成第二套机构授权真源。
