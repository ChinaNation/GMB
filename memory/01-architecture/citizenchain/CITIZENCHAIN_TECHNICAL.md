...
- 机构管理员集合真源归 `admins`，机构管理员是可任职人员。公权值结构为 `PublicAdmin { account_id, cid_number, family_name, given_name }`，非空公民 CID 必须与 `citizen-identity` 的 `AccountIdByCid` / `CidByAccountId` 双向真源一致；私权机构和个人多签保持 `Admin { account_id, family_name, given_name }`。管理员账户本身没有机构业务权限。
