...
- 创世机构：49,593 个公权机构和私权创世机构“中国公民链技术有限公司”统一在创世阶段上链。技术公司使用 `PrivateManage/PrivateAdmins`，其 CID、协议账户、三名管理员、三项固定岗位、2/3 阈值和法定代表人引用来自 runtime primitives 单一常量源；OnChina 运行期只读链上机构和链上 active admins，并用本地投影补齐展示字段，不生成第二套机构授权真源。
...
- 机构管理员集合真源归 `admins`，机构管理员是人，值结构统一为 `admin_account + family_name + given_name`；机构岗位和机构管理员任职关系真源归 `entity`。岗位的具体职责与授权由对应业务模块依据机构、岗位和现有硬规则判定，不建立通用 `role_permissions`。个人多签保持独立，不纳入机构岗位模型。
