...
### 11.3 CI 发布边界
- `citizenchain-wasm.yml` 的 push 自动 CI 只编译当前源码 WASM，不查询链上版本、不读取 SSH Secret、不连接服务器；根目录私密 `scripts/citizenchainwasm.sh` 自动触发 workflow 后，才允许使用 `GMB_SSH_KEY` 查询链上 `spec_version` 并在 CI 工作区临时提升构建版本。该入口只生成和校验 WASM，不自动执行链上升级。
- `citizenchain-ci.yml` 的 push 与 `mode=ci` 只做桌面端打包检查和本次 run artifact 上传，不读取 Tauri updater 签名私钥、不发布 GitHub Release、不部署服务器。
- `scripts/citizenchain.sh` 输入 `s` 时以 `mode=release` 自动触发 updater 签名、`citizenchain-latest.json` 和 GitHub Release；输入 `b` 时以 `mode=deploy` 自动构建并在本次 CI 全部成功后使用 `GMB_SSH_KEY` 滚动部署 Linux 服务器。两个动作互相隔离，不再由一次无参数 `workflow_dispatch` 同时发布和部署。
