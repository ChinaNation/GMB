...
`citizenchain/onchina` 是公民链 workspace 成员 crate，承接链上中国平台、多机构工作台、注册局业务、行政区、机构登记、管理后台和链侧凭证能力。任意机构可在办公室服务器安装节点后手动启动 OnChina；首次管理员冷钱包登录后由链上 active admin 关系确定并绑定本节点机构。
...
- 工作台模型：登录钱包账户属于哪个链上机构管理员集合，就进入哪个机构工作台；同一钱包属于多个机构时先选择机构。注册局是 `workspace` 的一类，司法院、立法院、学校、公司、公益组织等机构按自己的工作台 UI 进入“操作 / 显示 / 记录”页面。
- 数据两层：链上最小身份 + 承诺哈希(选择性/绑定触发上链)；链下明细存本市内嵌 PostgreSQL + 本地/NAS 文件仓库(文件哈希上链验真)。
- 创世机构：现有公权机构已经统一在创世阶段上链。OnChina 运行期只读链上机构和链上 active admins，并用本地投影补齐展示字段，不生成第二套公权机构真源。
...
- 机构管理员集合真源归 `admins`，只保存钱包账户集合；机构岗位和机构管理员任职关系真源归 `entity`。岗位的具体职责与授权由对应业务模块依据机构、岗位和现有硬规则判定，不建立通用 `role_permissions`。个人多签保持独立，不纳入机构岗位模型。
...
### 11.3 CI 发布边界
- `citizenchain-wasm.yml` 的 push 自动 CI 只编译当前源码 WASM，不查询链上版本、不读取 SSH Secret、不连接服务器；根 `deploy/` 本地控制台显式触发 WASM CI 后，才允许使用 `GMB_SSH_KEY` 查询链上 `spec_version` 并在 CI 工作区临时提升构建版本。控制台源码可追踪，密钥仍只从 Keychain/GitHub Secrets 注入。该入口只生成和校验 WASM，不自动执行链上升级。
- `citizenchain-ci.yml` 的 push 与 `mode=ci` 只做桌面端打包检查和本次 run artifact 上传，不读取 Tauri updater 签名私钥、不发布 GitHub Release、不部署服务器。
- 根 `deploy/` 控制台选择“正式 Release”时以 `mode=release` 自动触发 updater 签名、`citizenchain-latest.json` 和 GitHub Release。“部署服务器”保留为独立生产入口，先从 `institution-catalog.json` 展示44个权威节点，再选择单个节点；控制台只允许部署当前远端提交已有成功 CI 的 Linux amd 产物，不重新触发 Release。
- 每个权威节点的服务器 IP、节点身份 Ed25519 私钥、GRANDPA 验证私钥和 SSH 部署私钥按 `node-01` 至 `node-44` 隔离保存在 macOS Keychain。网页只返回 IP、公开 PeerId、公开 GRANDPA 公钥和“已配置/缺失”状态；私钥录入后不得回传。保存、更换与部署均逐次要求 Touch ID。
- 节点私钥保存前必须从私钥推导公开身份，并分别与 `institution-catalog.json` 的 PeerId 和 GRANDPA 公钥精确匹配；不匹配时禁止写入。部署时只把选中节点的密钥写入权限为 `0600` 的节点身份文件和 `gran` keystore，清理远端临时文件，并真实检查 systemd、RPC health 与 Authority/Validator 角色。
