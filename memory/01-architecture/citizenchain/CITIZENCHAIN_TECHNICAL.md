...
- 创世机构：49,593 个公权机构和私权非营利法人“公民链技术发展基金会”统一在创世阶段上链。公民链基金会使用 `PrivateManage/PrivateAdmins`，其 CID、协议账户、一名程伟管理员、同一钱包的三项固定岗位任职、机构阈值 2 和法定代表人引用来自 runtime primitives 单一常量源；OnChina 运行期读取链上机构、admins、岗位权限和有效任职，并用本地投影补齐展示字段，不生成第二套机构授权真源。
...
- 机构管理员集合真源归 `admins`，机构管理员是可任职人员。公权值结构为 `PublicAdmin { admin_account, cid_number, family_name, given_name }`，非空公民 CID 必须与 `citizen-identity` 的 CID↔钱包真源一致；私权机构和个人多签保持 `Admin { admin_account, family_name, given_name }`。管理员账户本身没有机构业务权限。
...
- `public-admins` 的 `AdminAccounts` 只接受当前四字段 `PublicAdmin` SCALE 布局，`private-admins` / `personal-admins` 只接受三字段 `Admin` 布局；旧纯账户、旧合并姓名和历史 storage migration 均已删除，不保留兼容或双轨。公权身份资料当前允许为空，私权/个人姓名必须非空。
...
- 开发期通用 `election-campaign` 骨架已决定删除。未来每种具体公权选举业务在 `runtime/public/` 下新增独立模块；具体业务模块定义规则，选举投票、计票和结果快照统一归 `election-vote`。

当前模块：
- `legislation-yuan`（idx25）
- `election-campaign`（idx32，待删除骨架；删除后 index 留空）
