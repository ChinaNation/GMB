...
- 根 `deploy/` 控制台选择“正式 Release”时以 `mode=release` 自动触发 updater 签名、`citizenchain-latest.json` 和 GitHub Release。“部署服务器”是与本地工作区、当前 HEAD 和 Release 解耦的独立生产入口：从 `institution-catalog.json` 选择节点后，只消费 GitHub `main` 最新成功 CitizenChain CI 的 Linux amd artifact；目标服务器使用 GitHub 短期签名地址直接下载，本机只传服务配置和节点密钥，不下载或转传安装包。
