...
- 机构管理员集合真源归 `admins`，机构管理员是人，值结构统一为 `admin_name + admin_account`；机构岗位和机构管理员任职关系真源归 `entity`。岗位的具体职责与授权由对应业务模块依据机构、岗位和现有硬规则判定，不建立通用 `role_permissions`。个人多签保持独立，不纳入机构岗位模型。
- `public-admins` / `private-admins` 的 `AdminAccounts` 当前存储版本为 v4；从旧 `Vec<AccountId>` 管理员集合升级到 `Vec<InstitutionAdmin>` 时，runtime 内置一次性 storage migration，把旧钱包账户翻译为默认姓名“管理员”+原钱包账户。查询路径只认 v4 新结构，不保留双轨兼容。
