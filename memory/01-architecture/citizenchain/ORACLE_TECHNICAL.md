...
- 远程管理 Tunnel：`nrcgch-rpc`
- 私有 RPC hostname：`nrcgch-rpc.crcfrcn.com`
- Tunnel origin：`http://127.0.0.1:9944`
- Access 应用：`CitizenChain RPC`
- Access 策略：只允许指定 Service Token 的 `Service Auth`，不得增加 `Everyone`、交互式 `Allow` 或 `Bypass`
- staging Worker：只通过三项远端 Secret 保存 RPC HTTPS URL 和 Access 服务令牌；不得把令牌、Tunnel token 或完整私有 URL 写入仓库、安装包、日志或命令文档
