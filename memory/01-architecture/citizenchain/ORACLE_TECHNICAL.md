...
- 唯一发布路由为 `chain.crcfrcn.com -> http://127.0.0.1:18080`，该路由不是 CitizenChain `9944` RPC。
- 原 `nrcgch-rpc.crcfrcn.com` 已无 DNS，当前没有可供 Worker 使用的私有 RPC hostname。
- staging/production Worker 仍存在 `CHAIN_URL`、`CHAIN_ID`、`CHAIN_SECRET` Secret 名称，但 Secret 值不可回读，不能作为链路可用证明。

目标边界保持不变：Worker 私有 RPC 必须使用独立 hostname、`Service Auth` 和 Tunnel 转发到 `127.0.0.1:9944`；不得增加 `Everyone`、交互式 `Allow` 或 `Bypass`，也不得把令牌、Tunnel token 或完整私有 URL 写入仓库、安装包、日志或命令文档。该目标当前尚未部署完成。
...
4. 不携带 Access 服务令牌访问私有 RPC hostname 必须返回 `403`；携带 Worker 专用服务令牌后必须到达本机 RPC。
