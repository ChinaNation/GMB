...
- 事实源:本文是流程导读;字段与编码以代码为准
  ...
  - `citizenchain/runtime/otherpallet/citizen-identity/src/lib.rs`(链上身份 + 人口计数)
...
- **两步快照、同块强约束**:
  1. `prepare_population_snapshot(scope)`:经
     `CitizenIdentityReader::population_count(scope)` 读当前计数(必须 >0),
     存 `PendingPopulationSnapshots[发起人]{eligible_total, scope,
     prepared_at=当前块}`;
  2. 同块创建特别案(`vote_type=SPECIAL`)时消费:`prepared_at != now` 报
     ...
     作用域存 `referendum_scope`。
- **分母固化、资格实时**:`cast_referendum_vote` 对每张票实时
  `can_vote(who, scope)` 校验;快照只决定通过门槛分母,不产生选民名单。
- **消费端全量校验**(`can_vote`):身份存在(注册时已验公民签名并锁定
  CID↔钱包一对一)+ 状态 NORMAL + **护照有效期窗口内**(链上时间戳按 UTC+8
  折算 YYYYMMDD,过期或未生效即拒,时间戳缺失 fail-closed)+ 居住地在作用域内;
