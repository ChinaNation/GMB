...
- **建案事务内快照**:特别案创建时由 `legislation-vote` 先从发起机构
  ...
  `CitizenIdentityReader::create_population_snapshot(scope)`。人口为零或后续建案、
  绑定失败时整笔事务回滚，不存在公开准备交易、调用者缓存或待消费中转存储。
  `eligible_total` 固化进 `Proposal.citizen_eligible_total`，`snapshot_id` 只绑定到
  `VotingEngine::ProposalPopulationSnapshotIds`；普通案和重大案不创建人口快照。
