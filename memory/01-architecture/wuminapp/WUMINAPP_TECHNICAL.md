...
wuminapp 的 P2P IM 技术路线已确定为“远程通信全节点 + 近场无网点对点通信”双通道架构：

- 远程通信：手机连接用户自己的通信全节点，通信全节点只保存端到端加密后的 IM 消息。
- 近场通信：不设计独立“局域网模式”，不要求同一路由器；Android 使用 BLE 发现 + Wi-Fi Direct 传输，iOS 使用 Multipeer Connectivity。
- 原生目录：Android 近场模块放在 `wuminapp/android/im/`，iOS 近场模块放在 `wuminapp/ios/im/`，两个 `im` 目录只承载 IM 近场通信功能。
- Flutter 层：`wuminapp/lib/im/` 承载统一消息层、加密身份、消息存储和传输抽象，通过 Platform Channels 调用 Android/iOS 原生能力。
- 公共全节点边界：公共归档全节点、普通全节点不承担 IM 信令、中继或消息存储；聊天内容不上链。
...
wuminapp/
├── android/
│   └── im/                 ← 预定：Android IM 近场原生模块（BLE + Wi-Fi Direct）
├── ios/
...
- 管理员主体使用 `AdminSubjectService` 的两层缓存：30 秒内存缓存负责同页面/相邻页面去重，`AppKvEntity` 持久化快照负责 App 重启后首屏显示；管理员变更、激活、手动刷新时清对应 subject，投票提交前仍链上复核
