...
- 钱包交易流水同步属于低优先级后台任务；监听 finalized 区块事件时必须复用 `WalletIsar` 队列，遇到本地库繁忙直接让路，不能和钱包页、治理页抢 MDBX 写锁。
...
- 机构分类卡片已内置：
  ...
  - 省储会 43
  - 省储行 43
- 省储会、省储行分类列表固定为一行两列展示，避免因不同 Android 机型逻辑宽度差异出现单列大卡或列数漂移；国储会单张卡片横跨整行显示到右侧边缘
...
`LocalTxEntity` 只记录本机钱包进入 wuminapp 后的余额变化流水，不补扫导入前历史；`WalletTxSyncCursorEntity` 记录每个钱包从哪个 finalized 区块开始同步以及最新同步高度。
...
| 钱包交易流水 | finalized 区块事件 `System.Events` | `wallet` + `transaction/shared` | 已实现：只记录本机开始跟踪后的事件 |
