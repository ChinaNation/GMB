...
CID 的高并发目标必须建立在结构化数据库、索引和省市范围分区之上。业务主数据只落 PostgreSQL,应用进程不保存第二套主数据。
...
- `citizens(province_code, city_code, archive_no, cid_number, wallet_pubkey, wallet_address)`
...
`citizencode-backend serve` 只做 schema 初始化、分区创建、内置联邦注册局机构管理员初始化和后台索引 worker 启动。确定性公权机构目录不得在每次服务启动时全量写入。
