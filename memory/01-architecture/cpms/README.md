...
- 每台电脑同一时刻登录一个管理员账号。
- 后端只监听 `127.0.0.1:8080`，局域网入口统一由 nginx 提供 `https://www.cpms.com/`。

## 交付与安装
- 正式安装包：`cpms-ubuntu24-amd64.run` 或 `cpms-ubuntu24-arm64.run`
- 主机安装：`sudo ./cpms-ubuntu24-<arch>.run`
- 终端访问地址：`https://www.cpms.com/login`
- 安装手册：安装后保存在 `/opt/cpms/docs/CPMS安装配置手册.pdf`

安装脚本负责：
- 安装并启动 PostgreSQL。
- 创建 CPMS 数据库与账号。
- 修正数据库 owner 和 schema 权限；正式数据库结构统一由后端 `MIGRATOR.run()` 创建。
- 安装前端静态文件到 `/opt/cpms/frontend`，并写入 `CPMS_FRONTEND_DIR`，由后端 8080 直接托管页面。
...
- 前端本地联调使用 `cd cpms/frontend && npm run dev`；正式部署不依赖 Vite dev server。

## 角色与权限
- `ADMIN`：初始化绑定或由管理员新增，可创建管理员、编辑管理员姓名、删除非初始管理员、维护地址、修改公民状态和选举资格。
- `OPERATOR`：录入档案、查询档案、下载/打印 ARCHIVE 二维码。
