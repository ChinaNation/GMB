...
- 机构管理员（`INSTITUTION_ADMIN`）通过 `wuminapp` 扫码绑定产生，最多 `3` 个（`K1/K2/K3` 各对应 1 个）。
- 支持多个系统管理员（`SYSTEM_ADMIN`），由机构管理员增删改查。
- 登录机制对齐 SFID：公钥账户 + 扫码 challenge 签名登录。
- 安装初始化时先扫码 SFID 下发的机构初始化二维码，验签通过后生成本机构 `3` 把二维码签名私钥。
- 每生成一把签名密钥即提供对应”机构管理员绑定二维码”，由 `wuminapp` 扫码后回传绑定签名。
- 当前版本（v1）密钥策略：机构内 3 把密钥并存（主用/备用/应急），按机构独立管理。
- 公钥登记二维码包含 `site_sfid`、`keys[]`、`sign_key_id`、`signature`，用于 SFID 机构管理员录入。
...
## 密钥边界
- 机构管理员登录公钥：用于 CPMS 管理员登录与权限控制。
- 机构管理员密钥来源：`wuminapp` 持有并签名绑定，CPMS 仅保存公钥账户。
- SFID 验签公钥：来源于各机构 CPMS 初始化生成的 `3` 把签名私钥对应公钥。
- 二维码签名私钥来源：每个机构安装初始化时生成，不参与机构管理员初始化流程。
