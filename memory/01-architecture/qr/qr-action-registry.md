...
|---:|---|---|---|---|---|---|
| 1 | `login` | UTF-8 `onchina` | `QR_V1|2|i|onchina|e|target_signer_public_key_without_0x` | OnChina | CitizenWallet | 先扫描 `k=5 wallet_code`（公民钱包账户详情出示），取 `b.account_id` 单向反查链上 Active 管理员记录后，再生成非空 `b.u` 的定向登录请求 |
