...
- 管理员列表展示**链上实名资料**(A2 `AdminProfile`):姓名/职务/任期/来源 + 实名 CID 号 + 账户 SS58;端侧由 `lib/citizen/shared/admin_profile.dart` 按机构码路由 Genesis/Public/Private/Personal Admins 解码(个人多签 kind=3 仅账户无资料)。机构(公权+私权)创建/关闭已收归 onchina,citizenapp 不再发起,管理员展示为**只读**(治理档可冷钱包激活)
...
- 多签 Tab 顶部标题为“多签”，右上角 `+` 仅提供新增个人多签 → `lib/transaction/personal-manage/personal_account_create_page.dart`
  （机构(公权/私权)多签创建/关闭已收归 onchina 控制台 + 冷钱包,citizenapp 不再发起机构创建/关闭）
...
- 如果交易已入块但没有成功事件，App 必须先解析 `System.ExtrinsicFailed` 并显示真实
  `PersonalManage / AdminsChange / PublicManage / PrivateManage` 模块错误；不得把“未找到成功事件”
  当作最终失败原因
