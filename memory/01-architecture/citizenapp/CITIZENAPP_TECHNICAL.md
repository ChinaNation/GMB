...
- 手动发布需要配置的 GitHub Secrets：
  - `CITIZENAPP_RELEASE_KEYSTORE_BASE64`：release keystore 文件的 base64 内容
  - `CITIZENAPP_ANDROID_STORE_PASSWORD`：keystore 密码
  - `CITIZENAPP_ANDROID_KEY_ALIAS`：签名 key alias
  - `CITIZENAPP_ANDROID_KEY_PASSWORD`：签名 key 密码
  - `CITIZENAPP_RELEASE_KEYSTORE_SHA256`：可选，用于校验 keystore 文件 SHA-256
