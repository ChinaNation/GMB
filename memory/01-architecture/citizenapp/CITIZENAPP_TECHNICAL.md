...
- 治理机构详情页固定数据立即显示：机构名称、类型、身份 ID、制度账户、内部门槛等均来自本地静态注册表或制度常量，不等待链上 RPC
...
| 提案 | 业务治理 pallet `propose_*` | `lib/proposal` + 各业务模块目录 | `onchain_tx`（交易签名）+ CID 快照签名字段 | Runtime 升级已接入，多签转账在 `lib/duoqian-transfer` |
...
- `memory/05-modules/citizenapp/onchain/ONCHAIN_TECHNICAL.md`（普通链上转账）
...
- 链上支付模块：`memory/05-modules/citizenapp/onchain/ONCHAIN_TECHNICAL.md`
