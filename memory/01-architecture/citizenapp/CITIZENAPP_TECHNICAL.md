...
- 电子护照不再写 `myid.*` 本地档案状态；`MyIdService` 只读默认热钱包，再按 `CidByWalletAccount → CidRegistry/WalletAccountByCid → VotingIdentityByCid/CandidateIdentityByCid` 读取 finalized 身份闭环。
- 用户资料：
...
- `identity_wallet_account` 展示为“投票账户”，来源是通过 CID 反向绑定校验的默认钱包地址。
- `identity_cid_number` 展示为“身份 CID 号”，来源是 `CidByWalletAccount` 与 `WalletAccountByCid` 相互印证的永久 CID；身份值不重复保存 CID。
