...
- 管理员列表展示**链上实名资料**(A2 `AdminProfile`):姓名/职务/任期/来源 + 实名 CID 号 + 账户 SS58;端侧由 `lib/citizen/shared/admin_profile.dart` 按机构码路由 Public/Private/Personal Admins 解码(个人多签 kind=2 仅账户无资料)。机构(公权+私权)创建/关闭已收归 onchina,citizenapp 不再发起,管理员展示为**只读**(治理档可冷钱包激活)
