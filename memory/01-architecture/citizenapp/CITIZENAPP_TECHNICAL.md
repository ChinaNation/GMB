...
citizenapp 的 P2P IM 技术路线已确定为“信息 Tab 统一入口 + 钱包账户聊天身份 + Cloudflare 密文 mailbox 互联网聊天 + 近场无网点对点通信”架构：

- 用户入口：公民端在“多签”Tab 与“交易”Tab 之间新增“信息”Tab；互联网聊天和近场聊天的消息都在“信息”Tab 集中显示，用户不选择底层通信模式。
- 聊天账户：CitizenApp 钱包账户就是用户可见聊天账户，也是聊天窗口内发起既有转账功能时的收付款账户；钱包私钥只用于设备绑定证明和链上转账签名，不作为 IM 消息加密密钥。
- 互联网聊天：CitizenApp 通过 Cloudflare Worker / D1 / R2 组成的不可信密文 mailbox 投递 OpenMLS `ImEnvelope`；Cloudflare 只能保存密文、加密附件和必要投递元数据，不保存聊天明文或 OpenMLS 私钥。
...
- 删除边界：区块链节点通信节点聊天路线、`citizenchain/node/src/im/` mailbox、桌面设置页“通信节点功能”、CitizenApp“设置通信节点”、`im_node_pairing` 固定码和 `/gmb/im/1` 直连投递都已从当前实现中删除，不得作为正式聊天入口恢复。
- 原生目录：Android 近场模块放在 `citizenapp/android/im/`，iOS 近场模块放在 `citizenapp/ios/im/`，两个 `im` 目录只承载 IM 近场通信功能。
- Flutter 层：`citizenapp/lib/im/` 承载统一消息层、信息 Tab 数据模型、加密身份、支付提示、消息存储、发送队列和自动传输路由，通过 Cloudflare transport 或 Platform Channels 调用近场能力。
- 成熟组件优先：端到端加密主选 OpenMLS；外层协议使用 Protobuf；传输、附件分片和近场能力优先复用成熟库或系统框架，不自研底层通信协议和加密算法。
- 公共节点边界：公共归档节点、普通节点和桌面区块链软件不承担 IM 信令、中继或消息存储；聊天设备公钥、KeyPackage、密文投递状态和撤销状态都属于 IM 通信体系。
...
- 广场与聊天：广场媒体/feed、IM 密文 mailbox 继续走 Cloudflare Worker/D1/R2/KV/DO；区块链节点不承担聊天中继或媒体存储。
...
- 会员体系为四档，**会员档 `membership_level` 与身份档 `required_identity_level` 解耦**（`plans.ts`）：访客身份含两档——自由会员 `visitor`、民主会员 `visitor_pro`（权益对齐投票公民、身份仍匿名，`required_identity_level='visitor'`）；投票公民会员 `voting`；竞选公民会员 `candidate`。**订阅资格精确匹配**（`identityEligibleForPlan`：`required_identity_level === identity_level`，**禁止降档/越级**）：访客身份只能订 visitor/visitor_pro，投票只能订 voting，竞选只能订 candidate（`assertCheckoutEligibility` 对所有档位含访客都读链确认，`eligibleMembershipLevels` 同）。**发帖额度按所购会员套餐**（`membershipPlan(level).quota`，民主即得投票额度；竞选专属发帖仍以 `membershipLevel==='candidate'` 闸门）。官网 `citizenweb/src/pages/Membership.tsx` 与 App `lib/my/membership/membership_page.dart` 均按「一张访客卡内自由/民主左右切换（默认自由）」呈现，非本人身份档卡订阅置灰。Stripe subscription webhook 写入 Cloudflare Worker / D1，App 内只展示会员状态与档位介绍，不直接提供支付按钮；`resolveMembershipEntitlement` 以链上 storage 为资格真源。
- 会员套餐的业务计价唯一使用美元：自由会员 `visitor` `$2.99/month`、民主会员 `visitor_pro` `$9.99/month`、投票公民会员 `voting` `$9.99/month`、竞选公民会员 `candidate` `$99.99/month`。价格→Stripe price 映射走 `STRIPE_PRICE_VISITOR` / `STRIPE_PRICE_VISITOR_PRO` / `STRIPE_PRICE_VOTING` / `STRIPE_PRICE_CANDIDATE`（Cloudflare secret/vars，民主 price 须单独在 Stripe 建）。`plans[]` 暴露 `price_currency = usd`、`price_usd_cents`、`price_usd_monthly`。Stripe Checkout 可按 Stripe 能力让用户使用本地法币或 USDC 完成支付，但本地币种仅属于支付呈现 / 换汇结果，不改变会员套餐计价；USDT 不作为目标支付方式。
- 认证用户是链上已绑定 `cid_number` 的钱包账户；非认证用户是未绑定 `cid_number` 的钱包账户。普通动态 / 普通文章三档会员都可发布但额度不同；竞选动态 / 竞选文章只有竞选公民会员可发布。当前 runtime 的 `campaign` 链上发布仍按 `VotingIdentityByAccount` 拦截，竞选公民会员资格由 Cloudflare Worker 作为 App 业务服务端强制校验；若未来要求链上也强制 Candidate 身份，必须按 runtime 二次确认规则单独修改。
...
- CitizenChain 只负责发布交易入块、每条发布扣 1 元、竞选发布权限校验、发布索引和事件；阶段 4 已落地 `citizenchain/runtime/otherpallet/square-post/`，pallet index `36`、call index `0`，链上只记录 `post_id`、`owner_account`、可空 `cid_number`、`post_category`、`content_hash`、`storage_receipt_id`、`storage_until`、`created_block`。
- Cloudflare Worker 负责钱包签名登录、官网 Stripe Checkout 创建、官网 Stripe webhook、会员和链上身份资格校验、Images/Stream 一次性上传授权、manifest R2 上传授权、上传回执、Stream webhook、链上发布事件确认、帖子删除清理、推荐/关注/竞选 feed、关注关系、点赞/隐藏/不感兴趣/举报等推荐信号。2026-07-07 起 `GET /v1/square/membership` 返回 `plans[]`、链上身份资格、可订阅等级、支付状态和最终权益状态；`POST /v1/square/membership/stripe/webhook` 使用 `Stripe-Signature` 与 `STRIPE_WEBHOOK_SECRET` 校验官网订阅回调，并在写入 `square_memberships` 前强制校验 Stripe subscription item 的 Price 为 USD 且金额匹配三档会员。2026-07-08 起 `POST /v1/square/membership/stripe/checkout` 由官网传 `owner_account` 与 `membership_level` 创建 Stripe subscription Checkout；投票/竞选会员创建前先读取链上身份，不满足则拒绝。2026-07-08 起广场主媒体迁移到 `square_media_assets` 表，图片 provider 为 `cloudflare_images`，视频 provider 为 `cloudflare_stream`。2026-07-08 起 `POST /v1/square/uploads/prepare` 必须携带 `content_format`、`title_length`、`text_length`，Worker 按当前有效会员计划先拒绝超额声明；`complete` 再读取 R2 manifest、校验 manifest hash、owner、分类、正文、标题、媒体数量和 `square_media_assets` 一致性，防止绕过 App 伪造声明。2026-07-08 起 `DELETE /v1/square/posts/{post_id}` 只允许作者本人调用，先删除 Images/Stream provider 资产和 R2 manifest，再把 D1 `square_posts` 标记为 `deleted` 并清空 `title/text`、删除 `square_media_assets` 行、按上传估算值回收 `storage_used_bytes`；链上 `SquarePosts` 不改写。
- App 端发布闭环当前口径：`lib/8964/services/square_api_client.dart` 负责 Worker 登录、会员、上传接口、`content_format/title_length/text_length` 声明和帖子删除接口；`lib/my/user/user.dart` 在「我的」Tab 钱包和通讯录之间提供会员入口，只读展示当前订阅状态和三档权益；`lib/8964/services/square_upload_service.dart` 负责生成内容 manifest、在 prepare 阶段取得 `post_id/storage_receipt_id` 和 Images/Stream 上传 URL，并按 Worker 返回的当前会员计划做交互前置校验，最终仍以 Worker 强校验为准；`lib/8964/chain/square_chain_service.dart` 负责编码并提交 `SquarePost.publish_square_post`；`lib/8964/services/square_publish_service.dart` 串联“余额校验、链上扣费入块、媒体上传、Worker 确认 feed”。动态/文章详情页的“修改”视为新发布：新 `post_id/content_hash/storage_receipt_id` 入块并确认成功后，再调用 Worker 删除旧帖 Cloudflare 数据；新发布失败时不触碰旧帖。
- 阶段 6 已在 Worker 侧接入链上发布确认：`citizenapp/cloudflare/src/chain/rpc.ts` 通过 `CITIZEN_CHAIN_RPC_URL` 与两项 `CITIZEN_CHAIN_RPC_ACCESS_*` Secret 访问 Access 保护的 HTTPS 上游并读取指定区块 `System.Events`，`src/chain/square_event.ts` 解码 `SquarePostPublished`，`src/posts/confirm.ts` 在事件字段、上传记录和 R2 manifest 全部一致后写入 `square_posts.post_state = published`。Worker 链调用器只允许内部固定的 `state_getStorage` 和 `author_submitExtrinsic`，不接收 App 指定的 method 或 RPC URL。
...
- `PUT /v1/square/uploads/dev-put` 只是本地 Miniflare/R2 验收代理，只有 `SQUARE_DEV_UPLOAD_PROXY=1` 时可用，生产环境不得开启。
...
- Worker 远端命令固定为 `npm --prefix citizenapp/cloudflare run migrate:staging`、`npm --prefix citizenapp/cloudflare run deploy:staging`、`npm --prefix citizenapp/cloudflare run migrate:production`、`npm --prefix citizenapp/cloudflare run deploy:production`；执行远端命令前必须先把 `wrangler.toml` 中 staging/production 的 D1 `database_id` 和 KV `id` 占位值替换为 Cloudflare 实际资源 ID。
- `GET /v1/chain/bootstrap` 属于 Cloudflare 边缘启动清单接口，返回 `citizenapp.chain.bootstrap.v2`：`chain`、`light_client`、`p2p`、`services`、`security`、`degradation`。该接口只治理链身份、公开 bootnodes 和服务发现，不返回 checkpoint、轻同步资产 URL/摘要或 RPC URL，不代理 JSON-RPC，不接触私钥；`signed_extrinsic_relay.enabled` 仅在 Worker 显式配置 `CHAIN_EXTRINSIC_RELAY_ENABLED=1` 且服务节点 RPC 已配置时为 `true`，path 固定 `/v1/chain/extrinsics/relay`。
...
- staging 曾用已废弃的单一链 RPC Secret 完成阶段 12/13 历史验收；2026-07-10 起当前代码不再读取该旧配置。当前已建立 `CitizenChain RPC` Access 应用、Service Auth 策略和 `nrcgch-rpc.crcfrcn.com -> 127.0.0.1:9944` Tunnel 路由，并在 staging Worker 配置 `CITIZEN_CHAIN_RPC_URL`、`CITIZEN_CHAIN_RPC_ACCESS_CLIENT_ID`、`CITIZEN_CHAIN_RPC_ACCESS_CLIENT_SECRET` 三项 Secret；废弃的 `SQUARE_CHAIN_RPC_URL` 已删除。Oracle 服务器 connector 尚未在线，因此带令牌请求当前在通过 Access 后返回 tunnel unavailable，不能把 Cloudflare 控制面配置当作完整私有链路验收。R2 阶段 11、链确认负向 smoke 和 runtime metadata 阻塞结论仍保留为历史事实。
- 卡 20260705-citizenapp-worker-prod-url-no-fallback 已完成 Cloudflare production 部署：Worker URL 为 `https://citizenapp-square-api.stews87-fawn.workers.dev`，R2 bucket 为 `citizenapp-square-media`，D1 为 `citizenapp-square-db-production` / `0c5a0924-83ef-4347-bacc-b3f6f36da460`，KV 为 `citizenapp-square-production-FEED_CACHE` / `b72bbbcb36d240acb317fdaf79ce46f4`。production 已完成远端 D1 迁移、`/health` smoke 和未登录 401 验证；App 已内置该 URL 为唯一默认并删除本机兜底。production R2 预签名 Secret（`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_ACCOUNT_ID`）与链 RPC 三项 Secret 待配置；纯文字聊天走 D1，不依赖链 RPC。
- App 端广场/聊天 Worker 地址默认即线上生产 `SquareApiConfig.prodBaseUrl = https://citizenapp-square-api.stews87-fawn.workers.dev`（聊天 mailbox 与广场共用同一 Worker）；`CITIZENAPP_SQUARE_API_BASE_URL` 编译期 define 仅作显式覆盖（HTTPS，或本机 `http://127.0.0.1` 联调）。已删除本机兜底与生产 fail-fast，真机 debug/release 不传任何 define 也默认直连 Cloudflare。
...
- 发布流程当前状态：App 先用 finalized 余额校验至少 `2.11 元`（ED 1.11 元 + 发布费 1 元）→ 钱包签名登录 Worker → Worker 校验会员、容量、`post_category` 与 `content_format` 额度并在 prepare 阶段生成 `post_id/storage_receipt_id`、manifest 上传 URL 和 Images/Stream 上传 URL → App 提交链上 `publish_square_post` → runtime 扣 1 元并校验竞选权限 → App 等待入块 → 入块后 App 上传 manifest 到 R2、上传图片/视频到 Images/Stream 并调用 complete → Worker 读取 R2 manifest 复核真实内容额度和媒体资产一致性 → App 调用 Worker 确认接口 → Worker 用链上事件、上传记录、R2 manifest 和 `square_media_assets` 交叉校验后写入正式 feed → App 刷新 Worker feed。
- 链上未入块、余额不足或后台发布流程失败时，App 用 `AppKvEntity` 保存当前钱包的广场发布草稿；草稿只用于用户继续编辑或再次发起发布，不是链上发布成功或 feed 可见的真源。
- Worker 链 RPC 只允许由远端 Secret `CITIZEN_CHAIN_RPC_URL` 提供 Access 保护的 HTTPS 地址，并由 `CITIZEN_CHAIN_RPC_ACCESS_CLIENT_ID`、`CITIZEN_CHAIN_RPC_ACCESS_CLIENT_SECRET` 提供服务令牌；三项必须成套存在，缺失任一项时 relay 对 App 保持关闭。R2 S3 预签名所需变量同样只允许放在 Cloudflare 远端变量或 Secret 中；仓库、CitizenApp、R2 manifest、bootstrap 响应和 relay 响应都不得保存链 RPC 私密地址或 Cloudflare 凭证。
