...
- 公权机构包：安装包内置 `assets/public_institutions/`，发布期从已完成链上 `PublicManage` 投影的 OnChina 真实 HTTP 接口导出；2026-07-04 起创世快照只包含国家/省/市公权机构 49,581 条，镇级和后续新增机构通过链投影增量进入本地缓存。正式 `genesis_hash/state_root/public_institution_root` 必须等待本轮 CI WASM 成功后重新 bake 冻结。citizenapp 公民端不按 OnChina 管理端“公权机构 / 市公安局 / 教育机构”等后台功能 tab 分流或排除。
