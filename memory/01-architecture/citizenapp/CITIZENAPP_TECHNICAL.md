...
- Cloudflare staging API 唯一入口为 `https://www.crcfrcn.com/api-staging`，由 `CitizenApp API Staging` Access 应用只允许维护账户访问；R2 bucket 为 `citizenapp-square-media-staging`，D1 为 `citizenapp-square-db-staging` / `4ba85b05-657a-46ac-ab19-8bbd84fe850a`，KV 为 `staging-FEED_CACHE` / `91133becebc24f27e`。2026-07-12 staging 已清空后按唯一 `0001_square_core.sql` 重建，旧附件中继表和旧用量表不存在；本次 Worker 版本为 `692d472a-49ec-47e5-912d-51cf6e178545`，production 版本为 `418f3d65-ea13-4d40-a045-a66ba84822cc`。未通过 Access 的 staging HTTP 验收返回预期 302，`workers.dev` 与 Preview URL 均关闭。
...
- CitizenApp 聊天、广场、会员和媒体共用同一套 Worker 源码，但 Cloudflare 远端严格分为 staging 与 production 两套 Worker、D1、KV、R2、路由和 Secret。唯一人工发布与运行态测试入口是根 `deploy/` 本地控制台，其不含密钥的源码由 Git 追踪：测试部署无需密码，生产部署逐次通过 Touch ID。对应 Secret 只保存在 macOS Keychain，网页只显示配置状态和中文用途。部署动作依次执行依赖安装、TypeScript 检查、Worker 测试、远端 D1 可读检查、Secret 同步、部署和真实健康检查；staging 强制 `sk_test_`，production 强制 `sk_live_`。独立“会员真实测试”不发布 Worker：使用被 Git 忽略的官方 `deploy/.runtime/cloudflared` 打开 Access 人工批准窗口，批准后以每轮随机钱包对现有 staging 执行真实链身份、sr25519 签名、远端 D1 与 Stripe test 矩阵；日志逐项区分 `PASS`、Worker→Stripe `BLOCKED` 和 `FAIL`，退出前清理 Stripe 测试资源并验证 D1 零残留。
- 2026-07-14 会员 staging 真实验收由上述部署控制台执行完成：最终日志为 PASS=17、BLOCKED=4、FAIL=0。14 个业务断言覆盖资格拒绝、预付时长、真签名取消、异档守卫、同档续费、升降档预览、降档确认和 D1 落库；本机 Stripe test 成功创建 active 订阅，B3 卡取消、D2 升档差价建单、E1 USDC 建单、F1 卡订阅建单均真实在 Worker→Stripe 返回 525。退出时 Stripe 测试订阅与客户两项资源的取消/删除响应已验证，随机钱包在 staging D1 的挑战与会员行均为 0。
- `citizenapp/cloudflare/migrations/0001_square_core.sql` 是清空数据库后的唯一重建基线，不是可重复执行的增量迁移。双环境部署脚本禁止自动重放该基线；检测到未来新增且未审核的迁移文件时必须停止发布，先单独完成 D1 迁移审查和真实数据库验收。
