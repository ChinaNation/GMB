...
- P2P Chat 路线：聊天 Tab + 钱包账户聊天身份 + Cloudflare 密文 mailbox 互联网聊天 + 近场无网点对点通信；区块链节点通信节点聊天路线已删除，不再作为正式聊天链路；Android 近场模块规划为 `android/chat/`，iOS 近场模块规划为 `ios/chat/`
...
citizenapp 的 P2P Chat 技术路线已确定为“聊天 Tab 统一入口 + 钱包账户聊天身份 + Cloudflare 密文 mailbox 互联网聊天 + 近场无网点对点通信”架构：
...
- 互联网聊天：CitizenApp 通过 Cloudflare Worker / D1 / R2 组成的不可信密文 mailbox 投递 OpenMLS `ChatEnvelope`；Cloudflare 只能保存密文、加密附件和必要投递元数据，不保存聊天明文或 OpenMLS 私钥。
...
- 删除边界：区块链节点通信节点聊天路线、`citizenchain/node/src/chat/` mailbox、桌面设置页“通信节点功能”、CitizenApp“设置通信节点”、`chat_node_pairing` 固定码和 已删除的节点聊天协议 直连投递都已从当前实现中删除，不得作为正式聊天入口恢复。
...
- 广场与聊天：广场媒体/feed、Chat 密文 mailbox 继续走 Cloudflare Worker/D1/R2/KV/DO；区块链节点不承担聊天中继或媒体存储。
