...
- 公权机构包：安装包内置 `assets/public_institutions/`，发布期从已通过 `check-gov --strict` 的 SFID 真实 HTTP 接口导出；2026-06-19 重新创世版本 1 当前包包含 43 省、248643 条公民端完整公权机构。citizenapp 公民端不按 SFID 管理端“公权机构 / 市公安局 / 教育机构”等后台功能 tab 分流或排除。
...
- `sfid_number` 在 citizenapp 展示为“身份ID”；缺失时显示“未绑定”。
...
- SFID 完成绑定后，citizenapp 通过状态接口同步结果；citizenapp 不创建电子护照绑定记录。
...
citizenapp 访问 SFID 只保留两条路径，路径由 `CITIZENAPP_SFID_ENV` 选择：

```bash
# 生产路径：不传该参数时默认也是 prod
flutter build apk --release \
  --dart-define=CITIZENAPP_SFID_ENV=prod
...
- `./scripts/citizenapp-run.sh` 只注入 `--dart-define=CITIZENAPP_SFID_ENV=dev_usb`，并且必须成功建立 `adb reverse tcp:8899 tcp:8899`。
