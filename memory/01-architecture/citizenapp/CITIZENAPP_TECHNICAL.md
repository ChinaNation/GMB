...
- staging 曾用已废弃的单一链 RPC Secret 完成阶段 12/13 历史验收；2026-07-10 起当前代码不再读取该旧配置，只接受 `CITIZEN_CHAIN_RPC_URL`、`CITIZEN_CHAIN_RPC_ACCESS_CLIENT_ID`、`CITIZEN_CHAIN_RPC_ACCESS_CLIENT_SECRET` 三项远端 Secret。新的 Access 应用、服务令牌、Tunnel 和三项 staging Secret 尚未在本步骤部署，完成前不得把历史 smoke 当作当前私有链路验收。R2 阶段 11、链确认负向 smoke 和 runtime metadata 阻塞结论仍保留为历史事实。
