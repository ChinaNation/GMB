...
- 会员体系统一为三档：访客会员 `visitor`、投票公民会员 `voting`、竞选公民会员 `candidate`。官网 `citizenweb/src/pages/Membership.tsx` 调 Cloudflare Worker 创建 Stripe Checkout Session；Stripe subscription webhook 写入 Cloudflare Worker / D1，App Store / Google Play 版本 App 内只展示会员状态和三档介绍，不直接提供支付按钮。访客会员不要求链上身份；投票公民会员必须有有效 `CitizenIdentity::VotingIdentityByAccount`；竞选公民会员必须有有效 `CitizenIdentity::CandidateIdentityByAccount`。Worker 每次计算会员最终权益时以链上 storage 为资格真源，App 端展示不作为权限依据。
- 会员套餐的业务计价唯一使用美元：访客会员 `$2.99/month`、投票公民会员 `$9.99/month`、竞选公民会员 `$99.99/month`。`plans[]` 暴露 `price_currency = usd`、`price_usd_cents` 和 `price_usd_monthly`，App 会员页只展示美元价格。Stripe Checkout 可按 Stripe 能力让用户使用本地法币或 USDC 完成支付，但人民币、港币、台币等本地币种仅属于支付呈现 / 换汇结果，不改变会员套餐计价；USDT 不作为目标支付方式。
...
- 阶段 6 已在 Worker 侧接入链上发布确认：`citizenapp/cloudflare/src/chain/rpc.ts` 按 `SQUARE_CHAIN_RPC_URL` 读取指定区块 `System.Events`，`src/chain/square_event.ts` 解码 `SquarePostPublished`，`src/posts/confirm.ts` 在事件字段、上传记录和 R2 manifest 全部一致后写入 `square_posts.post_state = published`。
...
- `GET /v1/chain/bootstrap` 属于 Cloudflare 边缘启动清单接口，返回 `citizenapp.chain.bootstrap.v1`：`chain`、`light_client`、`p2p`、`services`、`security`、`degradation`。该接口不返回 RPC URL，不代理 JSON-RPC，不接触私钥；`signed_extrinsic_relay.enabled` 仅在 Worker 显式配置 `CHAIN_EXTRINSIC_RELAY_ENABLED=1` 且服务节点 RPC 已配置时为 `true`，path 固定 `/v1/chain/extrinsics/relay`。
...
- staging 已配置 `R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY` 和 `SQUARE_CHAIN_RPC_URL`；R2 secret 与链 RPC 完整 URL 只允许配置在 Cloudflare 远端变量或 secret，不得写入仓库。阶段 11 远端 smoke 已验证 `uploads/prepare` 可在不写 R2 对象的前提下预生成 `storage_receipt_id`，D1 记录停留在 `prepared` 且 `content_hash` 为空。阶段 12 已通过真实远端 R2 预签名上传和 Worker 链确认负向 smoke：临时内容完成 `prepare -> R2 PUT -> complete` 后，`POST /v1/square/posts/confirm` 返回预期 `409 square_event_not_found`，证明 Worker 已能读取链 RPC 指定区块事件。阶段 13 已确认测试热钱包可签名且余额 200 GMB；Cloudflare Tunnel/DNS/RPC 已恢复，但正向发布确认 smoke 阻塞于国储会节点当前 runtime metadata 未包含 `SquarePost/publish_square_post/SquarePostPublished`，`author_submitExtrinsic` 在交易池校验阶段返回 `TaggedTransactionQueue_validate_transaction` wasm trap，交易未进入交易池/区块，测试钱包 nonce 与余额保持不变。
- 卡 20260705-citizenapp-worker-prod-url-no-fallback 已完成 Cloudflare production 部署：Worker URL 为 `https://citizenapp-square-api.stews87-fawn.workers.dev`，R2 bucket 为 `citizenapp-square-media`，D1 为 `citizenapp-square-db-production` / `0c5a0924-83ef-4347-bacc-b3f6f36da460`，KV 为 `citizenapp-square-production-FEED_CACHE` / `b72bbbcb36d240acb317fdaf79ce46f4`。production 已完成远端 D1 迁移、`/health` smoke 和未登录 401 验证；App 已内置该 URL 为唯一默认并删除本机兜底。production R2 预签名 secret（`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_ACCOUNT_ID`）与 `SQUARE_CHAIN_RPC_URL` 待配置——仅影响发图/链上确认，纯文字聊天走 D1 不依赖。
...
- Worker 链 RPC 地址只允许通过部署环境变量 `SQUARE_CHAIN_RPC_URL` 提供；R2 S3 预签名所需 `R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY`、`R2_BUCKET_NAME` 只允许放在 Cloudflare 远端变量或 secret 中；仓库、CitizenApp、R2 manifest、bootstrap 响应和 relay 响应都不得保存链 RPC 私密地址或 Cloudflare 凭证。
...
| 投票 | 直连节点提交投票 extrinsic | `citizen/proposal` | 管理员投票已实现，联合公投提交待补齐 |
