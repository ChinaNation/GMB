...
- 外部接口：Cloudflare Worker 承接聊天控制面、广场、会员、支付和媒体资源。公民、机构、管理员与清算行身份统一由 smoldot 读取 finalized runtime storage。
...
- Cloudflare Worker 负责钱包签名登录、Stripe 订阅、会员和链上身份资格校验、统一资源限制、D1 原子额度预留、R2/Images/Stream 写入、上传回执、Stream webhook 实际时长/分辨率复核、链上发布事件确认、帖子删除和 feed。`citizenapp/cloudflare/src/limits/catalog.ts` 是所有请求体、文件、账户数量、周期用量和出站载荷的唯一硬上限；环境变量只能收紧。`POST /v1/square/uploads/prepare` 在调用媒体提供商前原子预留活动上传数、订阅周期图片数和视频秒数；`complete` 核销一次，删除帖子只回收实际存储总量而不返还周期上传额度。
...
- Worker 工程位于 `citizenapp/cloudflare/`，功能目录包括 `auth/`、`membership/`、`limits/`、`uploads/`、`storage/`、`posts/`、`feeds/`、`chat/` 和 `moderation/`。所有外部方法/path 先匹配路由白名单和正文上限，未知路由不得进入 D1。
...
- 广场媒体盈利保护由 Worker 强制执行：自由会员每月 300 图/30 分钟视频/1 个活动上传，民主与投票公民会员每月 1500 图/180 分钟/2 个活动上传，竞选公民会员每月 5000 图/1800 分钟/3 个活动上传。媒体估算成本达到有效订阅毛收入预算的 85% 时暂停新视频，达到 100% 时暂停全部新媒体；文字浏览、账户和 Chat 不受媒体熔断影响。
