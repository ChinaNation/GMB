...
- 机构管理员展示联合解码链上 `admins` 钱包集合、entity 岗位定义和任职关系，使用 `InstitutionAssignmentCard` 展示钱包、岗位、任期和来源。个人多签展示及管理员集合变更完全独立，不复用机构岗位模型。
...
- 会员体系为三档（ADR-036，**会员与身份彻底解耦**）：自由会员 `freedom`、民主会员 `democracy`、薪火会员 `spark`。`membership_level` 是纯付费订阅轴，**不再绑定任何身份档**——任意身份（访客/投票/竞选）可订阅任意会员档，全组合放行；`plans.ts` 不再有 `required_identity_level` / `identityEligibleForPlan`，会员侧不读链上身份，也不再有「身份≠档位」冻结或暂停收款（取代 ADR-033 规则5）。**权限二分（用户 2026-07-16）：发帖分类权限按身份档、用量额度按会员档，互不影响**。发帖用量额度按所购会员套餐（`membershipPlan(level).quota`）；发帖分类权限按链上身份档——竞选内容（`campaign`）只有竞选身份（`identity_level==='candidate'`）用户可发（`assertIdentityCanPublishCategory` 在 `prepareUpload` / `confirm` 读链身份校验，仅竞选帖读链），普通内容任意身份可发。竞选身份用户可不订阅或订阅任意档。官网 `citizenweb/src/pages/Membership.tsx` 与 App `lib/my/membership/membership_page.dart` 均以三张并列会员档卡呈现，任意档可订，卡内不含身份字段（身份改由电子护照 myid 展示）。Stripe subscription webhook 写入 Cloudflare Worker / D1；App 按当前状态显示“订阅会员 / 取消订阅 / 续订会员”，操作统一打开官网，不在 App 内嵌支付。会员权益真源=订阅是否有效（`subscriptionIsActive`：status active/trialing 且未过期）。
- 会员套餐的业务计价唯一使用美元：自由会员 `freedom` `$2.99/month`、民主会员 `democracy` `$9.99/month`、薪火会员 `spark` `$99.99/month`。价格到 Stripe Price 的映射统一使用 `FREEDOM_PRICE_ID` / `DEMOCRACY_PRICE_ID` / `SPARK_PRICE_ID`。`plans[]` 暴露 `price_currency = usd`、`price_usd_cents`、`price_usd_monthly`，并含会员权益之一 `chat_file_max_bytes`（聊天单文件上限：自由 10MB / 民主 100MB / 薪火 5GB）。Stripe Checkout 可按 Stripe 能力让用户使用本地法币或 USDC 完成支付，但本地币种仅属于支付呈现 / 换汇结果，不改变会员套餐计价；USDT 不作为目标支付方式。
...
- CitizenChain 只负责发布交易入块、每条发布按最低链上费用扣 0.1 元、竞选发布权限校验、发布索引和事件；`SquarePost` 当前 pallet index `36`、call index `0`，链上只记录 `post_id`、`owner_account`、可空 `cid_number`、`post_category`、`content_hash`、`storage_receipt_id`、`storage_until`、`created_block`。
- Cloudflare Worker 负责设备子钥钱包登录、Stripe 订阅、会员和链上身份资格校验、加密通讯录密文 CRUD、统一资源限制、D1 原子额度预留、R2/Images/Stream 写入、上传回执、Stream webhook 实际时长/分辨率复核、链上发布事件确认、帖子删除和 feed。登录 Session 不读取 `System.Account` 或余额；链身份/余额只在需要它的业务入口校验。`citizenapp/cloudflare/src/limits/catalog.ts` 是所有请求体、文件、账户数量、周期用量和出站载荷的唯一硬上限；环境变量只能收紧。`POST /v1/square/uploads/prepare` 在调用媒体提供商前原子预留活动上传数、订阅周期图片数和视频秒数；`complete` 核销一次，删除帖子只回收实际存储总量而不返还周期上传额度。
...
- Worker D1 新增 `resource_reservations`、`resource_usage`、`resource_totals`；`square_media_assets.resource_key` 指向统一限制表。KV session、session index 和身份缓存写入前同样校验实际 JSON 字节。Stripe secret、Cloudflare API token、Stream webhook secret、R2 冷归档只读签名密钥和链 RPC 地址只放部署环境。
...
- Cloudflare staging API 唯一入口为 `https://www.crcfrcn.com/api-staging`，由 `CitizenApp API Staging` Access 应用只允许维护账户访问；R2 bucket 为 `citizenapp-square-media-staging`，D1 为 `citizenapp-square-db-staging` / `4ba85b05-657a-46ac-ab19-8bbd84fe850a`，KV 为 `staging-FEED_CACHE` / `91133becebc24f27e`。2026-07-12 staging 已清空后按 `0001_square_core.sql` 重建；2026-07-14 已由部署控制台审核并应用 `0002_stripe_webhook_events.sql`，新增 Stripe webhook 事件占位和一次性付款永久去重表。本次 staging Worker 版本为 `1d516002-e731-4b07-bdad-0c174ee29473`；production 未在本轮变更。未通过 Access 的 staging HTTP 验收返回预期 302，`workers.dev` 与 Preview URL 均关闭。
...
- Cloudflare production API 唯一入口为 `https://www.crcfrcn.com/api`，R2 bucket 为 `citizenapp-square-media`，D1 为 `citizenapp-square-db-production` / `0c5a0924-83ef-4347-bacc-b3f6f36da460`，KV 为 `citizenapp-square-production-FEED_CACHE` / `b72bbbcb36d240acb317fdaf79ce46f4`。2026-07-12 production 已清空后按唯一目标基线重建，Worker 活动版本为 `54432c7a-3572-4f55-86cc-38a95b25c2d0`；production 与 staging 均关闭 `workers.dev` 和 Preview URL。Stripe webhook 为 `/api/v1/square/membership/webhook`，Stream 账户唯一 webhook 为 `/api/v1/square/uploads/stream/webhook`。Images/Stream 只签发短期私有交付地址，Feed 不保存长期播放 URL。
...
- Cloudflare WAF `citizenapp-api-edge-limit` 对 `/api/*` 与 `/api-staging/*` 按 IP 限制 60 次/10 秒并阻断 10 秒，排除 Stripe/Stream webhook；Worker 再按认证接口、上传、Chat、读取、写入分别使用 IP 哈希或钱包账户做精确限流。staging 额外由 Access 登录保护。
...
- CitizenApp 聊天、广场、会员和媒体共用同一套 Worker 源码，但 Cloudflare 远端严格分为 staging 与 production 两套 Worker、D1、KV、R2、路由和 Secret。唯一人工发布与运行态测试入口是根 `citizenconsole/` 本地控制台，其不含密钥的源码由 Git 追踪：测试部署、会员逻辑真实测试和 Stripe Sandbox 全链路验收无需密码，生产部署逐次通过 Touch ID。对应 Secret 只保存在 macOS Keychain，网页只显示配置状态和中文用途。部署动作依次执行依赖安装、TypeScript 检查、Worker 测试、远端 D1/迁移检查、Secret 同步、部署和真实健康检查；staging 强制 `sk_test_`，production 强制 `sk_live_`。Stripe Sandbox 全链路验收使用真实 sr25519 签名、真实 Stripe 测试 Checkout、真实 Sepolia USDC、真实 webhook 和 staging D1，并逐项输出 `PASS` / `BLOCKED` / `FAIL`；控制台状态接口保留脱敏后的最近任务事件，页面刷新或标签交接后可恢复完整日志。
- 2026-07-14 独立 Stripe Sandbox（`acct_1Trr2qQlQZ1x0Cw8`，`livemode=false`）整车验收由部署控制台真实执行完成，最终日志为 `PASS=8 / BLOCKED=0 / FAIL=0`：银行卡付款→subscription webhook→D1 授权、真 sr25519 签名取消、Sepolia USDC 8.97 付款→checkout webhook→D1 预付授权、同一真实 Stripe event 连续重放两次不延长时长、USDC→银行卡 122 天 trial 切换落地、切换后真签名取消均通过；测试账户 `0xc8d89abb0b769C6d34e506D9ea41be85fd19E666` 的 USDC 由 20.00 降至 11.03，构成真实链上付款旁证。验收首次发现的真实问题不是 525，而是卡→USDC 后旧卡 `customer.subscription.updated` 延迟到达并覆盖预付行；现以 D1 原子真源守卫拒绝无 `payment_switch=usdc_to_stripe` 标志的旧卡事件，真实复测已通过。此前“Worker→Stripe 固定 525”不再作为当前故障结论：同一路径在隔离 Sandbox 中已连续完成建单、取消和 webhook。退出清理结果为 `challenge=0 / membership=0 / payment=0`，production Stripe、Worker 和 D1 均未触碰。voting/candidate 成功身份与身份不匹配冻结/解冻仍缺真实链上身份钱包，明确未计入本次 PASS。
- `citizenapp/cloudflare/migrations/0001_square_core.sql` 是清空数据库后的重建基线，不是可重复执行的增量迁移；`0002_stripe_webhook_events.sql` 是已审核的 Stripe 事件幂等增量，当前仅在 staging 应用，production 需另行授权和真实数据库验收。双环境部署脚本禁止自动重放基线或未审核迁移；检测到未来新增迁移文件时必须停止发布并单独审查。
