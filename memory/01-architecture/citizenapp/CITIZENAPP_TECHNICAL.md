...
- 阶段 6 已在 Worker 侧接入链上发布确认：`citizenapp/cloudflare/square_worker/src/chain/rpc.ts` 按 `SQUARE_CHAIN_RPC_URL` 读取指定区块 `System.Events`，`src/chain/square_event.ts` 解码 `SquarePostPublished`，`src/posts/confirm.ts` 在事件字段、上传记录和 R2 manifest 全部一致后写入 `square_posts.post_state = published`。
...
- Worker 工程位于 `citizenapp/cloudflare/square_worker/`，阶段 3 已提供 `auth/`、`membership/`、`uploads/`、`storage/`、`posts/`、`feeds/`、`moderation/` 模块、D1 迁移、Wrangler 配置和 Vitest 测试；阶段 8 已补齐 staging/production 绑定模板和运维脚本，但当前仍未部署 Cloudflare 远端资源。
...
- Worker 本地验证命令固定为 `npm --prefix citizenapp/cloudflare/square_worker run typecheck`、`npm --prefix citizenapp/cloudflare/square_worker test`、`npm --prefix citizenapp/cloudflare/square_worker run migrate:local`、`npm --prefix citizenapp/cloudflare/square_worker run dev:local -- --port 8787`。
- Worker 远端命令固定为 `npm --prefix citizenapp/cloudflare/square_worker run migrate:staging`、`npm --prefix citizenapp/cloudflare/square_worker run deploy:staging`、`npm --prefix citizenapp/cloudflare/square_worker run migrate:production`、`npm --prefix citizenapp/cloudflare/square_worker run deploy:production`；执行远端命令前必须先把 `wrangler.toml` 中 staging/production 的 D1 `database_id` 和 KV `id` 占位值替换为 Cloudflare 实际资源 ID。
...
- staging 已配置 `R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY` 并通过真实远端 R2 预签名上传 smoke；当前仍未配置 `SQUARE_CHAIN_RPC_URL`，因此还不能执行链上确认 smoke。R2 secret 与链 RPC 只允许配置在 Cloudflare 远端变量或 secret，不得写入仓库。
- App 端广场 Worker 地址由 `CITIZENAPP_SQUARE_API_BASE_URL` 编译期 define 注入；生产构建未显式提供该值时直接 fail-fast，本地调试才默认使用 `http://127.0.0.1:8787`。生产值只允许 HTTPS Worker/API 地址。
...
- `citizenapp/cloudflare/square_worker/.gitignore` 必须忽略 `.dev.vars`、`.wrangler/`、`node_modules/`、`coverage/` 和 `dist/`；Cloudflare token、R2 access key、R2 secret key 不得写入仓库。
