...
- CitizenApp 聊天、广场、会员和媒体共用同一套 Worker 源码，但 Cloudflare 远端严格分为 staging 与 production 两套 Worker、D1、KV、R2、路由和 Secret。根目录私密脚本 `scripts/cloudflare.sh` 是唯一发布入口：输入 `c` 立即部署 staging，输入 `s` 立即部署 production，不再二次确认。对应 Secret 只保存在权限为 `600` 且被 Git 忽略的环境文件中。脚本依次执行依赖安装、TypeScript 检查、Worker 测试、远端 D1 可读检查、Secret 同步、部署和真实健康检查；staging 强制 `sk_test_`，production 强制 `sk_live_`。
