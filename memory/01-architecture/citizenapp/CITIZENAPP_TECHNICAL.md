...
- staging 已配置 `R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY` 并通过真实远端 R2 预签名上传 smoke；当前仍未配置 `SQUARE_CHAIN_RPC_URL`，因此还不能执行链上确认 smoke。阶段 11 远端 smoke 已验证 `uploads/prepare` 可在不写 R2 对象的前提下预生成 `storage_receipt_id`，D1 记录停留在 `prepared` 且 `content_hash` 为空。R2 secret 与链 RPC 只允许配置在 Cloudflare 远端变量或 secret，不得写入仓库。
