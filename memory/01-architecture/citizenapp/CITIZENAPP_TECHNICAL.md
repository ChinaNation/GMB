...
- Cloudflare staging API 唯一入口为 `https://www.crcfrcn.com/api-staging`，由 `CitizenApp API Staging` Access 应用只允许维护账户访问；R2 bucket 为 `citizenapp-square-media-staging`，D1 为 `citizenapp-square-db-staging` / `4ba85b05-657a-46ac-ab19-8bbd84fe850a`，KV 为 `staging-FEED_CACHE` / `91133becebc24f27e`。2026-07-12 staging 已清空后按唯一 `0001_square_core.sql` 重建，旧附件中继表和旧用量表不存在，Worker 版本为 `f8fbb3e0-b5b3-4055-bf69-d0f305f4a8bb`；未通过 Access 的 HTTP 验收返回预期 302，`workers.dev` 与 Preview URL 均关闭。
- staging 曾用已废弃的单一链 RPC Secret 完成阶段 12/13 历史验收；2026-07-10 起当前代码不再读取该旧配置。2026-07-12 重新实查 Cloudflare 控制面：`nrcgch-rpc` Tunnel 已有 1 个健康 connector，但唯一发布路由是 `chain.crcfrcn.com -> 127.0.0.1:18080`，原 `nrcgch-rpc.crcfrcn.com` 已无 DNS，当前没有指向 `127.0.0.1:9944` 的私有 RPC 路由。staging/production Worker 均存在 `CHAIN_URL`、`CHAIN_ID`、`CHAIN_SECRET` Secret 名称，但 Secret 值不可回读，不能据此宣称 RPC 可用；production bootstrap 明确关闭 signed extrinsic relay。App 能发交易来自 smoldot 轻节点 P2P 提交，不代表 Worker 的链身份读取和广场入块确认 RPC 已打通。
