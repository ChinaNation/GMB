...
- CitizenApp 聊天、广场、会员和媒体共用同一套 Worker 源码，但 Cloudflare 远端严格分为 staging 与 production 两套 Worker、D1、KV、R2、路由和 Secret。唯一人工发布入口是根 `deploy/` 本地控制台，其不含密钥的源码由 Git 追踪：测试部署无需密码，生产部署逐次通过 Touch ID。对应 Secret 只保存在 macOS Keychain，网页只显示配置状态和中文用途。动作依次执行依赖安装、TypeScript 检查、Worker 测试、远端 D1 可读检查、Secret 同步、部署和真实健康检查；staging 强制 `sk_test_`，production 强制 `sk_live_`。
