...
- Cloudflare staging API 唯一入口为 `https://www.crcfrcn.com/api-staging`，由 `CitizenApp API Staging` Access 应用只允许维护账户访问；R2 bucket 为 `citizenapp-square-media-staging`，D1 为 `citizenapp-square-db-staging` / `4ba85b05-657a-46ac-ab19-8bbd84fe850a`，KV 为 `staging-FEED_CACHE` / `91133becebc24f27bf10a00cb001f27e`。staging 已按唯一 `0001_square_core.sql` 清空重建并关闭 `workers.dev` 与 Preview URL。
...
- Chat 的 staging/production 分别使用独立 Cloudflare Realtime TURN 密钥和独立 Google FCM 服务账号密钥；production 已真实验证 TURN 300 秒 ICE 凭证与 FCM OAuth/HTTP v1 鉴权。APNs 凭证暂不配置，仅影响 iOS 后台推送，不阻塞 Android FCM、TURN、前台 Chat、广场或会员。
