...
- 外部接口：HTTP API（由 OnChina 提供，用于管理员目录、公权机构目录等非链上查询场景）；电子护照状态直接读取链上 `CitizenIdentity::VotingIdentityByAccount`，不再走 OnChina 本地状态接口。
...
- 认证用户是链上已绑定 `cid_number` 的钱包账户；非认证用户是未绑定 `cid_number` 的钱包账户。两者都能浏览和发布普通动态，唯一权限差异是认证用户能发布 `campaign` 竞选分类内容。
- 广场默认分类为推荐；用户可切换关注、竞选，后续可按产品需要增加最新分类。推荐流初期只做可解释规则，不做黑盒模型。
- 广场媒体内容不存链上，不改造 CitizenChain 全节点存储媒体；图片、视频、封面和 `manifest.json` 存 Cloudflare R2，经 Cloudflare CDN 访问。
...
- Cloudflare Worker 负责钱包签名登录、会员和容量校验、R2 上传授权、上传回执、链上发布事件确认、推荐/关注/竞选 feed、关注关系、点赞/隐藏/不感兴趣/举报等推荐信号。
- App 端发布闭环当前口径：`lib/8964/services/square_api_client.dart` 负责 Worker 登录、会员、上传接口；`lib/8964/services/square_upload_service.dart` 负责生成内容 manifest、在 prepare 阶段取得 `post_id/storage_receipt_id`，并在链上入块后上传 R2；`lib/8964/chain/square_chain_service.dart` 负责编码并提交 `SquarePost.publish_square_post`；`lib/8964/services/square_publish_service.dart` 串联“余额校验、链上扣费入块、R2 上传、Worker 确认 feed”。
...
- App 发布页已支持图片/视频选择、热钱包本机签名和冷钱包 QR 签名；普通动态所有钱包可发布，竞选动态在 App 端先按链上 `CitizenIdentity::VotingIdentityByAccount` 查询结果拦截，runtime 仍是最终权限真源。
- 阶段 5 的 R2 manifest 是 App 先生成的规范化内容清单，字段包含 `schema`、`owner_account`、`post_category`、`text`、`media_items[].file_name/content_type/byte_size/sha256`；`post_id`、R2 `object_key` 和上传对象列表由 Worker/D1 的 `square_uploads` 记录维护，不要求 App 在 prepare 前伪造。
...
- R2 object key 固定使用 `square/{owner_account}/posts/{post_id}/...` 前缀；App 不持有 R2 API key，生产上传必须先通过 Worker 获取 R2 S3 短期预签名 PUT URL。
- `PUT /v1/square/uploads/dev-put` 只是本地 Miniflare/R2 验收代理，只有 `SQUARE_DEV_UPLOAD_PROXY=1` 时可用，生产环境不得开启。
- Worker 侧 D1 只保存动态元数据、会员、登录挑战、关注、上传状态和推荐信号；KV 只做 session/feed 短缓存，不作为长期内容存储。
...
- 阶段 7 已完成本地真实 E2E：钱包登录 Worker、D1 会员校验、R2 manifest/image 上传、上传完成、`SquarePost.publishSquarePost` 入块、Worker 按区块事件确认并写入 `square_posts`、推荐 feed 返回已发布动态。该验收只使用本地 Miniflare/D1/R2 和临时 chainspec，不写入 Cloudflare 密钥或链 RPC 私密地址。
...
- 发布流程当前状态：App 先用 finalized 余额校验至少 `2.11 元`（ED 1.11 元 + 发布费 1 元）→ 钱包签名登录 Worker → Worker 校验会员/容量并在 prepare 阶段生成 `post_id/storage_receipt_id` → App 提交链上 `publish_square_post` → runtime 扣 1 元并校验竞选权限 → App 等待入块 → 入块后 App 上传 R2 并调用 complete → App 调用 Worker 确认接口 → Worker 用链上事件、上传记录和 R2 manifest 交叉校验后写入正式 feed → App 刷新 Worker feed。
- 链上未入块、余额不足或后台发布流程失败时，App 用 `AppKvEntity` 保存当前钱包的广场发布草稿；草稿只用于用户继续编辑或再次发起发布，不是链上发布成功或 feed 可见的真源。
- Worker 链 RPC 地址只允许通过部署环境变量 `SQUARE_CHAIN_RPC_URL` 提供；R2 S3 预签名所需 `R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY`、`R2_BUCKET_NAME` 只允许放在 Cloudflare 远端变量或 secret 中；仓库、CitizenApp 和 R2 manifest 都不得保存链 RPC 私密地址或 Cloudflare 凭证。
- 统一协议登记见 `memory/07-ai/unified-protocols.md` 的 `P-API-CITIZENAPP-002` 和 `P-TX-013`；阶段任务卡见 `memory/08-tasks/open/20260705-citizenapp-square-r2-worker.md`。
