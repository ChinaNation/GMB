...
infer_module() {
  ...
  case "$lower" in
    *ai编程系统*|*ai*system*|*task*card*|*任务卡*|*guardrail*|*workflow*|*claude*review*|*启动协议*|*需求分析协议*|*memory/*)
      printf '%s' "ai/system"
      ;;
    *permit*|*cid*|*绑定*|*验签*|*身份*|*授权*|*登录*)
      printf '%s' "citizencode/backend"
      ;;
    *cpms*|*实名*|*档案*|*柜员*|*审批*|*审计*|*二维码签发*)
      printf '%s' "citizenpassport/backend"
      ;;
...
impact_scope_for_module() {
  ...
  case "$module" in
    ...
    citizenchain/node)
      ...
      ;;
    citizencode|citizencode/backend)
      printf '%s\n' "citizencode/backend" "memory"
      ;;
    citizencode/frontend)
      printf '%s\n' "citizencode/frontend" "memory"
      ;;
    citizenpassport|citizenpassport/backend)
      printf '%s\n' "citizenpassport/backend" "memory"
      ;;
    citizenpassport/frontend)
      printf '%s\n' "citizenpassport/frontend" "memory"
      ;;
...
risk_points_for_module() {
  ...
  case "$module" in
    ...
    citizencode|citizencode/backend|citizencode/frontend|citizencode/deploy)
      printf '%s\n' \
        "CID 不得保存原始实名，也不能替代 CPMS 成为实名信任根。" \
