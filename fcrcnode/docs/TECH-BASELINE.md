...
## 1. 文档目标
- 定义 `fcrcnode` 的统一技术方案基线。
- 保证与 `fullnode` 使用同一技术路线、同一工程规范。
- 明确本软件独立边界：面向公民储备委员会用户，不与其他软件混合部署。

## 2. 产品定位与边界
- 产品名称：`fcrcnode`
- 目标用户：国储会 / 省储会 / 省储行相关角色
- 核心业务域：治理、审批、委员会流程、组织管理
- 非目标范围：全节点运行监控、出块奖励统计（由 `fullnode` 负责）

## 3. 技术栈（与 fullnode 保持一致）
