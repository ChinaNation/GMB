...
#
# 用法:
#   tools/sync_account_derive_vectors.sh           # 默认 check 模式(CI):重生后必须无 diff,否则退出码 1
#   tools/sync_account_derive_vectors.sh --write    # 本地刷新:重生 + 复制,允许并保留 diff(供 commit)
#
...
if ! git -C "${REPO_ROOT}" diff --exit-code -- "${CANONICAL}" "${DART_COPY}"; then
  ...
  echo "[sync]   修复:本地跑 'tools/sync_account_derive_vectors.sh --write' 后提交两份文件。" >&2
